...
발명 절차가 필요하면 [vocaflow-design](../../../.claude/skills/vocaflow-design/SKILL.md)(§A–§G)을 참고한다(선택).
...
2. 코드 전에 vocaflow-design §G2 세 줄(골격 · 서명 · N4)을 적어 두면 방향이 선명해진다(선택).
   큰 재설계(새 화면·전면 재설계)는 §G3 발산 4안 — 골격(G1 축)이 서로 다른 넷 — 을 `compare.md` 로 만들 수 있고,
   그때 각 안에 사용자 행동과 학습 효과를 적는다. **고르는 것은 사람이다**(에이전트가 승자를 정하지 않는다).
...
6. [06-workflow](../../../docs/design/06-workflow.md) 5단계 비평(골든 대조 · 포트폴리오 · 접근성)으로 캡처를 보고,
