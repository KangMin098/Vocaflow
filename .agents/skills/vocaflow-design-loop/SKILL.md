...
2. 큰 재설계만 A(명확성), B(학습 자산 기반 새 조작), C(통합)를 비교한다.
   각 안에 사용자 행동과 학습 효과를 적고 가장 적합한 안을 선택한다. C를 무조건 승자로 삼지 않는다.
...
6. [visual-critic](../vocaflow-visual-critic/SKILL.md)의 기준으로 문제를 우선순위화하고,
   작업 범위 안의 중요한 결함을 수정한 뒤 같은 조건에서 다시 검증한다.
