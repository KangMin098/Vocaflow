-- supabase/migrations/20260908035514_db_health_stats_criterion_pg_statistic.sql
--
-- stats_stale_tables 의 판정 근거를 활동 카운터에서 pg_statistic 실재 여부로 바꾼다.
--
-- 왜: pg_stat_user_tables.last_analyze / last_autoanalyze / n_live_tup 은 크래시·통계
-- 리셋에 전부 NULL·0 으로 돌아가는 **활동 카운터**다. 통계가 실제로 있는지와 무관하다.
-- 2026-09-08 실측 (재기동 9/07 15:39 이후 12시간 시점):
--   8MB 넘는 표 16개 중 옛 규칙이 stale 로 보고한 표 = 16
--   그중 pg_statistic 에 통계가 실재하는 표 = 16
--   통계가 진짜 없는 표 = 0
-- => 100% 오탐. 크래시가 날 때마다 16건의 가짜 치명 경보가 나온다 (finding 30).
--
-- 두 번째 절(n_live_tup 대 reltuples 드리프트)도 같은 함정에 빠진다 — n_live_tup 은
-- 리셋되고 reltuples 는 pg_class 에 남으므로, 재기동 직후엔 reltuples > 10,000 인 표가
-- 전부 걸린다. 지우지 않고 **카운터가 24시간 이상 늙었을 때만** 적용하도록 가둔다.
-- (24시간인 이유: autovacuum/autoanalyze 기본 주기가 하루 안에 한 바퀴 돈다.)
--
-- 이 함수의 다른 블록은 손대지 않는다. plpgsql 은 부분 교체가 안 되어 전문을 다시 쓴다.

create or replace function public.collect_db_health_metrics()
 returns integer
 language plpgsql
 security definer
...
declare
  ...
  v_relname text;
  v_heap bigint;
  v_counter_age_min numeric;
begin
  -- 통계 카운터의 나이. 리셋 시각이 없으면 postmaster 기동 시각을 쓴다.
  select round(extract(epoch from (v_now - greatest(
           coalesce(pg_stat_get_db_stat_reset_time(d.oid), pg_postmaster_start_time()),
           pg_postmaster_start_time()))) / 60.0)
    into v_counter_age_min
  from pg_database d
  ...
  -- (1) capacity — 테이블별 상위 25 (이력이 쌓이면 증가 속도가 나온다)
  insert into db_health_metrics (measured_at, axis, metric, value, dims)
  ...
  -- (1) capacity — 통계 신선도 (판정 근거 = pg_statistic 실재 여부)
  insert into db_health_metrics (measured_at, axis, metric, value, dims)
  select v_now, 'capacity', 'stats_stale_tables', count(*),
         jsonb_build_object(
           ...
           'criterion', 'pg_statistic',
           'counter_age_min', v_counter_age_min,
           'drift_check_applied', v_counter_age_min >= 1440,
  ...
  from (
    select s.relname,
           ...
           st.stat_cols,
           (st.stat_cols = 0) as never_analyzed
    from pg_stat_user_tables s
    join pg_class c on c.oid = s.relid
    cross join lateral (
      select count(*)::int as stat_cols
      from pg_statistic sx where sx.starelid = c.oid
    ) st
    where pg_total_relation_size(s.relid) > 8388608
      and (
        -- 통계가 실제로 없다 — 카운터와 무관하게 언제나 참인 근거
        st.stat_cols = 0
        -- 또는 통계는 있으나 크게 드리프트했다. 카운터가 어리면 믿을 수 없으므로 가둔다.
        or (v_counter_age_min >= 1440
            and abs(coalesce(s.n_live_tup, 0) - c.reltuples)
                > greatest(c.reltuples * 0.5, 10000))
      )
  ) x;

  -- (1) capacity — 한 번도 안 쓰인 인덱스의 무게 (판정 아님 · 목록만)
  insert into db_health_metrics (measured_at, axis, metric, value, dims)
  select v_now, 'capacity', 'unused_index_mb',
         ...
         jsonb_build_object(
           'count', count(*),
           'counter_age_min', v_counter_age_min,
           'unreliable', v_counter_age_min < 1440,
  ...
  -- (1) capacity — 통계 카운터의 나이 자체를 지표로 남긴다.
  -- 판정층이 "왜 이 값이 이렇게 나왔나" 를 되짚을 수 있어야 한다.
  insert into db_health_metrics (measured_at, axis, metric, value, dims)
  values (v_now, 'capacity', 'stats_counter_age_min', v_counter_age_min,
          jsonb_build_object(
            'stats_reset', (select pg_stat_get_db_stat_reset_time(d.oid)
                            from pg_database d where d.datname = current_database()),
            'postmaster_start', pg_postmaster_start_time(),
            'note', '재시작·리셋 이후 경과(분). 1440 미만이면 unused_index_mb 와 stats_stale_tables 의 드리프트 절은 믿지 말 것. pg_statistic 절은 카운터와 무관하게 유효하다.'
          ));
  ...
  -- (1) capacity — 회전 블로트 표본 (맨 마지막 · 로컬 타임아웃 25초)
  begin
...
comment on function public.collect_db_health_metrics() is
  'db-health 수집기. stats_stale_tables 는 pg_statistic 실재 여부로 판정한다 — 활동 카운터(last_analyze/n_live_tup)는 크래시에 리셋되어 2026-09-08 실측 16/16 오탐이었다.';
