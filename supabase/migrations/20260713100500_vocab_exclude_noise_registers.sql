-- 추출 노이즈 제거 ②: 추출 함수 제외 목록에 brand·abbreviation·proper_noun 추가.
-- select_book_chapter_vocab + select_article_vocab의 WHERE word_register NOT IN (...) 확장.
-- 나머지 로직(Phase 3 문맥-sense LATERAL 매칭 등) 불변.
...
AS $function$
  ...
  cand AS (
    SELECT DISTINCT ON (sd.word)
      sd.word, COALESCE(cs.sense_meaning, sd.meaning_ko) AS meaning_ko,
      COALESCE(cs.sense_v, sd.v_level) AS v_level,
      sd.cefr_level, COALESCE(cs.sense_pos, sd.pos) AS pos, sd.example_en, sd.verified AS verified,
      COALESCE(sd.word_register, 'standard') AS word_register,
