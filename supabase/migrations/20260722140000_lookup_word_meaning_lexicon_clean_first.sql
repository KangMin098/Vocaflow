-- lookup_word_meaning: lexicon_clean(청정) 우선 tier + coverage_lexicon(kaikki) 브리지 유지.
-- 무regression 전환 — 청정 우선, kaikki 폴백. Step 7(lexicon_clean 완성 후) kaikki tier(7,9) 제거 예정.
-- 순서: L1(shared_dictionary 1-5) → 청정 한국어(6) → kaikki 한국어(7) → 청정 영어(8) → kaikki 영어(9).
-- 설계: docs/proposals/lexicon-coverage-clean-architecture.md
...
BEGIN
  ...
  -- 6. (신규) lexicon_clean 한국어 — 청정 우선(kaikki 대체). 직접+굴절.
  RETURN QUERY
  ...
  -- 7. coverage_lexicon 한국어 (kaikki 브리지 — Step 7 제거 예정)
  RETURN QUERY
  ...
  -- 8. (신규) lexicon_clean 영어 gloss — 한국어 없으면 영어라도(청정).
  RETURN QUERY
  ...
  -- 9. coverage_lexicon 영어 gloss (kaikki 브리지 — Step 7 제거 예정)
  RETURN QUERY
