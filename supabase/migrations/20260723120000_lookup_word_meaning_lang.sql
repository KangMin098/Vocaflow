-- lookup_word_meaning 에 lang 반환 추가 — 외국어(French 등) 해소분에 언어 배지 표기용.
-- 선제형 외국어 사전(lexicon_clean.lang)의 값을 노출: 영어 tier='en', lexicon_clean tier=lc.lang.
-- 시그니처 변경(RETURNS TABLE 끝에 lang text 추가)이라 DROP + CREATE. 호출부(reader-queries.ts)는 필드명 접근이라 하위호환.
