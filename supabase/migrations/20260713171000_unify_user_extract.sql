...
-- 추출 경로 통합(1단계) — extract_vocabulary_for_user_v2(학습자 BYO 지문)를 책/글과 동일 규칙으로.
--   매칭: L1 direct + L2 inflected_forms → resolve_dict_headword(direct→cluster→규칙역굴절→파생strip) 일괄.
--   추가: 형태 POS 추론 sense 선택 + 노이즈 register 제외 + 표면형 표시(word=실제 형태).
--   유지: 학습자 레벨(i+1) threshold, track 스코어(0.70빈도+0.30track), 이미 아는 단어 제외, 시그니처.
--   ⚠️ word=표제어→표면형, matched_via_surface=표면형→표제어(해소 lemma)로 의미 변경.
...
BEGIN
  ...
  ELSE
    IF v_user_v IS NOT NULL AND v_user_v > 0 THEN
      v_eff_v := v_user_v; v_thresh := v_user_v + 1; v_level_source := 'auto_user_diagnostic';
    ELSE
      v_eff_v := GREATEST(v_text_v - 1, 1); v_thresh := v_text_v; v_level_source := 'auto_text_p75_fallback';
    END IF;
  END IF;
  ...
  resolved AS (
    SELECT iw.w AS surface, public.resolve_dict_headword(iw.w) AS hw FROM input_words iw
  ),
  cand AS (
