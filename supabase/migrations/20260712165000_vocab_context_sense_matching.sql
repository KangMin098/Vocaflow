-- Phase 3 — 문맥-sense 매칭 추출 (근본 sense/POS 오정렬 근절).
-- 추출 JOIN 이 lbv/lav.context_pos 로 meanings_ko 에서 문맥 POS 일치 sense 선택 →
--   그 sense 의 v_level 로 V≥6 필터 + 그 sense 의 gloss·pos 표시. 미백필(NULL)은 row 값 폴백.
-- 효과: creep(문맥 verb)→"기어가다" gloss · sole(문맥 adj, sense v5)→V≥6 탈락(기본용법 오추출 근절).
...
AS $function$
  ...
  cand AS (
    SELECT DISTINCT ON (bv.chapter_idx, sd.word)
      ...
      sd.word                        AS word,
      -- Phase 3: 문맥 POS 일치 sense 우선(gloss·v_level·pos), 없으면 row 폴백.
      ...
      COALESCE(cs.sense_pos, sd.pos)            AS pos,
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY ((s->>'v_level') IS NOT NULL) DESC
      LIMIT 1
    ) cs ON true
  ...
  norm AS (
    SELECT c.*, MAX(c.frequency_in_chapter) OVER (PARTITION BY c.chapter_idx) AS chapter_max_freq FROM cand c
  ),
  scored AS (
    SELECT n.*, public._extract_composite_score(n.frequency_rank, n.frequency_in_chapter, n.chapter_max_freq::int, n.v_level, n.verified, n.example_en, n.skill_level, n.bvl) AS composite_score
    FROM norm n
  )
...
AS $function$
  ...
  cand AS (
    SELECT DISTINCT ON (sd.word)
      ...
      COALESCE(cs.sense_meaning, sd.meaning_ko) AS meaning_ko,
      COALESCE(cs.sense_v, sd.v_level)          AS v_level,
      sd.cefr_level, COALESCE(cs.sense_pos, sd.pos) AS pos, sd.example_en,
      sd.verified AS verified,
      COALESCE(sd.word_register, 'standard') AS word_register,
    ...
    LEFT JOIN LATERAL (
      SELECT (s->>'v_level')::smallint AS sense_v, s->>'meaning' AS sense_meaning, s->>'pos' AS sense_pos
      FROM jsonb_array_elements(sd.meanings_ko) s
      WHERE av.context_pos IS NOT NULL AND s->>'pos' = av.context_pos
      ORDER BY ((s->>'v_level') IS NOT NULL) DESC
      LIMIT 1
    ) cs ON true
    WHERE COALESCE(cs.sense_v, sd.v_level) >= 6
  ...
  norm AS (
    SELECT c.*, MAX(c.frequency_in_article) OVER () AS article_max_freq FROM cand c
  ),
  scored AS (
    SELECT n.*, public._extract_composite_score(n.frequency_rank, n.frequency_in_article, n.article_max_freq::int, n.v_level, n.verified, n.example_en, n.skill_level, n.avl) AS composite_score
    FROM norm n
  )
  SELECT s.word, s.word AS lemma, s.meaning_ko, s.v_level, s.cefr_level, s.pos, s.example_en, s.word_register,
    s.frequency_rank, s.frequency_in_article, s.skill_level, s.composite_score,
    ROW_NUMBER() OVER (ORDER BY s.composite_score DESC, s.frequency_in_article DESC NULLS LAST, s.v_level ASC, s.word)::int AS sort_order,
    s.first_sentence
  FROM scored s
$function$;
