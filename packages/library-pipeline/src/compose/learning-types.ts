...
export const LEARNING_TYPES: Record<LearningTrack, LearningTypeSpec> = {
  csat_korean: {
    ...
    compose: {
      ...
      avgSentenceWords: 22,
      directives: [
        // ⚠️ 예전 문구는 "**한 문단으로** 쓴다" 였다. 두 가지 이유로 고쳤다(2026-08-19):
        //   ① 학령 밴드가 함께 넣는 "한 문단은 4~6문장" 과 **정면으로 모순**된다. 180~190어를
        //      평균 14어절로 쓰면 13문장이라 한 문단에 담을 수 없고, 두 지시를 동시에 지킬 방법이
        //      없다. 발주가 서로 다른 말을 하면 쓰는 쪽이 임의로 하나를 버리게 된다.
        //   ② 실제로 그 문구를 지킨 초안은 **구문 연습 문항이 0개** 나왔다. DCP 의 순서·삽입은
        //      문단 단위로 생성되고 문단당 4~6문장을 요구하는데, 13문장짜리 한 덩어리는
        //      그 자격을 못 채운다. 즉 이 유형의 핵심 활동을 이 문구가 막고 있었다.
        // 논지 전개 순서(주제문 → 근거 → 함의)는 그대로 남긴다 — 그게 이 유형의 본질이고,
        //   문단을 나눠도 지킬 수 있다.
        '주제문 → 근거 → 함의 순서로 논지를 전개한다. 문단을 나눠도 이 순서는 글 전체에 걸쳐 유지한다.',
