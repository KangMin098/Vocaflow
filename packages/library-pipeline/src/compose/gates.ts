...
export const COMPOSE_THRESHOLDS = {
   ...
   * 않는다(고유명사 조합에서 드물게 1~2개).
   *
   * ── 실측으로 다시 정한 값 (2026-08-19) ────────────────────────────
   * 처음에는 0.25 였다. 근거 없이 "노이즈보다 두 자릿수 위" 라고만 적혀 있었고,
   * 실제로 재 보니 **부분 전재를 통과시켰다.**
   *
   * | 관계 | 담김(추출 본문 기준) |
   * |---|---|
   * | 각자 취재 (연합↔헤럴드 KASA · dw↔bbc · bbc↔dw · 연합↔헤럴드 일식) | 0.0 · 1.1 · 1.3 · 1.3% |
   * | 부분 전재 (헤럴드가 연합 문장을 손질해 실음 · 포항) | **19.3%** |
   * | 부분 전재 (헤럴드가 연합 문단을 그대로 실음 · 로카르노) | **31.3%** |
   *
   * 전문 전재라면 90%를 넘지만 **실제로 만난 것은 부분 전재**다. 0.25 는 그 사이에 있어
   * 19.3% 를 "독립" 으로 통과시켰고, 그 전제 위에 지문 1편이 발행 대기까지 갔다.
   *
   * ── 표본을 늘려 확인 (같은 날, 한국 매체 짝 10건) ──────────────────
   * 독립: **0.0 · 0.0 · 1.1 · 4.6%** (4건)
   * 전재: **16.9 · 85.2 · 88.7 · 97.7 · 100.0 · 100.0%** (6건)
   *
   * 분포가 두 덩어리로 갈리고 **4.6% 와 16.9% 사이가 비어 있다.** 0.10 은 그 빈 구간의
   * 한가운데다 — 임의로 고른 값이 아니라 데이터가 비워 둔 자리다.
   *
   * ⚠️ 함께 드러난 사실: 한국 매체 짝의 **60%가 전재**였다. 연합뉴스는 통신사이고
   *   코리아헤럴드·코리아타임스가 그 원고를 자주 싣는다. 제목만 보고 센 "독립 2계통" 은
   *   실제의 약 40% 다 — 이 게이트가 없으면 그 60%가 전부 재저작 대상으로 올라온다.
   */
  syndicationContainment: 0.1,
  /**
