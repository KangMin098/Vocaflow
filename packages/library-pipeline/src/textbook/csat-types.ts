...
export const CSAT_READING_TYPES: readonly CsatType[] = [
  ...
  },
  {
    ...
    implemented: false,
    note:
      '서사 지문 전용. **드레인을 돌려 봤고 수율이 0/16 이었다**(2026-08-22) — 지문 16편이 모두 3인칭 ' +
      '설명문이라 ① 장면 속 인물이 없고(주어가 동물·사물이거나 "A student who delays…" 같은 일반화된 인물) ' +
      '② 전후 정서 대비가 없다(바뀌는 것이 감정이 아니라 설계·규칙·인식이다). ' +
      '근접한 둘도 각각 "후반 심경을 글이 끝내 안 적음"·"방향이 되돌아와 A→B 가 하나로 안 정해짐" 으로 반려됐다. ' +
      '⚠️ **지문 풀을 바꾸지 않는 한 드레인을 더 돌려도 같은 결과다** — 1인칭·과거시제 서사문을 ' +
      '별도 축으로 확보해야 살아난다. 배치가 만들어 낸 것을 쓰지 않고 0 으로 둔 이유는, ' +
      '억지로 만든 심경 문항은 학습자가 근거 없는 추론으로 풀어야 해서 없는 문항보다 나쁘기 때문이다.',
  },
  {
    key: 'claim',
    numbers: [20],
