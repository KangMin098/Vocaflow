...
function buildFromWindow(
  ...
  const eligible = candidates
    ...
    // ⚠️ **문장처럼 생긴 것만 남의 글에서 가져온다** (실측 2026-09-16).
    //   여기 겉모습 가드가 「낱말 수」 하나뿐이라, 남의 글에서 **조각**이 딸려 왔다 —
    //   따옴표로 열린 인용 토막 · `(2008) Minority HIV-1 …` 같은 참고문헌 꼬리 ·
    //   `# M3029S, NEB Luna …` 같은 카탈로그 번호.
    //
    //   그 결과가 지면에서 이렇게 보인다(V5 무관 2,500문항 실측):
    //
    //       정답 문장이 알파벳으로 안 시작    2,176 / 2,500 = **87.0%**
    //       정답 아닌 문장                     138 / 10,000 =   1.4%
    //
    //   **「글자로 시작하지 않는 줄」을 찍으면 87%를 맞힌다**(우연 20%). 영어를 한 줄도
    //   안 읽고 풀리므로 이 유형이 재려던 「글의 통일성」은 하나도 안 재진다.
    //   3인 검수 두 청크가 각자 같은 것을 짚었고(87.0% · 49.1%), 둘 다 이 자리를 가리켰다.
    //
    // ⚠️ **부호를 지우지 않고 후보에서 뺀다.** 지우면 남의 문장을 고쳐 싣는 것이 되고,
    //   그 문장은 원문에 없는 글이 된다. 후보는 넉넉하므로 버리는 쪽이 옳다.
    .filter((c) => /^[A-Z]/.test(c.text.trim()))
