...
export const EVAL_DIMENSIONS: readonly EvalDimension[] = [
  ...
  // ── 교육 방법 및 내용 ─────────────────────────────────────────────
  {
    ...
    ours:
      '**9/18 (문항 비중 14/28).** 결정론 5 + Claude Code 드레인이 쓴 생성형 4(요지·주제·제목·빈칸). ' +
      '남은 9는 도표·안내문 같은 지문 밖 재료가 필요하거나 아직 안 쓴 것이다.',
    howMeasured:
      '`coverage.mjs` 실측 — 유형 9/18 · 문항 14/28. ' +
      '⚠️ **문항 수를 세는 이유**: 빈칸은 유형으로는 1 인데 시험지에서는 4문항이라 무게가 다르다. ' +
      '생성형은 넣었다고 끝이 아니다 — 첫 파일럿에서 정답이 최장인 비율이 요지 100% 였고, ' +
      '길이만 보고 풀리는 문항 49건을 걷어낸 뒤에야 이 숫자가 됐다.',
    standing: 'inferior',
  },
  {
    key: 'explanation',
