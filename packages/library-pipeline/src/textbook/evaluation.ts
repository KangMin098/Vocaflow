...
export const EVAL_DIMENSIONS: readonly EvalDimension[] = [
  ...
  // ── 교육 방법 및 내용 ─────────────────────────────────────────────
  {
    ...
    ours:
      '**13/18 (문항 비중 18/28).** 결정론 5 + Claude Code 드레인이 쓴 생성형 8 ' +
      '(요지·주제·제목·빈칸·목적·주장·요약·내용일치). 남은 5 중 둘(도표·안내문)은 **지문 밖 재료**가 있어야 하고, ' +
      '심경·함의는 서사/비유 지문이 전제, 장문은 개별 유형이 먼저 서야 하는 묶음이다.',
    howMeasured:
      '`coverage.mjs` 실측 — 유형 13/18 · 문항 18/28. ' +
