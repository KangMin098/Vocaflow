...
export const EVAL_DIMENSIONS: readonly EvalDimension[] = [
  ...
  },
  {
    ...
    category: 'legal',
    label: '편향·차별 표현 심사',
    market: '검정 교과서는 한국교육과정평가원 심사를 거친다(양성평등·장애인 인식·다문화·지역/직업 편견). 문제집도 편집부 감수가 있다.',
    ours:
      '`bias-review.ts` 가 **사람이 볼 자리를 좁힌다** — 비하·낡은 호칭(목록 주입) · 성별 표시 직업어 · ' +
      '성별 대명사 균형. **판정하지 않고 아무것도 지우지 않는다**(발행은 사람 판단).',
    howMeasured:
  ...
  // ── 교육 방법 및 내용 ─────────────────────────────────────────────
  {
    ...
    ours:
      '**14/18 (문항 비중 19/28).** 결정론 5 + Claude Code 드레인이 쓴 생성형 9 ' +
      '(요지·주제·제목·빈칸·목적·주장·요약·내용일치·함의). 남은 4 중 둘(도표·안내문)은 **지문 밖 재료**가 있어야 하고, ' +
      '심경은 **드레인 실측 수율 0/16**(설명문 지문에는 장면 속 인물도 전후 정서 대비도 없다 — 1인칭 서사문 축이 필요하다), 장문은 개별 유형이 먼저 서야 하는 묶음이다.',
    howMeasured:
      '`coverage.mjs` 실측 — 유형 14/18 · 문항 19/28. ' +
      '⚠️ **문항 수를 따로 세는 이유**: 빈칸은 유형으로는 1 인데 시험지에서는 4문항이라 무게가 다르다. ' +
      '⚠️ **넣었다고 끝이 아니다** — 두 번 확인했다. ① 첫 파일럿에서 정답이 최장인 비율이 요지 100% 라 ' +
  ...
  },
  {
    ...
    ours:
      '**발행 대상 다섯 권 전부 빠짐없이 붙어 있다**(80·120·80·114·108문항 · 없음 0). 규칙으로 근거를 확정할 수 있는 것은 기계가 쓰고(전체의 6.9%), ' +
      '나머지는 Claude Code 배치가 지문을 읽고 쓴다 — 정답 근거를 인용하고 왜 다른 자리가 아닌지까지 밝힌다.',
    howMeasured:
      '`render-volume` 실측 — V2 80/80 · V3 120/120 · V4 80/80 · V5 114/114 · V6 108/108 (전부 없음 0). ' +
