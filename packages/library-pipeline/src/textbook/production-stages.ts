...
export const PRODUCTION_STAGES: readonly ProductionStage[] = [
  ...
  },
  {
    ...
    claude: {
      ...
      progress:
        '**실측 2026-09-14 — 25유형 880,176문항.** 8월 실측(17,206)의 **51배**다. ' +
        '\n\n' +
        '⚠️ **창고가 늘어난 것과 책이 좋아진 것은 다르다.** 재고는 조판에 실릴 때만 값을 한다 — ' +
        '실제로 지면에 선 것은 사다리 7권 **420문항**이고, 그 권들의 유형 폭이 이 단계의 성적표다: ' +
        '시중 권당 유형 수 중앙값(초등 4 · 중등 4 · 고등 9) 대비 **평균 2.202 · 7권 중 6권 우위**, ' +
        '시중 **최대**(초 7 · 중 8 · 고 17) 대비로도 **5권이 그 이상**이다 ' +
        '(`scripts/textbook/type-spread.mjs` · 스냅샷 2026-09-13). ' +
        '남은 것은 V1(초등 저학년) 3종뿐인데, 그 권은 소리·낱말 단계라 수능형 유형을 넣을 자리가 ' +
        '아니고 **시중 분모도 초등 한 칸뿐이라 저학년을 따로 못 가른다** — 미달로 읽기 전에 분모를 본다.',
    },
  },
  {
    order: 4,
  ...
  },
  {
    ...
    gap:
      ...
      '\n\n' +
      '**지면 기준 실측 2026-09-14** — 사다리 7권이 쓰는 지문 **89편 중 87편(97.8%)이 깨끗**하다 ' +
      '(`volume-contents.json` 의 `proofChecked`/`proofClean`). ⚠️ 위 4.62% 와 **분모가 다르다** — ' +
      '저쪽은 저장된 지문 전체이고 이쪽은 실제로 인쇄되는 지문이다. 조판이 보는 것은 뒤쪽이다.',
    worker: 'claude',
    claude: {
      role:
  ...
  },
  {
    ...
    claude: {
      ...
      progress:
        '**실측 2026-09-14 — 분모를 둘로 갈라 읽는다.** ' +
        '지면(사다리 7권 420문항) **420/420 = 100%**(조판 가능도 420/420). ' +
        '창고 **509,352/880,176 = 57.9%** — 낮아 보이는 것은 해설이 줄어서가 아니라 ' +
        '**창고가 두 배로 늘었기 때문**이다(8월 실측 17,206 → 880,176). 먼저 들어온 유형은 ' +
        '거의 다 찼고(vocab_choice 55,238/55,238 · unit_grammar 34,921/34,921 · insert 121,892/121,923 · ' +
        'order 88,508/88,539) 새로 들어온 유형이 비어 있다(blank_word 80,443/224,148 · ' +
        'grammar_fix 45,521/137,861 · unit_vocab 46,559/133,185 · word_order 14,903/48,855). ' +
        '⚠️ **어느 분모를 쓰는지 밝히지 않으면 같은 사실이 100% 도 되고 57.9% 도 된다.** ' +
        '조판 게이트가 보는 것은 지면이고, 드레인이 채울 몫을 세는 것은 창고다. ' +
        '\n\n' +
        '⚠️⚠️ **남은 것은 보유가 아니라 품질이다.** 3인 검수 차단 사유를 계열로 접으니 ' +
        '**해설이 1위(17.0% · 117문항)** 였다(`scripts/textbook/review-causes.mjs`). 그 지적이 ' +
        '가리킨 것은 생성기가 **확인할 수 없는 것을 단정하던 문장**이었고(「나머지 …는 앞뒤 내용과 ' +
        '어긋나지 않는다」 — 38,522문항), 2026-09-13 에 제거해 107,782문항을 다시 썼다. ' +
        '보유율만 보고 이 단계를 `done` 으로 올리면 그때 고친 것이 다시 안 보이게 된다.',
    },
  },
  {
    order: 7,
