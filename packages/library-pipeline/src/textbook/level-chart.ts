 ...
 *
 * ⚠️ **2026-09-01: V7~V9 를 고2 → 고3 으로 옮겼다.**
 *   그전까지 고3 버킷이 **존재하지 않았다** — 코퍼스의 상용 교재가 고2 까지라 규격을
 *   만들 수 없었고, 그래서 고3 권을 이웃(고2)에서 빌려 재고 있었다.
 *   평가원 수능 기출 13회분이 들어오면서 고3 창(68~172)이 처음 생겼다.
 *
 *   수능 대비 권을 **시중 고2 교재**가 아니라 **실제 수능**으로 재는 것이 맞다.
 *   ⚠️ **처음에 "V7 손집필 유형이 깎인다" 고 적었는데 틀렸다.** 그 근거는 `v_level=7`
 *   태그 재고를 센 값이었다(topic 11→1 · title 14→5). 그런데 조립기는 v_level 이 아니라
 *   **밴드 7 원글에 붙은 문항**에서 고르므로 모집단이 다르다 — 실측 V7 권은 그대로
 *   **15종**을 싣는다(topic 4 · title 6 · main_point 2 · purpose 3 · summary 3 · claim 3).
 *   즉 이 매핑 변경으로 권에서 사라진 유형은 **없다.**
 */
