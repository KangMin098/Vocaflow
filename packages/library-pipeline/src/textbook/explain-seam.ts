...
export function explainInsertSeam(item: CsatInsertItem): ItemExplanation | null {
  ...
  if (after) {
    ...
    // ⚠️⚠️ **자리를 근거로 삼지 않는다** (3인 검수 3회차 실측 2026-09-15).
    //   여기 있던 「**글의 마지막 자리이므로** "…" 가 끝을 맺는다」는 순환 논증이다 —
    //   「⑤가 답인 이유는 ⑤가 마지막이라서」라고 말하는 것이라 학습자가 쓸 수 없다.
    //   순서 유형에서 이미 없앤 것(「원문의 이음매와 다르다」)과 **같은 꼴**인데
    //   삽입의 끝자리 가지에만 남아 있었다. DB 실측 **29,121문항**이 이 문구를 싣고 있었다.
    //
    //   끝자리는 앞 문장만 있고 뒤 문장이 없다 — 이을 자리를 **사실로만** 보인다.
    //   되받이가 있으면 위 `anchor` 가지가 이미 근거를 댔고, 없으면 댈 근거가 없다.
    //   **이유를 못 대면 안 대는 것**이 지어내는 것보다 낫다(이 파일의 순서 쪽 원칙).
    optional.push(`글의 끝자리다 — 앞 문장 "${tail(before, 30)}" 뒤에 놓인다.`)
  }
