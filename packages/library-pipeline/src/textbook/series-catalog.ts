...
/**
 * ── 단행본은 왜 없나 (실측 2026-09-13) ──────────────────────────────
 *
 * 목표에 「단행본 생성」이 있어 코퍼스를 훑었다
 * (`textbook-corpus/manifest.json` · 문서 94편 · 출판사 6곳):
 *
 *   기출 제외 **75편이 전부 시리즈 소속** — 시리즈가 안 붙은 문서 **0편**
 *   스키마도 출판사 → 시리즈 → 권(`volume`) → 역할(본책·정답해설·워크북…)이라
 *   **단행본이라는 개념 자체가 없다**
 *
 * 그래서 단행본 상품을 **정의하지 않는다.** 근거 없는 단을 만들면 화면이 「낼 수 있다」고
 * 말한 뒤 조판이 빈 권을 낸다 — 이 파일 머리말이 경고한 그대로다.
 *
 * ⚠️ **구조는 막지 않는다.** 단행본은 곧 **1단짜리 시리즈**이고 `rungs` 길이에 제약이 없다.
 *   근거가 생기는 날 `SeriesDef` 하나를 더하면 그대로 돈다 — 코드를 고칠 필요가 없다.
 *   그 사실을 `volume-target.test.ts` 의 「단행본 — 1단 시리즈」가 잠근다.
 *
 * ⚠️ 이 표본은 6곳·79종이고 NE능률에 치우쳐 있다. **「시장에 단행본이 없다」가 아니라
 *   「이 표본에 0편이다」**가 실측이 말하는 전부다. 넓혀 재면 달라질 수 있다.
 */
/* ───────────────────────── 상품 라인 ───────────────────────── */
