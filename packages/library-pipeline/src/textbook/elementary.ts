...
/**
 * 초등 보기 수. **4 로 둔 근거는 아직 없다 — 실측이 여기까지 닿지 않았다.**
 *
 * 2026-08-30 에 시중 79종을 문항 단위로 세어 `middle-choice.ts` 의 "중등 4지선다" 가
 * 틀렸음을 확인하고 5 로 고쳤다(중등 93.8%가 5지). 같은 표에서 초등도 79.7%가 5지로
 * 나왔지만 **그 표본을 이 상수에 적용하면 안 된다** — 코퍼스의 "초등" 은
 * 달곰한 Literacy · 리딩튜터 스타터 같은 **초3~초6 독해서**이고, 이 파일이 만드는 것은
 * V1(초등 저학년)의 **파닉스 운율·기본어휘 뜻·철자 완성**이다. 장르가 다르다.
 * 저학년 파닉스 교재가 코퍼스에 들어오면 그때 다시 잰다.
 */
