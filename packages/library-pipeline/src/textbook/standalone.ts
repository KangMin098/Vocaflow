 ...
 *
 * ── ⚠️ 두 번째 시도도 물러섰다 — **문항 지문**에 대고 재 봤다 (2026-09-14) ──────
 *
 * 3인 검수 3회차에서 검수자가 「발췌가 조응 해소를 안 본다」고 짚었다(8건 중 4건 —
 * 미소개 고유명사 `Rogger`·`Poeppel`, 선행사 없는 `they`, `To show how,` 로 여는 도입문).
 * 위 결론은 **원글**로 잰 것이라 문항 지문에서는 다를 수 있어 다시 쟀다. 결과:
 *
 *   넓은 자 (`ANAPHORIC_OPENERS` 전체를 문항 지문 첫머리에)
 *     걸림 **30,077문항(8.7%)** · 표본 10건을 읽으니 **7~8건이 오탐**이다 —
 *     `It is a common practice…`(가주어) · `This systematic review aims…`(글 자체를 가리킴) ·
 *     `That winter I agreed to meet…`(시간 설정). 전부 자립하는 정상 지문이다.
 *
 *   좁은 자 (맨대명사 `He|She|They…` + 접속부사 `But|So|However…` 만)
 *     생성형 지문 2,265건 중 **맨대명사 0 · 접속부사 4**.
 *
 * **넓히면 멀쩡한 것을 3만 건 버리고, 좁히면 네 건을 잡는다.** 그 사이에 쓸 만한 자리가
 * 없다 — 위 첫 시도와 같은 결론이고, 이번에는 **문항 쪽 실측으로도** 같았다.
 * 그래서 조합 단계에도 **안 건다.** 검수자의 지적은 개별 문항에서 옳지만 규칙이 되지 않는다.
 */
