...
describe('해설은 확인하지 않은 것을 단정하지 않는다', () => {
  ...
  const FORBIDDEN = [
    ...
    '지문 그대로다',
    '바꾼 자리는',
    // ⚠️ **같은 자국이 꼬리에 한 번 더 있었다** (3인 검수 chunk-01, 2026-09-15):
    //   「이 유형은 규칙에 어긋나는 자리가 하나뿐이다」. 문항 만드는 법을 그대로 적은 것이라
    //   학습자에게는 **「근거를 못 찾겠으면 남는 하나를 찍어라」**로 읽힌다 — 위 `바꾼 자리는`
    //   과 기능이 같다. 오답 절의 **문구**는 손대지 않았다(이미 세 번 바꿔 봤고 세 번 다
    //   같은 지적을 받았다 — `explain-drain-export.mjs` §밑줄 두 유형도 연다).
    //   **문구를 고치는 일과 인쇄하면 안 되는 문장을 지우는 일은 다르다.**
    '자리가 하나뿐이다',
  ]
...
/**
 * **조판기는 마크다운을 렌더하지 않는다 — 별표는 별표로 인쇄된다.**
 *
 * `volume-document.ts` 는 `esc(a.explanation.text)` 로 이스케이프만 한다. 그런데 해설 두 곳이
 * `첫 **소리**` · `명사의 **수**` 를 담고 있어 지면에 별표 네 개가 그대로 찍혔다
 * (3인 검수 chunk-01 이 짚었고, 전수 미리보기 표본에서도 그대로 보였다).
 *
 * ⚠️ 저장소 **소스 문자열**을 훑는다. 함수 하나를 부르는 검사로는 다음에 다른 유형에
 *   같은 것이 생겨도 모른다 — 별표는 어느 해설에도 있으면 안 되는 것이다.
 */
describe('해설에 마크다운 표기를 남기지 않는다', () => {
  it('출력 문자열에 `**` 가 없다', () => {
    const src = readFileSync(new URL('./explain-items.ts', import.meta.url), 'utf8')
    // 주석은 제외한다 — 주석의 강조는 사람이 읽는 것이고 인쇄되지 않는다.
    const offenders = src
      .split('\n')
