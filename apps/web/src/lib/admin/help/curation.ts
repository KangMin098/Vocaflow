    ...
    screen: {
      ...
      steps: [
        ...
        },
        {
          ...
          done: '검수는 **상태를 바꾸지 않는다** — 다 봐도 검토 대기 그대로다. 세 가지(분절·낭독·단어)를 확인했으면 끝난 것이고, 다음은 게시다.',
        },
        {
          title: '게시',
          detail:
        ...
        },
        {
          ...
          done: '「드레인 큐」 카드의 대기 수가 넣은 만큼 늘어난다. 여기까지가 이 화면의 몫이고, 실제 생성은 Claude Code 를 돌려야 진행된다.',
        },
      ],
      fields: [
        {
      ...
      'Curated Books': {
        ...
        drain: {
          ...
          procedure: [
            ...
            {
              title: '넣기 전 자동 검사 세 가지',
              detail:
                'insert-batch 는 배치 전량을 먼저 본다. ① 정답 위치 쏠림 — 화면이 보기를 섞지 않으므로 한 위치가 50% 를 넘으면 거부한다(balance-answer-positions.mjs 로 재배치). ② 정답 길이 쏠림 — 정답이 **단독 최장**인 문항이 배치의 40% 를 넘거나, 한 문항에서 정답이 오답 평균의 1.25배를 넘으면 거부한다. 위치가 완벽해도 길이가 새면 지문을 안 읽고 제일 긴 것을 눌러 맞는다(2026-09-05 실측 95.1%). 정답을 줄여서 맞추지 말고 **오답을 정답만큼 구체적으로(내용은 틀리게)** 채운다 — 동률은 단서가 아니므로 통과한다. ③ 학습 카드 금칙어 — 근거 문장은 원문을 그대로 인용하므로, 뜻이 오직 집단 멸칭인 낱말이 질문·보기·근거 문장 어디에 있어도 거부한다. 셋 다 넣기 전에 걸리므로 반쪽만 들어가지 않는다.',
              done: '세 검사를 통과해야 삽입이 시작된다.',
            },
            {
              title: '이미 들어간 문항의 오답 다시 쓰기 (길이 편향 복구)',
              detail:
                'quiz-distractor-drain.mjs export --size 60 이 길이로 풀리는 문항만 뽑아 scripts/lcp/quiz-distractor/chunk-NN.json 으로 낸다(사양은 같은 폴더의 _PROMPT.md). 서브에이전트가 chunk-NN.out.json 을 채우면 import --commit 이 넣는다. import 는 ①정답 문장이 한 글자라도 바뀐 문항 ②여전히 정답이 단독 최장인 문항을 **넣지 않고 건너뛴 수를 출력한다** — 우회 플래그는 없다. export 는 이미 고쳐진 문항을 건너뛰므로 **몇 번을 다시 돌려도 안전하다.** ⚠️ 에이전트가 도는 중에 export 를 다시 돌리지 말 것 — 청크가 갈아치워져 에이전트가 처음부터 다시 한다(실측). 한 웨이브가 다 끝난 뒤에만 재추출한다.',
              done: 'import 출력의 「여전히 길이로 풀림」이 0 이고, 적재 후 정답이 단독 최장인 비율이 0% 다(2026-09-05: 2,406문항 전량 0%).',
            },
          ],
          verify: [
      ...
      steps: [
        {
          ...
          done: '마지막 장까지 넘겨 본문 아닌 것이 섞이지 않았음을 확인한다. 깨졌으면 게시하지 말고 소스 GET 으로 되돌린다 — 게시 뒤에는 되돌리는 비용이 훨씬 크다.',
        },
        {
          title: '낭독 연결 확인',
          detail:
            'LibriVox 패널은 gutenberg·standard_ebooks·librivox 도서에만 나온다. 로직 처리에서 자동 매핑이 되면 이미 챕터별로 연결돼 있고, 정합에 실패한 책만 손으로 맞춘다.',
          done: '챕터마다 오디오 행이 붙어 있다. **패널 자체가 안 보이면 그 소스는 낭독이 없는 것**이니 건너뛴다 — 빈 패널과 없는 패널은 다른 뜻이다.',
        },
        {
          title: '추출 단어 확인',
          detail:
            '「추출」을 누르면 실제 발행될 단어장과 같은 목록·순서가 나온다. 사전 미바인딩이 많으면 그 자리에서 등재 큐에 올린다(뜻 생성·등재는 Claude Code 배치).',
          done: '미바인딩 수를 확인했다. **0 이어야 게시할 수 있는 것은 아니다** — 등재 큐에 올려 두면 뜻이 나중에 채워지고 단어장은 그때 완성된다.',
        },
        {
          title: '게시',
          detail:
        ...
        },
        {
          ...
          done: '이 화면에서 바뀌는 것은 없다 — 확인만 하는 자리다. 문항이 잘못됐으면 여기서 고치지 말고 퀴즈 큐에 다시 넣는다(고치는 곳이 둘이 되면 갈라진다).',
        },
      ],
      fields: [
        {
