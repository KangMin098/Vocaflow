        ...
        fields: [
          {
            label: '드레인 실행 (N건 대기)',
            detail:
              'N = 실패 표시가 없고 아직 자동 단계에 있는 호. 누르면 큐가 빌 때까지 최대 200회 반복하며 한 호출에 한 단계씩 올린다. 중지는 진행 중인 단계를 끝내고 다음 호출부터 멈춘다. **이 숫자는 목록 필터와 무관하게 항상 전량 기준**이다 — 필터를 걸어도 드레인 대상은 줄지 않는다.',
          },
          {
            label: '검색 · 상태 칩 (목록 위)',
            detail:
              '카드는 30건씩만 그린다(대량 적재 후 969호가 되면서 전량 렌더가 한 화면 2.6MB·18초였다). 칩에 달린 건수와 단계 분포·멈춤 수는 **좁힌 목록이 아니라 전량**을 센 값이라, 필터를 걸어도 운영 판단에 쓰는 숫자는 그대로다. 목록 아래 "N건 중 M건 표시" 가 지금 무엇이 잘려 있는지 말해 준다.',
          },
          {
