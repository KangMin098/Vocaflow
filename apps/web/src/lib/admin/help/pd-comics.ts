    ...
    screen: {
      ...
      steps: [
        ...
        },
        {
          ...
          done: '차단 문구가 사라지고 발행이 열린다. **네 칸 중 하나만 비어도 그대로 막힌다** — 화면은 어느 칸인지 말해 주지 않으므로 넷을 다 본다.',
        },
      ],
      fields: [
      ...
      '소스 · 대량 적재': {
        ...
        steps: [
          ...
          },
          {
            ...
            done: '접어 둔 채로 다음 단계로 가면 된다 — **이 단계는 건너뛰는 것이 정상이다.** 열었다면 아래 「출발점 칩」이 필터를 통째로 덮는다는 점을 먼저 읽는다.',
          },
          {
            title: '출발점 칩부터',
            detail:
              '자유 검색은 저작권이 살아 있는 자료를 상위에 끌고 온다. 어댑터가 주는 출발점 칩은 검색어와 필터(컬렉션·연도·정렬)를 통째로 갈아끼운다 — 누른 뒤엔 이전 필터가 남지 않는다.',
            done: '검색어 칸과 필터가 칩의 값으로 바뀐다. **손으로 맞춰 둔 필터가 있었다면 그 순간 사라진다** — 칩을 먼저 누르고 그다음에 손질하는 순서가 맞다.',
          },
          {
            title: '발행 상한 확인',
            detail: '기본 1963. 칩을 누르면 이 값도 칩의 것으로 덮이므로 적재 직전에 다시 본다.',
            done: '적재 직전 화면의 상한이 의도한 연도다. 덮인 채로 담으면 **저작권이 살아 있는 호가 섞여 들어오고**, 그건 발행 단계의 PD 게이트에서야 걸린다.',
          },
          {
            title: '테스트 모드로 담기',
            detail:
        ...
        steps: [
          {
            ...
            detail:
              '갱신은 발행 27~28년 뒤에 등록해야 했다(1909년법). 화면이 호별로 볼 연도를 계산해 준다 — 1952년 발행이면 1979~1980년 갱신 편이다. 60년치를 뒤질 필요가 없다.',
            done: '"갱신확인 1979~1981년" 같은 범위가 시리즈 헤더에 보인다.',
          },
          {
            title: 'Catalog of Copyright Entries 에서 조회',
            detail:
              '만화는 정기간행물(Class B)이다. 해당 연도 갱신 편에서 간행물 이름으로 찾는다. 없으면 갱신되지 않은 것이고, 그때 퍼블릭도메인이다.',
            done: '찾았든 못 찾았든 **본 사실을 기록에 남긴다** — "없음" 도 근거다. 안 찾아본 것과 찾아서 없는 것을 구별할 수 없으면 PD 근거가 아니다.',
          },
          {
            title: '근거와 함께 기록 — 시리즈 단위',
            detail:
              '갱신은 호가 아니라 간행물 단위로 등록되므로 한 번 확인하면 그 시리즈 전체에 적용된다. 확인한 페이지 주소를 근거 URL 로 남긴다. "갱신 기록 없음"과 "권리자 공개"는 URL 없이는 저장되지 않는다.',
            done: '시리즈 배지가 "근거 확정 N/N" 으로 바뀌고, 발행 게이트가 열린다.',
          },
        ],
        fields: [
          {
            label: '근거 — 보호기간 만료',
            detail:
              '1930년 이전 발행. 발행연도만으로 확정되므로 갱신 여부를 볼 필요가 없고 근거 URL 도 요구하지 않는다.',
          },
          {
            label: '근거 — 갱신 기록 없음',
            detail:
              '1930~1963 발행분의 정상 경로. **근거 URL 필수** — "찾아봤는데 없더라"는 어딘가를 봤다는 주장이라, 어디를 봤는지 없이는 나중에 아무도 재검증할 수 없다.',
          },
          {
            label: '이 시리즈 N호에 기록',
            detail:
              '그 시리즈의 모든 호에 근거·확인자·확인시각을 함께 쓴다. 확인자와 시각은 서버가 붙이므로 조작할 수 없다.',
          },
          {
            label: '적용 대상 N호',
            detail: '펼치면 호별 발행연도·갱신 확인 창·현재 근거 상태를 볼 수 있다. 연도 미상 호는 갱신 창이 계산되지 않는다 — 원본에서 연도를 먼저 확인해야 한다.',
          },
        ],
        cautions: [
          '⚠️ **만화는 Stanford 판권갱신 DB 에 없다** — 그 DB 는 도서(Class A) 전용이다. 거기서 "검색해도 안 나온다"를 근거로 삼으면 틀린 확신이 된다. 만화책은 정기간행물(Class B)이라 Catalog of Copyright Entries 를 봐야 한다.',
          '근거 확정은 **법적 진술을 기록에 남기는 행위**다. 확인자(pd_checked_by)와 시각이 서버 기준으로 박히므로, 확인하지 않은 채 누르지 않는다.',
          '연도 미상 호는 갱신 확인 범위를 계산할 수 없다. 원본 스캔에서 발행연도를 먼저 확인한 뒤 판단한다.',
          '근거를 기록해도 **발행되지는 않는다** — 컷 이미지가 공개 스토리지에 올라가야(현대화 → 업로드) 발행 버튼이 열린다.',
        ],
        seeAlso: [{ label: '학습자 복원 만화 서가', href: '/comics/restored' }],
      },
