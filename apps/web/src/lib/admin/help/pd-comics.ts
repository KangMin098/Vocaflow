  ...
  'pd-comics': {
    ...
    screen: {
      ...
      steps: [
        ...
        },
        {
          ...
          detail:
            '취득 → 복원 → 컷 분할 → 대사 추출. 한 호출이 한 호의 한 단계다. 이미지 작업은 로컬 CLI(ffmpeg·tesseract)라 dev 서버에서만 돈다.',
          done: '각 행 stepper 가 대사 추출까지 채워지고 컷 수가 0 이 아니다.',
        },
        {
          title: 'Claude Code 로 대사 정제·현대화',
          detail:
            'OCR 원문은 그대로 못 쓴다(실측 통과율 33%). 정제·말풍선 좌표·모던 대사는 사람 판단이 필요해 Claude Code 가 맡는다. 테스트 · 모니터 탭 도움말의 드레인 절차 참조.',
    ...
    tabs: {
      '소스 · 대량 적재': {
        ...
        cautions: [
          '한 번에 최대 50건만 적재된다 — 그 이상 선택하면 초과분은 조용히 잘린다.',
      ...
      '테스트 · 모니터': {
        ...
        fields: [
          ...
          },
          {
            label: '작업 방식',
            detail:
              'hOCR = 소스가 준 텍스트 레이어를 그대로 썼다는 뜻이고, tesseract OCR = 이미지뿐이라 컷을 직접 읽었다는 뜻이다. 어느 쪽인지에 따라 오탈자 성격이 다르다.',
          },
          {
            label: '이 이슈 한 단계 진행',
