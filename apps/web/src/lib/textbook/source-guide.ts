...
export const SOURCE_GUIDE: Record<SourceKey, SourceGuide> & Record<string, SourceGuide> = {
  ...
  // ⚠️ 「물음으로 시작해 답으로 끝납니다」라고 적혀 있었으나 **실측과 어긋났다.**
  //   지금 담긴 것은 기사 전문이 아니라 Crossref 초록이고(153편 전부 98~165어),
  //   그중 51%가 「이 글에서 다루겠다」는 예고를 품고 34편은 답 없이 예고로 끝난다
  //   (2026-09-06 판정에서 그 34편이 전부 `fragmentary` 로 반려됐다).
  //   학습자에게 하는 약속이므로 지킬 수 있는 만큼만 적는다 — 전문을 수확하면 그때 고친다.
  frym: {
    label: '어린이 과학지',
    says: '과학자가 쓰고 어린이·청소년 심사위원이 읽은 논문의 요약입니다. 한 편이 짧습니다.',
  },
  ocean_facts: {
    label: '바다',
