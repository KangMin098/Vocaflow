...
import { SOURCE_QUEUES, isSourceBreakdownReason, type SourceQueue, type SourceBreakdownReason } from './source-operations'
...
export const QUERY_KEYS = ['view', 'issue', 'sort', 'q', 'source', 'queue', 'reason'] as const
...
export function parseSourceWorkspace(params?: Params): SourceWorkspaceState {
  ...
  return {
    ...
    queue: oneOf(read('queue'), SOURCE_QUEUES, 'p0'),
    reason: isSourceBreakdownReason(read('reason')) ? read('reason') as SourceBreakdownReason : null,
  }
}
