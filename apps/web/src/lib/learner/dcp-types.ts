...
export const CHOICE_DCP_TYPES = [
  ...
  // 만들 때는 `grade_dcp_item` 배열에 더하는 것을 빠뜨려 **정답을 맞혀도 오답으로 보였다**
  // (실측 프로브: 정답 제출 → `Unknown type`). 마이그레이션 `20260822140000` 이 두 배열에
  // `mood` 를 더해 계약 셋(파서·화면·채점)이 다 갖춰졌으므로 여기로 옮긴다.
  //
  // ⚠️ 같은 어긋남이 세 번째였다. `dcp-grade-records.integration` 이 이제 **재생용 선택지
  //   유형을 전부 실제로 제출해 본다** — 목록이 또 갈리면 그 테스트가 먼저 빨개진다.
  'mood',
  ...
  // ── 장문 ① 설명문 41~42 (2026-08-22) ────────────────────────────────
  // 같은 선택지 모양. ⚠️ `long_vocab` 은 **지문을 고쳐서 저장한다** — 낱말 하나를 문맥과
  //   어긋나게 바꿔 놓은 판이 학습자가 보는 지문이고, 선택지는 그 지문에서 따온 구절이다.
  //   원본을 저장하면 학습자가 찾을 것이 없다(적재기가 포함 여부를 검사한다).
  'long_title',
  'long_vocab',
] as const
