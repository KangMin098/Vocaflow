...
//
//   ① 히어로 "마음에 자리잡은 단어 **0개**"
//      `user_stats.known_word_count` 를 읽는데, 그 값을 채우는 유일한 함수
//      `refresh_user_known_word_count(uuid)` 를 **아무도 호출하지 않는다**
//      (전 리포 grep: `lib/srs/flush-actions.ts` 한 곳뿐이고 그마저 실행 경로 밖).
//      실제로는 단어 252개를 들고 있는 계정에 0을 띄웠다.
//      게다가 그 함수의 정의는 `stability >= 21` 이라, 채워졌더라도 신규~중급
//      학습자에게는 **몇 달 동안 0** 이다. 회고 화면의 주인공이 될 수 없는 수치다.
//
//   ② 히트맵 "28일 중 **1일** 학습"
