...
//
// ── 왜 직접 링크가 한 회차뿐인가 — 전수 확인 (2026-09-13 · 2026-09-15) ──
//
// 「우리가 못 찾은 것」이 아니라 **평가원이 지금 공개하는 것이 그만큼**이다. 다 뒤졌다:
//
//   · `boardID=1500234`(기출문제) — **가장 최근 수능만.** 실측 10행(2026 수능 8과목 + 2025 2행),
//     페이저 markup 없음. 여기 말고 기출 문제지가 있는 게시판은 없다
//   · 나머지 게시판 6개(`1500229`·`1500230`·`1500231`·`1500232`·`1500233`·`5000103`) —
//     `fileSeq` **0건**. 붙은 PDF 는 대리접수 양식·연구보고서(RRO…)·시행계획뿐이다
//   · 모의평가 안내 3쪽(`m=0201`·`0202`·`0203`) — `fileSeq` **0건**
//   · `old.suneung.re.kr`(옛 archive) — **DNS 가 안 풀린다**
//   · `www.kice.re.kr` — 169바이트 셸만 돌아온다(기출 목록 없음)
//
// **그래서 링크 모드는 「올해 수능」 전용이다.** 해마다 한 회차가 새로 생기고 옛 회차는 사라진다.
// 나머지 28회차에 파일 모드는 **대체 수단이 아니라 유일한 길**이다 — 화면 문구도 그렇게 쓴다.
// (역설적으로 이 저장소의 원본 30편이 평가원 현재 공개분보다 완전하다.)
//
