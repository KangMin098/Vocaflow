 ...
 * 인용이 지문 안에 있나.
 *
 * 그대로 `indexOf` 하면 타이포그래피 때문에 샌다 — PDF 에서 뽑은 지문과 사람이 쓴 인용문이 같은
 * 글자를 다른 코드포인트로 적는다(“ vs " · — vs -). 그래서 `normalizeForMatch` 로 접고 본다.
 *
 * ⚠️ **「인용 실패는 전부 지문 잘림과 겹친다」는 하루 만에 거짓이 됐다.**
 *   2026-09-15 실측: 못 찾는 11건이 **전부** `body_ok = false` 였다 — 그래서 「지문을 고치면
 *   같이 풀린다」고 적었다. 2026-09-16 에 추출기(단 나누기)가 고쳐지자 `body_ok = false` 는
 *   213 → 112 로 줄었는데 **인용 미정착은 11 → 40 으로 늘었고, 그중 36이 지문이 온전한 문항**이다.
 *
 *   원인은 매칭기가 아니라 순서다 — **지문이 새로 뽑히면 그 지문으로 쓰인 분석의 인용이 죽는다.**
 *   원문을 고치는 것은 분석을 무효로 만드는 일이고, 그 문항들은 `--redo` 로 다시 분석해야 한다.
 *   이 화면이 있는 이유가 정확히 이것이다: 원천을 고친 뒤 **무엇이 따라 깨졌는지**가 보여야 한다.
 */
