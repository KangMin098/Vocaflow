...
/**
 * **7축은 전부 종이에서도 잴 수 있는 것이다.**
 *
 * 해설 보유율 · 해설 길이 · 오답 배제 · 원문 인용 · 유형 수 · 지문 어수 · 선택지 수 —
 * 일곱 축 어느 것도 종이책이 못 하는 일이 아니다. 그래서 지수 1.200 을 넘겨도 그 문장의 뜻은
 * **「더 나은 종이책」** 이지 「종이가 못 하는 것을 한다」가 아니다.
 *
 * 이 제품이 종이와 다른 자리(개인별 복습 일정 · 오답 재출제 · 수준 맞춤 배본 · 즉시 채점)는
 * 벤치마크 밖에 있고, **관측이 있어야 잴 수 있다.** 그래서 화면은 그 관측 수를 그대로 적는다 —
 * 「종이가 못 하는 것을 우리는 한다」고 **주장하지 않는다.** 관측이 없으면 그것은 설계도지 사실이 아니다.
 */
export interface PlatformGap {
  /** 기출 문항을 학습자가 실제로 푼 횟수. 개인별 일정·오답 재출제의 유일한 근거다. */
  itemAttempts: number | null
  /** 조판된 권 수 — 수준 맞춤 배본의 재료. 있어도 배정 기록이 없으면 배본은 안 일어난 것이다. */
  renderedVolumes: number | null
  itemAttemptsError: string | null
}

/**
 * 문항 하나의 정답률을 쓸 만하게 추정하는 데 필요한 시도 수 — **81**.
 *
 * 근거를 지어내지 않는다. 이 저장소가 이미 쓰는 합격선에서 나온다:
 * `csat_stage_gates` 의 `item_accuracy` 임계가 **0.65~0.70** 이다. 그 근처의 비율 p 를
 * 95% 신뢰수준에서 ±0.10 으로 잡으려면
 *
 *   n = p(1−p) · (1.96 / 0.10)²  =  0.70 × 0.30 × 384.16  ≈  80.7  →  **81**
 *
 * 즉 시도가 81회보다 적으면 그 문항이 게이트를 넘는지 **말할 수 없다**. 개인별 복습 일정도
 * 오답 재출제도 문항 정답률 위에 서 있으므로, 이 수가 네 축 전부의 **필요조건**이다.
 *
 * ⚠️ **필요조건이지 충분조건이 아니다.** 전체 시도가 81을 넘어도 그것이 한 문항에 모여야
 *   그 문항을 잴 수 있다. 여러 문항에 흩어지면 여전히 아무것도 못 잰다 — 화면이 그렇게 적는다.
 */
export const MIN_ATTEMPTS_FOR_ACCURACY = 81

/**
 * 플랫폼 축을 지금 잴 수 있는가.
 *
 * 관측이 모자라면 **주장할 수 없다.** 「종이가 못 하는 것을 우리는 한다」는 관측 없이는
 * 설계도이지 사실이 아니다.
 */
export function platformMeasurable(g: PlatformGap): boolean {
  return g.itemAttempts != null && g.itemAttempts >= MIN_ATTEMPTS_FOR_ACCURACY
}
