...
vi.mock('@/lib/supabase/keyset-select', () => ({ keysetSelect: read }))
...
it('권한 때문에 빈 분석 조회를 배포 결함으로 캐시하지 않는다', async () => {
  read.mockImplementation(async (_query, _cursorOf, label) => label === '해부 문항' ? [{ id: '2026#32', type_id: 'R-BLANK', answer: 5 }] : [])
