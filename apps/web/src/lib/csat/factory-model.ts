...
export interface StageGauge {
  ...
  /**
   * 이 수가 **언제 잰 것인지** — 미리 계산된 집계표에서 읽은 값에 붙인다.
   *
   * 추정(`approx`)과 다르다: 값은 정확하지만 **시점이 지금이 아닐 수 있다.** 드레인을 막
   * 돌린 직후에는 아직 반영되지 않았을 수 있고, 그것을 모르면 "왜 안 늘었지" 로 읽는다.
   * (공정 ⑥ 해설이 읽는 `textbook_shelf_inventory_mv` 는 30분마다 갱신된다.)
   */
  note?: string
  /**
...
export const FACTORY_STAGES: readonly StageDef[] = [
  ...
  },
  {
    ...
    //   `factory.ts` 의 눈금 이름을 「게이트가 있는 밴드 중 지문 보유」→「지문으로 채우는
    //   밴드 중 재고 보유」로 고쳤다. 의도된 변경이었다 — S5 의 합격선은 `listening` 하나뿐이라
    //   **지문을 수확해서 채우는 칸이 아닌데** 옛 셈법이 그 칸을 늘 빨갛게 세웠기 때문이다.
    //
    //   그런데 여기 `gateGauges` 는 옛 이름 그대로였고, 이름이 안 맞으면 `state()` 가
    //   「못 잼」 눈금을 꽂도록 돼 있다. 그래서 ④는 **재고 87,556편을 두고 「못 잼」**으로
    //   서 있었다 — 안전장치는 제대로 작동했고, 아무도 그 경보를 끄지 않았을 뿐이다
    //   (`factory-model.test.ts` 가 그동안 **실패 상태로 남아 있었다**).
    //
    //   게이트 문장도 함께 고친다. 문장만 옛 뜻으로 두면 눈금과 다시 따로 놀고,
    //   그 어긋남을 잡는 장치는 **이름**뿐이라 문장 쪽은 아무도 안 본다.
    gate: '지문으로 채우는 밴드에 재고가 하나라도 있는가',
    gateGauges: ['지문으로 채우는 밴드 중 재고 보유'],
