...
/**
 * 예문이 표제어를 실제로 담고 있나 — 굴절·구를 감안한 판정.
 *
 * 어간 자르기만으로는 **불규칙 굴절**을 놓친다 (come/came · say/said · give/gave).
...
export function exampleContainsHeadword(
  ...
  // 구(phrase) — 낱말이 따로 떨어져 나타나므로("give it up") 통짜 포함으로는 잡히지 않는다.
  // 구를 이루는 **모든 낱말**이 문장 안에 있으면 그 구가 쓰인 것으로 본다.
  if (w.includes(' ')) {
    const tokens = w.split(/\s+/)
    const head = tokens[0]!
    const rest = tokens.slice(1).filter((p) => p.length >= 2)
    // 머리 동사는 굴절할 수 있다("brought about") — 나머지 조각은 그대로 나타난다.
    const headOk =
      ex.includes(head) ||
      inflections.some((f) => f.length >= 2 && ex.includes(f.toLowerCase()))
  ...
  // 굴절 — 어간 기준 (drive/driving · study/studies · run/running)
  const stem =
    w.length > 4 ? w.replace(/(e|y|ie)$/, '').slice(0, Math.max(4, w.length - 2)) : w
