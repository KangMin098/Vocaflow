  ...
  },
  {
    ...
    build: (p) =>
      recipe({
        ...
        // ⚠️ (주제 내 중심도)로 정렬하면 표제어는 훨씬 좋아지지만
        // ()
        // **파라미터 스윕이 음식·동물·스포츠 세 주제에서 암기 장치 열위를 잡는다.**
        // 연상 데이터가 최빈출어에 몰려 있어서다(top1k 216개 중 165개 보유). 정렬을 바꾸면
        // 그 낱말들이 뒤로 밀리고,  로도 못 넘는다 — 세 번 시도해 세 번 같았다.
        // 순위 자체는 백필해 뒀으므로(28,079행) **연상 커버리지가 채워지면 그날 바꾸면 된다.**
