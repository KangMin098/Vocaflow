  ...
  },
  {
    ...
    build: (p) =>
      recipe({
        ...
        //   · **주제 내 중심도(`rank_in_category`) 정렬 → 이것이 답이었다.** 다만 정렬만
        //     바꿔서는 세 번 다 같은 벽에 막혔다(음식·동물·스포츠 암기 장치 열위). 원인이
        //     로직이 아니라 **데이터**여서다 — 흔한 낱말을 뒤로 밀면 연상도 같이 밀린다.
        //     그래서 그 세 주제에 연상 117건을 채우고(`32eaae4b`) 넘었다: **스윕 55/55**.
        //     지금 표제어는 `aircraft · arrival · radar · cargo · jet · runway · cockpit`.
        //
        // ⚠️ 이 블록은 한동안 "`rank_in_category` 가 28,079행 전부 NULL 이라 선행 과제다" 라고
        //   적혀 있었다. 바로 다음 커밋이 백필하고 정렬까지 켰는데 주석만 안 따라왔다 —
        //   **읽는 사람에게는 이미 넘은 벽이 아직 못 넘은 벽으로 보였다.** 실측 2026-08-31:
        //   28,067/28,775 = **97.5% 채워져 있고 아래 `order_within` 이 실제로 그것을 쓴다.**
        //   (나머지 708행은 백필 뒤에 새로 들어온 것이라 순위가 없다 — 정렬에서 뒤로 간다.)
        filters: { v_level_min: 3 as never, v_level_max: 11 as never },
        objective: { kind: 'count', n: p.count ?? 500 },
        group_by: 'topic',
        group_order: 'size_desc',
        group_cap: p.group_cap ?? 150,
        // **주제 내 중심도로 정렬한다** — 위 블록의 결론. 빈도순으로 되돌리지 말 것:
        // 되돌리는 순간 `round · total · bank · park · career · clock` 이 앞자리로 온다.
        order_within: 'topic_rank',
