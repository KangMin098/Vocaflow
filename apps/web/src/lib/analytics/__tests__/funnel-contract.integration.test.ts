...
describe.skipIf(skipIfNoEnv)('퍼널 단계 계약 (실 DB)', () => {
  ...
  it('DB CHECK 제약의 단계 목록과 TS 유니온이 정확히 같다', async () => {
    ...
    for (const ev of TS_EVENTS) {
      const { error: insErr } = await db
        .from('funnel_events')
        .insert({ event: ev, anon_id: '__contract_probe__', surface: 'contract-test' })
      if (!insErr) accepted.push(ev)
    }

    // 정리 — 프로브 행은 남기지 않는다.
    await db.from('funnel_events').delete().eq('anon_id', '__contract_probe__')
  ...
  it('목록에 없는 단계는 DB 가 거부한다 — 오타가 새 단계를 만들지 못하게', async () => {
    const { error } = await db
      .from('funnel_events')
      .insert({ event: 'teacher_hub_veiw', anon_id: '__contract_probe__' })
    expect(error, '오타 이벤트가 통과했다 — CHECK 제약이 열려 있다').not.toBeNull()
    await db.from('funnel_events').delete().eq('anon_id', '__contract_probe__')
  })

  it('주체(user_id·anon_id)가 둘 다 없으면 거부한다', async () => {
    const { error } = await db.from('funnel_events').insert({ event: 'visit' })
    expect(error, '주체 없는 행이 들어갔다 — 퍼널에 이을 수 없는 기록이다').not.toBeNull()
  })
})
