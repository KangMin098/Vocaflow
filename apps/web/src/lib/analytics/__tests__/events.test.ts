...
describe('허용 이벤트 목록', () => {
  ...
  const DEFINED: PublicEventName[] = [
    ...
    // 2026-09-19 — `/hub` 재설계의 서명 관측. 속성은 고른 수·최대 수(숫자)뿐.
    'hub_curve_interacted',
    // 2026-09-19 — `/dashboard` 「기억의 지층」 층 펼치기. 속성은 층(닫힌 열거형)·낱말 수(숫자)뿐.
    'retrospect_layer_opened',
    // 2026-09-19 — `/text/new` 저장. 속성은 커버리지(정수)·챕터 수뿐 — 원문·제목 없음.
    'text_created',
    // 2026-09-15 — 기출 해설의 「지문 지도」. 이 화면의 전제는 «근거를 눌러 가며 지문 위에서
    // 풀이를 재구성한다» 인데, 그 전제는 **몇 개를 여는지**로만 확인된다. seq 가 1에서 멈추면
