...
import type { RecentSessionRow } from './reads'
import type { DictationConfig, DictationItem, WordResult } from './types'

// ── 읽기 4종은 `./reads` 로 이사했다 ────────────────────────────────
//
// 이 파일은 완주 시 `recordGameScore`(`'use client'`)를 부르므로 파일 전체가
// `'use client'` 다. 서버 컴포넌트가 여기서 함수를 꺼내면 **함수가 아니라 클라이언트
// 참조**가 오고, 부르는 순간 `TypeError` 로 화면이 통째로 죽는다
// (2026-09-06 `/dictate` 허브가 그렇게 죽었다).
//
// 호출부를 흩지 않으려고 여기서 다시 내보낸다. **서버에서 쓸 때는 `./reads` 를 직접
// import 할 것** — 이 줄을 통해 가져가면 같은 자리에서 다시 죽는다.
export {
...
export type { DictationOverview, WeaknessRow, RecentSessionRow } from './reads'
