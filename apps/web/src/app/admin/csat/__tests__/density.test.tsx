...
const BUDGET: Record<string, { chunks: number; chars: number }> = {
  ...
  //
  // 카탈로그 **+품목 층**(2026-09-23 · DD-76): 116 · 715 → **142 · 1,100**.
  //   이 화면은 「이 시리즈의 어느 권이 있나」까지만 말했다. 그런데 한 권을 냈다고 그 유형이
  //   끝나지 않는다 — 시중 출판사는 제도·시기·경쟁·재고를 보고 **다음 라인을 계속 발의한다.**
  //   그 층이 없으면 마지막 권을 찍는 날 화면이 초록으로 끝나고, 다음에 무엇을 할지 아무도
  //   모른다. 늘어난 자리 셋:
  //     · 시리즈 × (생애 자리 · 낸 권/단 · 옛 규격 권)         +약 12
  //     · 시리즈 × (왜 생겼나 — 계기·근거·날짜 · 다음 한 걸음) +약 8
  //     · 시장 칸 표 4행(우리/시장 · 빈 자리 또는 막힌 이유)   +약 10
  //   대신 **빼기도 했다**: 「안 만드는 것」 구획을 시장 칸 표 안으로 들이고(같은 물음의
  //   양면이다), 내신이 두 목록에 같은 이유로 적혀 있던 것을 시장 칸 행 하나로 합쳤다(−3).
  //   이어서 이 화면이 **⑨ 운영·개정**을 맡게 되며 제목과 부제가 한 줄씩 길어졌다(+26자) —
  //   「끝이면서 다음 바퀴의 시작」이라는 것을 제목이 말하지 않으면 메뉴의 ⑨ 와 안 이어진다.
  // 2026-09-24 — **걸음 머리띠**(공장 지도 ← · 걸음 번호 · 한 줄 · 들어오는 것 → 나가는 것 · 낱말 툴팁 ·
  //   앞뒤 걸음)가 모든 단계 화면 맨 위에 섰다. 처음 온 사람이 「여기가 어디고 다음은 어디인가」를
  //   코드 없이 읽게 하는 것이 그 일이다. 옛 제목(공정 번호 + 질문 한 줄)보다 덩어리가 는다 — 화살표를
  //   CSS 로 그리고 겹치는 소제목을 지워 +20 → +10 으로 줄였고, 남은 10 만 올린다. 실측 152.
  //   글자는 +148 — 다른 단계 화면은 머리띠가 공정 계약 세 줄을 대신했지만(접힘) 카탈로그는 계약이
  //   없던 화면이라 머리띠 글자가 그대로 더해진다. 실측 1,278.
  카탈로그: { chunks: 152, chars: 1280 },
  // 현황판 **+매대 줄**(2026-09-23 · DD-77): 99 · 484 → 100 · 503(+19자 한 줄).
  //   공정이 ⑧ 에서 끝나던 동안 이 화면은 여덟 칸이 통과하는 날 초록으로 끝났다 — 그런데
  //   그 시점의 실측은 「나간 19권 중 결재 1 · 학습자가 고른 권 3」이다. 라인·연구소와
  //   나란히 **매대**를 한 줄로 적어 「더 찍어서 풀리지 않는다」를 말하게 했다.
  현황판: { chunks: 110, chars: 510 },
  ...
  // 2026-09-24 — 걸음 머리띠의 한 줄 · 흐름 · 앞뒤 링크 글자(+46)만큼 올린다. 공정 계약 세 줄은
  //   「자세히 — 운영자용」 안으로 접혀 이 셈에서 빠졌다(접힌 것은 세지 않는다). 실측 1,496.
  집필: { chunks: 226, chars: 1500 },
