// apps/web/src/app/admin/textbook/sources/page.tsx
// 원문 적격 — 교재에 실을 수 있는 원문인가를 일곱 축으로 판정한 결과.
//
// **조작 버튼이 없다.** 판정은 재고 전체를 훑는 일이라 웹 요청 시간 안에 안 끝난다
// (실측 9~35초). 스캔은 Claude Code 로 돌리고 이 화면은 그 결과를 읽는다 —
// 절차는 화면 도움말(`lib/admin/help/textbook.ts` 의 `textbook/sources`)에 있다.
...
export default async function AdminTextbookSourcesPage() {
  await requireAdmin('/admin/textbook/sources')
