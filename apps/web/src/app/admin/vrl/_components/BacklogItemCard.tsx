...
export function BacklogItemCard({ item }: BacklogItemCardProps) {
  ...
  const ringClass = item.isCorePain
  ...
  return (
    <article
      ...
      className={`flex flex-col gap-2 rounded-[var(--r-lg)] border border-[var(--bd)] bg-[var(--bg)] p-3 shadow-[var(--sh-sm)] transition-shadow duration-[var(--dur-normal)] hover:shadow-[var(--sh-md)] ${ringClass}`}
    >
