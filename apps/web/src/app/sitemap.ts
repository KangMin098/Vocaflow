 ...
 * 그러면 도서를 발행해도 **재배포 전까지 사이트맵이 132개 그대로**이고,
 * 콘텐츠를 DB 에서 읽도록 만든 의미가 통째로 사라진다.
 *
 * ⚠️ **빌드 라우트 표의 `○` 로는 확인할 수 없다.** `○` 는 ISR 과 완전 정적을 구분하지 않는다 —
 *    revalidate 가 붙은 `/` · `/pricing` 도, 붙지 않은 `/about` 도 똑같이 `○` 로 찍힌다.
 *    실제로 붙었는지는 **`.next/prerender-manifest.json`** 에서 본다:
 *      routes['/sitemap.xml'].initialRevalidateSeconds  →  86400 (false 면 굳은 것이다)
 *
