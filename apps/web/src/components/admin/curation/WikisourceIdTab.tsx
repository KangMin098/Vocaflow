// apps/web/src/components/admin/curation/WikisourceIdTab.tsx
// LCP v2.0 Phase 15 — Wikisource page title 직접 입력 + 미리보기
...
export interface WikisourcePreview {
  source: 'wikisource'
  source_id: string
  title: string
  author: string | null
...
interface WikisourceIdTabProps {
  onPickPreview: (preview: WikisourcePreview) => void
}

export function WikisourceIdTab({ onPickPreview }: WikisourceIdTabProps) {
  const [titleInput, setTitleInput] = useState('')
  const [preview, setPreview] = useState<WikisourcePreview | null>(null)
  ...
  async function handleFetch() {
    ...
    try {
      const res = await fetch(
        `/api/admin/library/preview-wikisource?title=${encodeURIComponent(title)}`,
      )
      ...
      startTransition(() => {
        setPreview(data as WikisourcePreview)
      })
    } catch (e) {
      ...
      <div className="flex flex-col gap-2 rounded-[var(--r-md)] border border-[var(--bd)] bg-[var(--bg2)] p-4">
        ...
        <div className="flex flex-wrap items-stretch gap-2">
          <input
            ...
            aria-describedby={error ? 'wikisource-title-error' : undefined}
            aria-invalid={!!error}
          />
        ...
        <p className="font-body text-[11px] text-[var(--t2)]">
          Wikisource 페이지 URL 의 마지막 부분입니다. 공백은 자동으로 <code>_</code> 로 변환됩니다.<br />
          상위 페이지를 입력하면 챕터 sub-page (최대 100개) 가 자동 수집됩니다. 예:{' '}
          <code className="font-mono text-[var(--t2)]">
            https://en.wikisource.org/wiki/<strong>Pride_and_Prejudice</strong>
          </code>
        </p>

        <p className="rounded-[var(--r-sm)] border border-[var(--bd)] bg-[var(--bg)] px-3 py-2 font-body text-[11px] text-[var(--t2)]">
          📜 Wikisource 작품은 대부분 PD (저자 사후 70년 경과) — 하지만 페이지 메타에 저자
          생몰년이 노출되지 않아 자동 KR 저작권 룰이 적용되지 않습니다. 큐레이션 단계에서
          작품을 직접 확인 후 강제 publish 가 필요합니다.
        </p>
      </div>
