...
export function ConfirmButton({ visible, disabled, onClick }: ConfirmButtonProps) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`mt-4 inline-flex cursor-pointer items-center gap-2 rounded-[var(--r-md)] border-none px-6 py-2.5 font-display text-[13px] font-[700] text-white shadow-[0_4px_12px_rgba(59,130,246,0.25)] transition-all duration-[var(--dur-normal)] ${
        visible
