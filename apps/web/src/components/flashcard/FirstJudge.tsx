...
export function FirstJudge({ visible, onJudge }: FirstJudgeProps) {
  return (
    ...
    >
      ...
      <button
        onClick={() => onJudge('yes')}
        className="flex flex-1 items-center justify-center gap-2 rounded-[var(--r-lg)] border border-[rgba(34,197,94,0.25)] bg-[var(--bg)] px-4 py-3.5 font-display text-[14px] font-[600] text-[var(--success)] transition-all duration-[var(--dur-normal)] hover:-translate-y-0.5 hover:border-[var(--success)] hover:bg-[var(--success-light)] hover:shadow-[var(--sh-md)]"
      >
        <span className="text-[18px]">💡</span>
        <span>떠올렸어요</span>
      </button>
