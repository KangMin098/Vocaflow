...
// 다이얼로그를 띄우는 컴포넌트가 28개인데, 다이얼로그를 **여닫고 원래 자리가
...
test.describe('팝업을 닫으면 제자리', () => {
  ...
  test('커버리지 고지 — 이 스펙이 무엇을 안 재는지 숨기지 않는다', () => {
    // 다이얼로그를 띄우는 컴포넌트는 **29개**다(2026-08-25 실측 28 + 공통 껍데기 1).
    // 전부를 여기서 열 수는 없다. 다만 **못 여는 이유를 하나씩 적어 둔다** —
    // 이유 없는 면제 목록은 커버리지가 아니라 눈속임이고, 목록이 조용히 자란다.
    //
    // ⚠️ 2026-09-23(DD-68 · tines-mapping §28)부터 **`role="dialog"` 를 직접 그리는 파일과
    //    이 목록은 더 이상 같지 않다.** 팝업 껍데기가 `components/ui/Dialog.tsx` 하나로 모였고,
    //    학습자·관리자 팝업 대부분은 그 부품에 내용만 넘긴다. 그래서 여기서 세는 단위는
    //    "파일이 role=dialog 를 그리는가" 가 아니라 **"이 컴포넌트가 팝업을 띄우는가"** 다.
    //    ①~⑦ 축은 그대로다 — 껍데기가 공용이라도 트리거·복귀는 화면마다 다르기 때문이다.
    ...
    const EXCLUDED: Record<string, string> = {
      'ui/Dialog': '공통 껍데기 — 자체 화면이 없다(아래 REACHABLE 이 전부 이걸 쓴다)',
