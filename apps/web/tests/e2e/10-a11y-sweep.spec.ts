...
const TOUCH_BASELINE: Record<string, number> = {
  // ── 2026-08-25 갱신 (줄이는 방향) ────────────────────────────────
  // scripts/ux-bench 전수 계측이 화면별로 어떤 컨트롤이 몇 px 인지를 이름과 함께
  // 뱉으면서, 남아 있던 위반이 한 회차에 정리됐다. 실측 전/후:
  //   /plan 8 → 0 (자료 유형 칩·난이도 칩·설정 서랍 행 7종이 36~40px 이었다)
  //   /text/new 3 → 0 (되돌아가기·테마 전환·구조 토글 2종 · 36px)
  //   /pairflip 2 → 0 (모드 선택 min-h-[40px])
  //   /wordvault 1 → 0 (세그먼트 탭이 **폭** 43px — 세로만 44 로 맞춰 뒀다)
  //   /library/scripts 1 → 0 (주제 전환 min-h-[36px])
  // 이 스윕이 못 보던 것도 있었다: 선택자에 **a[href] 가 없어** 링크 위반은
  // 원리적으로 안 잡혔다(/dashboard 66x18 · /hub 310x40 · /diagnostic/history 126x20).
  // 그쪽은 ux-bench 가 잡고 고쳤다 — 여기 베이스라인에는 원래 없던 값이라 변화가 없다.
  //
