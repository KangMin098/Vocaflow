# Vocaflow — 에이전트 컨텍스트 상세 (원문 보존)

> 2026-09-17 `CLAUDE.md`(v06.34, 495줄)를 `AGENTS.md` 단일 출처로 나누면서 200줄 한도에 들어가지 않은
> **배경·사례·표 원문을 그대로** 옮겨 둔 곳이다. 규칙의 정본은 [AGENTS.md](../../AGENTS.md) — 둘이 다르면 AGENTS.md 가 우선하고,
> 이 파일은 "왜 그런 규칙인가" 를 찾을 때 읽는다. 새 규칙을 여기에 먼저 쓰지 않는다.
> 원문 링크는 저장소 루트 기준(`./docs/…`)으로 적혀 있다.

---

## §첫인상 — 원문

> (DD-66, 2026-09-21: 첫인상·이탈 방지 규칙 중 I1–I4 · I6 · I8 · D1 · D4 · D5 를 삭제했다. 남는 규칙은 I5 · I7 · D2 · D3.)
...
| D4 · D5 | (DD-66 로 삭제 — D5 회귀 `activation-path.test.ts` 도 함께 삭제) | |

### 모션

(모션 예산 상한 · 학습 중 화이트리스트 7종 · 「항상 금지」 목록은 DD-66 으로 삭제. 토큰은 [DESIGN_SYSTEM](../DESIGN_SYSTEM.md) §3.)

`prefers-reduced-motion` 은 **끄기가 아니라 낮추기** — 이동·회전·스케일은 제거하되
**페이드는 남긴다**(전부 0.01ms 로 죽이면 무엇이 바뀌었는지 알 수 없다).

(2026-09-06 실측 기록: "루프 애니메이션 금지" 로 적었더니 정당한 `Loader2` 로더 20여 곳이 걸렸다 —
규칙이 틀렸지 코드가 틀린 게 아니었다.)

### 설치된 외부 디자인 스킬과의 관계

`.claude/skills/` 에 13개 취향 스킬이 있었다(2026-09-18 부터 활성 2개).
**발명 엔진(§A–§G)** 은 **[.claude/skills/vocaflow-design/SKILL.md](./.claude/skills/vocaflow-design/SKILL.md)** 에 있고,
필요할 때 참고한다(선택). 제약 절 Part 2 §0–§8 은 DD-65, 남은 디자인 금지·게이트는 DD-66 으로 삭제했다.
...
## §to_regclass — 없는 테이블을 읽는 RPC 경위 (원문)
...
> 이후 변경은 [docs/CHANGELOG.md](../CHANGELOG.md) Unreleased 에만 적는다.

### 이번 세션 (Unreleased)
- **학습자 `/csat` 재설계 — 라우트 7 → 3, 「고르는 화면」에서 「굴러가는 루프」로** (`docs/csat-learner-brief.md`). 홈 = 오늘의 세션 카드 한 장 · 세션 = 한 문항 한 화면(①풀기 ②이해 ③한 줄) · 기록 = 숫자 셋. 문제지를 쪽째 그리지 않고 **문항 글만 브라우저에서 뽑아 18px 로 다시 흘려 넣는다**(reflow — 서버에는 해시 64자만) · 분석은 그 문장 안에서 열린다. Gate 1(전 회차 802): 경계 100% · 문장 앵커 98.9%(순번이 아니라 **길이열 정렬** — DB 지문의 쪽 번호가 순번을 밀었다) · Gate 2(375px) 19/19 · Gate 3(프로덕션 3G) 첫 문항 탭 1 · 1.3초 · Lighthouse 100. 분석 뷰는 `/admin/kice/*` 로 이전, drill·overlay 삭제. 풀이 기록은 기기에 먼저 쓰고 서버(`csat_session_attempts` · `csat_review_queue`)로 올린다 — 복습 큐는 합친 풀이를 **다시 돌려** 계산(두 기기가 갈라지지 않음). 마이그레이션 `20260917190000`(계측) · `20260917200000`(기록) 적용
- **학습자 앱 디자인 방향 재설정 — v07 「주묵(朱墨) 판면」** (`docs/design/00~05`). 의뢰서의 진단 셋 중 **하나가 틀렸다**: 학습자 표면의 `violet-*`·`indigo-*` 는 **0회**이고 보라는 `/admin` 전용 액센트다(그대로 고쳤으면 엉뚱한 데를 손댔다). 실측으로 가른 진짜 원인 셋 — ① **한글에 웹폰트가 없었다**: 폰트 4종이 전부 `subsets:["latin"]` 이라 `/dashboard` 텍스트 노드 144개 중 한글 108개(**75%**)가 100% 라틴 전용 스택을 선언하고 OS 기본꼴로 떨어졌다(27개는 모노 폴백). 소스의 `font-display` 1,193 + `font-body` 848 = **2,041번의 지정이 한글에는 전부 무효**였다. ② 형태 언어가 한 종류 — `border-[var(--bd)]` 681 · `rounded-md` 484 이고 공용 `Card` 를 쓰는 파일은 447개 중 **5개**뿐이었다(시스템이 있어서 같은 게 아니라 **없어서** 같은 기본값으로 수렴). ③ 색이 없었다 — 글자색 **136/144(94%)** 가 같은 잉크 한 색의 알파 3단계, 액센트를 글자로 쓴 노드 0개, `public/` 이미지·로고 자산 **0개**. → 3안 시안 비교 후 B 확정. **Hahmlet + IBM Plex Sans KR** 도입(클래스 이름은 그대로 두고 스택만 교체 = 마크업 0줄로 70 라우트가 따라온다 · 한글은 `preload:false` 필수 — `unicode-range` 수백 조각 전량 preload 는 화면을 안 깨고 느리게만 만든다) · 그림자 8종을 **헤어라인 링**으로(컴포넌트 235곳 무수정으로 카드가 판면이 된다) · radius 두 벌(6~38px)을 한 램프(2~8px)로 · **주묵 `#C0392B`** 를 면적 브랜드 색으로(라이트 5.20:1 · 다크 `#E0705C` 5.27:1) · **오답을 빨강에서 뺐다**(`--srs-1`·`--learn-error`·`--slot-error-*` — 토큰이 「정답률 빨간 글씨 압박 금지」의 구멍이었다) · 이모지 제거(세션 머리 6 · 채점 4 · 판정 2) · **`DecayUnderline`**(밑줄 두께 = R(t) — FSRS 없이는 못 긋는 선이라 폰트를 베껴도 이건 못 베낀다) · 회귀 `learning-tone` 18 → **24**(변이 3종 전부 잡힘). 계측기 2종 신규(`scripts/design/capture-learner.mjs` · `measure-identity.mjs`)
- **평가원 문제지에 해설을 얹는다** (`/csat/overlay`) — 원본을 우리 서버에 복제하지 않고 오버레이만. 학습자가 떨어뜨린 PDF 를 `crypto.subtle` 이 그 자리에서 SHA-256 해 **해시 64자만** 보내고, PDF.js 가 **브라우저 메모리에서** 렌더한 위에 좌표로 상자를 얹는다. 프록시 없음 · **「해설 얹힌 PDF 내려받기」 없음**(변형 복제물이 된다) · 파일을 IndexedDB 에도 안 남긴다. 좌표는 `pdfjs-dist` 로 뽑았다(Poppler 가 아닌 이유 = 그리는 쪽도 PDF.js 라 좌표계가 같다) — 문제지 **30개 · 문항 번호 45/45 · 사정권 ①~⑤ 840/840 · 고아 0**. 단 경계 한 숫자에 전부 달렸고 **두 번 틀렸다**(x=400 못박기 → 2019 오염 · 두 여백의 중간 262 → 전 회차 66.3%). 둘 다 「고아 0」이라 조용했다 — 기호는 주인을 찾았고 다만 **틀린 주인**이었다. 경계 회귀를 **앵커 데이터까지 확장**(문자열 값은 `exam_id`·`sha256` 뿐 — 인용 한 줄을 넣으면 잡힌다). 마이그레이션 `20260913013946`(`csat_exams.paper_form` — 선지 순서가 형마다 달라 자리로 가리키려면 대조가 필요하다)
- **지문이 첫 `①` 에서 끊겨 있었다** — `choiceStart()` 가 유형을 모른 채 줄머리 `①` 을 선지 블록으로 오인. 지칭·어휘·어법·무관·삽입·도표는 ①~⑤ 가 본문에 박히는 것이 설계인데 거기서 잘렸다. M1809#30 은 958자 중 **106자**(10%)인데 `body_ok=true` — `suspectBody` 아홉 신호가 전부 「지문에 남은 흔적」을 보므로 **깨끗하게 잘린 조각을 하나도 못 잡고**, 인용 대조 게이트도 같은 잘린 지문을 대조해 함께 통과했다. 지문 **26문항 복원**(줄어든 것 0) · 빠진 검사 추가(번호 중앙값의 0.5 미만 · 적중 6·오탐 0) · 남은 6문항은 단 나누기 실패라 `body_ok=false` 로 격리 · **11문항 재분석**(게이트 PASS) · 회귀 T16·T17(변이로 19건 잡힘). 곁가지로 **적재의 「재실행 안전」이 거짓이었던 것**도 고쳤다 — jsonb 가 키 순서를 바꿔 저장하는데 `JSON.stringify` 로 비교해 전량 적재마다 802행이 늘었다(813 → 22 → **0** 으로 증명)
- `/admin/db` 가 읽는 화면에서 **보는 화면**이 됐다 — 라이브 계기판(15초 폴링 · 신호 6종 임계는 전부 `pg_settings` 실측) + 경보 표 + **조치 실행**(허용 목록 7종: safe 4 · guarded 3은 사유 필수. `VACUUM FULL`·`DROP INDEX` 는 여전히 SQL 복사만). 마이그레이션 `20260906190000`(`admin_db_health_live`) · `20260906190500`(`db_health_action_log` + `admin_run_db_health_action`). 실측 전후 가시 텍스트 **9,497 → 2,037자**(-79%) · 가장 긴 덩어리 440 → 48자 · 풍선말 16개로 설명을 옮김. 중간에 지침이 아니라 **내 코드가 틀린 것 셋**이 나왔다 — `cron.job_run_details.jobname` 은 없는 컬럼이라 적용은 성공하고 호출은 42703 으로 죽었고, 실패를 `raise` 로 올렸더니 감사 로그가 함께 롤백돼 실패 5건 중 0건만 남았고(자율 트랜잭션 없음 → `ok=false` 결과값으로 전환), 「30자 넘는 덩어리 = 산문」 규칙은 경보 **제목**을 산문으로 셌다(제목은 짐이 아니라 화물 → 데이터 문자열 제외). 초과 개선으로 평상시 접힌 위 경보 **0 → 5행**(볼 것이 없으면 «지금» 세부를 접고, 생기면 자동으로 편다) · 탭 제목에 상태 표시 · 44px 미만 터치 타깃 10곳·axe 대비 위반 3건 동반 수정 · 회귀 100
- 「변경 금지」 색 목록이 **없어진 색을 지키고 있었다** — 유령 예외 6색(Flashcard gradient, 저장소 0건) 삭제 · 누락 2색(`#5CB8E0` · `#FCD34D`) 추가. 직전 항목에서 PairFlip 그러데이션을 「예외 대상 아님」이라 적은 것은 **정정**한다(지침이 명시적 예외로 두고 있다). 목록 밖 하드코딩 **308건**은 측정만 기록하고 고치지 않는다 — 예외 범위를 먼저 정하지 않으면 전부 오탐이 된다. 문서가 스스로 검증되게 `learning-tone.test.ts` 를 7 → **18**로(예외 색 실재 검사 + 검사 자기무력화 가드)
- D6·모션 예산 회귀 — 붙이려고 실측했더니 **지침 자체가 틀린 것 2건**이 나왔다. 「루프 애니메이션 금지」는 정당한 로더 20여 곳을 걸었고(→ 판정 기준을 「끝나는 상태가 있는가」로), 「트로피 금지」는 CLAUDE.md 범위(진행률 100% 축하)보다 넓었다(→ 되돌림). 실제 위반 3건 고침 — 완료 화면의 빨간 시도횟수 배지(`--error-*` → `--warning-*`) · 하드코딩 `100ms`·`600ms` → 토큰. 회귀 `components/__tests__/learning-tone.test.ts` 7종, 변이 4종 전부 잡힘. 아케이드(`components/game/`)는 예외 명시
- D5(가입 후 첫 학습까지 ≤3전환)를 **회귀로 잠갔다** — 규칙이 지침에만 있고 코드에 닿지 않았다. 실측 **3전환**(①가입→`/hub` ②미진단→`/diagnostic` ③진단완료→`/flashcard/play` 직행). 경로가 세 파일에 나뉘어 한 곳만 바뀌어도 4전환이 되는데 화면은 멀쩡히 뜬다 — `app/__tests__/activation-path.test.ts` 5종이 세 걸음을 잠그고, 변이 검사로 「/hub 경유」·「준비면 경유」 둘 다 잡히는 것을 확인했다 (규칙 D5 와 이 회귀는 2026-09-21 DD-66 으로 삭제)
