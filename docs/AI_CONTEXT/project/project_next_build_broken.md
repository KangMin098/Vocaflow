...
**후속:** ① CI 의 `next build` job 은 PR #41 로 추가됨(✅) ② SWC minify surgical 복원(ort 청크만 제외) — 미착수(불가 확정, 아래) ③ `eslint.ignoreDuringBuilds` 를 다시 false 로 **완료(v06.117, `feat/scriptquiz-chapter-quiz` `448fb96`)** — lint 부채 청산(bookId no-unused-vars ERROR + aria 3건) 후 0 error 확인, 풀 `next build` EXIT 0 검증. ④ `.turbo/*.log` gitignore — 미착수.
