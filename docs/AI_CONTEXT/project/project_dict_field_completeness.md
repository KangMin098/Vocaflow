...
**도서 5권씩 배치 채굴 루프(사용자 계획, 재개 가능)**: 목적=사전만 개선(도서 transient·용량 절약). 단계: (1) `library_seed_catalog`에서 SE seed 5개 pick(필터: `imported_to_books IS NOT true` AND `NOT COALESCE((curation_meta->>'dict_mined')::boolean,false)` AND NOT EXISTS in library_books) → `library_books` 직접 INSERT(status='queued', admin_enqueue_book는 is_admin_or_curator 가드라 MCP 불가) — **insert 시 id AND source_id 함께 캡처**(삭제 후 마킹용). (2) `pnpm dlx tsx scripts/lcp/reprocess-all-se.mjs --commit --ids <uuid,...>` (fetch+segment+winkNLP, 권당 수십초, 거대챕터 skip 있음). (3) 위 추출-평가 쿼리로 gap 수리+shared_words 동기화. (4) lbv/chapters/book_curation_jobs/library_books DELETE + seed `curation_meta.dict_mined=true` 마킹. **진행: 3배치(15권)·mined 14·수리 ~18단어**(bid·tender·pardon·pin·rear·rage·whip·variation·span·merit·fling·strand·furnish·worship·dread·vow·mock·distress). **yield 4→4→1 diminishing**(다양 성인 산문 고yield·동화/시극 저yield). 잔여 SE ~1,423. **근본 사전 전체 이슈 미발견**(전부 per-word 다의어). curation_status는 이미 전부 'done'이라 추적에 부적합→curation_meta 플래그 사용.
