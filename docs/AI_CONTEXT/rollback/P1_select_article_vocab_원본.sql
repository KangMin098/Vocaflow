-- ═══════════════════════════════════════════════════════════
-- P1 rollback baseline: select_article_vocab 원본 (2026-06-20 dump)
--
-- 원본 게이트: WHERE sd.v_level >= COALESCE(art.article_v_level, 4)
-- 이 함수를 원래대로 복구하려면 본 파일 전문을 그대로 apply.
-- ═══════════════════════════════════════════════════════════
...
AS $function$
  ...
  cand AS (
    ...
    WHERE sd.v_level >= COALESCE(art.article_v_level, 4)
