...
| 표제어 | 들어 있던 뜻 | 실제 |
|---|---|---|
| `uncountable` | 셀 수 없이 많은 | + **(문법) 불가산의** — 예문이 문법 용어인데 앞면엔 없었다 |
| `daisy` | 데이지, **들국화** | 들국화는 산국. 다른 꽃이다 |
| `schoolhouse` | **교사** | 한글만 보면 敎師(사람)로 읽힌다 → 학교 건물, 교사(校舍) |
| `washer` | 세탁기 | + **와셔**(나사에 끼우는 받침쇠) — 예문이 수도꼭지 고무 패킹이었다 |
...
### T13 — T6 뜻 백로그 잔여 425건 (2026-08-17)

T6 이 진단만 하고 남긴 968건 중 T7 이 543건, T13 이 나머지 425건을 맡았다.
**적용 90건 · skip 334건 · 기존 sense 소실 0 · `pos` 교정 7.**

**⚠️ 이 백로그의 성격이 예상과 달랐다.** 425건 중 **334건(79%)이 뜻 결함이 아니라 표제어 결함**이다 —
굴절형(`goes`·`fled`·`reinvests`) · 복수형(`mice`·`subtypes`) · 고어 철자(`inclose`·`develope`) ·
비자립 차용어 조각(`facto`·`guerre`·`mortis`) · 소문자 고유명사(`bible`·`february`) · 중복 계열
(`nighty`/`nightie`·`bio-medical`/`biomedical`). `meanings_ko` 를 어떻게 고쳐도 해소되지 않고
**표제어 병합·삭제로만** 처리된다 — 즉 보류 중인 "표제어 정규화" 와 같은 건이다.

실제로 고친 것 중 가장 나빴던 것:

| 표제어 | 문제 |
|---|---|
| `mes` | 세 뜻 중 **둘이 날조** — "me 의 변형" · "SNS 에서 me 의 복수형(여러 셀카)". 영어에 me 의 복수형은 없다. 실제로는 프랑스어·스페인어 낱말 |
| `sat` | **SAT(미국 대학입학시험)이 `v_level` 1**(A1 대역)로 박혀 있었다. 초급자가 `sat` 의 뜻으로 시험 이름을 학습 |
| `aids` | 소문자 `aids`(aid 의 3인칭 단수)에 "에이즈"가 **같은 v3** 로 나란히. `A good dictionary aids students` 를 보고 에이즈를 떠올린다 |
| `lice` | 카드 앞면이 **글자 하나 "이"** — 치아·지시대명사·숫자 2와 충돌해 아무것도 못 가르침 |
| `borne` | **"(아이를) 낳았다"** 누락 — `born`/`borne` 을 가르는 바로 그 경계 |
| `spanish` | "스페인인"으로 적어 한국인 최다 오류 `He is a Spanish`(x) 를 **사전이 직접 유발**. `a Spaniard` 가 맞다 |
| `fagot` | 뜻·예문 다 맞는데 **경고가 없었다.** 뜻이 아니라 사용역 공백이 위험한 유일한 유형 |

**정정** — 앞서 `flewed` 를 "아동 오류형, 표제어 제거 대상" 으로 적었으나 **실재 형용사**다.
사냥개의 늘어진 윗입술(`flews`)을 뜻하고 셰익스피어 『한여름 밤의 꿈』 `so flew'd, so sanded` 가 용례다.
제거가 아니라 sense 추가로 처리했다.

**하네스 결함 2건 수정** — head 게이트의 `core()` 가 (a) 선행 괄호만 벗기고 **후행 괄호**를 남겨
`환자들 (patient의 복수형)` 형식에서 10건이 헛되이 탈락했고, (b) core 만 정규화하고 head 는 그대로 비교해
`달(月)` ↔ `달 (月)` 공백 차이로 또 탈락했다. **괄호를 위치 무관 전부 제거 + 양쪽 동일 정규화** 로 고쳐
탈락 12 → 1.

### `pos_set` 에 축약 잡값 2,936건 (T13 에이전트 발견 → 정규화 완료)

`["n.","noun"]` · `["adj.","adjective","v.","verb"]` 처럼 **마침표 붙은 옛 라벨이 정식 라벨과 함께**
들어 있었다. 품사 필터가 `n.` 을 매칭하지 못하므로 조용히 새는 값이다.
`n. pl.` · `v. 3sg.` · `abbr.` · `num.` · `adj./n.` · `adjective(orographic)` 같은 변종까지 있었다.

축약 → 정식 매핑 + 괄호·`pl.`/`past`/`3sg.` 꼬리 제거 + 슬래시 분해 + 중복 제거로 일괄 정규화했다.
**결과: 잡값 0 · 서로 다른 라벨 29종 · `pos` ∉ `pos_set` 여전히 0 · 다품사 표제어 7,196.**
...
### 삭제가 전파되지 않는 파생 스냅샷 — gitignore 라 영원히 안 보인다

**사전에서 지운 12건이 `tools/en-wordlist.txt`(8/11자, 64,028단어)에 전부 살아 있었다.**
`scripts/comic/pd/build-wordlist.mjs` 가 `shared_dictionary(word + inflected_forms)` 를 덤프한
PDCP OCR 판정용 어휘 목록인데, **`/tools/` 가 gitignore 대상이라 `git status` 에 뜨지 않는다.**
DB 를 아무리 정리해도 이 스냅샷은 그대로 남는다.

**다만 영향은 제한적이다** — `--lexicon` 플래그는 **기본 꺼져 있고**
[PD_MODERNIZE_MODEL.md](../../../scripts/comic/docs/PD_MODERNIZE_MODEL.md) §"채택하지 않은 것"
에서 이미 **평가 후 반려**된 경로다(방언 대사를 오탐하고 정작 잡아야 할 병합·왜곡은 못 잡음).
즉 "OCR 이 지금 `railro` 를 승인하고 있다" 가 아니라 "켰다면 승인했을 것" 이다.

재생성했다: **64,028 → 65,487단어**, 삭제분 잔존 **12 → 0**, 원형 10건 전부 존재.
PD 문서에 재생성 시점("표제어 삭제·대량 추가 후")을 명시했다.

⚠️ **일반화**: DB 를 파일로 덤프해 두는 곳은 삭제가 전파되지 않는다. 그런 파생물이
