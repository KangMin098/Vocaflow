...
- **추출 누락 실단어 폐쇄**: 마이그 `extraction_residual_harvest`(`select_extraction_residual()`) — 도서·기사 토큰 중 core·coverage·kaikki 어디에도 없는 실단어(고유명사·해소가능 제외) + 문맥 first_sentence 수확. **2,186 후보 → 문맥 기반 Opus 배치 → 903 실영어 적재**(`source='content_residual'`), 1,283은 외국어·OCR·방언·고유명사로 정확 제외. 도서 not_found 2,332→1,932·실단어후보 1,478→1,082(잔여=비영어). `dwelt`(불규칙 굴절)·esports·secularization 등 회수. coverage_ko 77,501→78,404. `coverage-residual-{harvest,apply}.mjs`.
- **참고 목록 완성도 — 굴절 해소 + core-제외**(마이그 `coverage_lists_inflection`·`_core_exclusion`·`coverage_book_chapter_param`): 도서·기사 참고 함수가 직접 lemma만 보던 것 → **`en_inflection_bases` 해소**로 굴절형 롱테일까지 회수(P&P 18→적용대상 대폭↑). 부작용(character의 과잉stem base가 kaikki obscure 고어 오매칭) → **core-제외 가드**(토큰이 core면 학습소관→배제) + cov.word 길이≥4로 정제(P&P 54 클린). **성능**: `p_chapter_idx`로 챕터별 처리(큰 도서 11s→1.2s) + direct-core 선필터. FE `getChapterCoverageWords` 챕터 전달. → 7만 롱테일이 굴절형으로 나와도 참고 목록에 적용.
