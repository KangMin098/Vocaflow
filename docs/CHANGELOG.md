...
- **Phase 8 도서 5권씩 배치 채굴 루프(자동화 `scripts/lcp/dict-mine-batch.mjs`)**: 5권 적재→추출·평가→사전 보완→도서 삭제(transient, 용량 절약)→전체 학습대상 반복. seed 5개 직접 INSERT + `reprocess-all-se --ids`(fetch+winkNLP) + `select_book_chapter_vocab` 집계(단일-sense·sort_order≤40·rank≤8000 후보를 등장 도서수 합산 누적) + 삭제 + `curation_meta.dict_mined` 표시. **자동 50권 run 완료 → 2,903 후보 누적** → 등장 도서수 순 상위 220 검토 → **실 gap 16 수리**(drift·flush·quarrel·sin·despair·bundle·thrill·vain·blaze·spy·shield·thrust·divine·retreat·surge 동사/형용사 sense 누락 + **ah** abbreviation "암페어시" 오분류→interjection 교정). 수동 15권에서 별도 ~18단어(bid·tender·pardon·pin·merit·fling·strand·furnish·worship·dread·vow·mock·distress 등). **누적 mined ~65권·수리 ~34단어**. 핵심: 등장 도서수=임팩트 정렬(17-20권 등장 최고가치), 220위 이하는 대부분 정확한 단의어→yield 포화. 근본 사전 전체 이슈 미발견(전부 per-word 다의어). SE seed ~1,370 다중 세션 반복.
