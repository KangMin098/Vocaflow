...
- **오디오 전달 방식 확정 — 브라우저 TTS** (제품 결정). Sound 면을 `fallback`(0.7 가중) → `full` 로,
  D26 을 `partial` → `ready` 로(오디오 세트 0.92 → 0.98). 녹음 부재는 채점표의 `audio_playable`
  근거 문구("녹음 0 · TTS 합성 300")로 계속 보인다. 결정이 만든 새 실패 지점 2건도 같이 막았다 —
