...
### 상한 감시가 `10_000` 을 못 보고 있었다 — 만든 다음 사이클에 드러났다

만화 패널부터 재고(최대 90·209행 — **상한에 안 닿는다**, 가정 대신 실측) 다른 큰 테이블로
넘어가다 **내가 만든 감시의 구멍**을 찾았다. 정규식이 `d+` 만 봐서 숫자 구분자를 쓴
`.limit(10_000)` 을 **통째로 놓치고 있었다.** 고치자 다섯 곳이 나왔다:

- **`memory-horizon`** (3곳 · `/dashboard` 회고) — `.limit(10_000)`·`.limit(50_000)` 셋 다
  1,000에서 잘렸다. "얼마나 쌓였나"·"되찾은 단어" 를 세는 곳이라 **학습이 쌓일수록 더 크게
  틀린다**. ⚠️ 이 파일은 바로 아래에서 `.in()` 을 500개씩 **병렬로** 쪼개고 있었다 —
  쪼개는 법을 알면서 기본 읽기만 안 쪼갠, 이 저장소에서 반복되는 모양이다
- **`admin/retention`** (2곳) — 리텐션은 "며칠에 걸쳐 돌아왔나" 를 세는데 **최근 1,000건만
  보고** 계산하고 있었다. 분기 진단이 근거로 쓰는 수치다 — 틀린 채로 결정에 들어간다
- **`VocabularyLevelMap`** — 상한을 **안 적었는데 그게 곧 1,000행**이다. 1,945행 계정에서
  레벨 분포가 절반만 세어진 채 "내 어휘 지도" 로 떴다(여기도 사전 조회는 500씩 쪼개고 있었다)

감시가 자기 구멍을 메운 뒤 스스로 잡아냈다 — **통과한 검사도 왜 통과했는지 봐야 한다**는
이 루프의 세 번째 사례다.


### 브랜딩이 코드에만 있었다 — 조판 기록 + Admin 관측면 (2026-08-30)

`85b340ce` 가 조판기를 디자인 토큰에 붙이고 판권면을 만들었지만 **관측면이 없었다.**
팔레트·서체·판권면은 패키지 상수이고 조판 결과는 `--out volume-v5.html` 로컬 파일이라,
`/admin/textbook` 이 읽을 레코드가 하나도 없었다 — 화면은 있는데 브랜딩만 안 보이는 상태였다.

- 마이그레이션 `20260830140000_textbook_volume_renders` — 조판 기록 테이블(권당 1행,
  `band` PK 덮어쓰기 → 재실행 안전). RLS `SELECT` = `is_admin_or_curator()`, 쓰기는 조판기만
- `brandFingerprint()` (FNV-1a 8자리) · `brandSpecRows()` 신설 (`textbook/brand.ts`) —
  **색을 DB 에 복사하지 않고** 규격의 요약만 남겨 옛 규격으로 찍힌 권을 가려낸다
- `render-volume.mjs` — 조판 후 기록 upsert. 기록 실패는 조판 실패가 아니므로 죽이지 않고
  경고 + 마이그레이션 이름을 찍는다
- `/admin/textbook` — KPI `조판된 권` + 블록 2 신설(브랜드 규격 · 조판된 권).
  빈 표와 못 읽은 표를 구별하고, 색 칸에 hex 를 글자로 함께 적는다
- 화면도움말 `help/textbook.ts` — fields 3 · 드레인 단계 1(토큰 고쳤으면 재조판) · verify 1 · recovery 1
- 회귀 11 (brand 6 + 콘솔 렌더 5)
...
평탄화해 들여온 결과 `saw`(see 의 과거형)에 `hand saw`·`chain saw` 가, `over`(부사)에
크리켓 연어가, `march`(행진하다)에 `in March` 가 붙어 있었다.
**빈칸은 학습자가 아무것도 안 배우지만 틀린 연어는 외운다** — 컴포저의 collocation 블루프린트가
그 값을 그대로 목차로 쓴다.

- 새 파이프라인 `scripts/dict/w0830-homonym.mjs` (chunk → 서브에이전트 → apply).
  대상 34,195행 중 시험 밴드 **6,456 낱말 / 323 청크**, 현재 **4,400 낱말 판정·적재**
- 오염 없음 4,329(**98.4%**) · 지운 항목 연어 50 · 반의어 54 · **유의어 0**.
  `saw`·`over`·`march`·`may`·`he`·`stem`·`transport` 전부 제거
- **되돌릴 수 있게 지운다** — 지운 값을 `field_provenance.t6_removed` 에 보존.
  청크 입력을 다시 읽어 대조하므로 서브에이전트가 지어낸 값은 못 지운다(`not_shown` 0)
- 대체값을 만들지 않고 `example_en`·`meanings_ko` 순서도 건드리지 않는다 —
  섞으면 무엇이 지워지고 무엇이 채워졌는지 못 가린다
- **오염보다 흔한 결함이 따로 드러났다**: 연어가 전부 맞는데 전부 부차적 뜻에만 붙어 있고
  대표 뜻에는 하나도 없는 낱말(`pet` `pin` `swallow` `sole` `tender` `rose` …).
  지울 게 없어 이 배치로는 못 고치므로 `COREFILL-QUEUE.json` 으로 뽑았다(77건)
- 배치 중 자체 결함 수정 — 부수 발견을 `already` 판정 뒤에 모아 재실행 시 큐가 빈 채로
  덮이고 있었다(실제로 184→83 으로 덮임). 수집 순서를 바꿔 271건 복구
- 폐기한 수치를 기록에 남겼다: `meaning_ko`↔`senses` 불일치 SQL 휴리스틱 31,571건은
  의역에도 걸려 **근거로 쓸 수 없다**. 반증 불가능한 대체 지표는 667행
  ([dict-defects-d0830.md](./reports/dict-defects-d0830.md))


### 각색 드레인 — 소재 적합성은 규칙으로 안 갈린다 (그리고 필터는 효과가 없었다)

초등 각색을 이어 하다가 나온 것들이다.

- 초등 5편 추가 작성 → 게이트 통과 4편 적재. 1단 원글 **7 → 11편**
- ⚠️ **`source='original'`(우리가 쓴 글)은 각색 대상이 아니다** — `chk_original_needs_batch` 가
  `compose_batch_id`·`composed_spec` 을 요구하는데 원본의 spec 은 각색본을 설명하지 않는다.
  첫 적재가 이 제약에 걸려서 알았다. export·import 양쪽에서 막는다(한 행 때문에 배치가
  통째로 터지는 것보다 세어서 건너뛰는 편이 낫다)
- ⚠️ **표본 18편 중 7편(39%)이 소재부터 부적합**했다 — COP28 정책 · 육류 감축 논쟁 ·
  네덜란드 전쟁사 · 사망 원인. 초등 지시문이 금하는 것들인데 export 는 라이선스·레벨·길이만 봤다

`register` 로 `argumentative` 1,546편을 제외했다. **그런데 부적합률은 39% → 약 44% 로,
측정상 나아지지 않았다** — 남은 부적합 글이 전부 `expository` 이기 때문이다. `register` 는
"주장하는가" 를 가르지 이 밴드가 필요한 **"눈에 보이는가"** 를 가르지 않는다.

필터는 남겨 둔다(`argumentative` 는 정의상 이 밴드의 소재가 아니다). 다만 **이것으로 문제가
풀렸다고 적지 않는다** — 실제 대책은 채우는 쪽의 판단이고, 그래서 필요한 편수의 **두 배**를
뽑아 두고 안 맞는 것은 비워 둔다. import 가 그 수를 센다.

### 상한은 **RPC 에도 걸린다** — `/fit` 이 파는 수치가 조용히 낮아지고 있었다

앞 항목(테이블 1,000행 상한)을 RPC 로 이어 물었다. **같이 걸린다** — 그리고 아무도 모르고 있었다.

```
표면형 1,500개 → textfit_resolve_levels 1회: 1,000행 · 500개씩 쪼개면: 1,499행
```

`resolveLevels` 는 표면형을 **한 번에** 보내고 있었다. 1,000을 넘으면 나머지는 해석되지
않은 채 남고, 그건 **미지어로 세어져 "내가 아는 비율" 이 실제보다 낮게** 나온다 —
그 수치가 랜딩의 1차 CTA(`/fit`)가 파는 것 자체다. 같은 파일의 **폴백은 이미 400개씩
쪼개고 있었다**(`.in()` 길이 때문에) — RPC 쪽만 안 쪼갠 것이다.

- `chunkedRpc` 를 `paged-select.ts`(상한 지식이 사는 곳)에 더하고 두 자리에 적용:
  `textfit_resolve_levels`(표면형 해석) · `curriculum_bands`(교육과정 밴드 — 잘리면
  실제로는 교과서에 있는 낱말이 "없는 단어" 처럼 보인다)
- **조각 하나가 실패하면 전체를 실패로 돌린다** — 반쯤 푼 결과를 정상이라고 부르면
  그게 더 나쁘다(호출부가 그대로 미지어로 센다)
- 회귀 8종(`paged-select.test.ts`) — 끝까지 받는가 · 딱 맞아떨어질 때 마지막을 잃지 않는가 ·
  빈 입력에 RPC 를 때리지 않는가 · 실패를 삼키지 않는가

앱이 부르는 RPC 99개 중 SETOF/TABLE 반환이 88개다. 그중 **인자 배열 크기만큼 행이 나오는**
둘이 위 두 개였다.


### "한 권이 시중 교재 분량에 닿는다" 의 기준 20 은 근거가 없었다

채점표가 `units.length >= 20` 으로 판정하고 라벨은 **"시중 교재 분량"** 이었다. 그런데 같은
저장소가 시중 9종을 재서 남긴 값은 **최소 5 · p25 7 · 중앙값 10 · p75 12** 다.
**20 은 p75 보다도 위**였다 — 시장을 참칭하는 임계값이었고, 반증은 같은 폴더의
`market-spec.json` 에 있었다. `middle-choice.ts` 의 "중등 4지선다" 와 같은 종류의 결함이다.

그 탓에 18단원짜리 권이 "시중 분량 미달" 로 잡혔다. 실제로는 시중 중앙값의 1.8배다.

- `MARKET_UNITS_PER_BOOK` 을 `market-spec.json` 에서 읽어 쓴다. 기준은 **중앙값 10**
- `detail` 에 근거를 남긴다 — `1/10단원 (시중 9종 실측 중앙값 · p25 7 · p75 12)`
- ⚠️ **테스트가 틀린 임계값을 굳혀 두고 있었다** — `expect(c.detail).toBe('1/20단원')`.
  회귀 3종으로 바꿔 실측에 고정했다(919 테스트 통과)

**시중 중앙값으로 다시 재니 그림이 달라진다** (20단원 → 10단원):

| 단 | 적합도 20단원 | 적합도 10단원 | 자동 검수 |
|---|--:|--:|--:|
| 2 초등 고 | 87.6% | **99.3%** | 9/9 |
| 3 중1-2 | 93.4% | **97.7%** | 9/9 |
| 4 중3 | 95.0% | **98.5%** | 9/9 |
| 5 고1 | 75.7% | **92.6%** | 9/9 |
| 6 고2 | 80.3% | **90.3%** | 9/9 |
| 7 고3 | 77.1% | **89.7%** | 9/9 |

20단원을 채우려면 재고에 남은 것을 끌어다 메워야 해서 구성이 시장과 멀어졌다.
**한 권을 시장 분량으로 만들면 구성도 시장에 가까워진다.**

⚠️ **앞서 기록한 카탈로그 용량(합 56권)은 틀린 값이다.** 그 사이 `volume-pool.mjs` 의
페이징 누락이 고쳐졌고(밴드 하나가 1,000편을 넘는데 앞 1,000편만 읽고 있었다),
V5 재고는 988 → **3,004편**이었다. 10단원 기준 실제 카탈로그는
**2 · 5 · 18 · 91 · 86 · 6 = 208권**이다.

### 레벨 적응 게이트는 지어져 있었는데 그것을 쓰는 파이프라인이 없었다

사다리 아래 세 단이 비어 있는 원인을 좇다가 나온 것이다. `compose/adaptation.ts` 의
`runAdaptationGates` 를 **부르는 스크립트가 하나도 없었고**, 6,627편 중 각색본은 3편뿐이었다.

- `adapt-drain-export.mjs` — 각색 허용 라이선스(`cc_by`·`cc0`·`public_domain`)이고 목표보다
  위 밴드인 원본을 **피드를 돌아가며** 뽑는다. 각색 가능 원본이 **6,006편** 있다
- `adapt-drain-import.mjs` — 게이트를 돌려 넣는다. critical 은 I17 서가 중복 하나이고,
  규격 밖(어수·평균 문장 길이)은 import 가 막는다. 경고를 달고 통과한 편수를 반드시 찍는다
- 첫 실행: 초등 6편 작성 → 게이트 **6/6 통과 · 건너뜀 0 · 경고 0** → 1단 원글 **1 → 7편**
- **재실행 안전 실증** — 2회차 "이미 있음 6 · 적재 0편"

⚠️ 첫 적재가 `library_articles_source_check` 에 걸렸다. `source` 는 실제 피드만 허용한다.
각색본은 **원본의 `source` 를 그대로 이어받는 것이 맞다** — 각색해도 귀속은 원 발행처이고,
각색이라는 사실은 `adapted_from_id` 와 `feed_id='adapted'` 가 나른다.

`cc_by_sa` 는 대상에서 뺐다 — 파생물도 같은 조건으로 공유해야 하는데 서가 약관이 그것을
감당하는지 확인되지 않았다. 모르는 채로 쓰는 것보다 빼는 편이 싸다.

### ScriptQuiz — 카탈로그가 자라면 대기열이 조용히 틀린다 (2026-08-30)

Fables 28장 전량(140문항)을 채워 챕터 퀴즈가 **1,340문항 / 189챕터 / 8도서** 가 되면서
PostgREST 1,000행 상한 셋을 새로 넘겼다. 셋 다 오류 없이 **화면만 틀리는** 종류다:

- `list_book_chapter_quiz_catalog` RPC — 챕터마다 한 행. 189행 / 대상 3,373행이라 드레인 중 상한을 넘는다. `pagedSelect` 로 페이지네이션 (`lib/scriptquiz/questions.ts`).
- 대기열 `texts` / `scores` — 잘리면 읽은 챕터가 "안 읽음", 푼 챕터가 "안 풂" 이 된다. 게다가 `data ?? []` 로 오류를 삼켜 **실패도 같은 모습**이었다. `pagedSelectIn` 으로 교체 — 실패 시 throw (`lib/scriptquiz/queue.ts`).
- 회귀 3 (`queue-paging.test.ts`) — 1,200행 상황에서 `readTotal` 1,200 · `confirmed` 1,200 · 조회 실패 시 예외.

진입밴드 퀴즈 드레인 실측: **271 발행 도서 · 대상 3,373장 · 완료 189장(5.6%) · 퀴즈 있는 도서 6권.**


### 카탈로그 용량 — "한 권을 만들 수 있다" 와 "여러 권을 줄 수 있다" 는 다르다

지금까지 잰 것은 전부 **밴드당 책 한 권**이었다. 학습자가 늘면 같은 책을 돌려주게 되는데
그 한계가 어디에도 안 찍혔다. 한 권이 원글 몇 편을 쓰는지 세어 재고로 나눠 봤다
(`render-volume.mjs` 에 한 줄 추가 · 20단원 기준):

| 단 | 원글 재고 | 한 권이 쓰는 원글 | 겹치지 않는 책 |
|---|--:|--:|--:|
| 1 초등 저학년 | 1 | — | **0권** |
| 2 초등 고학년 | 145 | 86 | **1권** |
| 3 중1-2 | 171 | 62 | **2권** |
...
합계 56권. 시중 교재도 한 권을 다수가 공유하므로 "1권" 이 곧 실패는 아니다. 다만
**사다리의 아래쪽과 맨 위가 얇아서**, 초등·중1-2 학습자는 한두 권으로 카탈로그를 소진한다.
앞서 찾은 재고 편중(V5·V6 에 원글이 몰려 있다)이 학습자가 체감하는 단위로 나타난 것이다.

### 유형을 새로 열면 적합도가 **먼저 떨어진다** — 함정을 코드에 적었다

V7(고3) 조판 적합도가 69.4% 로 일곱 단 중 가장 낮았다. 원인은 재고에 `blank`·`title`·
`topic`·`main_point` 가 **0건**이라는 것이었다(시중 고등 구성의 49.8%를 차지하는 유형들).

`topic` 을 드레인으로 채우며 예상 밖의 일이 나왔다:
...
`rungMix(band, available)` 는 목표를 **재고에 있는 유형 안에서 다시 정규화**한다. 없던 유형이
하나 생기면 그 유형이 곧바로 큰 목표 비중(여기서는 17.5% · 21문항)을 갖고, 조금만 넣으면
미달분이 그대로 감점된다. **한 청크만 넣고 "효과 없음" 으로 읽으면 정반대로 읽는 것이다.**

- `rung-mix.ts` 에 실측 표와 함께 적었다 — 새 유형은 목표 비중을 채울 만큼 한 번에 넣는다
- 회귀 2종이 이 세 수치를 그대로 재현한다(917 테스트 통과)

### 주제(23번) 문항 20건 — V7

정답 위치 ①~⑤ 각 4건 균등 · 해설 196~268자(시장 규격 75~473 안) · 원문 인용과 오답 배제
전건 · 지문 20/20 규격 안. 범용 병합기(`merge-chunk.mjs`)에 **정답 길이 편향** 검사를 넣어
import 단계가 아니라 작성 단계에서 잡도록 했다 — 첫 청크에서 1건이 import 에 걸렸던 문제다.

⚠️ 처음 만든 "정답이 유일한 최장/최단" 검사는 **한두 글자 차이도 결함으로 셌다.** 그건 풀이
단서가 될 수 없으므로 문턱을 10% 로 고쳤다 — 고칠 수 없는 것을 고치라고 하는 검사는 무시된다.

### 정렬 없는 페이지네이션이 우위 지수를 통째로 왜곡하고 있었다

`market-benchmark.mjs` 의 `fetchAll` 에 **`.order()` 가 없었다.** Postgres 는 ORDER BY 가
없으면 순서를 보장하지 않으므로 페이지마다 같은 행이 또 나오고 그만큼 다른 행이 빠진다.
재고가 13만 행으로 늘자 이것이 터졌다 — **희귀 유형이 표본에서 통째로 빠졌다.**

| 축 | 정렬 없음(버그) | 정렬 후(실제) |
|---|--:|--:|
| A3 오답 배제 | 65.2% | **88.0%** |
| A5 유형 다양성 | 8종 · 0.571 ❌ | **25종 · 1.786 ✅** |
| **종합(기하평균)** | **0.884** | **1.247** |

재고는 하나도 안 변했다. **측정이 틀렸던 것이다.** 이 저장소가 IA 수집에서 이미 겪은
버그와 같다(CLAUDE.md §PDCP `sort[]=identifier asc`) — 두 번째로 같은 자리에서 밟았다.

### 생성형 15유형이 정답 쏠림 검정에서 빠져 있었다

`item-health-report.mjs` 의 `SHAPE` 에 없는 유형은 **"단답이라 답지가 없다"** 로 분류된다.
5지선다인 생성형 15종(topic·title·blank·main_point·mood·purpose·claim·implication·
summary·content_match·long_* 5종)이 전부 거기 걸려 **검정 대상 밖**이었다. 드레인으로
수천 건을 넣을 참이라 지금 막지 않으면 쏠림이 조용히 쌓인다.

등록하자마자 **이전에 안 보이던 9건**이 드러났다 — `vocab_choice` 정답 쏠림 χ²=38.4 ·
`title` 지문 규격 밖 13/40(**한 건은 14,420어**) · `long_*` 5종 100%(장문 유형이라
90~200어 규격이 애초에 안 맞는다 — 규격 쪽을 유형별로 나눠야 한다).

### 제목(24번) 문항 10건 — V5

`item-drain` 3단 구조로 Claude Code 가 채웠다. 정답 위치를 ①~⑤ 에 **2건씩 균등**하게
흩었고(정답 번호 쏠림 χ²=2.6 ✅), 해설은 177~290자로 전부 시장 규격(75~473) 안이며
원문 인용·오답 배제 100%다. 지문은 **10/10 규격 안**(178~199어).

- import 의 "정답이 오답 평균의 1.27배 — 길이만 보고 풀린다" 게이트에 1건이 걸려
  오답 길이를 손봤다. 같은 검사를 채움 스크립트에도 넣어 다음부터는 먼저 잡는다
- V5 조판 유형-학년 적합도 **67.4% → 75.7%** (title 2 → 12편). 10문항으로 +8.3%p

### D0830 사전 배치 완주 — 뜻마다 예문, 예문마다 해석

시중 다의어 교재의 본체는 뜻 목록이 아니라 **뜻을 갈라 주는 문장**이고, 국내 교재는 예문마다
해석을 단다. 우리는 예문 48,730개 중 해석이 **한 건도 없었다**. 세 파이프라인을 Claude Code
배치(export → 서브에이전트 → apply)로 완주했다.

| 지표 | 배치 전 | 완주 후 |
|---|---|---|
| 뜻별 예문 (`meanings_ko[].example`) | 9,645 | **72,266** |
| 예문 한국어 해석 (`example_ko`) | **0** | **72,271** (낱말 47,763 / 48,969 = 97.5%) |
| `base_word` 어족 키 | 3,414 | **10,400** (2개 이상 계열 2,379) |

- `w0830-senseex` 634/634 · `w0830-family` 376/376 · `w0830-exko` 533/533 — 세 파이프라인
  모두 재실행 후 미적재 **0** 확인
- 서브에이전트가 억지 예문 대신 `skip` + 사유를 남긴 항목이 다수 — **억지 예문은 빈칸보다 나쁘다**
  (학습자가 그걸 외운다). 뜻 갈래 중복 · 파생형이 뜻으로 들어옴 · 표제어 오철자 · 멸칭 등
  부수 발견은 각 `FLAGGED.json` 에 모였다 (senseex 1,871 · exko 953 · family 7,865)
- 남은 것은 T6 회차 — 동음이의어 오염 판정, 뜻 갈래 정리(⚠️ `meanings_ko` 인덱스를 바꾸면
  이번에 채운 예문이 엉뚱한 뜻에 붙는다), 문장이 아닌 예문 교체.
  설계는 [dict-defects-d0830.md](./reports/dict-defects-d0830.md)


### 시중 구성에 맞추는 기능이 옵트인이었다 — 기본값을 켬으로

조판된 책의 **유형-학년 적합도**(시중 밀도 대비 총변이 겹침)를 처음 재 보니 V5 가 31.3%
였다. 120문항 중 순서 40 + 삽입 40 = **67%가 두 유형**이었다. 원인은 조합기가 아니라
`--market-mix` 가 **꺼진 기본값**이었다는 것 — 켜면 같은 재고로 67.4% 가 나온다.
시장에 맞는 책을 만드는 것이 옵트인일 이유가 없다.
...
| V4 중3 | **89.1%** | 38.3% |
| V5 고1 | **67.4%** | 31.3% |
| V6 고2 | **80.3%** (20단원) | 45.2% (**18**단원) |

모든 밴드에서 낫고, V6 는 단원 수까지 18 → 20 으로 늘었다 — 깎인 축이 없다.

- `volume-pool.mjs` `loadVolume` 의 `marketMix` 기본값 `false` → **`true`**. 끄려면
  `--no-market-mix`
- `render-volume.mjs` · `explain-drain-export.mjs` **셋을 함께** 뒤집었다 — 둘이 어긋나면
  "드레인이 겨냥한 책" 과 "조판된 책" 이 달라진다(그 파일의 기존 드리프트 경고 그대로다)

**남은 격차의 원인은 재고다.** 고등 재고 8,027건 중 시장 1위 유형 `blank`(목표 22.6%)이
**28건**, `title`(13.5%)이 15건, `topic`(9.4%)이 31건뿐이고, 시장이 거의 안 쓰는
`blank_word`(0.7%)가 2,366건 · `grammar_fix`(0.8%)가 1,553건이다. **재고가 시장과
뒤집혀 있다** — 결정론으로 만들 수 있는 것만 쌓였기 때문이다.

⚠️ **학년 커버리지 5/7 실측** — V1(원글 1편)과 V7(고3)은 재료가 없어 0단원이다.

### 한 학습자가 자기 단어의 **49%를 못 보고 있었다** — 1,000행 상한

PostgREST 는 한 응답에 **1,000행까지만** 준다. 더 크게 요청해도 **오류가 아니라 조용히
1,000행**이 온다. 실측(2026-08-30):

```
실제 행수(count exact): 1945   단일 select: 1000   페이지네이션: 1945
```

`.limit(10000)` 은 "넉넉히 받겠다" 가 아니라 **"1,000에서 끊기는 것을 모르고 있다"** 는 표시였다.
학습자 표면 네 곳이 그 상태였고, 전부 오류 없이 수만 틀렸다:

- **`growth-stats`** — 상태 띠의 "다시 볼"·"새 단어" 분포. 셸 최상단이라 **모든 화면**에 뜬다
- **`useHubStats`** — WordVault 히어로 총계와 4버킷. 상한을 안 적었을 뿐 결과는 같다.
  같은 분포를 서버에서 따로 세는 띠와 **행 집합이 달라져** 한 화면 안에서 두 수가 어긋난다
  (이 저장소가 이미 겪은 사고다 — 리본 135 vs Vault 20)
- **`browse-queries`** — 둘러보기 목록·칩 개수·듣기 큐. 1,000을 넘는 단어는 **아예 안 보였다**
- **`study-queries`** — 상태 필터 학습 세션. "새 단어 N개로 시작" 이 N 보다 적게 열렸다
  (여기 있던 `STATE_SCAN_LIMIT = 1500` 은 **실제로 1,000이었다** — 지웠다)

학습자 밖에서도 셋 — 챕터 단어장 lemma 수집(`chapter-words-queries`, 큰 챕터일수록 조용히
적게 줬다) · 무결성 결함 집계(`admin/dict/queries`) · VCB 후보 풀 2곳(정렬된 상위 N 이라
자르는 것 자체는 의도지만 **적힌 수가 오지 않아** 실제 값으로 정정).

전량이 필요한 곳은 `lib/supabase/paged-select.ts`(같은 날 다른 세션이 만들었다)로 바꿨고,
`row-cap-lies.test.ts` 가 **`.limit(N)` 의 N 이 1,000을 넘으면 실패**시킨다 —
그 숫자는 지킬 수 없는 약속이다.

### 교재 조판기가 브랜드 밖에 있었다 — 토큰 SSoT 연결 + 판권면

`render-volume.mjs` 가 **자기 팔레트를 따로 갖고 있었다.** 디자인 토큰과 대조하니 다섯 항목이
전부 어긋나 있었다. 루트 CLAUDE.md 는 "CSS Variables 로 테마 제어 — 하드코딩 금지" 라고
적어 두었는데, 조판기는 그 규칙 밖이라 아무도 안 봤다. **교재는 학습자가 손에 쥐는 물건이라
여기서 어긋나면 브랜드가 어긋난 것이다.**

| 조판기(이전) | 토큰 | |
|---|---|---|
| `--accent #7a3b2e` 테라코타 | `--active-ink #7E5A1B` | 골드 잉크 |
| `--ink #1a1a1a` | `--t1 #1A1714` | 웜 잉크 |
| `--bg #fbfaf7` | `--bg #FBFAF6` | 종이 |
| `--line #d8d4cd` | `--bd #E0DBD0` | 괘선 |
| `Iowan Old Style` | `Lora` | v06.39 영문 시그니처 |

- `brand.ts` — **값을 다시 적지 않는다.** `@vocaflow/design-tokens` 에서 읽는다
  (`library-pipeline` 에 workspace 의존성 추가). 토큰이 바뀌면 교재도 같이 바뀐다
- 회귀 13종이 두 곳의 값을 대조한다 — 은퇴한 팔레트 5색이 다시 나타나면 실패한다
- `buildColophon` — 판권면(제목·사다리 자리·판차·발행일·검수·출처 정책). 상업 교재가
  반드시 싣는 것인데 없었다. **출처 정책이 빠지면 공개 저작물 근거가 사라진다**
- `ladderStrip` — 뒤표지 시리즈 표시. 시중 교재가 전체 사다리를 싣고 현재 권을 표시하는
  이유는 독자가 **다음에 무엇을 볼지** 알아야 하기 때문이다. 숫자만으로는 그 일을 못 한다
- `volumeCssVariables` — 세 테마 상태(무표시·light·dark). 라이트를 bare `:root` 에 두어
  테마를 고르지 않은 독자가 색을 잃지 않게 한다
- 실조판 대조: 토큰 4색 반영 · 옛 팔레트 0건 · Lora 3곳 · 패키지 **915 테스트 통과**

### 교정 5단계 — 초교만 기계가 보고 있었다 (재교·삼교 신설)

상업 교재 8단계 중 5번 "교정(초교·재교·삼교)" 은 세 번 훑는 자리인데, 기계가 보는 것은
**초교(`isPrintablePassage` = 인용 잔해·용어풀이) 하나뿐**이었다. 오탈자도 표기 일관성도
아무도 안 봤다.

- `proofread.ts` — 재교(구두점 앞 공백 · 연속 공백 · 반복 낱말 · 괄호 짝) +
  삼교(아포스트로피/큰따옴표 모양 · `-ise/-ize` · 대시 혼용). **고치지 않고 표시만 한다** —
  지문 수정은 원문 개작이라 사람이 정한다
- `scripts/textbook/proofread-report.mjs` — 저장 지문 상시 점검. 읽기만 하므로 재실행 안전
- 실측 지문 1,668편 중 **77편(4.62%)에 결함** — 아포스트로피 31 · 구두점 앞 공백 29 ·
  반복 낱말 15 · 큰따옴표 9 · 대시 3 · 괄호 2
- 회귀 15종. 패키지 전체 **902 테스트 통과**

**실측이 규칙 셋을 고쳤다** — 지어낸 표본이었으면 셋 다 놓쳤을 것들이다:

| 오탐 | 무엇이었나 | 고친 방법 |
|---|---|---|
| `opinions ... it's` | 말줄임표 앞 공백은 정상 | `.` 뒤가 `.`/`…` 면 건너뛴다 |
| `Some bacteria (e.g.` | 문장 분리기가 `e.g.` 에서 끊어 괄호가 두 조각에 걸쳤다 (54건 중 대부분) | 괄호는 **글 전체**로 센다 → 54건/28편 → 2건/2편 |
| `we’ve` + `"quote"` | 굽은 아포스트로피와 곧은 큰따옴표는 서로 다른 종류 | 아포스트로피와 큰따옴표를 **따로** 본다 |

`Durand Durand`(Barbarella 악당 이름)처럼 **겹친 고유명사**는 중복이 아니라서, 대문자로
시작하면 "지운다" 대신 "확인한다" 로 말을 바꾼다.

⚠️ 측정 중 **DB 문항이 15,911 → 12,923 으로 줄었다**(다른 세션이 같은 DB를 쓰는 중).
결함률 변화를 개선으로 읽으면 안 된다 — 모집단이 움직였다.

### `irrelevant` 41문항에 결정론 해설 — 시장 지수 A1 0.994 → 0.996

시중 79종 대비 우위 지수(`market-benchmark.mjs`)에서 **A1 해설 보유율만 1.0 미만**이었다.
결손 103건의 내역은 `irrelevant` 41 · `order` 31 · `insert` 31 인데, 앞의 41 은
**작성기 자체가 없어서** 비어 있었다(`DETERMINISTIC_EXPLAIN_TYPES` 미포함).

- `explainIrrelevant` — 도입부와 나머지 문장이 **함께 쓰는 내용어**가 화제이고, 그 낱말을
  하나도 잇지 않는 문장이 답이다. 겹침이 없다는 사실을 낱말로 보여 준다 —
  "흐름이 어색하다" 는 인상이라 학습자가 스스로 확인할 수 없다
- 41/41 전량 작성(못 씀 0) · 중앙 417자 · 오답 배제 100% · 원문 인용 100% —
  시장 3규격(길이 p25~p90 · 배제 53.6% · 인용 49.7%)을 한 번에 넘는다
- 기능어 목록에 `including`·`make`·`used` 류를 넣었다 — 내용어처럼 보이지만 화제를
  나르지 않아 근거로 들면 해설이 약해진다(실측 표본에서 발견)
- 회귀 7종 추가(25 통과) — 표본은 **DB 실측 payload** 그대로. 지어낸 모양으로 통과시키면
  적재할 때 전부 null 로 떨어진다

⚠️ 남은 A1 결손 62건(`order`·`insert`)은 **수능 인쇄 형식으로 변환되지 않는 문항**이라
이음매가 성립하지 않는다 — 해설을 지어 넣을 것이 아니라 문항 쪽을 고쳐야 한다.

### 상태 띠가 말하는 수를 **DB 와 직접 대조**한다 — 모든 화면에 뜨는 숫자

이 루프에서 지어낸 수치가 세 번 나왔다 — `/wordvault?view=review` 의 하드코딩 `12개` ·
`QuickStartCard` 의 기본값 `12·15·8` · 공개 요금제 지표. **셋 다 화면은 멀쩡했고 에러도 없었다.**
그런 것을 잡을 수 있는 자는 DB 대조뿐인데, 그 자를 가진 화면이 `/wordvault` 하나였다.

상태 띠는 셸 최상단이라 **모든 화면에서 보인다** — 여기가 틀리면 어디를 봐도 틀린 수를 본다.
`38-hub-ribbon-truth.spec.ts` 가 "새 단어"·"다시 볼" 칩을 `vocabularies` 실측과 대조한다.

- 판정은 앱과 **같은 함수**(`getMemoryState`)를 쓴다 — 테스트가 임계값을 다시 쓰면
  앱이 바뀔 때 조용히 다른 것을 재게 된다
- 이름은 레지스트리(`MEMORY_ATTENTION_LABEL`)에서 가져온다 — 문구를 테스트에 베끼면
  라벨이 바뀔 때 **아무것도 못 찾고 지나간다**
- 수는 **`aria-label` 에서** 읽는다. 시각 숫자만 보면 보조기기가 듣는 값과 갈라져도 모른다
  (이 저장소는 그 둘이 어긋난 적이 있다 — 리본 135 vs Vault 20)
- 대조할 낱말이 0이면 **통과로 세지 않는다**. 빈 상태 문장("아직 시작 전이에요")이 떴을 때는
  거꾸로 "정말 0인가" 를 확인한다 — 2026-08-27 에 교사가 보낸 낱말이 그 문장에 가려졌었다

⚠️ 처음 판은 통과했지만 **선택자가 맞아서가 아니라 두 칩의 렌더 순서 덕**이었다.
`filter({ hasNotText })` 는 본문을 보는데 두 칩을 구별하는 문구는 aria-label 에 있다.
통과한 검사도 왜 통과했는지 봐야 한다.

정적 스캔도 함께 돌렸다 — 학습자 컴포넌트에 남은 **하드코딩 지표 기본값은 0건**
(`QuickStartCard` 가 마지막이었다).
...
(`filter=state:*` · `?q=` · `?level=` …). 그런데 **다른 화면에 같은 구멍이 있는지는
아무도 몰랐다** — 이 사고는 타입·린트·빌드를 전부 통과하고 화면도 오류 없이 뜬다.

`lib/__tests__/orphan-query-params.test.ts` 가 내부 링크의 파라미터를 전부 모아
저장소 어디에서도 읽히지 않는 것을 잡는다. 켜자마자 **후보 14 → 실제 4**:

- **`/dictate/setup?textId=`** (`lib/learner/gateway.ts`) — 받는 쪽이 읽는 이름은 `text` 다.
  "이어서 받아쓰기" 를 눌러도 **자료가 안 실린 빈 준비 화면**이 열렸고 학습자는 방금 읽던
  글을 다시 골라야 했다. 아무도 모르고 있던 일곱 번째다 → `?text=` 로 정정
- **`/flashcard?word=<id>`** (`CompletionState`) — `/flashcard` 는 `searchParams` 를 한 줄도
  읽지 않는다. "어려웠던 단어" 를 눌러도 그냥 허브가 열렸다(링크가 장식이었다)
  → 실제로 그 낱말을 보여 줄 수 있는 `/wordvault/browse?q=` 로
- **`/signup?plan=pro`** · **`/signup?invited=true`** — 받는 쪽이 만들어진 적 없다.
  결제 경로 자체가 없어 받아 줄 곳도 없다 → 파라미터 제거
  (주소에 남은 마커는 "이런 동작이 있다" 는 신호로 읽힌다)

나머지 10은 계측기 쪽이었고, **그 오탐들이 감시의 설계를 정했다**:
`?period=` 는 `sp.period` 로 읽히고 있었다(서버 컴포넌트 별칭) → 별칭을 읽는 자로 인정한다.
`?r=` 은 네 곳 다 **주석**이었다(옛 공유 주소를 설명하는 문장) → 주석을 걷어내고 센다.
`?per_page=` 는 외부 API URL 이다 → `seed-fetchers` 제외.
...
**피드당 3~16편**이었다. 소스가 부족한 게 아니라 **우리 코드가 첫 페이지에서 멈추고 있었다.**

찾아 고친 상한 (전부 지표를 깨뜨리지 않는 조용한 실패였다):

| # | 상한 | 실측 |
|---|---|---|
| 1 | 위키미디어 continuation 부재 | 3왕복 18편(전부 중복) → 56편(중복 0) |
| 2 | 위키미디어에 크롬 위장 UA → 429 | 0건 → 184건 |
| 3 | USGS·NOAA 목록 쪽번호 미지원 | 피드당 13~16편 → 908편 |
| 4 | NOAA 본문 선택이 "가장 긴 HTML" | 211편 중 25편 → 115편 |
| 5 | VOA RSS 창이 URL 파라미터(`count=20`) | 피드당 3~15편 → 936편 |
| 6 | `applyArticleCurationSpec` 이 `maxItems` 로 재절단 | 200건 받아 15건으로 잘림 |
| 7 | 배치 SOURCES 표에 futurity 누락 · plos `essay` 미배선 | 0편 → 188편 / 15편 → 222편 |
| 8 | **DB CHECK 제약에 `futurity` 없음** | INSERT 전량 거절 → 확보 영구 0 |

8번이 가장 깊었다 — 어댑터·spec·register 매핑·회귀 테스트가 2026-08-21 에 다 들어왔는데
`library_articles_source_check` 만 갱신되지 않아, 목록도 본문 추출도 성공하는데 저장이
불가능했다. 화면에는 "담은 것 0" 으로만 보여 소스가 비어 있는 것과 구분되지 않았다.
마이그레이션 `20260830020000_add_futurity_to_article_source_check` (사용자 승인 후 적용).

6번도 성격이 같다. `maxItems` 는 **매일 도는 경로가 넘치지 않게** 두는 정책이지 품질
규칙이 아닌데 대량 확보 경로에도 그대로 걸려 있었다. 게다가 `listVoaFeed`·`listNasaFeed`·
`listFuturityFeed` 는 `limit` 파라미터를 **받아 놓고 `void _limit` 으로 버리고 있었다** —
고칠 자리는 새 기능이 아니라 죽어 있던 파라미터였다.

**소스 순위를 두 번 뒤집었다** — 근거는 매번 실측이었다.

- 1차: DCP 문항 유형 커버리지로 wikipedia 를 1순위로 잡았다가, 적재된 제목이
  *Fire Emblem Warriors · Ariana Grande 곡 · 소련 프리깃 Silnyy* 라 주제 분류기로 다시 쟀다.
  wikipedia 주제 적합률 5~11% · NOAA·NASA·VOA 61~78%. **커버리지 지표는 길이에 교란되고
  주제를 못 본다.** wikipedia 대량 확보 중단(전량 확보 시 3.1GB · 적합 11%).
- 2차: 분석 후 V-Level 을 보니 VOA 는 주제 적합률 61~66% 인데 평균 **V3.8**,
  수능 대역(V5~8) 적중이 **16.5%** 였다. Learning English 라 의도적으로 단순한 문장이다.
  **주제 적합도와 레벨 적합도는 다른 축이다.**

**확보 결과 (최종)** — 지문 재고 **779 → 6,627편**. 수능 대역(V5~8)에서 쓸 수 있는 것 **5,504편**.

| 소스 | 소진 | 상류 실측 | 확보 | 평균 V | 대역 내 |
|---|---|---|---|---|---|
| `futurity` | 298p **소진** | 2,879 | 2,879 | 5.2 | 다수 |
| `plos/essay` | 54p **소진** | 1,541 | 1,534 | 6.0 | 1,563 |
| `usgs` | 52p·23p **소진** | 793 | 728 | 5.1 | 652 |
| `nasa` | 36p **소진** | 314 | 403 | 5.5 | 356 |
| `voa` | 창 전량 **소진** | 936 | 249 | 3.8 | 41 |
| `noaa` | 21p·14p **소진** | 115 | 115 | 4.9 | 101 |
| `wikipedia` | **의도적 보류** | 6,993+ | 92 | 6.0 | 16 |
| `the_conversation` | ND — 우회 | — | 71 | 5.1 | **0** |

문종별 (사용 가능 · 대역 내): **논증문 26 → 1,543편** · 설명문 3,869편.
논증문은 수능 문종의 핵심이면서 가장 얇았던 축이고, ND 게이트를 건드리지 않고
`plos/essay` 로 대체해 59배로 늘렸다.

USGS·NOAA 는 목록 전 페이지를 DB 와 대조해 **미확보 0** 을 확인했다. 거절된 65편은
본문 200단어 미만인 지진 속보·예산 공지라 거절이 맞다.

**상류 총량을 세 번 과소 보고했다** — usgs 282→567 · plos essay 15→1,541 · futurity 372→2,879.
매번 `--pages` 예산이 먼저 끝난 것을 소진으로 읽을 뻔했고, 표에 **`소진` 과 `예산소진` 을
구분해 찍게 해 둔 것**이 세 번 다 잡아냈다. 셀 수 없는 것을 세지 않기보다,
**못 센 것을 못 셌다고 표시하는 것**이 값이 나갔다.

**비용** — DB 2,195 → 3,387MB(+1,192MB). 편당 약 200KB. Cycle 2 에서 위키백과 전량 확보를
3.1GB·적합률 11% 로 추산해 중단한 판단이 여기서 값을 했다 — 같은 예산으로 적합률 26~65% ·
대역 적중 85~100% 인 지문을 얻었다.

**확보 다음 관문은 사람이다** — 문항은 `published` 지문에만 생기고, 확보한 것 대부분은
`ready` 에 서 있다(`process-queue` 가 스스로 적어 둔 대로 "검수·발행은 Admin ⑦ 에서
사람이 한다"). 이건 결함이 아니라 의도된 품질 관문이고, 사용자가 준 목표("원문 모두 확보")
범위 밖이라 임의로 발행하지 않았다.

**회귀 26종 추가** — `wikipedia-continuation` 5 · `user-agent-policy` 10 ·
`list-pagination` 7 · `noaa-body-selection` 4. 전부 네트워크를 타지 않고 **배선**을
검사한다(수량이 아니라) — 수량으로 재면 소스가 바뀔 때마다 깨진다.


### 탭에 브랜드가 **두 번** 나오고 있었다 — 25화면

루트 `layout.tsx` 의 `title.template` 이 `"%s | Vocaflow"` 인데 페이지도 `· Vocaflow` 를
적고 있었다 → **"Flashcard 학습 · Vocaflow | Vocaflow"**. 제목을 선언한 111곳 중
대부분은 이미 템플릿에 맡기고 있었으니 25곳이 규칙을 어긴 쪽이었다.
`28-screen-identity` 는 **겹치는지**만 보므로 전부 똑같이 두 번 붙으면 그대로 통과한다 —
이 값을 재는 자리가 없어서 눈에 보이는 채로 남아 있었다.

- 25화면에서 접미 제거(`'Flashcard 학습'` · `'Today'` · `'WordVault — 학습'` …)
- 랜딩은 `title: { absolute: … }` — 브랜드를 앞에 세우는 편이 검색결과에서 맞으므로
  템플릿을 거치고 제목 전체를 직접 정한다
- ⚠️ **접미가 가리고 있던 이름 충돌이 드러났다** — `/text/[id]/comic` 과 `/comics/adapted`
  가 둘 다 `'만화'` 였다(접미가 달라 우연히 갈라져 있었다) → `'만화로 읽기'`
- 회귀 `src/app/__tests__/page-titles.test.ts` 3종 — 브랜드 중복·정적 제목 충돌·스캔 공집합.
  `admin/` 13화면(`'… — Vocaflow Admin'`)은 **미처리로 표시**해 두었다(제대로 고치려면
  `admin/layout.tsx` 자체 템플릿 + 13페이지라 한 커밋이 40파일을 넘는다)

실측: 화면 정체 훑기 **261/261 = 100%** (프로덕션 빌드).

### 시중 교재를 자로 삼아 교재 파이프라인을 잰다 — 우위지수 0.636 → **1.296**

"시중보다 월등하게" 는 숫자가 없으면 이겼는지도 모른다. 그래서 기준선을 **실제 시중 교재
79종 5,214쪽에서 재서** 고정하고, 우리 산출물을 같은 자로 쟀다.

- **`market-spec.json`** (`scripts/textbook-corpus/market-spec.mjs`) — 시장 규격을 기계가 읽는 형태로.
  표준 발문 **23종**(서로 다른 시리즈 2곳 이상에서 쓰인 것만) · 학년대별 지문 어수 6구간 ·
  해설 규격(중앙 143자 · 오답배제 53.6% · 인용 49.7%) · 단원 구성(권당 10단원 · 단원당 17쪽).
  **집계와 업계 공통 발문형만 담는다 — 지문·해설 원문은 담지 않는다**
- **`market-benchmark.mjs`** — 6축 우위지수(기하평균). 한 축이 0 에 가까우면 종합도 끌어내린다
- **`explain-items.ts`** 결정론 해설 7유형 신설 — 해설 보유율 **2.7% → 80.3%**(13,814/17,206).
  `explain.ts`(순서·삽입)가 15%에 그치는 것과 달리 이 7유형은 **정답 근거가 문항 안에 이미 있어**
  100% 쓸 수 있다. 규격 준수: 길이 99.4% · 오답배제 54.1% · 인용 99.6%
- **`explain-fill.mjs`** 적재 13,351건 — `answer_key` 를 덮지 않고 키만 더한다(정답 키 전건 보존 확인)
- 낡은 문서 교정 — `production-stages.ts` 가 "수능 5유형 · 문항 7/28" 이라 적고 있었으나
  실측은 **25유형 17,206문항**이었다. 해설 진척도 "28건" → 실측 13,814건
- 리뷰에서 잡은 품질 결함 2건: 인용을 앞에서 잘라 **바뀐 낱말이 안 보이던 것**,
  `those AI-focused data centers` 에서 형용사를 명사로 지목해 **사실이 틀린 해설**이 나오던 것
- 회귀 18종 신설 (표본은 DB 실측 payload 그대로) · textbook 전체 302 통과

### 유형은 난이도가 아니라 **학년의 신분증**이다 — 유형-학년 적합도 23.5% → 95.0%

시장 밀도를 재고 나서야 보였다. **V3(중1-2) 교재 120문항 중 80문항이 수능 순서·삽입**이었다.
재고에 그것밖에 없어서였지 그 학년 교재라서가 아니었다.

- `market-spec.json` `typeDensity` — 학교급별 유형 등장률(‰) 실측:
  `word_order` 초등 36.7 / 중등 19.9 / 고등 7.3 · `order` 초등 **0** / 중등 3.8 / 고등 30.4 ·
  `blank` 초등 3.4 / 중등 2.8 / 고등 **64.5**. 유형은 학년을 가른다
- `rung-mix.ts` — 단수별 구성을 **이 표에서 유도한다**(손으로 정하지 않는다).
  문턱 2‰(값이 뚝 떨어지는 자리) · 총변이거리로 적합도를 잰다
- `compose-unit.ts` — 비중 배분 경로 신설(최대잉여법). 뼈대/덧붙임 이분법은 비중을 정수 칸으로
  바꾸며 작은 유형을 0 으로 눌렀다(`order` 11%가 0칸이 됐다). 3단 폴백으로 재고가 허용하는
  만큼만 양보하고 **양보 횟수를 센다**(`mixRelaxed`)
- `SCHOOL_ITEM_TYPES` + `itemWordSpec` 확장 — 학교 시험 축 7유형은 수능 창(90~200어)에
  전량 걸려 **13,351문항이 어느 권에도 안 실렸다.** 문단 40~200어 · 문장 6~40어로 창을 가른다
- `volume-pool.mjs` `SCHOOL_TYPES` · `render-volume.mjs` `renderSchool()` —
  4지선다·밑줄 5지·단답·배열을 조판한다. `renderExtra` 가 `choices.length === 5` 를 요구해
  전부 null 로 떨어지던 자리다. 해설도 `rationale_ko` 를 함께 본다(534문항이 있는데 "없음" 이었다)
- **실측(20단원 120문항 · 검수 9/9 유지)**: 유형-학년 적합도
  V3 23.5→**77.2%** · V4 33.5→**95.0%** · V5 31.3→**67.4%** · V6 40.4→**80.3%**.
  V7 은 0% — 그 밴드에 문항이 없다(재고 결손이지 조합 결함이 아니다)
- 기본은 꺼져 있다(`--market-mix`) — 이미 완성된 권이 조용히 달라지면 안 된다
- 회귀 15 신설 · library-pipeline 전체 818 통과
### 1,000행 상한 전수 점검 — 스캐너를 만들고 교재 계열 15곳을 고쳤다

같은 함정에 **다섯 번째**였다. 주석으로는 못 막는다는 것이 증명됐으므로 기계가 훑게 했다.

- `scripts/lib/scan-unpaged-queries.mjs` 신설 — `.from().select()` 체인에 `range`·
  `limit(≤1000)`·`single`·`head:true`·공용 페이저가 **하나도 없고**, 표가 1,000행을 넘고,
  `.eq('*_id'|'word'|'slug')` 같은 좁힘도 없을 때만 의심으로 낸다. 실측 조회 882건 중
  의심 **144 → 130** (교재 계열은 16 → **1**, 남은 1건은 50개씩 끊어 묻는 오탐이라 주석으로 명시)
- **살아 있던 결함**: `volume-pool.mjs` 가 원글을 페이징 없이 읽어 **V5(3,055편)·V6(2,339편)를
  1,000편에서 자르고 있었다.** 조판과 해설 드레인이 공유하는 로더라 두 밴드의 모든 권이
  앞 1,000편만 보고 만들어지고 있었다. 고친 뒤 실측: V5 원글 1,000 → **3,004편** ·
  문항 풀 40,896 · 유형-학년 적합도 67.4% → **75.7%**
- `fetchAllPaged(db, build)` 공용 페이저 추가(`volume-pool.mjs`) — `.in()` 이 아닌 조회용.
  `fetchAllIn` 과 짝을 이룬다
- 고친 곳 15: `volume-pool` · `item-drain-export` · `new-types-health` · `refresh-dcp-items` ·
  `write-drain-export`(원글 + **슬롯 번호** — 잘리면 이미 쓴 슬롯을 다시 쓴다) ·
  `write-drain-import` · probe 7종(`stock`·`type-yield`·`source-yield`·`sentence`·
  `middle-short`·`grammar-yield`·`caption`) · `media-probe`(808낱말 — 아직 안 잘리지만 상한 근접)
- 효과 실측: `stock-probe` 가 V5 **2,958편** · V6 **2,293편**을 보고한다(이전에는 안 보이던 재고)

### 재고 15,911 → **136,512** · 해설 **99.95%** · 우위지수 **1.452**

페이징 결함을 고치자 대상 원글이 981 → 4,356편으로 늘었고, 그만큼 재고가 늘었다.
(적재 도중에도 다른 세션의 ACP 수집이 돌아 원글이 3시간에 3,908편 더 늘었다 —
승인받은 규모 95,800 보다 커진 이유다. 사후 보고.)

- `--prune` **2,988건 삭제** — 4지선다 2,886 전량 포함. 학습 기록 0건이라 잃은 이력 없음
- `--commit` **123,579건 적재** — 신규 `unit_vocab`·`unit_grammar` 는 전부 5지선다(min=max=5)
- `explain-fill` **123,579건 해설 적재** → 보유율 **136,450/136,512 = 99.95%**
- **A3 모집단 교정** — 단답형(빈칸 낱말 쓰기·어법 고쳐 쓰기)에는 배제할 오답이 없다.
  없는 것을 못 했다고 세니 단답이 64%가 되면서 A3 가 62.5% → 45.3% 로 떨어졌는데,
  **선택지 있는 문항만 보면 99.8%** 였다. 시장 기준선 53.6% 도 객관식 해설 위주 값이라
  같은 모집단으로 맞췄다 → 1.862
- 최종 7축: A1 1.000 · A2 1.535 · A3 1.862 · A4 2.008 · A5 1.786 · A6 1.146 · A7 1.161
  → **종합 1.452** (목표 1.200)

### 원글 조회에 페이징이 없어 **71%를 못 보고 있었다** (같은 함정 네 번째)

`store-new-types.mjs` 아래쪽에는 "PostgREST 는 1000행에서 자른다 · 같은 함정에 세 번째다"
라는 경고가 적혀 있는데, **정작 원글 조회 자신**이 안 걸려 있었다.

- 실측: 쓸 수 있는 글 **3,356편** 중 스크립트가 본 것은 **981편(29%)**
- 이것이 이상 현상 셋을 한꺼번에 설명한다 —
  낡음 판정이 `1,571 → 1,295 → 0` 으로 흔들린 것(보는 글 집합이 매번 달랐다) ·
  4지선다 2,886건이 "낡음 0건" 으로 보고된 것(그 글들이 조회 밖이었다) ·
  "새로 넣을 문항" 수치가 **29% 표본** 기준이었던 것
- `range()` 페이징 추가 후: 글 3,356편 · 낡음 **2,988건**(4지선다 2,886 포함) ·
  신규 적재 대상 20,902 → **79,921**
- 함께 고친 조용한 건너뛰기 — 원본 문단을 못 찾으면 세지도 않고 넘어가던 것을
  `uncomparable` 로 유형별 보고. 그리고 **보기 수 규격은 문단 없이도 판정**하도록
  앞으로 뺐다(저장된 값만으로 알 수 있는 것을 재생성 대조에 걸어 두면 안 된다)

### 낡음 판정이 수능 3종만 보고 있었다 — 54건 → **1,571건**

`store-new-types.mjs` 머리말은 "매 실행이 기존 문항도 다시 잰다" 고 적어 두었지만,
실제로 재는 유형은 `irrelevant`·`vocab_choice`·`grammar_choice` **셋뿐**이었다.
2026-08-22 에 중등 4종이 생겼는데 검출기가 따라오지 않았고, 그래서 `MIDDLE_CHOICES` 를
4→5 로 고친 직후에도 **"낡음 0건"** 이라고 답했다.

- `rebuilders` 에 `unit_vocab`·`unit_grammar` 추가
- `staleSignature` 신설 — **유형마다 "달라졌다" 의 뜻이 다르다.**
  문장만 대조하면 보기 수가 바뀐 것을 못 잡는다(실제로 그랬다).
  어휘는 보기 수·대상 낱말·문장, 어법은 밑줄 수·문장을 본다
- 지문 규격도 유형별로 자를 가른다(`MIDDLE_TYPES` → 40~152어) —
  중등 유형에 수능 창(90~200어)을 대면 멀쩡한 문항이 전량 "규격 밖" 으로 잡힌다
- 실측 낡음 54 → **1,571** (unit_vocab 1,026 · unit_grammar 491 · 나머지 54)

### "중등 4지선다" 는 근거 없는 한 줄이었고, 재고의 24%를 시장 밖으로 보냈다

- `market-spec.json` `choiceCount` 신설 — 보기 수를 **문항 단위**로 센다(쪽 단위로 세면
  한 쪽에 여러 문항이 있어 값이 흐려진다). 실측:
  초등 5지 **79.7%** · 중등 5지 **93.8%** · 고등 5지 84.7%
- `middle-choice.ts` 가 "수능 5지선다 · **중등 4지선다**" 라고 적고 있었고 근거는 없었다.
  그 한 줄 때문에 `unit_vocab` 2,848 + `unit_grammar` 1,287 = **4,135문항(재고의 24%)**
  이 4지선다로 만들어졌다. `MIDDLE_CHOICES` · `MIDDLE_GRAMMAR_UNDERLINES` 를 5 로 정정
- **지문 창도 같이 넓혔다** (`MIDDLE_ITEM_WORDS` 120 → **152** = 중1 p90 실측).
  두 규격은 같이 지켜야 성립한다 — 실측 수율:
  4지·40~120 `vocab 45.0% / grammar 27.6%` → 5지·40~120 `45.0% / **16.6%**`(창이 좁아 −40%)
  → 5지·40~152 `**48.7%** / 24.0%`(시장 창으로 −13%)
- `elementary.ts` 는 **건드리지 않았다** — 코퍼스의 "초등" 은 초3~초6 독해서이고
  그 파일이 만드는 것은 저학년 파닉스다. 장르가 달라 근거가 닿지 않는다(주석에 명시)
- 벤치마크 **A7 선택지 수 규격 적합률** 신설 — 숨어 있던 축이라 종합이 1.373 → **1.158** 로
  내려갔다. 후퇴가 아니라 **안 보이던 결함이 드러난 것**이다
- 수율 측정 결함 자기교정 — `ORDER BY` 없는 `limit` 으로 두 실행이 다른 사전 표본을 써서
  "2.2배 향상" 이라는 허상이 나왔다. 결정론 표본으로 다시 재 실제 수치를 얻었다
- 회귀 2 신설(상수를 시장 실측에 고정) · library-pipeline 830 통과

### 해설을 다 채우고, 학습자에게 실제로 보이게 했다 — 보유율 83.4% → **99.4%**

- `explain-seam.ts` — 순서·삽입 **이음매 해설**. `explain.ts` 는 표면 단서가 정답 쪽에
  유일하게 많을 때만 쓰므로 커버리지가 낮았고(그 문턱은 옳다 — 단서가 오답을 더 가리키는
  경우가 34.4% 라 단서만으로 "왜" 를 쓰면 **틀린 해설**이 나온다).
  그런데 이 두 유형에는 다른 확실한 사실이 있다 — **원문 그 자체**다.
  정답이 만드는 이음매와 오답이 만드는 이음매를 **인용**한다(추론이 아니다).
  적재 2,755건 · 오답배제 100% · 인용 100% · 중앙 303~390자
- `explainShortInsertSeam` — 자리가 5곳이 안 되는 삽입 1,016건. 3~4문장짜리는
  **교재에는 못 써도 학습 화면에서는 그대로 풀리는데** 그 화면에 해설이 없었다.
  인쇄 규격만 우회하고 인용 잔해·자리 범위 안전장치는 그대로 건다
- **`DcpPlayer` 가 해설을 보여 준다** — 이전에는 순서·삽입에 정답 순서만 보여 주고
  **왜 그렇게 이어지는지는 한 글자도 안 보여 줬다.** 채워도 닿지 않으면 채운 게 아니다
- `pickExplanationText()` (`lib/learner/dcp.ts`) — 해설 키가 둘(`explanation_ko` ·
  `rationale_ko`)이라는 사실을 **한 곳에만** 둔다. 화면마다 각자 고르게 두면 또 갈린다
- **종합 우위지수 1.306 → 1.373** (A1 0.994 · A3 오답배제 55.2%→62.5%)
- 남은 62건(order 31 · insert 31)은 수능 형식 변환 자체가 안 된다 — 재생성이 필요하다
- 회귀 13 신설 · library-pipeline 828 + web 113 통과

- 측정 교정 — 벤치마크가 `explanation_ko` 만 세고 있었다. 해설은 **두 이름으로 산다**
  (생성형 드레인은 `rationale_ko`) — 둘 다 학습자 화면과 조판이 읽는데 한쪽만 세어
  534문항이 "없음" 으로 잡혔다. **종합 우위지수 1.296 → 1.306**


### 게임 19종이 **전수 훑기 분모 밖**에 있었다 — 한 번도 재지 않은 화면들

`(app)/play/*` 19화면(아케이드 게임)이 학습자 전수 훑기 어디에도 들어 있지 않았다.
`learnerRoutes()` 가 `src/app/(main)` **한 그룹만** 훑었기 때문이다. 그 파일 머리말은
"새 라우트를 스윕에 넣지 않으면 그 화면은 영영 안 재진다" 고 이미 적고 있었는데,
같은 일이 **라우트 단위가 아니라 그룹 단위로** 다시 일어났다.

- 분모 **43 → 62**. `(app)` 편입 + `/play/*` 를 파일 시스템에서 읽어 `SESSION_ROUTES` 에
  자동 합류(손 목록이면 새 게임이 또 빠진다 — 스윕이 게임 안에서 버튼을 눌러 검증 계정
  기록을 오염시키게 된다). `redirectOnlyRoutes()` 도 두 그룹을 함께 본다
- **진짜 결함 1건 — 게임 19종의 브라우저 제목이 전부 같았다.** `page.tsx` 가 전부
  `'use client'`(`next/dynamic`+`ssr:false`)라 `metadata` 를 내보낼 수 없어 루트 기본 제목
  "Vocaflow — 영어 스크립트 기반 종합 학습" 을 달고 있었다 — 탭·히스토리·북마크가 모두 같은
  이름. 라우트별 `layout.tsx`(제목만) 19개 + 이름은 `GAME_CATALOG` 소유.
  `/wordblitz`(허브)와 겹치지 않게 `<이름> 플레이 · Vocaflow`
  (`(main)` 세션이 `/flashcard` vs `/flashcard/play` 를 가르는 방식과 같다)
- **나머지 19×3 은 계측기가 틀린 것이었다.** 셋(26 동선·27 키보드·28 정체)이 저마다
  `document.querySelector('main')` 한 줄을 들고 있었고, 게임 화면에서 **셋 다 같은 오답**을 냈다
  (막다른 길 / Tab 40번으로 본문 못 닿음 / 본문에 h1 없음). 원인은 ① 첫 진입에 브리핑
  (`role="dialog"`)만 뜨고 게임은 일부러 마운트하지 않아 `<main>` 의 innerHTML 이 **0자**이고
  ② `<main>` 과 SessionFrame 의 중첩이 `(main)`·`(app)` 에서 반대라 이름·닫기가 본문 밖이다.
  → 판정을 `tests/e2e/utils/content-scope.ts` **한 곳**으로 옮겼다(열린 모달은 본문으로 센다).
  ⚠️ 중첩을 바꾸는 쪽은 이미 해 보고 되돌렸다 — 28 이 5건 깨진다(SessionFrame 주석)
- 정적 추적기 `lib/a11y/touch-target.ts` 가 이 저장소의 지배적 표기 `py-s-N`·`h-s-N` 을
  **아예 못 읽고 있었다**(className 12,470개 중 180개가 판정 불가로 조용히 빠졌다).
  `s-N` 은 전부 N×4px 이라 기본 스케일과 값이 같다 → 접두만 허용. 확인 대상 50 → **55건/32파일**.
  더불어 `touch-target.test.ts` 가 `00cf9704`(간격 4px 정렬) 이후 계속 빨간불이었다 —
  `py-2` 를 12px 로 적은 기대값이 틀렸다(위아래 각 8px = 16px). 그 세션도 연결 끊김으로 끝났다

실측(프로덕션 빌드 · `NEXT_DIST_DIR=.next-sweep`): 동선 **296/296** · 키보드 **171/171** ·
정체 **261/261** · 탭대상 @390px **0건** · 동선 @390px **296/296**.
회귀 4종 추가(`play-titles.test.ts` — 새 게임에 제목이 빠지거나 카탈로그와 어긋나면 빨개진다).

### `cefrj-import.mjs` 가 스스로 적어 둔 **사후 단계가 실행된 적이 없었다** — 난이도 화해 (T7)

`scripts/cefrj-import.mjs` 머리말은 이렇게 적고 있다 —
"사후 단계 (별도 SQL): `UPDATE shared_dictionary FROM _staging_cefrj_wordlist`".
그 단계가 돌지 않았다. **권위 있는 밴드는 `cefrj_wordlist_band` 라는 옆 칸에 6,098행 들어와 앉아 있고,
학습자와 컴포저가 읽는 `cefr_level` 은 그것과 무관하게 남아 있었다.**

CEFR-J 근거가 있는 6,098 낱말로 대조한 결과(2026-08-30 실측):

| | |
|---|--:|
| 일치 | 2,236 (36.7%) |
| **우리가 더 어렵게 매김** | **3,279 (53.8%)** — 그중 2단계 이상 1,164 |
| 우리가 더 쉽게 매김 | 583 (9.6%) |

어긋남이 한쪽으로 쏠려 있다. 실물: `chocolate`=B2 · `notebook`=B2 · `bath`=B2 ·
`temple`=C1 · `cop`=C1 · `sunny`=C1 (전부 CEFR-J A1). **그리고 이 행들의 `cefr_confidence` 는
0.90 이었다** — 근거 없는 값에 확신 수치가 붙어 있으면 없느니만 못하다
(`field_provenance` 에 `cefr_level` 키는 아예 없었다. ipa·synonyms 는 있는데).

사전 전체로는 C1+C2 가 **38,235행(78%)** 이었다. 등급이 아니라 **쓰레기통**으로 쓰이고 있었다.
수준별을 파는 제품에서 이건 부수 결함이 아니라 본체 결함이다.

**한 일** — `cefrj_wordlist_band` 가 있는 행만, 그것을 정본으로 삼아 3,862행 화해.
바꾼 행에 `field_provenance.cefr_level = 'cefrj-wordlist-1.6'` 를 남긴다(되돌릴 손잡이 ·
다음 배치가 다시 추측하지 않게 하는 표식). `scripts/dict/w0830-cefrj-reconcile.mjs`
...
**손대지 않은 것**: CEFR-J 근거가 없는 42,864행. 대조할 것이 없으면 고칠 것도 없다 —
"C1/C2 78%" 라는 더 큰 문제는 근거를 먼저 구해야 풀린다.
...
### ScriptQuiz 진입밴드 드레인 착수 — 배치 왕복 + 정답 쏠림 가드

학습 루프에서 유일하게 비어 있던 칸이다 — 챕터당 본문 11,080/11,080 · 단어장 10,923 ·
**퀴즈 129(1.2%)**. 전량 11,080장이 아니라 **학습자가 실제로 도달하는 장**으로 좁혀
진입밴드 **40권 1,091장**을 대상으로 잡았다(사용자 승인).

- `generate-chapter-quiz.mjs` 에 `export-batch` / `insert-batch` 추가.
  챕터당 content→저술→insert 왕복 3회는 1,091장에 맞지 않는다.
  **검증은 기존 `validateQuestions` 를 그대로 쓴다** — 배치용을 따로 두면 규칙이 갈린다.
  대상 필터: `chapter_v_level ≤ 7` + 300~6,000단어 + **이미 문항이 있는 장 제외**(재실행 안전).
- ⚠️ **정답 위치 쏠림 가드 신설** — 첫 배치 5문항이 전부 `correct_index=0` 이었다.
  화면은 보기를 섞지 않고(`scriptquiz` 렌더러에 shuffle 없음), 기존 1,019문항은
  **253/256/261/249** 로 고르다. 쏠리면 학습자가 본문을 읽지 않고 첫 보기만 찍어도 맞는다 —
  문항이 있어도 학습이 일어나지 않는다. 보기 3개 이상 문항의 최대 점유가 50%를 넘으면
  **넣기 전에 막는다**(문항이 8개 미만이면 경고). 실제로 이 가드가 첫 배치를 잡아 냈다.
- 넣기 전에 **전량을 먼저 검증**한다 — 챕터별로 넣다가 중간에 실패하면 반쪽만 남는다.
- 첫 산출: `Winnie-the-Pooh` ch.1 5문항(근거 문장 포함 · 정답 위치 2·1·0·3·2).
...
단어장 10,923 · **ScriptQuiz 129(1.2%)**) 계측 쪽을 확인했는데, 거기서 더 급한 것이 나왔다.

`fit_worksheet_printed` 가 `PublicEvent` 유니온에는 있는데 `ALLOWED_EVENTS` 배열에는 없었다.
`track()` 은 목록에 없으면 **조용히 반환**하고, 운영 빌드에서는 개발용 `console.error` 도 안 나온다.
→ 호출부(`WorksheetControls`)는 멀쩡히 `track()` 을 부르는데 **한 건도 나가지 않는 상태**였다.

하필 `events.ts` 자신이 그 이벤트를 이렇게 적어 두었다 —
*"학습지 인쇄 — **가입보다 강한 의도 신호**. 링크 복사는 '괜찮네' 지만 인쇄는 오늘 수업에
쓰겠다는 뜻이다. … 어떤 표에도 흔적이 남지 않는다(인쇄는 브라우저에서 끝난다).
파생으로 대체할 수 없다."* 10만 산술이 걸린 교사 채널(CAC 0)의 신호다.

**왜 타입이 못 막았나**: `readonly PublicEventName[]` 는 **전수를 요구하지 않는다** —
배열은 빠진 이름을 잡아 주지 않는다. 그래서 유니온과 목록이 조용히 갈렸다.

**왜 테스트도 못 잡았나**: 있던 테스트가 `toEqual([...7개])` 로 **불완전한 목록을 고정**하고
있었다. 빠짐을 잡는 검사가 없고 늘어남만 막고 있어서, **고치려 하면 테스트가 막는** 상태였다.

- `EVENT_REGISTRY: Record<PublicEventName, true>` 로 바꿔 **타입 검사가 빠짐을 막게** 했다.
  검증: 항목 하나를 지우면 `TS2741 Property 'fit_worksheet_printed' is missing` 로 빌드가 선다.
- 테스트를 **빠짐 + 늘어남 두 방향**으로 고쳤다(둘 다 있어야 한다). 44 passed.

### "지금 딱 맞아요" 레일이 12칸인데 고1에게 채울 책이 **5권**이었다

앞 항목에서 챕터 배지를 붙였는데, 추천이 그 책을 바닥에 깔면 두 화면이 서로 다른 말을 한다.
그래서 추천 쪽을 재고 이어 붙였다.

**실측 — 책 단위 i+1 판정(고1 V5 기준)**: hard **273** · challenge 30 · easy 7 · **ideal 5**.
316권 중 273권이 좌절 구간이고, "지금 딱 맞아요" 레일(12칸)은 5권밖에 못 채운다.

- **추천 점수** — hard 감점(−25)을 **읽을 수 있는 장의 비율만큼 완화**한다. 지우지는 않는다
  (책을 통째로 읽기엔 어렵다는 판정 자체는 옳다). 비율을 쓰는 이유: 500장에서 20장(4%)이
  열린 것을 "진입 가능" 이라 부르면 대작이 전부 앞으로 나온다.
  실측 효과 — V5 에서 **86권이 상승(최대 134계단) · 상위 100 진입 16권**.
  상위 30 은 그대로다(책 전체가 맞는 42권이 정당하게 차지하고 있다).
- **새 레일 "이 책은 여기부터"** — hard 책 중 읽을 수 있는 장이 3개 이상인 것.
  기존 레일에 섞지 않았다: "모르는 단어가 적당해요" 라는 설명이 거짓말이 된다.
  정렬은 **비율 × log(개수)** — 비율만 쓰면 3장 중 3장인 짧은 책이 이기고,
  개수만 쓰면 486장 중 315장인 대작이 항상 이긴다.

  | 학습자 | 지금 딱 맞아요 | 이 책은 여기부터 |
  |---|---|---|
  | V4 중3 | 2권 | **27권** |
  | V5 고1 | 5권 | **103권** |
  | V6 고2 | 4권 | **143권** |
  ...
  선정 예: `King Lear` 24/24 · `A Yankee in the Trenches` 15/16 —
  책 라벨(p75)은 V8 hard 인데 모든 장이 사정권이다.
- 선정·정렬은 `rankStartHereBooks()` 로 추출했다 — 컴포넌트에 인라인이면 회귀를 테스트로
  잡을 수 없다(앞 항목에서 같은 이유로 데인 자리다). 회귀 12건 추가(총 18).

### 316권 서가가 우리 타겟에게는 **대학원용**이었다 — 책 라벨이 진입로를 가리고 있었다

발행을 316권으로 늘린 뒤 "우리 타겟(한국 고등학생)이 실제로 읽을 수 있는가" 를 처음 쟀다.
V-레벨 정의는 저장소 것을 그대로 썼다(`vocaflow_levels`: **V5=고1 · V6=고2 · V7=수능 1-2등급**).

**책 단위 분포** — V5 **2권** · V6 15권 · V7 92권 · **V8~V9(대학·대학원) 187권(59%)**.
진입밴드(V2~V4) 20권은 전부 1~3분짜리 그림책이다. 고등학생 눈에 서가는 대학원용이었다.

**그런데 학습자는 책이 아니라 챕터를 읽는다.** 챕터로 다시 재니 진입밴드가 이미 있었다 —
V4 83개(32권) · **V5 263개(87권)** · **V6 936개(222권)** · V7 2,520개(285권).
책 라벨은 `p75`(상위 25% 어휘)라 **책 안의 쉬운 챕터를 가린다.**
그리고 `chapter_v_level` 을 읽는 화면은 리더 안 `ChapterSidebar` 뿐이었다 —
**책을 열기 전에는 볼 수 없었다.** 즉 진입로가 없던 게 아니라 **발견 단계에 노출되지 않았다.**

- **데이터** `scripts/lcp/backfill-chapter-level-histogram.mjs` — 도서별 챕터 난이도 분포를
  `curation_metadata.chapter_v_hist` 에 적재(315권 · 재실행 대상 0 실측).
  **jsonb 키만 더한다** — 통째로 덮으면 synopsis_ko·genre_norm 이 사라진다. 마이그레이션 불필요.
- **판정** `countReadableChapters(hist, userVLevel)` — 경계는 글(article) 판정과 **같다**
  (gap ≤ 0 수월 · +1 딱 맞음). 갈리면 같은 학습자에게 자료 종류마다 다른 기준이 적용된다.
- **화면** 도서 카드에 `읽을 장 N` 배지 + 필터 칩 `📖 지금 읽을 챕터 있음`.
  **진단한 학습자에게만** 보인다 — 미진단 baseline(V5) 을 "내 수준" 이라 말하면 거짓말이 된다.
- **효과 실측** — 학습자에게 열리는 도서 수:

  | 학습자 | 책 라벨 기준 | 챕터 기준 | 읽을 수 있는 장 |
  |---|---|---|---|
  | V4 중3 | 22권 | **111권** | 422 |
  | **V5 고1** | 37권 | **246권** | **1,358** |
  | V6 고2 | 129권 | **311권** | 3,878 |
  | V7 수능 1-2등급 | 277권 | 315권 | 7,328 |

  고1 기준 **6.6배**. 콘텐츠를 더 만들지 않고 이미 있던 것을 보이게 한 결과다.
- 회귀 `readable-chapters.test.ts` 6건 — 글 판정과 경계가 갈리지 않는지 포함.

### 같은 결함을 하루에 세 번 만났다 — 1,000행 상한을 공용 헬퍼로 봉합

`/wordvault` 학습 자산에서 두 건을 더 찾았다. 둘 다 오류 없이 숫자만 틀렸다:

- 세트별 단어 수 `vocabularies .in(setIds)` — 상한 없이 받고 있었는데
  **한 사용자가 이미 1,945행**이다(실측). 과소 집계된 그 수로 목록 **정렬까지** 한다.
- 챕터 보유 판정 `shared_words .limit(10000)` — `.limit()` 을 크게 걸어도 소용없다.
  PostgREST 가 1,000에서 끊는다. 코드 주석의 전제("표준 세트는 소수 ≤1184단어")도 이미 틀렸다:
  실측 챕터 보유 세트 최대 **1,828행** · 큐레이션 전체 **26,390행**.
  잘리면 챕터형 세트가 '챕터 없음' 으로 판정돼 챕터 런처 대신 단어 브라우저로 샌다.

같은 버그를 세 번 각자 고치면 네 번째가 온다. `lib/supabase/paged-select.ts` 로 모았다:
- `pagedSelect` / `pagedSelectIn` — `.range()` 로 끝까지, `.in()` 은 조각내서.
  **오류를 삼키지 않는다**(0개와 조회 실패가 같아지면 안 된다).
- 앞서 각자 페이지네이션하던 `word-set-counts.ts` · `plan-material-queries.ts` 도 이 헬퍼로 통합.
- 규칙을 [CONVENTIONS.md](./CONVENTIONS.md) §Supabase 클라이언트 패턴에 못 박았다 —
  의도적으로 자르는 곳은 `.limit()` 을 쓰되 **왜인지 적는다**(실수와 구별되게).
- 회귀 `paged-select.test.ts` 7건 — 마지막 페이지가 가득 차면 한 번 더 묻는지 포함.

### 큐레이션 단어장 70개 중 **1개만** 학습자 선택기에 남아 있었다 — 상한이 잠식됐다

앞 결함("보이는데 시작할 수 없다")과 같은 부류를 다른 표면에서 훑다가 찾았다.
`fetchAvailableMaterials`(계획에 추가할 자료)가 두 군데서 조용히 잘리고 있었고 **오류는 없었다**:

- 단어장 `.limit(600)` 을 **카테고리 구분 없이** 걸어 둔 채 제목순 정렬 —
  발행 확대로 챕터 세트가 1,129 → **10,923** 이 되자, 발행 세트 11,128 중 600을 받아
  **큐레이션 70개 중 화면에 남은 것이 1개**였다(실측). 수능·고등·중등·어원·비즈니스·
  영어시험·테마 세트가 사실상 사라진 상태.
- 도서 `.limit(300)` — 발행이 316권이 되면서 제목순으로 **16권이 잘렸다.**

**표면 전수 점검 결과**(같은 부류를 찾으려고 다 훑었다):
| 표면 | 결과 |
|---|---|
| 아티클 학습 시작 | 이상 없음 — 발행 160편 전부 content·cefr_level·register 보유 |
| 공용 단어장 카탈로그(`fetchPublishedSets`) | 이상 없음 — `library_book`·`library_article` 을 이미 제외 |
| 계획 자료 선택기 | **두 곳 결함** (위) |

조치 — 목록 규칙을 `lib/learner/plan-material-queries.ts` 로 분리해 단일 출처로 만들었다.
서버 액션 안에 인라인으로 두면 회귀를 테스트로 잡을 수 없다는 것이 이번 결함의 교훈이다.
- 발행 도서·큐레이션 단어장은 **상한 없이** `.range()` 페이지네이션 (각각 수백·수십 규모)
- 도서 챕터 단어장은 **학습자가 등록한 도서로 한정** — 10,923개를 제목순 600으로 자르면
  어느 책의 챕터가 뜨는지가 사실상 무작위였다. 등록하지 않은 책의 챕터는 본문이 없어
  계획에 넣어도 쓸 수 없고, 책을 등록하면 `_enroll_book_subscribe_word_sets` 가 구독시키므로
  **도달 경로는 사라지지 않는다.** 한계가 카탈로그 크기가 아니라 학습자 수강 범위로 바뀐다.
- 결과 실측: 도서 300 → **316** · 큐레이션 단어장 1 → **70**(8개 카테고리 전부 복귀)
- 회귀 `plan-material-queries.integration.test.ts` — 개수를 **DB count 와 대조**한다.
  상한은 "지금은 충분한 숫자" 로 보이기 때문에 위험하다(5 passed).
...
`authenticator`(PostgREST 가 쓰는 롤)에 `statement_timeout=8s` 가 걸려 있어
**service_role 키를 써도 supabase-js 경로는 8초에 잘린다**. 상태 UPDATE 가 트리거
`trg_lb_publish_word_sets` → `publish_book_word_sets` 를 같은 문장 안에서 돌리는데,
챕터 100개가 넘으면 8초를 넘긴다. 타임아웃은 롤백이라 **부분 진행도 남지 않는다**.

DB 기본 `statement_timeout` 은 **2min** 이므로 MCP(`execute_sql`, postgres 롤) 경로로 잔여 49권을
발행해 **316/316** 을 채웠다. 같은 벽에 `republish_book_word_sets` 도 걸린다(재발행 41권 중 11권 실패
→ 같은 경로로 마무리). 챕터 100+ 도서를 다루는 스크립트는 이 8초를 전제해야 한다.

- 발행 **13 → 316권** (`ready` 0) · 챕터 단어장 1,129 → **10,923세트 / 627,198단어**
- 316권 규모 카탈로그 감사 재실행 — 실패 0. 배지 **316/316권**에서 정상(페이지네이션 11페이지·474ms).
  한 방 조회는 여전히 10,923 중 **1,000행만** 온다(감사에 기록용으로 남겨 근거를 보존).
- 2차 멸칭 재스캔(발행 316권) — 신규 후보 24종 중 **멸칭 4**(`gook`·`gooks`·`kaffirs`·`cretin`) 판정.
  lemma 자체가 멸칭이면 **표제어 재분류만으로 복수형까지 걸린다** — 굴절형을 따로 등재해야 하는 것은
  lemma 가 중립일 때뿐이다(`savage`/`savages` · `retard`/`retarded` · `oriental`/`orientals`).
- ⚠️ **자기 유발 결함 1건 수정** — 등재한 굴절형 7행에 `v_level` 을 비워 두어 품질 게이트
  **I1 이 312 → 319** 로 늘었다("지어낸 난이도를 넣지 않는다" 가 이 경우엔 틀린 판단이었다).
  난이도를 **lemma 에서 물려받도록** 고치고(굴절형의 난이도는 기본형과 같다 — 추출 RPC 도 표제어
  대표 v_level 로 밴드를 판정한다) 기존 행은 같은 스크립트 재실행으로 보수되게 했다. I1 **312 복귀**.
- 잔여 실패(내 변경 아님): I1 312 = 다른 세션의 사전 작업 진행분 ·
  `extraction-rpc` 골든셋 2건 = 같은 사전 작업이 바꾼 표제어(`discontented`·`animating`).
...
#### D0830 1차 실적 (2026-08-30 세션 종료 시점 실측)

| 지표 | 시작 | 종료 | 비고 |
|---|--:|--:|---|
| 뜻별 예문(`meanings_ko[].example`) | 9,645 | **50,901** | 5.3배 |
| 예문 한국어 해석(`.example_ko`) | **0** | **49,602** | 낱말 34,348 / 48,969 (70%) |
| 다의어(`senses` ≥ 2) | 7,970 | **16,154** | T1 기계 동기화 (2.03배) |
| `base_word`(어족 키) | 3,414 | **9,690** | 파생형 2개 이상 계열 **2,227** |
| 유의어 보유 | 19,591 | **21,650** | 코어밴드는 90.5% + 없음판정 629 |
| CEFR-J 화해 | — | **3,862행** | A1 750→1,260 · B1 2,922→3,226 |

**수능·EBS·교육과정 밴드 6,932 낱말은 완주**했다 — 유의어 미판정 31 · 반의어 미판정 22 ·
연어 99.8% · 학습자 노트 100%.

**남은 것** (파이프라인은 전부 재실행 안전 — `status` 로 이어서 돌린다):
`senseex` 634 중 371 완료 · `family` 376 중 320 · `exko` 533 중 402.
세션 API 한도로 중단됐고, 남은 청크는 `node scripts/dict/w0830-<batch>.mjs status` 가 알려 준다.

**배치가 스스로 찾아낸 자기 결함 4종** (전부 같은 턴에 고쳤다):
① 굴절 매처에 `choose`·`undertake`·`strive` 계열 누락 → 그 예문이 `no_headword` 로 조용히 버려짐
② 접미사 6자 제한이 `-lessness`·`-ification` 을 버림 (184행 회수)
③ 표제어 `constructor` 가 `Object.prototype` 을 타고 나와 apply 를 통째로 죽임
④ 예문이 게이트에 걸리면 그 뜻의 **해석까지** 함께 버려짐 (예문과 해석은 다른 칸이다)
...
zip 2개 안에 57개가 더 들어 있어 실제 규모가 알려진 것(15개)의 다섯 배였다.

- **5단계 + 자가검증** — `scan`(스캔·zip 전개·6축 분류) → `extract`(쪽 단위) → `analyze`(난이도·어휘)
  → `build-db`(SQLite+FTS5) → `build-md`(문서 카드 79 + 색인 9) → `verify`(목표 6개, 실패 시 종료코드 1)
- **원문은 저장소 밖** `d:/workspace/textbook-corpus` — 저작권 존속 상업 교재라 git 에 넣으면 배포가 된다.
  저장소에는 파이프라인 코드만 둔다
- **HWP 5.0 직접 파싱** (`hwp.mjs`) — 한글·LibreOffice 없이 CFB + raw deflate + `HWPTAG_PARA_TEXT`.
  `xlsx` 가 내보내는 CFB 리더를 쓴다. 리딩튜터 수능PLUS 본문 54,185자 확보
- **분류 6축**(학교급·학년대·유형·역할·출판사·시리즈) 빈 값 0 — 모르면 `미상` 명시값 + `low_confidence` 기록.
  빈 값을 두면 다음 실행이 완료로 세어 구멍이 영영 남는다
- **확장 지점 2개** — `sources.json`(원본 루트) · `overrides.json`(원문 근거로만 알 수 있는 것).
  파일 1개 추가 시 1개만 처리됨을 `verify` G5 가 실제로 실행해 확인한다
- **스캔 PDF 3건을 Windows 내장 OCR 로 해소** (`ocr-win.ps1` · `ocr.mjs`) — tesseract·한글오피스 없이
  `Windows.Data.Pdf` + `Windows.Media.Ocr(ko)`. 배율은 오염률을 재서 3.5 로 고정(2.0/2.5/3.0/3.5 → 4.0/4.7/4.8/**3.6%**).
  인식기 최대 변 10,000px 을 넘으면 **문서 전체가 실패**하므로 쪽마다 배율을 누른다.
  오인식 교정은 외부 사전 없이 — 깨끗한 76문서에서 3회 이상 나온 **13,506낱말**을 사전 삼아
  사전에 있는 형태로 바뀔 때만 적용(462/807 · 57%). 상태는 `ok` 가 아닌 **`ocr`** 로 구분한다
- **사본 관계 탐지** (`overlap.mjs`) — 8낱말 연속열 해시 1/64 샘플의 **포함률**. 24쌍 검출
  (같은 책 3 · 부분본 5 · 지문 일부 공유 16). `빠른독해 바른독해 구문독해.pdf` 는
  `2.빠른독해 바른독해_구문독해.pdf` 안에 100% 들어 있다. 모르고 세면 "N종 비교" 가 거짓이 된다
- 재실행 안전 결함 2건 수정 — `scan` 이 재스캔 때 `analysis` 를 버려 79건 전량 재분석하던 것,
  `analyze` 가 원본 해시만 봐서 **추출이 좋아져도 낡은 지표에 머물던** 것(추출 시각도 함께 본다)
- 실측: 본문 확보 **79/79 (100%)** — ok 76 · ocr 3 · 실패 0 · 5,214쪽 · 1,471만 자 ·
  FK 학년지수가 학년대와 단조 증가(초3~4 3.33 → 고2~고3 12.28) · verify **10/10**
- 산출물 크기: `corpus.db` 51.9 MB · `text/` 21 MB · 문서 카드 676 KB · 색인 173 KB · 원문 전량 md 20 MB(`--full`)
...
DB 직접 질의 + 저장소 실계수만 근거로 썼다.
리포트: <https://claude.ai/code/artifact/7c88e227-7299-416f-8ab1-862b299af86b>

**측정** — 가입 3(2개월 신규 0) · 학습기록 665 · 학급 0 · 학급 과제 0 · 퍼널 이벤트 0.
발행률은 도서 13/401 · 짧은 글 160/779 · 만화 1/969 인데 단어세트만 1,334/1,337 이다 —
**회전한 유일한 축이 사람 손이 안 든 축이었다.** 공급:수요 2,858:1 (판정 `risk` 유지).

**가장 중요한 발견은 원가 구조였다.** API 라우트 76개 중 런타임 LLM 호출 **0건**,
TextFit 은 분석당 DB 왕복 0~1회, 음성은 브라우저 `speechSynthesis`,
`llm_cost_usd` 합계는 도서 401권·짧은 글 779편 모두 $0.00 (기록된 AI 지출 총액 $0.50).
→ 학습자 1명당 변동비가 호스팅뿐이라 Pro ₩9,900 이 거의 전액 공헌이익이다.
뒷면도 같이 적었다(§6-2) — 원가가 0 인 건 효율이 아니라 **정액 구독에 숨어 계측되지 않기**
때문이고, 고정비는 수요가 아니라 공급이 만든다(DB 1,996 MB 중
`library_book_vocabularies` 하나가 1,120 MB = 56%, 발행 13권짜리 재고의 색인).

**즉시 정정 (§9 예외 — 표시광고법)** — 공개 화면 4곳이 존재하지 않는 기능을 고지하고 있었다.

- `/pricing` 특전 "음성(TTS) 자연스러운 OpenAI 보이스" → 재생은 기기 내장 음성
- `/pricing` FAQ "GPT-4o-mini 기본 · Pro 는 GPT-4o · TTS 는 OpenAI TTS-1" → 런타임 호출 0건
- `/privacy` 위탁 목록 `OpenAI(AI 분석·TTS)` → 저장소에 OpenAI 클라이언트 0개.
  실제는 Anthropic 이고 호출 지점이 전부 오프라인 제작 스크립트다. 다만
  `pending_words` 에 회원이 만난 낱말·문맥 조각이 39행 들어 있어 "일절 전송 안 함" 으로는
  쓰지 않고 그 경계에 맞춰 고쳤다
- `/privacy` 위탁 목록 `Toss·Stripe(결제)` → 결제 기능 자체가 없다(도입 예정으로 표기)

**회귀** — `components/marketing/__tests__/no-unbuilt-claims.test.ts` 3종.
금지어 목록이 아니라 **조건부**로 썼다: "런타임에 LLM 을 부르는 코드가 없으면 마케팅도
LLM 을 판다고 말하지 않는다." 진짜로 붙이는 날 앞 조건이 풀려 테스트가 조용해진다.
(옆 파일 `no-hardcoded-stats` 가 *수치* 를 막고, 이쪽이 *기능 주장* 을 막는다.)

**진단 절차 자체의 결함도 고쳤다** — §3 A2 가 `pg_stat_user_tables.n_live_tup` 을 쓰고 있었다.
추정치라 VACUUM 이후 크게 어긋난다(오늘 `shared_dictionary` 를 654 행으로 보고, 실제 48,962 —
75배 과소). 87개 테이블 전수 정확 카운트로 교체했다. 2회차 초안이 상위 테이블을 손으로
고르다 `library_book_vocabularies`(167만)·`spelling_norm`(31만)을 빠뜨려 공급 규모를
3분의 1로 잡았던 것도 같은 질의로 막힌다.

**F7(문서 드리프트) 부분 해소** — `CLAUDE.md` DB 통계를 실측으로 교체하고,
손으로 고치는 한 다시 낡는다는 경고를 블록 머리에 달았다
(테이블 87→105 · 함수 327→358 · migrations 428→510 · DB 350 MB→1,996 MB ·
`library_books` 20→401 · `vocabularies` 5,896→2,205).

### 선생님이 보낸 낱말이 **"오늘 할 일" 에 도달하지 못했다**

30배 곱셈(교사 3,500 × 학급 30)은 학생 30명이 **다시 오는** 데 달렸고, 다시 오는 이유는
`/hub` 의 오늘 할 일이다. 사이클 35에서 due 큐까지는 확인했지만 `/hub` 는 다른 질의를 쓴다 —
밟아본 적이 없었다.

**실측**: 학생이 선생님이 보낸 낱말 3개를 실제 UI 로 담고("담았어요" 확인, `vocabularies` 에
`origin='assignment'` · 뜻·표제어까지 정확히 적재) `/hub` 로 갔더니 화면이 **조금도 달라지지
않았다** — 여전히 "아직 시작 전이에요 — 5분이면 오늘 할 일이 생겨요" 와 무작위 표본 단어였다.

원인:

    isEmpty = total === 0 && attention === 0 && streak === 0
      total     — 진단 전이면 강제로 0/0
      attention — risk + shaky 만. 한 번도 복습 안 한 낱말은 4상태의 new 라 어디에도 없다

`new` 를 뺀 것은 원래 맞는 판단이었다 — 학습자가 스스로 뽑아 담던 시절엔 **담은 사람이
이미 본 사람**이다. **교사가 보낸 낱말이 생기면서 그 전제가 깨졌다.** 학생은 그 낱말을
본 적이 없고, 그것이 가장 먼저 할 일이다.

- `TodayStatus.fresh` 신설 — `growth-stats` 가 이미 `memory.fresh` 를 계산하고 있었다.
  상태 질의가 넘기지 않았을 뿐이다
- `isEmpty` 가 `fresh` 도 본다 — 안 보면 **할 일이 있는 사람에게 없다고 말하게 된다**
- ⚠️ `attention` 에 **합치지 않았다.** "복습이 급하다" 와 "아직 안 배웠다" 는 다른 일이고,
  합치면 기존 숫자의 뜻이 조용히 바뀐다. 띠에도 별도 칸(`새 단어`)으로, 색도 `--memory-new`
  (warm gray)로 구분한다 — 점 + 라벨 + 숫자 3중(색맹 대응)
- 회귀 4 — 비어 있지 않음 판정 · attention 미합산 · 기존 판정 불변 · 음수 방어

실측 확인: 담은 뒤 띠가 **"새 단어 11"**(기존 시드 8 + 과제 3 — 전부 미복습이라 정직한 수).
검증용 학급·과제·참여·낱말은 전부 삭제, 관련 표 0행 복귀.


### 빈 상태 — 화면이 **틀린 다음 할 일**을 말하고 있었다

목표: *교사·학생이 실제로 마주치는 빈 상태마다 "다음에 할 일" 이 보인다.*
상태를 만들어 직접 봤다(제품이 사람을 잃는 자리는 대개 빈 화면이고,
이 저장소는 `return null` 을 여러 곳에서 쓴다).

| 상태 | 화면이 말한 것 | |
|---|---|---|
| 교사 · 과제 보냄 · 학생 0 | "0/0 · 아직 학생이 없어요 — 초대코드를 나눠 주세요" | ✅ 원래 좋았다 |
| 교사 · 학급 1 · 학생 0 · **과제 0** | "학생 0명" + 코드 칩. 다음 할 일은 **"단어 보내기"** | ❌ |
| 학생 · 참여 중 · 받은 것 0 | 반 이름 **한 줄**. 그 외 아무것도 | ❌ |

- **보낼 대상이 없는데 "단어를 보내세요" 라고 하면 그 화면은 틀린 말을 하는 것이다** —
  교사는 시킨 대로 해도 아무 일이 안 일어난다. 학생이 0명이면 다음 할 일을
  **"학생 부르기"** 로 바꾼다. 들어온 뒤에 보내기로 돌아온다
- 학생 0명인 학급 줄에 코드가 무엇인지 적는다 — 실측했을 때 첫 화면에 `TSTEM1` 이라는
  칩만 덩그러니 있었다. 처음 온 교사가 그것이 초대 링크라는 걸 알 방법이 없었다
- 참여했는데 받은 것이 없는 학생에게 무엇을 기다리는지 말한다
  (`ReceivedAssignments` 는 빈 목록에서 `return null` 이라 아무것도 안 그린다).
  **들어온 직후가 정확히 그 상태**이고, 그때 침묵하면 학생은 잘못 들어온 줄 안다
- 회귀 5 (`renderToString`) — 두 분기를 **양쪽 다** 고정한다: 학생 0이면 "부르기" 이고
  "보내기" 가 **없어야** 하며, 학생이 있으면 반대다(초대 안내 줄도 사라진다)

검증용 학급·과제·참여 기록과 로봇 방문분은 전부 삭제 — 관련 표 모두 0행 복귀.


### 전수 훑기 축 신설 — "화면이 뜬다" 에서 **"화면이 뜨고 그 데이터도 온다"** 로

바를 올렸다. 기존 축은 전부 *화면이 뜨는가* 를 물었다(열림·조용함·연결·복귀·가로스크롤·탭대상).
그런데 **화면은 멀쩡히 뜨고 콘솔도 조용한데 그 화면의 데이터가 안 오는** 경우는 아무도 안 봤다.
실측으로 셋 다 확인했다 — `26-learner` · `30-admin` · `33-public` 의 `requestfailed`/`on('response')` 사용은 **0**.

- **요청 축 추가** (`tests/e2e/utils/net-watch.ts`) — 같은 출처 4xx/5xx 를 라우트별로 센다.
  401/403 은 **로그아웃 훑기에서만** 무해로 거른다(`NetContext`) — 로그인한 사람이 자기 API 에서
  401 을 받으면 그건 결함이지 잡음이 아니다. 계측기 자가검증(일부러 404 를 내고 잡히는지)까지 하고 붙였다.
- **에러 화면 판정 단일화** (`tests/e2e/utils/crash-screen.ts`) — `app/error.tsx`·`not-found.tsx` 는
  **HTTP 200 에 본문도 충분해서** 여섯 축이 전부 통과한다. 판정을 셋이 각자 적어 두었더니 실제로 갈라져 있었다:
  - `30-admin` 의 조건이 **죽어 있었다** — `X === false && false` 는 **항상 false**.
    관리자 33화면은 에러 경계로 떨어져도 **영영 초록**이었다.
  - `33-public` 은 `문제가 발생했어요`(에러 경계)를 **안 보고 있었다.**
  - 판정기 자체를 재는 것이 없어서 살아남은 버그라, `src/lib/__tests__/sweep-detectors.test.ts`(6) 신설.
    "정상 화면의 `다시 시도` 버튼을 에러로 오해하지 않는다" 가 핵심 케이스다 — 죽은 조건의 원래 의도가 그것이었다.
- **관리자 훑기 예열** — 이 훑기는 dev 우회가 `NODE_ENV==='production'` 에서 하드 게이트라
  **dev 서버에서만 돌 수 있고**, dev 는 라우트마다 첫 방문에 컴파일한다. 예열을 넣되
  브라우저가 아니라 **HTTP GET 으로만** 한다(첫 판은 33화면을 두 번 렌더해 **26분·타임아웃**이었다).

**측정** — 학습자 **220/220 = 100%**(분모 177 → 220, 라우트당 +1) · 관리자 **33화면 5축 전부 0** ·
공개 12화면 0.

### `/admin/vrl` — 분포가 **조용히 비어 있었다** (마이그레이션 `20260826160000`)

위 요청 축을 붙이던 중 dev 로그에서 발견. 화면은 **HTTP 200 으로 떴고** 어떤 축도 못 잡았다 —
서버 컴포넌트 안에서 RPC 가 죽으면 화면은 그냥 분포가 빈 채로 정상 렌더된다.

- **원인**: `dict_categorical_distributions()` 가 `shared_dictionary`(48,962행 · 155MB · 60컬럼)를
  **일곱 번 따로** 전체 스캔. 집계 하나가 1,206ms(EXPLAIN 실측) × 7 ≈ 8.4초 → `statement timeout`.
- **고침**: 필요한 7컬럼만 `MATERIALIZED` CTE 로 **한 번** 훑고 일곱 집계를 그 위에서 돌린다.
  두 번째 집계부터 16~19ms. **적용 전 같은 DB 에서 결과 동일성 대조**(`= 기존함수() → true`) — 순수 성능 변경.
  웜 405ms → **205ms**(최대 725 → 206, 편차 소멸) · `/admin/vrl` **9.17초 → 1.4초**.
- **가려져 있던 결함이 드러났다** — RPC 가 성공하기 시작하자 `/admin/vrl` 이 통째로 에러 경계로 떨어졌다.
  `jsonb_object_agg` 는 **그룹이 0행이면 SQL NULL** 을 준다. `shared_dictionary` 의 RLS 정책은
  `authenticated`·`service_role` 뿐이고 **`anon` 정책이 없는데**, dev admin 우회는 합성 admin 이라
  실 세션 없이 `anon` 으로 질의한다 → 일곱 개 전부 null → `Object.entries(null)`.
  그동안은 **RPC 가 항상 먼저 타임아웃해서** 상위 가드가 받아 주고 있었다.
  - `DictCategoricalDistributions` 타입이 전부 non-nullable 이라고 **거짓말**하고 있었다.
    소비자 대부분(`queries.ts`·`word-search.ts`·`critical-defects-detector.ts`)은 이미 `?? {}`·`?.` 로
    막고 있었고 **타입만 혼자 다른 말을 했다** → 사실대로 nullable 로 정정(`tsc` 0 오류).
  - 빈 분포를 **말없이** 그리지 않는다 — 막대 0개 상자는 "균일하다" 로도 "없다" 로도 읽힌다.

### QR → 학생 도착 경로를 **폰에서 처음 끝까지 밟았다**

목표: *QR 로 들어온 학생이 폰에서 이탈 없이 반에 도착하고 과제를 받는다.*
측정은 단계마다 ① 390px 가로 넘침 ② 44px 미만 조작 요소 ③ 콘솔 에러.

앞선 사이클들에서 조각을 이었지만 **한 번도 이어서 밟아 본 적이 없었다**
(`class_members` 0행 — 이 경로는 존재한 적이 없다). 검증용 학급·과제를 만들어 걸어 봤다.

| 단계 | 가로 넘침 | 44px 미만 | 콘솔 에러 |
|---|---|---|---|
| 익명 `/join/[code]` | 0 | 0 | 0 |
| `/signup?next=…` | 0 | 0 | 0 |
| 로그인 `/join/[code]` | 0 | 0 | 0 |
| 참여 후 `/teacher` | 0 | **3 → 0** | 0 |

- **`/teacher` 의 개설·참여 입력과 버튼이 높이 40px 이었다**(`h-10`). 저장소 자체 규칙은 44px 이고,
  하필 **학생이 참여 직후 처음 보는 화면**이자 교사의 주 동작이다 → `h-11`
- 나머지는 이어져 있었다: 학급 이름 · 받은 단어 · 과제 제목 · 낱말 · 담기 버튼까지 전부 도착
- 마케팅 내비 링크(`요금제` 37×44 · `소개` 25×44)는 **높이는 44**이고 너비만 짧다 —
  텍스트 링크의 간격 예외(WCAG 2.5.8)로 이 저장소가 이미 그렇게 둔 자리다. 손대지 않았다

**밟는 도중 알게 된 것 둘:**

1. 개발 모드에서 하이드레이션 **전에** 로그인 버튼을 누르면 폼이 **네이티브로 제출**돼
   `GET /login?` 로 되돌아온다(`auth/v1/token` 요청이 아예 안 나간다). 자격증명은 멀쩡했다 —
   **아무 메시지 없이 입력만 지워진다.** 프로덕션에서는 번들이 캐시돼 사실상 안 걸리지만,
   회귀에는 `networkidle` 대기를 넣어 이 함정에 다시 빠지지 않게 했다
2. `funnel_events` 에 `teacher_hub_view` 가 **처음으로 기록됐다** — 물려받은 계측이 실제로
   작동한다. (로봇 방문분은 지표를 오염시키지 않게 지웠다)

- 회귀 e2e 2 (`36-student-arrival`) — **학급·과제를 스스로 만들고 지운다**(공유 개발 DB).
  service role 키가 없으면 통째로 skip. 숨긴 요소(포커스 때만 나타나는 건너뛰기 링크)는
  세지 않는다 — 그것까지 세면 매 화면이 1개씩 걸려 **진짜 위반이 묻힌다**


### 종이 → 웹 — 학급 초대 QR (`qrcode-generator` 2.0.4)

목표를 올렸다: *교사의 종이 한 장이 학생 30명을 웹으로 데려온다.*
산술이 성립하는 유일한 경로가 **교사 3,500명 × 학급 30명**인데, 그 30배가 실제로
일어나는 자리는 교실에서 나눠 주는 종이다. 그 종이에 돌아올 길이 없었다.

**설계가 실측에서 한 번 뒤집혔다.** 처음에는 `/fit` 결과 공유 링크를 QR 에 담으려 했다:

```
/fit/s/<payload>   434자 → QR 81×81 → 30mm 인쇄시 0.37mm/모듈  ← 복사본에서 안 읽힘
/join/ABC123        32자 → QR 29×29 → 30mm 인쇄시 1.03mm/모듈
```

교실에 도는 유인물은 대개 **복사본**이다. 그리고 낱말은 이미 종이에 인쇄돼 있다 —
학생이 QR 로 얻어야 하는 것은 낱말이 아니라 **자기 반에 들어가는 길**이고, 들어가면
교사가 보낸 과제로 같은 낱말을 받는다(그 경로는 이미 이어 뒀다).

- `lib/worksheet/qr.ts` — SVG QR 단일 출처. **긴 주소는 거부한다**(`MAX_QR_URL_LENGTH`) —
  조용히 촘촘한 QR 을 내보내면 교사는 **인쇄해서 나눠 준 뒤에** 아무도 못 읽는 것을 안다.
  비트맵이 아니라 SVG 인 이유: 인쇄기 해상도로 다시 그려져야 모듈 경계가 또렷하다
- `components/worksheet/PrintSheet.tsx` — 두 자리(`/fit`, `/text/new` 추출)가 **같은 종이**를
  쓴다. 갈라지면 한쪽만 이름 칸이 없는 유인물이 같은 교무실에 돈다
- `ClassWorksheet` — 추출 결과 옆, 보내기 버튼과 나란히. 학급이 여럿이면 QR 이 가리킬 반을 고른다
- `/fit` 의 QR 은 학급이 아니라 `/fit` 을 가리킨다 — 그 종이를 만든 사람은 로그인조차 안 했고,
  다음 독자는 학생이 아니라 **그것을 집어 든 옆자리 교사**다

**검증**: 생성한 QR 을 렌더해 **독립 디코더(jsqr, 프로젝트 의존성 아님)** 로 되읽었다 —
354px(30mm 상당) · 170px · 113px(10mm 상당) 전부 정확히 복원. 인쇄 지면에서 잘라낸
219px QR 도 `https://vocaflow.app/fit` 로 디코드됐다. 내가 쓴 인코더를 내가 검사하지 않았다.

- 회귀 4(단위: 밀도·거부·크기) + e2e 3(쪽수·지문 미유출·**QR 실림**)


### 인쇄 학습지 — 교사에게 내놓는 **첫 실물** (`@media print` 가 0개였다)

목표: *교사가 지문 하나로 "수업에 그대로 쓸 수 있는 인쇄물"까지 이탈 없이 도달한다.*
산출물 ① 난이도 ✅ ② 어휘 목록 △ ③ **인쇄물 ❌** ④ 학급 배포 ✅ → **③ 을 지었다.**

교사의 수업 준비는 화면에서 끝나지 않는다. 유인물·쪽지시험이 교무실과 교실에 남고
옆자리 선생님이 그것을 집어 든다 — CAC 0 채널에서 브랜드가 **실제로 옮겨 다니는 경로**다.
기존 출구 셋(결과 링크 복사 · 단어 탭 복사 · 가입) 중 앞의 둘은 "다른 도구에 붙여넣어
알아서 만드세요" 였다.

- `Worksheet` — A4 두 장: **어휘 목록**(나눠 주는 것)과 **빈칸 확인**(걷는 것).
  교사는 대개 둘 다 만들므로 기본이 `both` 다. 학년·반·이름 칸은 한국 유인물의 관례다
- **지문은 인쇄하지 않는다.** `/fit` 은 붙여넣은 글을 저장하지 않고(그것이 이 화면의 약속),
  교과서·문제집 지문은 남의 저작물이다. 낱말과 뜻만 옮긴다
- 교육과정 표시를 함께 찍는다(`*` 초등 · `**` 중·고 · `·` 그 외 · 빈칸은 목록 밖) +
  고시 출처 한 줄 — 교사가 학교에서 "이 기준이 뭐냐" 는 질문을 받았을 때 그 줄이 답이다
- 색을 쓰지 않는다 — 학교 인쇄기는 대부분 흑백이고 회색조로 떨어진 옅은 토큰 색은 읽기 어렵다
- `fit_worksheet_printed` — **가입보다 강한 의도 신호**다. 링크 복사는 "괜찮네" 지만
  인쇄는 "오늘 수업에 쓰겠다" 는 뜻이다. 인쇄는 브라우저에서 끝나 어떤 표에도 안 남는다

**만들고 나서 인쇄해 보고 고쳤다.** 처음에는 학습지를 결과 화면 안에 두고
`visibility: hidden` 으로 나머지를 지웠다. **잉크는 정확했고** 화면 캡처로도
print 미디어 스크린샷으로도 멀쩡했다 — 그런데 **문서 높이가 그대로라
1장짜리 목록이 4쪽으로 인쇄됐다.** 교사는 그것을 인쇄기 앞에서, 종이 세 장을 버린 뒤에 안다.

`display:none` 은 조상과 함께 학습지까지 지워 쓸 수 없다. 그래서 학습지를
**`document.body` 직속으로 portal** 하고 `body > *:not(.vf-sheet)` 만 접는다 — 높이가 함께 접힌다.
실측: 목록 1쪽 · 빈칸 1쪽 · 둘 다 2쪽.

- 회귀 e2e 2 — **실제 PDF 를 만들어 쪽수를 센다.** 이 실패를 잡는 다른 방법이 없다.
  두 번째는 지문 유출 검사(원문 구절이 PDF 에 없어야 한다 — 압축될 수 있어 **없음만** 신뢰하는 한 방향 검사)


### 기사 낱말 드레인 — 학습자 적중 96.8% → 99.1%, 그리고 두 파이프라인이 같은 낱말을 따로 판정하고 있었다

방치돼 있던 `scripts/dict/article-lemmas` 청크(2026-08-20 생성, 1,428낱말)를 재측정하니
**978건이 그새 다른 경로로 해소**돼 있었다 — 실제 갭은 450낱말(8청크)뿐이었다(3.2배 부풀려진 값).
Claude Code 로 8청크 전부 판정해 **312낱말 적재**, `실제 적중 13,677/14,127 → 13,994/14,127`.

- `drain-article-lemmas.mjs --export` 가 `noise_blacklist`·`proper_noun_forms` 를 **보지 않던 것** 수정 —
  `skip: true` 는 어디에도 기록되지 않아 URL 조각·학명 종소명·중세 영어가 export 마다 다시 나왔다.
  두 목록을 제외하니 즉시 **37건**이 걸러졌다(133 → 96). 자매 드레인이 이미 판정한 것들이다.
- 실측으로 드러난 중복: article-lemmas 잔여 1,010건 중 **747건(74%)** 이 `pending_words` 큐와 겹쳤다.
  두 파이프라인이 같은 낱말을 각자 판정하고 있었다.
- `pending_words` 드레인 4청크(240낱말) 판정·적재 — 사전 +34 · 고유명사 +92 · 노이즈 +126.
  같은 날 유입 실측 **시간당 ~600건** — 남은 청크를 다 채워도 4.7시간분에 그친다(병목은 적재 방식).

### 수능 영어 출제 설계도 37/37 완성 — 185/185 = 100% ([CSAT_TYPE_BLUEPRINTS.md](./CSAT_TYPE_BLUEPRINTS.md))

"기출과 같은 수준으로 출제한다" 를 잴 수 있는 형태로 바꿨다 —
**분모 37 유형 × 요건 5 = 185칸**(R1 설계 서술 · R2 제약 실측 · R3 계측 대역 · R4 생성 절차 · R5 생성 검증).
분모는 문서가 아니라 기출에서 뽑았다(2023~2026 중 1회 이상 출제). 기준선 18.9% → **100%**.

- 새 스크립트 7 — `build-type-inventory` `build-bands-all` `measure-type-constraints`
  `measure-response-modes` `verify-blueprint-items` `test-blueprint-coverage` `test-exam-assembly`
  `render-blueprints`(문서 생성 — **`docs/CSAT_TYPE_BLUEPRINTS.md` 는 손으로 고치지 않는다**)
- **가족 제약 G1~G4** 신설 — 유형 하나(n=13~15)로는 Holm 을 못 견디므로 같은 설계 원리를 공유하는
  유형을 묶어 검정한다. G1 대응형 ①회피 0/199(p=6e-28) · **G4 신규** 순열형 ①회피 0/40 ·
  G3 한글 선지 3점 0/186 · G2 비대응형은 ①이 최빈 107/406
- **새 발견 — 짧은 응답은 한 유형이 아니라 둘이다.** 대본 7개년 28문항이 36~54낱말 14개 /
  127~163낱말 14개로 갈리고 **그 사이 55~126 구간은 0건**
- 계측기 결함 3건 수정 — 기호형(어법·어휘·삽입)을 한국어 선지로 오분류해 G3 가 죽어 있었고,
  장문 43~45 지문이 오염 임계 오적용으로 14개년 중 2편만 남아 있었고,
  순서형 선지 동점을 세어 "정답이 최장 13/13" 이라는 가짜 HARD 가 나왔다
- **회차 조립 검사** 신설 — 문항 규칙을 다 지킨 37문항이 모아 놓으니 정답 번호가 ①0 ②9 ③11 ④13 ⑤4 였다
  (기출은 14개년 내내 8~11). G1(대응형 ①회피)을 비대응형에까지 적용한 오용이었다 → 7/8/7/8/7 로 교정
- ⚠️ 이 100% 는 **양식 일치**의 100% 이지 정답성의 증거가 아니다
  (어휘 유사도의 정답 적중 27.3%, 기저 20%). 대리 대역 2건(L-MAIN·L-TODO)도 표시해 두었다

### 교육과정 기본 어휘 축 — **제품이 이미 갖고 있으면서 한 번도 쓰지 않은 것** (마이그레이션 `20260826140000`)

목표를 올려 물었다: **교사 한 사람의 수업 준비 1회가 이 안에서 끝나는가.**
① 난이도 판정 ② 어휘 목록 ③ 인쇄 가능한 학습지 ④ 학급 배포 — 실측 **2/4**
(저장소 전체에 `window.print`·`@media print` **0개**).

그런데 더 근본적인 것이 나왔다. `/fit` 은 "고1에게 92%" 라고 말하는데, 그 V-Level 사다리는
**자기참조**다(`LEVEL_LABEL` 이 `CATEGORY_VLEVEL` 을 따를 뿐 바깥 기준에 정박해 있지 않다).
교사가 그 숫자를 믿을 근거가 제품 안에만 있다.

**딥서치 결과**: 2022 개정 영어과 교육과정 **기본 어휘 3,000개** — `*` 800(초등) ·
`**` 1,200(중·고 공통) · 나머지 1,000. 교육부 고시 제2022-33호 [별책 14] pp.254-290.
**교과서 검정이 이 목록으로 이뤄진다**(KICE Word Lister). 교사·출판사·평가원이 이미 같은 말을 쓴다.

**그리고 그 데이터가 이미 `shared_dictionary.list_tags` 에 있었다**:

| 태그 | 수 | 평균 V | 표본 | 고시 |
|---|---|---|---|---|
| `kcurr2022_1` | 808 | 1.83 | be·in·have·it·he | `*` 초등 800 |
| `kcurr2022_2` | 1,211 | 3.68 | other·mean·through | `**` 중·고 1,200 |
| `kcurr2022_0` | 1,006 | 5.93 | accord·apparent·former | 그 외 1,000 |

한 낱말이 두 계층을 갖는 경우 **0건**. `kice-csat-13y` 5,254개(수능 13년치) 중
**3,108개가 교육과정 밖** — 그 자체가 교사에게 의미 있는 사실이다.

- `curriculum_bands` RPC (SECURITY DEFINER · anon) — `/fit` 은 로그인 없이 쓰는 화면인데
  익명은 `shared_dictionary` 를 못 읽는다(공개 경로는 `shared_words`+`lexicon_clean`)
- `lib/textfit/curriculum.ts` 가 밴드의 뜻을 소유한다 — **태그 이름의 숫자는 난이도 순서가
  아니다**(`_1` 이 초등이고 `_0` 이 가장 어렵다). 그대로 쓰면 화면에서 순서가 뒤집힌다
- `CurriculumPanel` — 큰 숫자는 **밖**이다. 안에 있는 것은 수업에서 다룰 이유가 적고
  밖이 유인물·각주·사전 찾기를 부른다. 목록 밖 낱말을 뜻과 함께 나열해 그대로 가져다 쓴다

**만들고 나서 틀린 것을 찾아 고쳤다** — 고시 목록은 **원형만 싣는다**:

```
teach ✓ / teacher ✗    compute ✓ / computer ✗    differ ✓ / different·difference ✗
```

파생어는 규칙으로 인정되는 것이 이 목록의 관행이라, 그대로 대조하면 `teacher` 가
"교육과정 밖" 으로 세어진다. 표본 지문에서 **밖 10개 → 실제 6개**였다.
사전의 `derived_forms` 로만 물려준다(**접미사를 추측해 만들지 않는다**) — `beautiful` 은
`beauty.derived_forms` 에 없어 여전히 밖이다. 지어내지 않은 정직한 한계다.
화면도 "4개는 원형에서 인정 (teach → teacher)" 로 대조 방식을 밝힌다.

⚠️ **수능 표시는 파생형으로 확장하지 않는다.** 밴드는 "이 목록에 속하는가" 라는 분류지만
출제는 **일어난 일**이다 — `teacher` 가 나왔다고 `teach` 가 나온 것이 아니다.

- 조회 실패는 `null` 이다(빈 Map 이 아니다). 빈 Map 을 돌려주면 **모든 낱말이 "밖"** 이 되어
  멀쩡한 지문에 "밖 47개" 라는 거짓 경보가 나간다. `null` 이면 칸을 통째로 감춘다 —
  **틀린 숫자보다 없는 칸이 낫다**
- `unresolved`(사전에 없는 토큰)는 세지 않는다 — `Prague` 같은 고유명사가 "교육과정 밖 어휘" 로
  잡혀 숫자가 부풀고, 그 사실은 `breakdown.unresolved` 가 이미 따로 말한다
- 회귀 7 (단위) + e2e 1 — **익명 실행 권한이 빠지면 유효한 지문도 칸이 통째로 사라지고
  페이지는 200 인 채 사다리도 멀쩡하다.** 눈으로는 못 잡는다


### 수능 설계도 — 어휘 유사도의 상한을 숫자로 박고, 홀드아웃을 20/20 으로 닫았다 (§10.38~10.40)

- ⭐ **어휘 유사도는 정답을 모른다(P10.18 기각)** — 지문과 가장 닮은 선지를 정답으로 고르면
  **24/88 = 27.3%**, 기저 20% 대비 이항검정 **p=0.0618**, Wilson 95% 신뢰구간 **19.1~37.4% 로 기저 포함**.
  문항의 **71%(65/91)에서 어휘로는 오답이 더 그럴듯하다**(빈칸 75% · 제목 79%).
  빈칸은 **24%가 정답과 지문의 겹침이 0**이다
- ⭐ **그래서 지금까지의 모든 대역은 "정답인가" 가 아니라 "기출처럼 보이는가" 를 재고 있었다** —
  표면 96.1% 와 선지 16/16 은 **양식의 일치**이지 정답성의 증거가 아니다. 그 구분이 수치로 고정됐다
- ⭐ **대조: 손판독은 같은 자리에서 정답을 가른다** — 추상도 +0.295 · 반향 +0.315, 둘 다 p=0.0001.
  남은 축(바꿔 쓴 재진술)은 **원리적으로** 못 재는 것이 아니라 **이 도구로** 못 재는 것이다.
  ⚠️ 다만 생성 문항의 저자가 나이므로 **내가 판독하면 블라인드가 아니다** — 저자 아닌 판독자가 필요하다
- ⭐ **홀드아웃 20/20 (P10.19)** — 16/20 에서 막혀 있던 4칸이 전부 **접근성 계열**이었고,
  원인은 루프가 **오답만 고르고 정답은 고정**한 것이었다(H2 접근성 0.1149 vs 대역 상단 0.0565).
  `fit-choice-band.mjs` 가 **정답 선지 후보**도 받도록 확장 → 정답 3개로 18/20, 오답 11개로 **20/20**
- **파이프라인 요구량 실측** — 후보 풀 곡선 4개 **12/20** · 6개 15/20 · 8개 18/20 · **9개 20/20(닫힘)**.
  → **문항당 오답 후보 9개 이상 + 정답 후보 3개.** 최소치로는 60% 밖에 못 닫는다
- 명제 125 → **127**: P10.18(기각) · P10.19(SOFT) 신설, P10.12 갱신
- 커버리지 **판정 99.2%(126/127) · 전수 65.4% · 규칙 22.0%**
- ③ 최종 지도 정리 — **양식은 맞출 수 있고 정답성은 이 도구가 모른다.** 둘을 합쳐 "몇 % 부합" 이라 쓰면 거짓말이 된다
- 회귀 — design-spec 14규칙 위반 0 · 모평 홀드아웃 0 · 선지규칙 C1 3관문 · D1 규칙 3관문


### 큐가 572건 중 489건이 쓰레기였다 — 청소하고 5권을 되살렸다

앞 항목에서 "83권 되살리기 전에 묵은 메시지를 치워야 한다" 고 적었고, 치우려고 세어 보니
**큐의 85%가 처리하면 안 되는 것**이었다:

| 큐에 있던 것 | 수 | |
|---|---|---|
| 이미 `ready` 인 도서 | 311 | **전부** 메시지가 도서의 `updated_at` 보다 오래됐다 = 이미 지나간 일 |
| 도서가 **삭제되고 없음** | 168 | 소스 GET 복귀(DELETE 시맨틱)로 사라진 행. 처리하면 그냥 실패한다 |
| 이미 `published` | 10 | 위와 같음 |
| `failed` 도서의 묵은 메시지 | 82 | 되살리면 새 메시지 위에 겹친다 |
| `queued` | 1 | 직전 항목의 트리거 검증분 |

전부 `read_ct = 0` 이었다 — **한 번도 읽힌 적이 없다.** dev 는 `get_lcp_config()` 가 NULL 이라
cron worker 가 early return 하고, 드레인은 `/api/lcp/dev-drain-queue` 가 도서 단위로 돌기
때문에 큐를 읽지 않는다. 그래서 6월부터 쌓이기만 했다(가장 오래된 것 2026-06-03).

- 489건 archive(삭제 아님 — `pgmq.a_library_pipeline` 으로 이동) · 실패분 82건도 archive
- **되살리기 전에 네트워크를 직접 쟀다** — 실패 사유가 전부 `fetch failed` 였으므로
  그것이 아직 유효하면 되살려도 그대로 다시 죽는다. 실패 도서 5권의 실제 수집 주소
  (`standardebooks.org/ebooks/<id>/text/single-page`)를 curl 로 받아 **5/5 성공 · 평균 2.2초**.
  일시적 오류였다는 것이 확인됐다
- 5권 되살림(전량 83 이 아니라) — 처리는 dev 드레인 UI 가 하고 **나는 그것을 돌릴 수 없다**.
  검증 없이 83권을 `queued` 로 바꾸면 화면만 "83개 대기" 가 된다

**결과 실측**: 큐 **572 → 6** · 메시지 6건 / 도서 6권 = **1:1**(트리거가 정확히 한 번씩) ·
`failed` 83 → **77** · archive 누적 896.

남은 것: `/admin` → LCP 큐 드레인으로 6권을 처리하고 성공률을 본 뒤 나머지 77권 결정.
...
`fetchDueGameWords` 는 새 낱말을 잘 다룬다 — `next_review_at` 이 NULL 인 것을
`nullsFirst: true` 로 **맨 앞**에 두고, `lemma ?? word` 로 표제어가 없어도 논다.
그런데 **`.neq('meaning','')` 로 뜻이 빈 낱말을 거른다**(빈 뜻으로는 문제를 못 만든다).
과제 낱말을 `w.m ?? ''` 로 넣고 있었으니, 뜻 없이 온 낱말은 단어장에 들어가고도
**영영 안 풀리는 죽은 낱말**이 됐다.

- 사전 조회를 **한 번에 두 가지**로 — 표제어 실재(FK)와 뜻. 이미 하던 조회에 컬럼 하나를 더한다
- 교사가 보낸 뜻이 우선(학생이 과제 카드에서 본 것이 그것이다), 없으면 사전에서 채운다.
  공백뿐인 뜻도 빈 것으로 본다 — DB 는 통과시키지만 게임은 거른다
- 둘 다 없으면 빈 뜻으로 넣되 **수를 세어 화면이 사실대로 말한다** —
  "N개는 뜻이 비어 있어 단어장에는 담겼지만 게임에는 나오지 않아요".
  조용히 버리지도, 조용히 죽이지도 않는다. (사전은 소문자 표제어만 가지므로
  고유명사가 이 경우다 — 실측: `Prague` 는 표제어가 아니다)
- 회귀 10 (기존 8 → +2): 사전 채움 · 교사 뜻 우선 · 공백 처리 · 셀 수 있음
- 확인한 것: 학습자 쿼리에 `origin` 필터 없음 · `lemma` NULL 을 요구하는 조회 없음 ·
  새 낱말이 due 큐 맨 앞에 온다


### 로그인한 사람이 랜딩에 서 있었다 — 미들웨어에서 /hub 로

랜딩을 만들 때 남겨 둔 것을 닫는다. 미들웨어가 `/` 를 그냥 통과시켜 **로그인한 사람도
가입 CTA 가 걸린 마케팅 화면**을 봤다(헤더에는 "로그인" 버튼까지 있었다). 주소만 치고
들어오는 재방문자에게 이미 내린 결정을 다시 묻는 화면이다.

- `middleware.ts` — `if (user && pathname === '/') return redirectTo('/hub')`.
  세션 조회(`getUser`)는 그 위에서 이미 하므로 왕복이 늘지 않는다
- **페이지 안에서 세션을 읽어 갈라 놓지 않는다** — `/` 는 `revalidate = 86400` 인 ISR 이라
  페이지가 auth 를 조회하는 순간 캐시가 깨지고 **크롤러가 받는 화면까지 매 요청 렌더**된다.
  미들웨어는 캐시 앞에서 돌기 때문에 익명 요청은 그대로 캐시된 랜딩을 받는다
  (이것이 랜딩 작업 때 "다음 사이클" 로 미뤄 뒀던 이유이고, 자리를 옮기니 그 이유가 사라진다)
- 회귀는 `landing-contract.test.ts` 에 붙인다(7종) — 런타임 동작이라 다른 테스트로는 안 잡혀
  **소스에 규칙이 있는지**를 못 박는다. ⚠️ 단언을 `pathname === '/'` 로 쓰면
  `skipsStatusCheck` 안의 같은 줄에 걸려 **규칙이 없어도 통과**한다 —
  로그인 조건과 붙어 있는지(`user && pathname === '/'`)를 본다

⚠️ **순서가 중요했다** — 이 리다이렉트를 먼저 넣었으면 모바일에서 `/teacher` 로 가는
**마지막 길이 끊겼다.** 랜딩 CTA 가 그때까지 유일한 통로였고, 바로 앞 커밋의
`MobileUtilityBar` 가 생긴 뒤에야 안전하다. 미들웨어에 그 사실을 주석으로 남겼다.

### `단어장에 담기` 가 **아무것도 담지 않고 있었다** (마이그레이션 `20260826123000`)

초대 구간을 이은 뒤 그 **다음**을 밟았다. 학생이 학급에 들어오면 교사가 보낸 낱말을
받는다 — 그 낱말이 학습 자료가 되는 것이 교사 사슬의 **존재 이유**다.

`markAssignmentCollected` 는 `class_assignment_progress.collected_at` 만 찍었다.
**학생의 `vocabularies` 에는 아무것도 들어가지 않았다.** 그런데 버튼은 `단어장에 담기` 이고
누르면 `담았어요` 로 바뀌며, 교사 대시보드는 그 숫자를 세어 "N명이 담았어요" 라고 말했다.
화면도 DB 도 오류를 내지 않는다 — **하지 않은 일을 했다고 말할 뿐이다.**

- 담기가 실제로 `vocabularies` 에 넣는다. 낱말은 **과제 행에서 다시 읽는다** —
  클라이언트가 보낸 목록을 믿으면 남의 낱말도 넣을 수 있다(RLS 가 소속 학급만 열어 준다)
- **`origin='assignment'` 신설** — `origin` 에는 삭제 의미가 붙어 있다.
  `unenroll_library_book` 은 도서 해지 시 `origin='shared_set'` 낱말을 지운다.
  그 값을 재사용하면 **무관한 도서를 해지했을 뿐인데 선생님이 보낸 단어가 함께 사라진다.**
  `'manual'` 은 "학습자가 직접 넣었다" 는 뜻이라 사실과 다르다
- **`lemma` FK 함정** — `vocabularies.lemma` 는 `shared_dictionary(word)` 를 참조한다
  (실측 `vocabularies_lemma_fkey`). 사전에 없는 값이 **하나만 섞여도 일괄 upsert 전체가
  거부**돼 학생은 과제를 통째로 못 담는다. 먼저 사전에 물어보고 있는 것만 표제어를 채운다
  (없으면 NULL — FK 는 NULL 을 허용한다). 낱말 하나 때문에 나머지 스무 개를 잃지 않는다
- 이미 갖고 있던 낱말은 건드리지 않는다(`ignoreDuplicates`) — 과제가 FSRS 상태를
  초기화하면 그동안의 학습이 지워진다
- 순서: **낱말 저장 → 기록.** 뒤집으면 "담았어요" 인데 단어장은 빈, 지금 고치는 그 상태가 된다

### `/teacher` 에 **나가는 링크가 하나도 없었다**

학급을 만들고 초대 링크를 나눠 준 교사가 그다음 할 일이 화면 어디에도 없었다.
"보낸 단어" 구역은 보낸 것이 없으면 `return null` 이라 **기능이 있는지조차 안 보인다.**

보내는 기능은 이미 있었다 — `/text/new` 에서 지문을 붙여넣고 낱말을 뽑으면
`SendToClassButton` 이 나온다. 그런데 링크가 **한 방향뿐이었다**:
추출 화면은 학급을 알고(없으면 `/teacher` 로 보낸다) 학급 화면은 추출을 몰랐다.

- 학급이 있을 때만 "다음 — 우리 반에 단어 보내기" 를 보여준다(없으면 만드는 것이 순서다)
- 회귀 8 (`assignment-vocab.test.ts` 6 + 기존 갱신) — `origin` 값·NULL lemma·
  빈 표면형 걸러내기·FSRS 미기입. **`origin` 이 틀리면 몇 주 뒤 엉뚱한 순간에 낱말이 사라진다**
- 실측: `origin='assignment'` + NULL lemma 삽입 통과 · 기본값(difficulty 6·stability 0) 적용 확인 후 삭제.
  학습자 쿼리에 `origin` 필터가 없어 담긴 낱말은 단어장에 그대로 보인다


### 교사 경로가 **세 곳에서 끊겨 있었다** — 초대 링크 (마이그레이션 `20260826120000`)

분기 진단의 산술은 하나의 경로만 성립한다고 했다 — **교사 3,500명 × 학급 30명**
(광고로는 허용 CAC 가입당 ₩400 을 못 맞춘다). 그 경로를 끝까지 밟아 봤다.
`classes`·`class_members`·`funnel_events` 모두 0행이라 **한 번도 밟힌 적이 없는 길**이었다.

**끊긴 곳 셋:**

1. **초대가 링크가 아니었다.** 복사 버튼이 클립보드에 넣는 것은 맨 코드 6자(`ABC123`).
   그것을 받은 학생은 ①주소를 찾아 ②가입하고 ③`클래스` 화면을 찾아 ④붙여넣어야 했다 —
   ③은 학생이 스스로 도달할 이유가 없는 화면이고, 넷 중 하나만 틀려도 끝이다
2. **가입이 복귀 경로를 버렸다.** 로그인은 `resolveReturnTo` 로 `?next=` 를 지키는데
   가입은 무조건 `/hub` 였다. **초대받은 학생은 전원 신규 가입자**이므로, 링크만 만들었다면
   가입하는 순간 학급 연결이 사라졌을 것이다
3. **메일 인증 경로도 같았다.** `emailRedirectTo` 에 `next` 가 없었다
   (콜백은 이미 `safeInternalPath(next)` 우선인데 아무도 실어 보내지 않았다)

- `/join/[code]` 신설 — 익명에게 **가입 전에** 학급 이름·인원을 보여준다. 코드가 틀렸을 때
  가입을 마친 뒤 알려 주면 그 사람은 계정만 만들고 떠난다. 잘못된 코드에는 **가입을 권하지 않는다**
- 마이그레이션 `peek_class_by_code` — `STABLE SECURITY DEFINER`, anon 실행 허용.
  **교사 신원은 돌려주지 않는다**(이름·인원만). 코드를 아는 사람은 이미 그 학급에 들어갈 수
  있으므로 노출은 코드가 주는 권한보다 좁다
- 참여는 **버튼 한 번**을 거친다 — 이 주소는 공개라 링크 미리보기·prefetch 가 그냥 열 수 있고,
  GET 이 가입을 일으키면 마우스를 올린 것만으로 들어간다
- `noindex, nofollow` + sitemap 제외 — 초대는 **받은 사람의 것**이다. 코드가 검색 결과에
  실리면 초대가 초대가 아니게 된다 (`public-routes-in-sitemap` 이 새 라우트를 잡아
  이유를 적게 했다 — 설계대로 작동)
- 링크 모양은 `lib/teacher/invite-link.ts` 한 곳 — 한 글자만 어긋나도 학생이 도착하지 못하고
  **그 실패는 교사 쪽에 보이지 않는다**(화면은 "복사됨" 이라고 말한다)
- 새 퍼널 이벤트는 **더하지 않았다**. 익명의 링크 열람은 `auth.uid()` 가 없어 기록할 수 없고
  (`record_funnel_event` 가 의도적으로 버린다), 로그인 뒤의 참여는 `class_members` 로 파생된다.
  `funnel.ts` 의 "파생으로 못 재는 둘뿐" 원칙 유지
- 회귀 8 — 링크 모양 4 + 익명 RPC 계약 4(실행 권한·0행·반환 컬럼·비로그인 join 무효).
  **GRANT 가 빠지면 유효한 초대까지 전부 "확인할 수 없어요" 로 보인다** — 정상 실패 화면과
  구별되지 않아 눈으로는 못 잡는다
- 실측: 검증용 학급 1개를 만들어 `/join/TSTJN1`(정상) · `/join/tstjn1`(대소문자) ·
  `/join/ZZZZZZ`(없는 코드) 익명 렌더 확인 후 **삭제**(classes·class_members·funnel_events 다시 0행)


### 수능 설계도 — 논지 통합 요구 축 신설 · 배점 효과는 유형 혼합비였다 (§10.36~10.37)

§6.12 가 "못 잰다" 고 남긴 **논지의 선명함** 자리에, 못 재는 것은 그대로 두고 **옆에 잴 수 있는 축**을 세웠다 —
`measure-thesis-spread.mjs` (예측은 `data/thesis-spread-prereg.md` 에 측정 전 등록).
정답 선지와 **각 문장**의 IDF 유사도 분포로 집중도 · 유효 문장 수 · 진입 문장 비율을 잰다.

- ⭐ **빈칸과 대의파악이 계측으로 갈린다(P2.5)** — 빈칸 43 vs 주제·제목 36, Holm 3검정:
  집중도 **0.617 vs 0.426** · 유효문장 **2.32 vs 3.90** · 진입비율 **0.360 vs 0.610**, **셋 다 Holm 0.0001**.
  빈칸 정답은 **두 문장 남짓**, 대의파악은 **네 문장**에 걸쳐 있다 —
  초안 §2 "능력 정의 5군" 을 **계측으로 뒷받침하는 첫 사례**
- ❌ **배점은 통합 요구와 무관하다(P10.16, 사전 등록 후 기각)** — 합산으로는 **예측과 반대 방향**으로
  세 측도 모두 유의했는데(Holm 0.007~0.027) **유형별로 층화하면 전부 사라진다**
  (빈칸 2.34 vs 2.24 Holm 1.000 · 주제 3.56 vs 3.96 Holm 1.000 · 제목은 3점 3문항이라 판정 보류).
  원인은 **유형 혼합비** — 3점 38문항 중 **31문항이 빈칸**이다.
  초안 §3 의 "난도 다이얼 = 근거와의 거리" 는 이 측도로 지지되지 않는다
- ⭐ **P0.7 이 이번엔 방금 만든 도구에서 걸렸고, 미리 층화한 덕에 틀린 결론을 발표하지 않았다** —
  §10.22 에서는 합산을 믿고 발표한 뒤 되돌렸다. 규율이 값을 한 자리다
- **생성은 이 축에서 기출과 구분되지 않는다(P10.17)** — 겨냥 안 한 축, 집중도 0.450 vs 0.536(Holm 0.400) 등
  셋 다 null, 판별로도 상쇄 아님. ⚠️ **n=15 저검정력 — "같다" 가 아니라 "차이를 못 봤다"**
- 명제 122 → **125**: P2.5(HARD 후보) · P10.16(기각) · P10.17(SOFT) 신설
- 커버리지 **판정 99.2%(124/125) · 전수 64.8% · 규칙 22.4%**
- ③ 품질 축 지도 정리 — 표면·선지는 맞추고(겨냥한 축), 통합 요구는 구분 안 되고,
  **바꿔 쓴 재진술은 여전히 도구가 없다**. 다음 후보는 블라인드 손판독


### 수능 설계도 — D1 이 NONE 에서 SOFT 로 · 초안 §1 사슬 가설 기각 · "17편" 두 번째 정정 (§10.33~10.35)

청소된 표본과 하나의 대역 정의 위에서 D1(지문 선정)을 다시 시도했다.

- ⭐ **D1a·D1b 신설 (HARD)** `scripts/csat/verify-d1-rules.mjs` — 지문은 내용어를 되풀이한다
  (최빈 ≥2회 · 반복어가 어휘의 3% 이상). 세 관문 통과: 청소된 코어 **235편 위반 0** ·
  돌연변이 **2/2** · 모평 홀드아웃 **44편 위반 0**
- ⚠️ **초안 §1 "사슬 밀도" 가설 기각(P1.6)** — 인접 문장 어휘 사슬이 안 끊기는 지문은 **191편 중 27편(14%)**,
  끊긴 비율 중앙 **43%**. 수미상관 51.7% 도 기저 대비 **p=0.8085 구분 안 됨**.
  **기출 지문은 어휘를 이어 붙여 고른 글이 아니다**
- ⭐ **대신 두괄식이 나왔다(P1.7)** — 최빈 내용어가 **첫 문장**에 66.5~67.7%, 기저(임의 두 문장, 순열 20,000회)
  대비 **p<0.0001**. 끝 문장 48.3% **p=0.986 구분 안 됨**. **주제문은 앞에 온다, 뒤가 아니다.**
  "첫 **또는** 끝" 으로 합치면 83.2% **p=0.066 — 또 지워진다**(P0.7 의 형제)
- **② 결정 커버리지**: HARD 4/8 그대로, **SOFT 3/8 → 4/8**, **없음 1/8 → 0/8** — "설계도에 없다" 칸이 사라졌다.
  D1a·D1b 는 "글이 아닌 것"(나열·목록)을 걸러낼 뿐 **어떤 글을 고를지는 못 정하므로 SOFT 다**
- ⚠️ **직전 사이클의 "17편" 두 번째 정정(P10.5)** — 그 코퍼스 536편 중 **460편(86%)이 이 저장소가 합성한 글**이고
  통과 17편 중 **14편이 그 합성물**이었다. 진짜 외부 소스는 **76편**, 통과 **3편(3.9%)**
- ⚠️ **그 3편도 글이 아니었다(P10.15)** — 전부 NASA 이미지 캡션이고 `1 min read` · `HQ Web Team` ·
  `Image Credit:` 같은 **부속물이 대역을 통과시키고 있었다.** 기출 쪽(84.4% 오염)과 **같은 결함이 소스 쪽에도** 있었다.
  소스도 청소하면(119편) 통과 3편이 **완전히 다른 3편**(factbook 백과나열 · owid 블로그공지 · NASA APOD 안내문)이고
  **셋 다 논설문이 아니다 → 장르 게이트 포함 0편**
- ⭐ **§10.14 의 결론(0편)은 살아남고 그 근거는 여전히 틀렸다** — 같은 결론에 세 번 다른 길로 도착했고 맞는 길은 세 번째다
- 새 규율 **P0.9 "소스" 를 셀 때 우리가 만든 것을 코퍼스에서 뺀다**(`compose_batch_id is null`)
- 명제 117 → **122**: P1.6(기각) · P1.7(HARD 후보) · P1.8(HARD) · P10.15(HARD) · P0.9(규율) 신설, P10.5·P10.4 재정정
- 커버리지 **판정 99.2%(121/122) · 전수 63.9% · 규칙 22.1%(27/122)**. 규칙 19.7% → **22.1%**
- 회귀 — design-spec 14규칙 위반 0 · 모평 홀드아웃 0 · 선지규칙 C1 3관문 · D1 규칙 3관문


### 공유 카드 — 한 단어 안에서 굵기가 갈리고, 각색 만화는 카드가 아예 없었다

상세 19개의 head 를 뜯어본 뒤 **카드를 실제로 열어 봤다.** 상태 200 · 유효한 PNG 인데
눈으로 보기 전에는 알 수 없는 것이 세 가지 더 있었다.

**1. `A Chr`**ist**`mas Carol`** — 한 단어 안에서 굵기가 갈렸다(`Pr`**agu**`e` 이후 두 번째).
한글 서브셋 요청에 `Vocaflow`·숫자 같은 **라틴이 섞여 있어서** 이 폰트가 라틴 글리프를
*일부만* 갖게 됐고, Satori 가 가진 글자는 이 폰트로 없는 글자는 기본 폰트로 그렸다.
"한글 있을 때만 싣기" 방어는 반쪽이었다 — 한글 배지 하나면 다시 실린다.
→ **한글만 받는다.** 두 폰트의 글자 범위가 겹치지 않으면 섞일 수가 없다.
→ 손으로 들고 있던 `SUBSET_TEXT` 목록도 없앴다(**빠진 글자는 조용히 사라진다**).
  이제 호출부가 `ogCardText(props)` 로 **그 카드의 글자**를 넘긴다 — 잊을 것이 없다.
  캐시는 글자 집합을 키로, 상한 32.

**2. 긴 제목이 카드 밖으로 잘려 나갔다.** 제목 div 의 `display: 'flex'` 때문에 글자 전체가
**하나의 flex 항목**이 되어 줄바꿈 없이 뻗었다(`flexWrap`·`maxWidth`·`width` 로는 안 고쳐진다).
텍스트 컨테이너로 되돌리니 흐른다. 상한 110자의 근거를 실측으로 고정 —
카탈로그 최장 제목은 **218자**(논문 글), 110자로 자르면 52px 에서 세 줄로 들어간다.

**3. 만화 카드도 오타 제목을 쓰고 있었다.** 표의 `series_title` 을 그대로 읽었다.
`pd_comic_series(title)` 을 임베드해(anon 으로 읽힌다 — 마이그레이션 불필요) 화면과
**같은 규칙**(`display-title.ts`)을 쓴다. 아카이브 원본 표기는 부제로 내려 밝힌다.
실측: `Baffling Mysteries #18` / `Atomic War! #1` (부제에 원본 문자열).

**4. 각색 만화(CCP) 상세는 메타가 통째로 비어 있었다** — 고정 제목 `만화 미리보기` 하나,
canonical 없음, og:image 없음, description 20자. 지금 발행 1권이라 안 보이지만,
복원 만화 113호가 제목 하나였을 때와 같은 구조다(늘어난 뒤엔 이미 색인에서 빠진 뒤다).
→ `generateMetadata`(책 제목·저자·canonical) + `opengraph-image` 신설.
  `OG_QUERIES.comicAdapted` 는 조건이 `book` 과 다르다 — 이 화면은 `applyBookReadGate`
  (status 만)를 쓴다. `book` 을 그대로 쓰면 **화면엔 있는데 카드만 비는** 도서가 생긴다.

- 회귀 `og-font.test.ts` 5 — 서브셋에 라틴이 남으면 실패. `og-queries` 회귀는 종류가
  늘어 자동으로 13개
- 실측 확인: 도서·글·복원 만화·각색 만화 카드 4종 + 최장 제목 + 한글 전용 카드(`/fit/s`)


### 수능 설계도 — D1 을 채우러 갔다가 **자가 오염되어 있었다** (§10.29~10.32)

② 결정 커버리지의 빈칸 D1(지문 선정)에 규칙을 세우려고 봉투를 재다가 양 극단이 추출 결함임을 알았다.

- ⚠️ **논설문 코어 237편 중 200편(84.4%)에 문항 부속물이 섞여 있다** — 한글 180편 · 발문 조각 112편 ·
  각주 표시 166편. 2022/37 은 시험지 **두 단이 섞였고**(2257자), 2019/35 는 **잘렸다**(178자)
- ⭐ `scripts/csat/clean-passage.mjs` 신설 후 이동 측정 — 글자수 중앙 **−32** ·
  **문장당 낱말 +1.63(19.75→21.38, +8.3%)** · 낱말 길이 **−0.00** · 어휘 다양도 **+0.004**.
  즉 **낱말 길이·어휘 다양도 위의 판정은 안전하고 문장당 낱말 위의 것은 틀렸다**
- v6 표면 재채점 49/51 = 96.1% 로 총합 동일, **깨지는 칸이 바뀐다**(문장당 낱말 8/9→**9/9**) —
  30번의 "밖" 은 계측 결함이었다. 청소기 자신도 탐욕 매칭으로 본문을 삼킨 버그가 있어 고쳤다(2025/31 −62% → 0/237)
- ⚠️ **§10.14 의 "쓸 수 있는 소스 0편" 은 대역 폭의 산물이었다.** 그 스캔만 홀로 25~75분위를 썼고
  나머지 게이트는 10~90분위를 쓴다. 같은 코퍼스(파생 허용 library_articles **536편**)를 한 정의로 재면
  좁은 대역 **0편** · **표준 10~90 대역 17편(3.2%)**. §10.15 의 전제가 산물이었다
- ⚠️ **"낱말 길이와 문장당 낱말은 거의 배타적" 은 검정 없이 단정한 것이었다.** Fisher 양측:
  §10.14 보고값 2 vs 기대 5.5 **p=0.1267 유의하지 않음** · 좁은 대역 재현 4 vs 4.9 p=0.79 **효과 없음** ·
  10~90 대역 58 vs 68.7 **p=0.0123 유의**. **방향은 살아남지만 크기는 과장이었다**
- 명제 114 → **117**: **P10.14**(HARD · 추출 오염 84.4%) · **P0.8**(규율 · 게이트 대역은 한 정의) ·
  **P10.13**(SOFT · 배타성) 신설, **P10.5 정정**(REJECTED → SOFT) · P10.4 갱신
- **D1 은 못 채웠다** — 뜻 있는 보편 후보(최빈어 반복 ≥2 · 인접 문장 겹침 >0 · 낱말 길이 봉투)는
  수능에서 이미 위반이 나오고, 위반 0 인 것들은 너무 헐겁다. 대신 그 자리의 **자를 고쳤다**
- 커버리지 **판정 99.1%(116/117) · 전수 64.1% · 규칙 19.7%(23/117)**. 규칙 18.4% → 19.7%.
  전수는 64.9% → 64.1% — 회차 개념이 없는 규율 명제(P0.8)와 소스 명제(P10.13)가 분모에만 더해진 희석
- 회귀 — design-spec 14규칙 위반 0 · 모평 홀드아웃 위반 0 · 선지규칙 C1 세 관문 통과 · 층화 결론 불변
...
기출 지문 5편에 선지만 새로 써서 한 번만 쟀다 — 지문 품질이 통제되므로 **선지 작성만** 시험대에 오른다.

- ❌ **예측 반증** — 예측 혼동도 5/5 · 네 측도 ≥16/20, 실제 **3/5 · 9/20 = 45%**.
  사전 등록한 반증 조건("3/5 이하")에 정확히 걸렸다
- ⚠️ **빗나간 방향이 매번 뒤집힌다** — 지침 안 보고 0.0135(v1~v4) → 지침 따라 **0.1391**(1판) →
  규칙 외우고 **0.0056**(2판, p=0.0053). **세 번 다 한쪽으로.** 사람은 표면 어휘 겹침을 눈으로 못 어림한다
- 원인 둘: 한 오답이 혼동도 **1.000**(정답 문장 틀을 낱말 그대로 뒤집음) ·
  오답의 지문 낱말 되쓰기 0.56~0.86 (기출 오답 중앙 **0.40**) — **선지를 지문에서 베꼈다**
- ⭐ **규칙 C1 신설 (HARD)** `scripts/csat/verify-choice-rules.mjs` — design-spec 이 회차 구조에
  하는 일을 **선지**에 한다. 세 관문 통과: 전 회차 오답 364개 위반 0 · 돌연변이 잡힘 ·
  **모평 홀드아웃 오답 60개 위반 0**. 뜻: **기출은 정답과 어휘가 통째로 겹치는 오답을 안 쓴다(혼동도 < 0.50)**
- 같이 세우려던 "되쓰기 < 0.85" 는 **모평에서 4건 예외**(최대 1.000) → 규칙이 아니라 **대역**으로 남김
- ⭐ **계측 루프 신설** `scripts/csat/fit-choice-band.mjs` — C1 을 통과하는 조합 중 대역 중앙에
  가장 가까운 것을 고른다. 1판·2판 오답을 합친 풀(문항당 8개)에 돌리니
  혼동도 대역 안 **3/5 · 1/5 → 5/5**, 네 측도 45% · 50% → **80%**. **같은 재료, 고른 방법만 다르다**
- 남은 4칸은 전부 **정답 선지**(접근성) 탓 — 루프가 오답만 고르고 정답은 고정했기 때문.
  정답 후보도 넣으면 닫힌다(v6 23번에서 실증)
- 명제 111 → **114**: **P10.10**(HARD · C1) · **P10.11**(REJECTED · 말로 된 지침으로는 못 맞춘다) ·
  **P10.12**(SOFT · 계측 루프는 수렴한다) 신설, P10.7 갱신
- 커버리지 **판정 99.1%(113/114) · 전수 64.9%(74/114) · 규칙 18.4%(21/114)** — 셋 다 상승
- 회귀 — design-spec 14규칙 위반 0 · 돌연변이 14/14 · 모평 홀드아웃 위반 0 · 선지규칙 3관문 통과


### 도서 13권도 고아였다 · 만화는 **주소 자체가 흔들리고 있었다**

같은 검사를 네 서가로 넓혀 실측:

| 서가 | sitemap | 링크로 닿음 | 고아 |
|---|---|---|---|
| 글 | 160 | 160 | 0 (이번에 해소) |
| 도서 | 13 | 0 → **13** | 13 → **0** |
| 복원 만화 | 105 | 105 | **3** |
...
- **도서** — `BookGridCard` 가 `<button onOpen>` 이었다. 시트(넷플릭스식 상세)는 그대로 두고
  **겉만 링크**로 바꿨다: 평범한 좌클릭은 예전처럼 시트, 수식키·새 탭은 실제 상세 페이지
  (`/library/books/[id]` 는 이미 공개이고 h1·구조화 데이터·공유 카드를 갖췄다)
- **복원 만화 3건은 링크 문제가 아니었다.** 개수는 양쪽 105 로 같은데 **가리키는 행이 다르다** —
  `Atomic War!` 1~4호가 IA 식별자만 다른 중복으로 9행(숨는 5행) 올라와 있고,
  `list_pd_comics` 의 `distinct on` 동점 처리가 `panels_total desc, created_at` 인데
  **그 두 값이 9행 모두 같다**(36/36/36/32 · 타임스탬프 동일). 승자가 실행 계획에 달려 있어
  sitemap 과 화면이 같은 만화에 서로 다른 주소를 쓴다. 검색엔진에는 중복 콘텐츠로 보이고
  공유된 링크가 다음에 다른 호를 가리킬 수 있다
  → `_pending_pd_comic_pick_stable.sql` (**미적용** — 정렬 끝에 `i.id`. 데이터는 안 건드린다.
  중복 5행 정리는 별개이고, 정리해도 파이프라인이 다시 실을 수 있으므로 결정성이 먼저다)
- 회귀 `33-public-surface` 를 서가 4개로 일반화 — 진입면 1홉 + `?…` 2홉을 따라가
  sitemap 집합과 대조. 복원 만화만 허용 고아 4 (사유·해소 조건 명시, **줄어들기만 한다**)


### 고아 페이지 — sitemap 이 알리는 160개를 사이트 안 어디서도 가리키지 않고 있었다

`/library/scripts` 를 익명으로 받아 보면 글 제목 160개가 다 들어 있는데 **상세로 가는
`<a>` 가 0개**였다. 원인 둘:

1. 글 카드가 `<button onClick>` 이었다 — 주소가 없다. 비로그인이 누르면 학습 시작이
   실패해 `window.alert` 가 떴다(막다른 골목)
2. 시리즈 드릴다운이 `useState` 였다 — 글 목록에 주소가 없다. 뒤로가기·공유·새로고침이
   전부 진입면으로 돌아갔다

화면에서는 잘 동작하니 아무도 몰랐다. 대가는 조용하다 — 크롤러는 sitemap 으로 주소를 알아도
그 페이지가 무엇에 속하는지 모르고(내부 링크가 곧 문맥이다), 방문자는 목록에서 글로 갈 수 없다.

- `ArticleCard` → `<Link href="/library/scripts/[id]">`. 로그인 클릭은 그대로 가로채
  한 홉을 아끼고, 실패하면 `alert` 대신 그 주소로 보낸다. 수식키 클릭은 가로채지 않는다
- `ScriptsBrowser` 드릴다운을 `?series=` 로 (만화 서가 `/comics/restored` 와 같은 방식).
  `/library/scripts` 는 `revalidate=60` → `force-dynamic`. 없는 키는 진입면으로 떨어진다
- 실측: 진입면 시리즈 링크 6 → 각 시리즈의 글 링크 합집합 **160/160**
  (topic 47 · easy 34 · listen 30 · argue 25 · data 15 · reference 9. `news` 는 발행 0편이라 트랙 없음)
- 회귀: `33-public-surface` 에 **고아 금지** 추가 — sitemap 의 글 주소 ⊆ 링크로 닿는 주소.
  손으로 센 숫자를 쓰지 않는다(글이 늘면 두 집합이 함께 늘어야 한다)

**남은 고아** (같은 방식으로 실측): 도서 **13/13 전부** (`/library/books` 카드가 `<button onOpen>`
— 모달이라 주소가 없다) · 복원 만화 3 (닿는 수는 105 로 같은데 3개가 어긋난다).
...
그렇다면 **같은 구조가 다른 곳도 무력화하고 있지 않은가** — 굴절형·파생형이 표제어로도
등재돼 있으면 L2~L4 가 같은 이유로 건너뛰어진다. 규모부터 쟀다.
...
숫자만 보면 철자 변이(183)보다 열 배지만, **뜻을 보면 이야기가 다르다:**

| 굴절형 | 원형 |
|---|---|
| `acting` 연기 | `act` 행동하다 |
| `accounting` 회계 | `account` 계좌 |
| `airing` 환기, 통풍; 방영 | `air` 공기, 대기 |
| `according` ~에 따르면 | `accord` 합의; 일치 |

**뜻이 실제로 같은 경우는 2,035개 중 1건**(`ski jumping`/`ski jump`)뿐이다.

즉 이것들은 명사화·형용사화로 **뜻이 달라진 독립 단어**이고, L1 이 먼저 걸리는 것이 **맞다** —
`acting` 을 `act` 로 해석하면 "연기" 가 "행동하다" 가 된다. L5 주석의 **"의미 위험 0"** 이
정확히 이 구분을 말한다. 철자 변이만 뜻을 바꾸지 않고 통합할 수 있다.

**결론: 고칠 것이 없다.** 그리고 이 확인이 앞 항목의 수정 범위를 좁혀 준다 —
`variant_of` 는 **철자 변이 183쌍에만** 쓰이고, 굴절·파생에는 손대지 않는다.
(사전 품질의 방증이기도 하다 — 2,035개의 뜻이 전부 원형과 구분돼 있다.)

### 수능 설계도 — 오답 매력도 판정을 **유형별로 층화**해 자기 정정 (§10.22~10.25)

직전 사이클은 "생성 오답의 혼동도가 기출보다 낮다(합산 0.0155 vs 0.0530, Holm 0.024)" 를
겨냥하지 않은 축의 진짜 격차로 보고했다. **그 판정은 세 유형을 합쳐 잰 산물이었다.**

- 유형별 기출 중앙이 전혀 다르다 — **빈칸 0.0000** · 주제 0.0809 · 제목 0.0758.
  기출 오답 364개 중 **217개(59.6%)** 가 정답과 낱말을 하나도 안 겹친다. 빈칸에선 그것이 정상값이다
- 층화(v1~v4, Holm 3검정): 빈칸 **8/8 대역 안**(0.135) · 제목 구분 안 됨(0.069) ·
  **주제만 다르다(0.041)** → 깨진 것은 **주제 하나**. 합산은 **유형 혼합비**를 재고 있었다
- 그 오진으로 만든 v5 도 재채점하면 **더 나쁘다** — 네 측도 대역 통과 v4 14/16 → **v5 8/16**.
  v5 가 넘긴 것은 사실상 **멀쩡하던 빈칸 두 문항**(0.1946 vs 대역 상단 0.0900)이었다
- **v6** — v4 의 지문 9편·나머지 8문항을 그대로 두고 **23번 선지만** 대역 중앙을 겨냥해 재작성.
  선지 대역 **16/16 = 100%**(네 값 모두 중앙 근처)이면서 표면 계측은 **49/51 = 96.1% 로 불변** —
  한쪽을 고치며 다른 쪽을 깨지 않았다
- 새 도구 `scripts/csat/check-choice-band.mjs` — 지문용 `check-passage-band.mjs` 의 선지판.
  **유형별로만** 재고 합산 API 를 두지 않았다. 진입점일 때만 CLI 가 도는 가드 포함
- `score-generated-bait.mjs` — 층화 절 추가, **판정을 층화에서 읽도록** 교체.
  합산 Holm 이 전부 1.000 이어도 "같다" 로 적지 않는다
- 명제 109 → **111**: **P0.7**(규율 — 유형마다 대역이 다르면 합산 검정을 판정 근거로 쓰지 않는다) ·
  **P10.9**(기출 오답의 59.6% 는 정답과 어휘를 전혀 공유하지 않는다) 신설,
  **P10.7 정정**(전면 기각 → 주제 한정) · **P10.8 정정**(REJECTED → SOFT, v6 16/16)
- 커버리지 **판정 99.1%(110/111) · 전수 64.0%(71/111) · 규칙 18.0%(20/111)**.
  규칙 16.5% → 18.0%. 전수는 64.2% → 64.0% 로 0.2pp 내려갔는데, 회차 개념이 없는
  **작업 규율 명제(P0.7)가 분모에만 더해진 정의상 희석**이다 — 검사 범위가 준 것이 아니다
- 회귀 — design-spec 14규칙 위반 0 · 돌연변이 14/14 · 모평 홀드아웃 위반 0 (전부 재실행)


### 정정 — 철자 통합이 깨진 원인은 `spelling_variants` 가 아니라 **계층 순서**였다

앞 항목에서 "원인은 데이터다 — `spelling_variants` 가 47,890 중 114개만 채워져 있다" 고 적었다.
**틀렸다.** `resolve_dict_headword` 를 열어 보니 **L5 는 규칙 기반**이다 —
`-ize↔-ise` · `-or↔-our` 등 정규식 변환이고 `spelling_variants` 컬럼을 **쓰지 않는다.**
게다가 이미 **양방향**이다.

진짜 원인은 순서다. **L1(정확 일치)이 먼저**라, 두 철자가 **모두 표제어로 등재돼 있으면**
각자 자기 자신에 걸려 L5 까지 가지 않는다.
...
`spelling_variants` 를 채우는 것은 이 문제에 아무 효과가 없다 — 그쪽을 손댈 뻔했다.

### 통합 마이그레이션 준비 (미적용)

`supabase/migrations/_pending_spelling_canonical.sql` — **구조만** 만든다.

- `shared_dictionary.variant_of` — 이 표제어가 다른 표제어의 철자 변이일 때 그 정본.
  **행을 지우지 않는 이유**: `labour` 를 5개 발행 세트가 참조한다. 표제어를 지우면
  그 세트의 낱말이 뜻을 잃는다. 행은 남기고 "정본이 누구인지" 만 표시한다
- 자기 참조 금지 CHECK + 부분 인덱스
- `resolve_dict_headword_l1()` — L1 규칙을 한 곳에 적어 둔다

⚠️ **이 파일만으로는 동작이 바뀌지 않는다.** 본체가 아직 이 함수를 부르지 않는다.
본체의 L1 을 바꾸려면 함수 전체를 다시 써야 하는데(SQL 은 본문 일부만 못 고친다),
그건 **백필과 같은 단계에서** 하는 편이 낫다 — 구조·데이터·동작이 한 번에 켜지고
문제가 생기면 그 커밋 하나만 되돌리면 된다.
지금 조각을 떼어 둔 덕에 다음 diff 가 "본체에서 L1 세 줄이 이 호출로 바뀌었다" 로 읽힌다 —
**나머지 계층에 손대지 않았음을 눈으로 확인할 수 있다.**

**정본을 미국식으로 두는 근거**: 학습자는 한국 중·고등학생~성인이고 국내 표준은 미국식이다.
반면 본문은 영국 고전이 많아 영국식 표면형이 자주 나온다 — 그래서 영국식을 **버리는 게 아니라
표면형으로 인식하고 정본으로 모은다.** L5 가 양방향이라 반대 방향도 계속 된다.

### 영/미 철자 중복의 전체 규모 — **183쌍**, 그중 66개는 학습자가 두 항목으로 만난다

앞 사이클에서 `-ise/-ize` 112쌍을 쟀다. 다른 계열도 같은 상태인지 확인했더니 **더 심했다.**

| 계열 | 쌍 | 발행 세트에 | **둘 다 발행 세트에** |
|---|---|---|---|
| `-ise/-ize` | 112 | 74 | 20 |
| **`our→or`** (colour · honour · labour · neighbour) | **50** | | |
| `tre→ter` (centre · theatre) | 11 | | |
| `ogue→og` (catalogue · dialogue) | 6 | | |
| `ll→l` (traveller · jeweller) | 4 | | |
| **위 네 계열 합** | **71** | **63 (89%)** | **46 (65%)** |
| **총계** | **183** | **137** | **66** |

`-ise/-ize` 보다 훨씬 심한 이유는 분명하다 — `colour` · `honour` · `neighbour` · `theatre` 는
**고전 문학의 기본 어휘**다. 도서가 영국 고전 위주라 본문에 영국식이 나오고, 미국식도 따로 등재돼 있다.

**세트 등장 실측** — 이론이 아니라 지금 학습자가 만나는 수치다:

| 단어 | V-Level | 뜻 | 세트 등장 |
|---|---|---|---|
| `labor` / `labour` | 5 / 5 | **둘 다 "노동; 분만; 수고하다"** | **17 / 5** |
| `color` / `colour` | 1 / 1 | 둘 다 "색, 색깔" | 6 / 2 |
| `neighbor` / `neighbour` | **2 / 1** | 이웃 | 2 / 3 |
| `theater` / `theatre` | **3 / 2** | "극장 (미국식 철자; 영국식 thea…" | 1 / 3 |

`labor` 는 **17개 세트**, `labour` 는 **5개 세트** — 22개 세트에서 같은 단어를 따로 배운다.
`neighbor`/`neighbour` 와 `theater`/`theatre` 는 **난이도까지 갈린다**(V2/V1, V3/V2).

그리고 `theater` 의 뜻풀이에 **또 손으로 메모가 적혀 있다** — "(미국식 철자; 영국식 theatre)".
`civilise` 때와 같다. **문제는 여러 번 인지됐고 매번 뜻풀이 안에 적혔지만,
`spelling_variants` 컬럼으로는 한 번도 옮겨지지 않았다**(47,890 중 114개만 채워져 있다).

측정만 했다. 데이터 변경이라 승인 대기(선택지는 앞 항목 참조).

### 영/미 철자 통합이 데이터에서 끊겨 있다 — 112쌍, 그중 20쌍은 학습자가 두 번 만난다

`resolve-headword` 테스트 실패("영/미 철자 변이는 해석한다 · 의미 위험 0")를 파고들었다.
...
| 그중 발행 세트에 등장 | **74쌍** |
| 도서 어휘에 등장 | 101쌍 |
| **둘 다 발행 세트에** | **20쌍** ← 같은 단어를 두 항목으로 만난다 |

뜻이 **글자 그대로 같은** 경우가 다수다:
`authorisation`/`authorization` 둘 다 "허가, 인가, 권한 부여" · `emphasise`/`emphasize` 둘 다 "강조하다".
일부 뜻에는 `(BrE civilise)` 같은 상호 참조가 있다 — **누군가 이미 인지하고 손으로 적어 뒀다.**

**더 나쁜 것**: 같은 단어인데 난이도가 갈린다.

| | V-Level |
|---|---|
| `emphasise` / `emphasize` | **V6 / V5** |
| `optimisation` / `optimization` | **V8 / V7** |

학습자에게 같은 단어가 다른 시점에 나온다는 뜻이다.

**원인은 데이터다.** `shared_dictionary.spelling_variants` 컬럼은 있고 5계층 해석기의 L5 가
그것을 본다. 그런데 **47,890 표제어 중 114개에만 채워져 있고**, 문제의 쌍은 전부 빈 배열이다.
그래서 L1(정확 일치)이 먼저 걸려 자기 자신을 돌려준다.

**선택지**(데이터 변경이라 승인 대기):
- ㉮ 한쪽을 정본으로 정하고 다른 쪽을 `spelling_variants` 로 연결 — 한국 영어 교육은 미국식이
  표준이지만 **도서는 고전(영국 작가 다수)이라 본문에는 영국식이 나온다.** 정본을 미국식으로 두면
  영국식 표면형은 L5 로 해석된다
- ㉯ 양방향으로만 연결하고 표제어는 둘 다 유지 — 안전하지만 세트에는 여전히 둘 다 들어간다

어느 쪽이든 **v_level 을 맞추는 것이 함께 가야 한다** — 지금은 같은 단어가 두 난이도를 갖는다.

### CI 가 실 DB 회귀 24개를 조용히 건너뛰고 있었다 — 그중 다섯이 권한·RLS 검사다

새로 만든 회귀들이 CI 에서 실제로 도는지 확인하다 나왔다.
`verify` job 에 **환경변수가 없다.** 실 DB 를 치는 테스트는 전부 `skipIf(env 없음)` 이라
**전량 건너뛴다** — 초록불인데 아무것도 안 본 상태다.

| | |
|---|---|
| 실 DB 통합 테스트 | **24개** |
| 그중 권한·RLS | `privilege-escalation` · `rls-surface` · `word-set-rls` · `session-rls` · `assignment-rls` |

`apps/web/CLAUDE.md` 가 "실 DB 권한 상승 공격 8건 포함" 이라 적어 둔 그것들이다.
코드는 있고 실행은 없었다.

→ `verify` job 에 시크릿을 넘긴다. 없는 저장소에서는 지금과 **똑같이 skip** 되므로(빈 문자열
→ `skipIf`) 안전하고, 넣는 순간부터 24개가 실제로 돈다.

### 전체 테스트 1,642 중 6 실패 — 소유자를 갈랐다

⚠️ **`vitest` 가 6 실패를 내고도 `exit 0` 을 줬다**(이 세션에서 세 번째 —
`JSON.parse` 통과하는데 HTML 깨짐 · `exit 0` 인데 빌드 OOM · 200 인데 빈 카드).
종료 코드만 보면 "전체 통과" 로 읽힌다.

| 실패 | 원인 | 처리 |
|---|---|---|
| `on-p-contrast` | **내가 랜딩에 `bg-[var(--p)] text-white` 를 썼다** | **고쳤다** — `text-[var(--on-p)]` |
| `touch-target` 2건 | 다른 세션의 커밋(`00cf9704` 간격 4px 정렬) | 그 세션 소관 |
| `content-quality-gate`(I10) · `extraction-rpc` 스냅샷 2 | **내 사전 개선의 알려진 파급** — 예문 backfill 이 `_extract_composite_score` 를 통해 추출을 바꿨다 | **재발행 승인 대기**(대기 목록 ①) |
| `resolve-headword` | 영/미 철자 통합이 깨졌다 (아래) | 승인 대기에 추가 |

### 영/미 철자가 사전에 둘 다 등재돼 통합이 깨졌다 — 112쌍

`resolve_dict_headword('optimization')` 이 `optimisation` 대신 **자기 자신**을 돌려준다.

| 표제어 | 등재일 |
|---|---|
| `optimisation` (영) | 2026-05-31 |
| **`optimization` (미)** | **2026-08-15** |

미국식이 **표제어로 직접 등재**되면서 5계층 해석기의 L1(정확 일치)이 먼저 걸린다.
같은 계열이 **112쌍**이다(`-ise/-ize` 만 세어서 그렇고 `-our/-or`·`-re/-er` 을 더하면 늘어난다).

테스트 이름이 "의미 보존 원칙 — 영/미 철자 변이는 해석한다 (같은 단어 · 의미 위험 0)" 다.
**통합은 의도된 설계였고 데이터가 그것을 무력화했다.** 학습자에게는 같은 단어가 둘로 갈리고
단어장도 갈라진다. 데이터 변경이라 승인 대기에 올린다.

### 빈 공유 카드를 자동으로 잡는다 — 이미지를 열어 보지 않고

앞 사이클에서 만화 카드가 **제목 `Vocaflow` 짜리 빈 카드**로 나갔던 것을 사람 눈으로 잡았다.
그 실패는 자동 검사를 전부 통과했다:
...
이 세션에서 같은 모양이 세 번 나왔다 — `JSON.parse` 는 통과하는데 HTML 이 깨지고,
`exit 0` 인데 빌드는 OOM 으로 죽고, 200 인데 카드가 비었다.
**지표가 아니라 결과물을 봐야 하는 자리**가 있다. 그런데 사람이 매번 열어 볼 수는 없다.

카드의 경우 **어디서 갈리는지는 분명하다** — 조회가 행을 못 주는 순간이다. 그 지점을 잡는다.

- `lib/seo/og-queries.ts` — 카드가 읽는 쿼리의 **단일 출처**(표·컬럼·필터).
  세 `opengraph-image.tsx` 가 이것으로 URL 을 만든다
- 회귀 `og-queries.integration.test.ts`(10) — **같은 쿼리를 실제로 날려** ① select 가 200 인지
  ② 행이 오는지 ③ 제목이 비지 않았는지 ④ 키를 넣은 URL 도 같은 모양인지
- **검사가 실제로 잡는지 확인했다** — 원래 실수(`source_archive`)를 일부러 되살리니
  3개가 실패하고 메시지가 원인을 지목했다:
  `pd_comic_issues 의 select 가 거부됐다 — 컬럼 이름을 표 기준으로 맞출 것 (RPC 반환 이름과 다를 수 있다)`
  통과하는 테스트가 아무것도 막지 못하는 경우를 이 세션에서 이미 겪었으므로, 확인하고 되돌렸다
- 리팩터 후 렌더 재확인: 만화 카드 **27,616 bytes — 리팩터 전과 같은 바이트**
...
공유될 때 그림 없는 카드가 뜬다는 뜻이다. 그 한 번의 마찰이 확산을 깎는다
(`/fit/s/[payload]` 가 같은 이유로 이미 OG 이미지를 갖고 있다).

원천 표지를 기다리는 대신 **우리가 아는 것으로 그린다** — 제목·저자·난이도·분량·출처.
표지 파이프라인이 생겨도 이 카드는 유효하다(그때는 표지를 배경으로 얹으면 된다).

- `opengraph-image.tsx` (edge · 1200×630) — 조회 조건을 화면과 **같게** 맞춘다
  (published + copyright_safe). 갈라지면 없는 글의 카드를 그리게 된다
- **본문은 그리지 않는다** — 발행 160개 중 25개가 CC-BY-ND(개작 금지)이고, 발췌를
  이미지로 재구성하는 것이 그 경계에 닿는다. 제목·서지 정보는 어느 라이선스에서도 안전하다
- ⚠️ **한글 폰트를 무조건 실으면 라틴 글자가 섞여 보인다.** 한글 폰트의 라틴 글리프와
  Satori 기본 폰트가 글자마다 갈려 `Pr**agu**e` 처럼 굵기가 들쭉날쭉해진다(첫 렌더가 그랬다).
  이 카드에 들어가는 것은 원문 제목·저자·출처라 대부분 영어다 →
  **한글이 실제로 있을 때만** 싣는다
- 실측(dev): 200 · image/png · 1200×630 · 23.8 KB.
  `Prague` / `Wikivoyage contributors` / `B2 · V5 · 13,942 words` / `wikivoyage`

도서 13 · 만화 105 는 아직 없다 — 같은 패턴으로 만들 수 있지만 데이터가 달라 각각 검증이 필요하다.
...
| | 흐름 | lemma |
|---|---|---|
| 도서(LCP) | analyze → INSERT → **`backfill_book_lemmas(book_id)` 호출** | 94.8% 채움 |
| 글(ACP) | analyze → INSERT → **(없음)** | **100% NULL** |

`analyze-article.ts` 의 INSERT 매핑에 `lemma` 가 없는 건 도서도 마찬가지다 —
도서는 **INSERT 뒤에 RPC 로 채운다**(`api/lcp/process/route.ts:178`).
글에는 그 호출이 없고, **부를 함수 자체도 없었다**(`backfill_article_lemmas` 부재).

즉 "한 줄을 빠뜨린" 것이 아니라 **단계 하나가 통째로 없었다.**

- `supabase/migrations/_pending_backfill_article_lemmas.sql` — **미적용**(승인 대기).
  본문은 `backfill_book_lemmas` 를 그대로 따른다: ① 사전 직접 매칭 ② `en_inflection_bases`
  후보 중 사전에 있는 것(`en_negation_preserved` 로 부정 뒤집힘 차단 · 축약어 제외).
  **두 파이프라인이 다른 규칙을 쓰면 같은 낱말이 도서와 글에서 다른 표제어에 붙고
  학습자의 단어장이 갈라진다** — 그래서 새로 설계하지 않고 같은 규칙을 쓴다
- 못 찾으면 **NULL 로 남긴다** — 틀린 표제어를 붙이는 것보다 없는 편이 낫다
- `WHERE lemma IS NULL` 이라 **재실행 안전**. 사전이 자라면 다시 돌려 남은 NULL 을 줄인다

**코드(ACP 라우트에 호출 추가)는 아직 넣지 않았다** — RPC 가 없는 동안 그 코드는
"없는 함수를 부르는 줄" 이 된다. 마이그레이션 적용 후에 넣는 것이 순서다.

승인 대기가 둘로 나뉜다:
- **기존 132,476행 백필** (데이터) — 발행 160개 글의 뜻 적중 78% → 99%
- **파이프라인 수정** (마이그레이션 + 라우트 한 줄) — 앞으로 들어올 글의 재발 방지

### 짧은 글 어휘 132,476행의 `lemma` 가 **전부 비어 있다** — 굴절형 21%가 뜻 없이 나간다

앞 사이클에서 "글 619개 중 ~70개가 게시 불가(추출 0단어)" 를 쟀다. 그 원인을 파고들었더니
게시 여부보다 큰 것이 나왔다.

`select_article_vocab` 이 0행을 주는 글을 열어 보니 **어휘는 있었다**(71행). 다만
`lemma` 가 **71행 전부 NULL** 이라 사전 조인이 통째로 실패했다. 범위를 넓혀 보니:

| status | 글 수 | lemma 전부 NULL |
|---|---|---|
| ready | 619 | **619** |
| published | 160 | **160** |

**779개 글, 어휘행 132,476개 — 한 번도 채워진 적이 없다.**

표본 3,000행으로 잰 회복 가능성:

| | |
|---|---|
| `word` 로 사전 직접 매칭 | 2,350 (**78.3%**) |
| `resolve_dict_headword(word)` 로 해석 | 2,974 (**99.1%**) |

즉 지금은 **굴절형 약 21%가 뜻 없이 학습자에게 보이고**(`acting` 은 사전에 없고 `act` 가 있다),
lemma 를 채우면 99%가 연결된다. 이것은 게시 대기 글만의 문제가 아니다 —
**이미 발행된 160개 글이 그 상태로 나가 있다.**

같은 계열의 결함을 이 세션 초반에 WordBlitz 에서 고쳤다(D-4 — 표면형으로 키를 잡아
사전 적중이 71.3% 였고 lemma 키잉으로 100% 가 됐다). 여기는 손대지 않은 쪽이었다.

**제안(승인 대기)**: `library_article_vocabularies.lemma` 백필 —
`resolve_dict_headword(word)` 로 132,476행. 데이터 변경이라 결정이 필요하다.
효과는 두 갈래다: ① 발행 160개 글의 어휘 뜻 적중 78% → 99% ② ready 619개 중
게시 불가 ~70개 해소.

측정만 했다. 아무것도 바꾸지 않았다.

### 발행 대기 콘텐츠의 게이트 통과율 실측 — 결정에 필요한 숫자

승인 대기 중인 "대량 발행" 을 판단하려면 **몇 개가 실제로 통과하는지**가 필요한데,
그동안 샘플 30건만 확인된 상태였다. 전수·대표본으로 다시 쟀다(코드 변경 없음).

| | 대기 | 발행 가능 | 실패 | 실패 원인 |
|---|---|---|---|---|
| **도서** | 303 | **288 (95.0%)** | 15 | I10 발행세트 드리프트 **13** · I8 `book_v_level` 결측 **2** |
| **짧은 글** | 619 | **약 549 (89%)** | 약 70 | 추출 비어있음(0단어) |

- 도서는 **전수**(303권을 60권씩 5배치 — 한 번에 돌리면 `select_book_chapter_vocab` 에서
  statement timeout 이 난다). 글은 150개 표본(11.3% 실패)이고 40개 표본(10.0%)과 일치한다
- `book_v_level` 결측 **2권**은 게이트 없이 직접 셌다(추정이 아니라 실수)

**결정에 미치는 영향**:
- I10 13권은 **내 사전 개선이 만든 드리프트**다(예문 8,171건 backfill → `_extract_composite_score`
  의 +0.10 예문 보너스 → 추출 결과 변화). 승인 대기 ①(**재발행**)을 하면 그대로 해소된다.
  즉 ① 다음에 ②를 하면 **301권**이 열린다(288 → 301)
- 남는 2권은 `book_v_level` 을 채우면 된다 — 별개 작업이고 데이터 변경이라 승인 대상
- 색인 표면 전망: 지금 **279** → 도서·글까지 열면 **약 1,100**

측정만 했고 아무것도 발행하지 않았다. `PLATFORM_AUDIT.md` §순서 규칙("측정 → 기록 →
(별도 결정) → 수정")과 같은 이유다 — 같은 턴에 고치면 측정이 오염된다.

### robots 가 `/admin` 을 막지 않고 있었다 — 파생의 전제가 어긋나 있었다

공개 표면 스펙을 CI 에 넣고, "빠진 유형" 을 자동으로 잡는 **역방향 검사**를 만들다가 나왔다.

`robots.ts` 는 `PROTECTED_PREFIXES` 에서 파생한다("보호 목록에 추가하면 robots 가 따라온다").
그런데 **`/admin` 은 그 목록에 없다** — 학습자 보호 경로 목록이고, admin 은 3층 가드
(미들웨어 + `requireAdmin` + `requireAdminApi`)가 따로 지킨다. 파생의 전제가 어긋난 것이다.

실측 결과 `disallow` 에 admin 이 한 줄도 없었다. 크롤러가 관리 콘솔 전체를 훑으며
로그인 리다이렉트에 크롤 예산을 쓰고, 관리 화면의 주소 구조가 그대로 드러난다.
→ `ADMIN_PREFIX` 를 명시적으로 넣고 **왜 파생이 아닌지**를 주석에 남겼다. 회귀가 못 박는다.

### 공개 동적 라우트가 sitemap 에 유형째 빠지는 것을 자동으로 잡는다

`sitemap-routes-exist` 는 sitemap → 파일 방향만 본다("올린 주소에 화면이 있는가").
그 방향으로는 **아예 안 올린 것**을 못 잡고, 실제로 그 구멍으로 두 번 샜다:
짧은 글 160개(주소 자체가 없었다) · `/comics/adapted/[bookId]`(공개 상세인데 유형이 빠졌다).

- `public-routes-in-sitemap` — 파일 시스템에서 **동적 라우트를 전부 찾아**,
  보호되지 않은 것이 sitemap 생성기에 덮이는지 본다.
  색인 대상이 아니면 `NOT_INDEXED` 에 **이유를 적어야** 통과한다
- 이 검사가 처음 돌자마자 위의 robots·admin 결함을 드러냈다

### `/comics/adapted` 상세를 sitemap 에 (콘텐츠 278 → 279)

도서를 각색한 CCP 만화. 비로그인도 3컷 미리보기를 보는 **공개 상세**인데 빠져 있었다.
발행이 1개라 지금은 작지만, 여기서도 표를 직접 읽지 않고 `fetchComicCatalog` 를 쓴다 —
"이중 published 게이트" 를 담은 화면의 **단일 출처**이고, 조건을 옮겨 적으면 반드시 갈라진다.

- CI e2e 에 `33-public-surface` 추가 — 비로그인 검색 착지점이 이제 상시 검사된다
- e2e 9/9 통과(8.6분): 리더 `h1` 승격과 랜딩 변경이 기존 화면을 깨지 않았다
- 콘텐츠 상세 **279** = 도서 13 · 복원 만화 105 · 각색 만화 1 · 짧은 글 160
...
(`[bookId]` 등 — "시나리오 스펙의 몫"). 합리적인 설계지만 결과가 이랬다:
**공개 콘텐츠 상세 278개가 통째로 검사 밖.** 하필 그것들이 sitemap 이 검색엔진에
알리는 바로 그 주소이고, 방문자가 이 제품에서 **처음 보는 화면**이다.
게다가 스윕은 로그인 상태로 돈다 — 검색 방문자는 로그인하지 않았다.

- 새 스펙 `33-public-surface` — **비로그인**·390px 로 sitemap 대표 6종을 본다.
  대상을 **sitemap 에서 뽑는다**: 손으로 적으면 sitemap 과 갈라지고, 그러면
  "검색에 알렸지만 아무도 안 본 화면" 이 다시 생긴다
- 검사: 200 · h1 존재 · 가로 넘침 · 44px 터치 타겟 · 콘솔 에러

**첫 실행에서 나온 것**:

| | |
|---|---|
| **도서 상세에 `h1` 이 없다** | 제목이 리더 헤더의 `h2` 뿐이었다. 검색 착지점인데 "무엇에 관한 페이지" 인지 기계도 스크린리더도 알 수 없었다 → `user-preview` 모드에서 `h1` 로 승격(admin-review 는 검수 도구 안의 구획이라 `h2` 유지) |
| 랜딩 로고·푸터 링크 44px 미만 | **내가 만든 것**이다. 높이·폭 모두 확보 |
| 글 상세 출처 링크 44px 미만 | 역시 내 것. 푸터의 독립 링크라 문장 속이 아니다 |

오탐 둘을 검사에서 걸러냈다:
- **건너뛰기 링크**(`sr-only`, 1×1) — 포커스될 때 커진다. 크기로 재면 항상 걸리는데
  그건 이 링크의 존재 이유(Calm UI)와 반대다
- **문장 속 인라인 링크** — 출처 표기를 44px 로 키우면 문단이 깨진다.
  WCAG 2.5.8 도 "inline in a sentence" 를 예외로 둔다

기존 학습자 UI 3건(`/fit` · `/library/books` · 도서 리더)은 **베이스라인**으로 남겼다 —
손대면 이 사이클의 범위를 넘는다. 베이스라인은 **줄어들기만 해야 한다.**
(스펙을 안 만들면 다음 것도 안 보인다. 실제로 도서 상세는 한 번도 검사된 적이 없어
h1 이 없는 것조차 아무도 몰랐다.)
...
### 🟢 프로덕션 재검증 + 가짜 초록 하나를 잡았다 (v08.7)

다른 세션이 `TeacherClient` 의 미사용 import 를 쓰면서 **빌드가 다시 통과**한다(exit 0).
그동안 dev 서버에서만 재던 것을 **프로덕션 빌드 위에서 전부 다시 쟀다.**
...
| 학습자 탭 대상 @390px | **0건** |
| 공개 화면 6축 (로그아웃) | **전 축 0** |
| 팝업 제자리 | **7/7** (관리자 1건은 정상 skip) |

- ⚠️ **가짜 초록을 하나 잡았다.** 관리자 탭 대상 ratchet 이 프로덕션에서 **2.1초 만에 "0건"** 으로
  통과했다. 바닥선이 218인데 0이면 축하할 일이 아니라 **계측이 죽은 것**이다 —
  `DEV_ADMIN_BYPASS` 는 `NODE_ENV==='production'` 에서 코드가 무조건 무력화하므로
  관리자 라우트가 전부 로그인으로 튕겼고, 스캐너는 그것을 "잴 것 없음" 으로 넘겼다.
  기존 가드는 `.env.local` 의 **글자만** 읽고 있었다
- `adminReachable(page)` 신설 — 플래그가 아니라 **서버에 물어본다**(`/admin` 이 로그인으로
  튕기는가). 관리자 스펙 4곳(훑기 2 · 팝업 1 · 탭 대상 1)에 걸었다.
  이제 프로덕션에서는 **이유를 밝히고 skip** 하고, dev 에서는 그대로 218건을 잰다
- 같은 이유로 관리자 `390px 가로 스크롤` 테스트도 프로덕션에서 조용히 통과하고 있었다 — 함께 해소

### 🚪 공개 표면은 어느 훑기의 분모에도 없었다 (v08.7)

전수 훑기가 둘 있었지만 **둘 다 로그인한 뒤의 화면만** 봤다(학습자 45 · 관리자 33).
랜딩 `/` · `(marketing)` · `(auth)` **12화면은 한 번도 훑은 적이 없다** —
하필 검색·공유가 도착하는 정문이고, 모든 가입자가 반드시 통과하는 길이다.

- `33-public-sweep` 신설 — **로그아웃 상태로** 6축(열림·조용함·연결·복귀·가로스크롤·탭대상).
  로그인 상태로 재면 `(auth)` 4개가 리다이렉트로 튕겨 **초록인데 검증 0** 이 된다
- 지금 **전 축 0** (12화면)
- 고친 것: 마케팅 헤더·푸터(공용 · 푸터 링크가 17px 이었다) · `(auth)` 테마 전환 ×4 ·
  `(auth)` 로고 ×4 · 비밀번호 보기 28×28 · **가입 필수 동의 체크박스 3개**(`Checkbox` 공용 ·
  주석이 "30px 보장" 이라 적고 있었다) · 비밀번호 찾기 73×13 · 요금제 결제 토글 ×2 ·
  `/dev` 가로 스크롤 22px
- **탭 대상 규칙을 한 곳으로 모았다**(`utils/tap-target.ts`) — 32·33 두 스펙이 복제 중이었다.
  규칙도 두 단계로 정확해졌다:
  · 아이콘 전용(글자 ≤1) → **양쪽 44px** · 글자 있는 것 → **높이 44 + 너비 24**
  · 옛 규칙 `min(너비,높이) ≥ 44` 는 푸터의 "소개"(2글자)를 `26×44` 로 걸었다 —
    높이는 이미 44인데 **글자가 짧아서** 너비가 26이다. 거기 가로 여백을 강제하는 것은
    접근성이 아니라 디자인 변경이다
  · 인라인 링크 면제도 **글자 수가 아니라 문맥**으로 바꿨다. "이미 계정이 있으신가요? **로그인**"
    의 3자 링크는 걸리고 푸터 "소개"(2자)는 빠지던 것이 정확히 거꾸로였다
- ⚠️ 관리자 ratchet **205 → 218** — 회귀가 아니라 위 규칙 교정으로 **빠져 있던 13건이 드러난 것**.
  옛 규칙이 "라벨이 긴 `<a>`" 를 인라인으로 보고 면제해 "소스 GET (대량)" 같은 내비 링크를 통째로 뺐다
...
### pending_words 팬아웃 판정 — 사전 +243 · 큐가 처음으로 줄었다

⚠️ **큐끼리 물려 있다.** TCP 드레인을 무인으로 돌린 몇 시간 사이
`topic_corpus_queue` 85,179 → 40,411 인데 `pending_words` 는 8,962 → 14,577 로 **늘었다** —
TCP 수확이 사전 갭을 거기에 쌓는다. **자동화한 큐가 자동화 안 된 큐를 채우고 있었다.**

- `.claude/agents/pending-words-judge.md` + `.claude/commands/pending-words-drain.md` —
  청크당 서브에이전트 팬아웃. 판정 넷(add / proper_noun / noise / defer)의 계약을 사양에 명문화
- 실측 2물결 10청크(600낱말) · **형태 문제 0 · 적재기 검증 탈락 0**
  → 사전 **47,890 → 48,133**(+243) · proper_noun_forms +97 · noise_blacklist +165
  → `pending_words` 14,577 → **14,332** (TCP 유입에도 처음으로 감소)
- 5물결 34청크(2,040낱말) 누적 · 무결성 34/34 · 검증 탈락 0
  → 판정 합계 add 694 · defer 618 · proper_noun 271 · noise 308
  → **사전 신규 표제어 +640**(물결별 적재기 보고 243+141+168+88) ·
    proper_noun_forms +271 · noise_blacklist +308
  ⚠️ **표 델타를 성과로 쓰지 않는다.** `shared_dictionary` 총량은 같은 시간에 VCB 등
  다른 세션도 쓰기 때문에 이 작업의 몫이 아니다 (실측: 5차 적재 전후 표는 +400 인데
  이 적재의 신규는 +88 이었다). 근거는 적재기가 스스로 보고한 "새로 넣을 것" 수치다.
- ⚠️ **팬아웃에 claim 이 없었다** — 3차에서 두 에이전트가 "내 청크에 이미 다른 판정본이
  있었다" 고 보고했다. DB 큐는 `FOR UPDATE SKIP LOCKED` 로 막혀 있는데 **청크 폴더에는
  그런 장치가 없었다.** DB 는 안전했지만(적재기가 이미 있는 표제어를 건너뛴다 — "이미 등재
  437건") 낭비된 것은 판정 노동이고 위험한 것은 판정본끼리의 덮어쓰기다.
  → `scripts/lib/claim-chunks.mjs` — 30분 회수(compose 큐와 같은 값) · `--max` · `--release` ·
    `--done-dir`. **네 개의 팬아웃 명령이 같은 구현을 공유한다**(bash 를 네 곳에 복사하면 갈린다).
    자체검사 13건(`claim-chunks.selftest.mjs`).
  → 만들자마자 실결함이 나왔다: `chunk-*.json` 글롭이 **자기 산출물 `chunk-00.out.json` 도
    잡아** 이미 판정한 청크 6개를 다시 잡았다. 규칙을 거꾸로 적용해 산출물을 걸러내고 회귀로 잠갔다.
- ⚠️ **속도 정정** — "팬아웃 51낱말/분 vs 유입 4낱말/분 → 순배출 47" 은 **드레인 한 대 기준**이었다.
  실제로는 TCP 드레인이 **3대** 돌고 있어 유입이 훨씬 컸다(편당 0.15~0.38낱말).
  지금은 팬아웃이 유입을 못 이긴다. 다만 TCP 는 유한하고(잔여 20,519) 마르면 유입이 0 이 된다.
- 파일럿에서 배운 것을 사양에 되먹였다 — legal `word_register` 8종 명시 · **굴절형은 add 가 아니다** 절
  (`headsets`·`thinkers` 를 넣으면 틀린 형태가 사전에 영구히 남는다)

⚠️ **낭비의 진짜 원인은 드레인이 아니라 적재 방식이었다** (소스별 실측):
  `ted:catalog`(카탈로그 전량 투입) 수확률 **4.7%** vs `ted:sleep`(주제 `discover`) **15/15**.
  TEDx 여부는 원인이 아니다(TEDx 2.9% · 그 밖 5.0% — 둘 다 낮다).
  → 화면도움말에 "적재는 discover 로, 카탈로그 전량 투입 금지" 를 실측과 함께 박았다.
...
### ⭐⭐ 수능 설계도 — 표면 96.1% 뒤에 숨은 것: **내 오답이 정답과 안 닮았다** (§10.18~10.21)

v4 의 96.1% 는 **내가 겨냥한 축**이라 순환이었다. **오답 매력도는 겨냥하지 않았다** —
§6.12 의 IDF 측도를 걸었고, IDF 는 **기출과 생성을 합친 코퍼스**에서 만들었다.

| 측도 | 생성(v1~v4) | 기출 | raw p | Holm |
|---|---|---|---|---|
| 지문 미끼 | 0.0246 | 0.0213 | 0.277 | — |
| 접근성 | 0.0303 | 0.0250 | 0.251 | — |
| 미끼 격차 | −0.0058 | −0.0037 | 0.639 | — |
| **혼동도(정답↔오답)** | **0.0155** | **0.0530** | **0.0060** | **0.024 ✓** |

- ⭐ **내 오답은 정답과 어휘를 안 겹친다** — 기출의 1/3. v1·v3·v4 는 **중앙값 0.0000**
- **기출 오답은 정답의 어휘를 물어 쉽게 지워지지 않는다.** 내 오답은 "명백히 다른 이야기" 라
  지문을 안 읽어도 셋은 지울 수 있다 — **표면 계측이 못 잡는 것의 실례**

### ⚠️ 고치려다 반대편으로 넘어갔다 (v5)

| | v4 | **v5** | 기출 |
|---|---|---|---|
| 혼동도 | 0.0184 | **0.1496** | 0.0531 (상단 **0.1190**) |

- **상단도 넘었다**(p=0.002). ⚠️ **§10.11 의 "게이트는 양측" 을 오답 쓰기에서 또 놓쳤다**

### ⭐ 합산 null 이 상쇄의 산물이었다

- 다섯 판 합산은 혼동도 0.0423 vs 0.0531, **Holm 1.000 "구분 안 됨"** — **그 null 은 가짜다**
- **v1~v4 는 낮고 v5 는 높다. 합치면 지워진다.**
  §6.14 의 G1 교훈이 **분석이 아니라 생성 쪽에서 다시 걸렸다**
- 스크립트에 **Holm 보정과 판별 분해**를 넣어 이 함정을 드러내게 고쳤다

새 스크립트 `score-generated-bait.mjs` · 새 자료 `generated-set-v5.json`

명제 107 → **109** · 판정 99.1% · 전수 63.6% → **64.2%**

### ⭐⭐ 수능 설계도 — 병목을 찾고 **천장을 뚫었다**: ③ 76.5% → **96.1%** (§10.14~10.17)

**유형별 대역**을 뽑아 전수 스캔하니 소스 재고가 **0편**이었다.
...
| 낱말 길이 ∩ 문장당 낱말 | **2** (독립이면 5.5 기대) |
| **넷 다** | **0** |

- ⭐ **병목은 낱말 길이이고 문장당 낱말과 거의 배타적이다** —
  **어려운 낱말을 쓰는 글은 문장이 짧고, 문장이 긴 글은 낱말이 쉽다**
- 이것이 v1~v3 가 76~82% 에서 멈춘 이유다 — 세 소스가 배타성의 서로 다른 쪽에 앉아 있었다

### ⭐⭐ 고르지 않고 **겨냥해 썼다** — v4 **96.1%**

`check-passage-band.mjs`(새 스크립트)를 자로 삼아 대역 안에 들 때까지 고쳐 썼다.
초안은 낱말 길이 **6.06**(상단 초과) · TTR **0.720** 으로 2/5 였고, 두 번 고쳐 **5/5**.

> **기출은 어휘를 다양하게 쓰지 않는다** — 핵심어를 되풀이해 결속을 만든다(TTR 중앙 0.650).

| 판 | 낱말 길이 | 어휘 다양도 | 글자수 | **전체** |
|---|---|---|---|---|
| v1 | 6/9 | 5/9 | 9/9 | 82.4% |
| v2 | 8/9 | 7/9 | 7/9 | 80.4% |
| v3 | 8/9 | 6/9 | 8/9 | 76.5% |
...
### ⚠️ 그런데 이 96.1% 는 품질의 증거가 아니다

- v1~v3 는 계측 축을 **겨냥하지 않아서** 독립 판정이었다. **v4 는 그 축을 겨냥해 썼으므로 순환**이다
- **v4 가 답하는 것은 하나뿐 — 그런 글을 쓸 수 있는가. 답은 예다.**
  그리고 그것이 파이프라인의 **작문 목표**가 된다
- ⚠️ 계측이 맞아도 **논지의 선명함·오답 매력도는 안 재진다**. 표면만 맞춘 것이다
- ⚠️ **지문 길이 실수를 네 번 반복했다**(v1·v3·v4 초판·v4 2판). 유형별 중앙표를
  **네 번째에야** 만들어 맞췄다 — 먼저 만들었으면 한 번에 끝났을 일이다

새 스크립트 `check-passage-band.mjs`(파이프라인 작문 게이트로 그대로 쓸 수 있다) · 새 자료 `generated-set-v4.json` · `type-bands.json`

명제 105 → **107** · 판정 99.1% · 전수 63.8% → **63.6%**

### ⚠️⭐⭐ 수능 설계도 — 게이트가 **단측**이었다 · 세 소스로 세 번 만들어도 **총점이 안 오른다** (§10.11~10.13)

**두 번째 정정.** §10.8 의 "32편 통과" 게이트도 틀렸다 — 난도를 **하한만**(≥) 걸어
**기출보다 어려운 글도 통과**시켰다. 기출 369편의 10~90 분위는 양쪽이 있다:
낱말 길이 **4.361~5.497** · 8자 이상 **0.108~0.265** · 문장당 낱말 **10.3~25.2**.
...
- ⭐ **자체 작문이 0 이 아니라 16편** — §10.1 의 "한 편도 못 닿는다" 는 하한 게이트가 만든 값이었다
- ⭐ **PLOS 는 4 → 0** — 편집 논평이 낱말 길이 5.91 · 8자 이상 0.323 으로 **기출 상단 초과**.
  **"어려울수록 좋다" 가 아니다**

### ⭐⭐ 세 소스로 같은 9문항을 세 번 만들었다 — 76~82% 에서 천장

| 판 | 소스 | 낱말 길이 | 문장당 낱말 | **전체** |
|---|---|---|---|---|
| v1 | cc0 자체작문 B2 | 6/9 · 19% | **9/9** · 29% | **82.4%** |
| v2 | eLife cc_by | **8/9** · 68% | 7/9 · **81%** | **80.4%** |
| v3 | cc0 대역 중앙 | **8/9** · 27% | **4/9** · 33% | **76.5%** |

- **실패 축이 소스마다 옮겨 갈 뿐 총점은 안 오른다.** 두 축을 동시에 맞추는 소스가 세 번 다 없었다
- **원인은 게이트가 유형별이 아니라는 것** — 빈칸의 문장당 낱말 대역은 14.50~24.00 인데
  전체 대역은 10.3~25.2 다. 전체로 고르면 유형 대역을 벗어난다
- ⚠️ **나는 지문 절단을 세 번 반복했다**(v1 659자 · v3 570~800자) — 절차의 실수이지
  설계도의 한계가 아니지만, 세 번 반복했다는 사실을 적어 둔다
- → **다음 지렛대는 소스가 아니라 게이트의 해상도**다(유형별 대역)

새 자료 `generated-set-v3.json`

명제 104 → **105** · 판정 99.0% · 전수 63.5% → **63.8%**

### ⚠️⭐⭐ 수능 설계도 — **"0/310" 은 내 표본 절단이었다** · 실제 통과 소스로 **어휘 축이 닫혔다** (§10.8~10.10)

**자기 정정.** 앞 항목의 *"세 관문 동시 통과 0/310 … 구조적 공백"* 은 틀렸다.
외부 소스를 `left(content, 1400)` 으로 **잘라서** 쟀고, 자르면 머리글 보일러플레이트 비중이 커져
장르 게이트에서 떨어진다. **§7.5 의 "조용한 절단" 을 또 밟았다 — 이번엔 내가 SQL 에서 잘랐다.**

**전수 스캔 결과 — 32편이 세 관문을 통과한다**

| usgs 14/19 · voa 6/72 · noaa 4/12 · plos 4/5 · wikipedia 2/5 · **elife 2/2** | **합 32** |
|---|---|
| 통과 0: original 0/455 · nasa 0/33 · simple_wikipedia 0/35 · owid 0/13 | |

- "구조적 공백" 은 **과한 결론**이었다. 재고가 얇을 뿐 0 이 아니다
- ⚠️ 32편은 **회차당 28지문을 한 번 채울 분량** — 계속 출제하려면 채널을 넓혀야 한다

### ⭐⭐ 실제 통과 소스(eLife digest, cc_by)로 다시 만들었다 — **어휘 축이 닫혔다**

| 축 | v1 (CC0 자체 작문 · B2) | **v2 (eLife · cc_by)** |
|---|---|---|
| **낱말 길이** | 6/9 · 중앙값 **16%** | **8/9 · 중앙값 68%** |
| 문장당 낱말 | **9/9** · 29% | 7/9 · **81%** |
| 지문 글자수 | **9/9** · 11% | 7/9 · 20% |
| **전체** | **82.4%** | **80.4%** |

- ⭐ **어휘 난도 축이 소스만 바꿔서 닫혔다** — 백분위 중앙값 16% → **68%**(기출 중앙 근처).
  §9.9 의 진단("남은 것은 어휘 난도 하나")이 **맞았다**
- ⚠️ **대신 문장이 길어졌다**(81%). **두 축이 동시에 맞는 소스는 아직 없다** —
  v1 은 문장·길이가 맞고 어휘가 안 맞으며, v2 는 그 반대다
- → **출제 절차는 80%대에서 안정적이고, 남은 20% 는 소스가 정한다.**
  소스 게이트에 **문장당 낱말(≈19.6)** 을 더하면 좁힐 수 있다(§10.7 기준표에 이미 있다)

새 자료 `generated-set-v2.json` · `score-generated-set.mjs` 에 `--set` 인자 추가

명제 103 → **104** · 판정 99.0% · 전수 63.1% → **63.5%**
