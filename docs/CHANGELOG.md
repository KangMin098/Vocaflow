...
### sitemap 이 만화 중복 5개를 광고하고 있었다 — 화면은 105호, sitemap 은 110개

앞 커밋에서 "`super-mystery-comics` 33호의 `issue_no` 공백은 별개 결함이고 파이프라인이
채울 수 있다" 고 적었다. **틀렸다.** 실측해 보니 그 시리즈는 **권-호 체계**다:

```
v01n02 · v01n04 · v01n05 · v01n06 · v02n01 · v02n03 …
```

단일 `issue_no` 로는 표현할 수 없다 — `n` 만 쓰면 v01n02 와 v02n02 가 같은 2 가 되어
정렬이 오히려 깨진다. **`null` 이 정직한 상태**이고 slug 정렬이 정답이었다.

대신 옆에서 진짜 결함이 나왔다. `Atomic War!` 는 **4개 호가 9행**인데,
같은 만화의 다른 스캔본(IA 업로드가 여럿)이 각각 발행된 것이다. 그래서:

| | 만화 수 |
|---|---|
| 서가 · 이어읽기 (`list_pd_comics`) | **105** (호마다 대표 하나) |
| sitemap (표 직접 조회) | **110** |

차이 5개는 검색엔진에 **중복 콘텐츠**로 나가고, 서가에서 도달할 수도 없는 고아 URL 이다.

- `comicEntries` 를 표 직접 조회에서 **`list_pd_comics` RPC** 로 바꿨다 —
  서가·이어읽기가 쓰는 바로 그 함수다. 도서에서 세운 원칙("sitemap 조건을 화면과 똑같이")과
  같되, 조건을 손으로 맞추는 대신 **같은 함수를 부른다**(더 안전하다)
- 이어읽기는 대표가 아닌 호에서 이웃을 만들지 않는다 — 없는 순서를 지어내지 않는다
- 절단 회귀의 대조 대상을 표 총계에서 **RPC 대표 수**로 바꿨다.
  표를 세면 중복 5개만큼 항상 어긋나고, 그 어긋남이 절단으로 오해된다
- sitemap 콘텐츠 283 → **278**(도서 13 · 만화 **105** · 글 160). 총 URL 289

**숫자가 줄었지만 정확해졌다.** 중복을 광고하지 않는 쪽이 색인에 낫다.

**남은 결정**: `Atomic War!` 중복 5행을 발행 취소할지. 데이터 변경이라 승인 대기에 올린다.

### 만화 상세에 이전 호·다음 호 — 연재물인데 출구가 목록 하나였다

글에 이어읽기를 붙인 것과 같은 이유다. 만화는 **연재물**이라 다음 호로 이어 읽는 것이
가장 자연스러운 행동인데, 이 화면의 출구가 "복원 만화 목록" 하나였다. 그 목록은
**시리즈 단위**라 한 호를 다 읽은 사람이 다음 호로 가려면 두 번 거슬러 올라가야 했다.

- `listPdComics(client, seriesKey)` 를 재사용한다(이미 시리즈 필터를 지원한다 — 새 RPC 불필요)
- ⚠️ **호수(`issue_no`)를 믿고 정렬하지 않는다.** 실측(2026-08-26):
  - 발행 중 가장 큰 시리즈 `super-mystery-comics`(**33호**)는 호수가 **전부 `null`**
  - `Atomic War!` 는 **9호가 번호 4개**를 나눠 쓴다
  → 호수가 있으면 호수로, 없으면 `slug` 로. 어느 쪽이든 순서가 흔들리지 않게 한다
- 실측(dev): `v06n06` → 이전 `v06n05` · 다음 `v07n01`.
  **호수가 전부 null 인 바로 그 시리즈**에서 slug 폴백이 올바른 연재 순서를 만들었다

기록만 남긴 것:
- 도서 13권에는 넣지 않았다 — 수가 적고 카탈로그(`/library/books`)가 이미 13개를 전부 링크한다.
  글 160 · 만화 110 과 달리 이어읽기의 이득이 거의 없다
- `super-mystery-comics` 33호의 `issue_no` 가 비어 있는 것은 **별개 결함**이다.
  slug(`…-csv06n06`)에는 권·호가 들어 있으니 파이프라인이 채울 수 있다.
  제목에 호수가 안 붙고 정렬이 slug 에 의존하게 되는 대가를 지금 치르고 있다

### 글 상세의 출구가 로그인 CTA 하나뿐이었다 — 이어읽기

앞 사이클에서 글 160개에 주소를 만들었다. 그런데 **읽고 나면 갈 곳이 없었다** —
출구가 로그인 CTA 하나였다. 크롤러도 같다: 글 160개가 sitemap 으로만 이어져 있고
**서로를 가리키지 않으면** 검색엔진이 그것을 하나의 묶음으로 읽지 않는다.

- 같은 `article_v_level` 에서 **분량이 가까운 순** 셋을 건다.
  - 왜 V-Level 인가: `feed_label` 은 흩어져 있고 가장 큰 묶음이 `null`(41개)이라 기준이 못 된다.
    난이도는 이 제품이 이미 쓰는 축이고 "이 글이 편했다면 다음은 이것" 이 §학습원칙과 같은 말이다
  - 왜 분량 근접인가: 무작위는 요청마다 달라져 캐시와 안 맞고, 최신순은 같은 글만 계속 나온다
- 실측(dev): `Prague` 하단에 `Kyoto 약 44분` · `Most people care about farm animals… 약 15분` ·
  `Childhood stunting… 약 15분`

확인했으나 손대지 않은 것:
- `ScriptsBrowser`(목록)는 **주제 트랙 단위**라 개별 글 카드가 없다. 여기에 링크를 넣으려면
  UI 재설계에 가까워지고, sitemap 이 이미 160개를 직접 제출하므로 색인은 된다.
  이어읽기가 글끼리 잇는 것으로 내부 링크의 실질을 채운다
- `(main)` 레이아웃이 공개 상세에도 학습자 사이드바를 그린다 — 비로그인에게 보호 경로 목록을
  보여주는 셈이라 전환에 좋지 않을 수 있다. 다만 **불필요한 DB 조회는 없었다**
  (`fetchTodayStatus` 가 `if (!user) return null` 로 먼저 빠진다). 내비게이션을 바꾸는 것은
  제품 결정이라 기록만 남긴다

### 짧은 글 160개에 주소가 없었다 — 색인 표면 123 → 283

카탈로그를 하나씩 읽다가 `/library/scripts`(Dispatches)에서 걸렸다. 글이 **160개** 있는데
**개별 글로 가는 링크가 하나도 없었다.** 그 라우트(`/library/scripts/[bookId]`)는 상세가 아니라
**리졸버**였다 — `startArticleLearning` 후 `/text/[textId]` 로 보내는 40줄짜리.
비로그인이 글 주소로 오면 그게 실패하고 **목록으로 튕겼다.**

그런데 준비는 이미 끝나 있었다:

| | |
|---|---|
| 데이터 | `library_articles.content` — 본문이 같은 표에 있다 |
| 정책 | `anyone_read_published_safe_articles` — **익명에게 열려 있다** |
| 라이선스 | PD 76 · CC-BY-SA 43 · CC-BY 16 · CC-BY-ND 25 (ND 는 개작 금지지 표시 금지가 아니다) |

**도서·만화와 결정적으로 다르다** — 도서 본문(`content_chunks`)은 `authenticated` 만 읽는데,
글은 본문까지 공개다. 즉 검색엔진이 제목이 아니라 **실제 문장**을 읽을 수 있고,
"NASA 우주 영어" 같은 롱테일은 제목이 아니라 본문에 걸린다. 그게 이 카탈로그의 값이다.

- 리졸버를 **공개 상세 + 리졸버**로 확장(도서 상세와 같은 갈래):
  로그인 → 학습 시작으로 리다이렉트 · 비로그인 → **공개 미리보기**(제목·난이도·본문·출처) + 로그인 CTA
- 출처·라이선스를 화면과 구조화 데이터 **양쪽에** 남긴다 — CC 는 출처 표시가 **조건**이라
  장식이 아니라 준수 사항이다. `Article` + `isBasedOn` + `license`
- 모르는 라이선스 코드는 URL 을 **지어내지 않는다**(키를 뺀다)
- sitemap 콘텐츠 상세 **123 → 283**(도서 13 · 만화 110 · 글 160). 총 URL 294
- 회귀 69 (Article 구조화 5 + 글 포함 1 + 기존)

실측(dev): `Prague | Vocaflow` · 200 / 255 KB · 본문이 SSR 에 들어감 ·
출처 `wikivoyage` · `CC BY-SA 4.0` 표기.

**남은 것**: 목록(`ScriptsBrowser`)이 카드 모달로 열려서 **개별 글로 가는 내부 링크가 없다.**
sitemap 이 직접 제출하므로 색인은 되지만, 내부 링크가 없으면 페이지 권위가 전달되지 않는다.
...
### ⭐⭐ 수능 설계도 — **난도만으로는 소스를 못 고른다**: 세 관문 교집합 **0/310** (§10.5~10.7)

§10 은 "외부 수집의 52.9% 가 난도를 통과한다" 로 끝났다. **그 소스들을 실제로 열어 보니
문항이 안 만들어진다** — NASA 는 `2 Min Read` 머리글에 **논지 없는 기술적 서술**,
OWID 는 `Cite this article` 보일러플레이트에 1인칭, APOD 는 내비게이션 메뉴가 본문에 섞여 있다.

**수능 지문은 논지가 있는 3인칭 설명문**이고, 빈칸·주제·제목은 논지가 없으면 아예 안 만들어진다.

| | 1인칭 | 고유명사 | 보일러플레이트 |
|---|---|---|---|
| **기출 369편** | **0.00%** | **1.82%** | **0.0%** |
| nasa | 0.00% | **23.68%**(13배) | **96.7%** |
| owid | **1.39%** | 8.23% | **100.0%** |
| the_conversation | 0.88% | 5.86% | 0.0% |

**장르 게이트 통과율**: the_conversation **96.2%** · original **94.6%** · nasa **3.3%** · owid **0%**

### ⭐⭐ 세 관문 교집합 — **0/310**

| 소스 | 난도 | 장르 | 파생 | **셋 다** |
|---|---|---|---|---|
| original 자체 작문 240 | **0** | 94.6% | ✅ | **0** |
| the_conversation 26 | 15 | **96.2%** | ❌ **cc_by_nd** | **0** |
| nasa 30 | 20 | **3.3%** | ✅ | **0** |

- **자체 작문은 장르가 맞고 난도가 0% · 외부 수집은 난도가 맞고 장르가 걸린다**
- **The Conversation 만이 둘 다 통과하는데 No Derivatives 로 막힌다**
- ⚠️ 장르 지표는 **대리**다("논지가 있는가" 는 직접 못 잰다) — **하한에서 이미 0**이다

### §10.7 — 파이프라인에 줄 구체 기준 (임계값은 전부 기출 실측)

| 관문 | 기준 |
|---|---|
| 난도 | 낱말 길이 **≥ 4.914** · 8자 이상 **≥ 18.7%** · 문장당 낱말 **≈ 19.6** |
| 장르 | 1인칭 **≤ 4.92%** · 고유명사 **≤ 15.93%** · 숫자 **≤ 2.06%** · 보일러 **없음** |
| 라이선스 | public_domain · cc0 · cc_by · cc_by_sa (**cc_by_nd 제외**) |

> 가장 가까운 길은 **자체 작문의 난도 목표를 올리는 것** — 장르·라이선스는 이미 통과했고
> **한 관문만 넘으면 되며 그 관문을 파이프라인이 직접 통제한다**(8자 이상 낱말 4.8% → 18.7%).

새 스크립트 `measure-source-genre.mjs`

명제 101 → **103** · 판정 99.0% · 전수 64.4% → **63.1%**(분모 증가)

### ⭐⭐ 수능 설계도 — ③ **82.4%** 로 올리고, **소스 난도 격차**를 숫자로 냈다 (§9.9 · §10 신설)

나머지 4문항도 원 소스 길이로 되살렸다.
...
> **길이를 고치면 길이 축이 전부 들어오고 어휘 축은 그대로다.** 진단이 확인됐다 —
> **(a) 절단은 내 실수였고 사라졌다. 남은 것은 (b) 소스 어휘 난도 하나다.**

### ⭐⭐ §10 신설 — "소스 선별은 파이프라인 고도화 전제" 의 크기

기출 읽기 지문 **311편**과 파이프라인 소스를 **같은 자**로 쟀다(사전 없이 계산되는 지표만).
⚠️ **TTR 은 첫 120낱말로 잘라서** 쟀다 — 길이를 안 맞추면 길이 차이를 어휘 차이로 읽는다.

| | 기출(중앙) | `original` 240편 | 외부 수집 70편 |
|---|---|---|---|
| 평균 낱말 길이 | 4.914 | 4.154 (**0%** 통과) | **5.077** (70% 통과) |
| 8자 이상 낱말 | **18.7%** | **4.8%** (0% 통과) | **19.0%** (54% 통과) |
| 문장당 낱말 | 19.556 | 16.909 | **19.545** |

- ⭐ **자체 작문(source=original)은 0/240** — 한 편도 수능 난도에 못 닿는다. 8자 이상 낱말이 **네 배** 차이
- ⭐ **외부 수집은 37/70 = 52.9%** — 문장당 낱말이 **19.545 vs 19.556** 으로 거의 일치.
  **전제는 이미 절반은 참이다.** 문제는 선별이 아니라 **자체 작문의 난도 설정**이다
- ⚠️ **라이선스가 난도 통과분의 41%를 막는다** — 37편 중 The Conversation 15편이 **cc_by_nd(No Derivatives)**.
  빈칸을 뚫는 것이 파생물 제작이다. 파생 가능한 것은 **22/70 = 31.4%**(NASA 20 · PLOS 1 · OWID 1)
- → 파이프라인 고도화의 실제 과제는 **"더 잘 고르는 것" 이 아니라 파생 가능한 라이선스의 고난도 소스를 늘리는 것**

새 스크립트 `measure-source-gap.mjs`

명제 99 → **101** · 판정 99.0% · 전수 64.6% → **64.4%** · 규칙 18.2% → **17.8%**(분모 증가)
