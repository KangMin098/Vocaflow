...
- **Phase 8 도서 5권씩 배치 채굴 루프(진행형)**: 5권 적재→추출·평가→사전 보완→도서 삭제(transient, 용량 절약)→전체 학습대상 반복. seed 5개 직접 INSERT + `reprocess-all-se --ids`(fetch+winkNLP) + 평가 + 삭제 + `curation_meta.dict_mined` 표시. 배치1(Wollstonecraft·Huck Finn·Gadfly 등) 4건 수리(merit·fling·strand·furnish). 누적 추출-평가 수리 13단어(현대/협소 뜻만·대표 뜻 누락 패턴). SE seed ~1,428 다중 세션 반복.
