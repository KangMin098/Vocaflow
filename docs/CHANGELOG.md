...
### ⭐ 수능 설계도 — 빈칸 정답은 오답보다 **짧다** (§6.14), 그리고 ⑤번 선지 추출 버그 (§7.5)
...
| | 관측 | 기저 | p |
|---|---|---|---|
| 정답의 평균 길이 순위 (577문항) | 3.046 | 3.000 | 0.407 → **기각** |
| 인접 문항 정답이 같은 비율 (748쌍) | 16.8% | 18.4%(다중집합 순열) | 0.305 → **기각** |
| **빈칸만 (61문항)** | **2.180** | 3.000 | **0.0001 · Holm 0.0004** |

- ⭐ **빈칸에서 정답은 오답보다 3.7자 짧다.** 선지에 관해 **Holm 을 견딘 첫 발견**이다
  (§6.12 의 두 측도는 raw 로만 유의했고 Holm 에서 무너졌다)
- 관문 전부 통과 — G4 부호검정 **14/16 회차**(동점 1 제외) p=0.0042, 계단이 아니라 상시.
  교란(배열 관행 × 번호 편중)은 **관측 번호 분포를 그대로 쓰는 귀무**로 재검 p=0.0001
- **정답 배열에는 제약이 없다** — 같은 번호가 네 번 잇따른 회차가 실제로 있다.
  평가원은 번호를 **개수만**(E5) 통제하고 순서는 통제하지 않는다
- 학습자 역산(§6.14.6) — 63.9%는 찍기엔 낮다. 쓸모는 반대쪽이다:
  **"길고 지문 낱말이 잔뜩 박힌 선지" 는 정답처럼 보이지만 대개 미끼다**

### ⚠️ 그 과정에서 **두 번째 조용한 절단**을 잡았다 — `choicesOf` (§7.5)

교란 검사가 "⑤ 자리만 평균 68.6자(나머지 43자)" 를 뱉었다. 관행이 아니라 **버그**였다 —
⑤ 는 끊어 줄 마커가 없어 블록 끝까지 먹었고, **2022#34 의 ⑤ 는 1,029자로 35번 문두를 품고 있었다.**

| | 수정 전 | 수정 후 |
|---|---|---|
| 자리 간 최대 격차 | 25.9자 | **2.7자** |
| 빈칸 평균순위 | 2.369 | **2.180** (효과가 커졌다) |
| 순서(R-ORDER) "정답이 길다" 3.50 | raw p=0.047 | **사라짐** — ⑤ 부풀림의 산물 |
| 전체 추출률 | 96.6% | **93.5%** |
| 순서대응 판정 가능(읽기) | 27 | **33** |

- **추출률 하락은 후퇴가 아니다** — 전에 **틀린 값으로 세던** 문항을 이제 거부한다.
  남은 실패는 대부분 듣기 2단 조판 뒤섞임이라 원래 못 쓰던 것들
- ⑤ 가 나머지 중앙값의 3배를 넘으면 **그 문항을 버리는** 가드 추가 (조용히 쓰는 것보다 낫다)
- **소비처 13개 전부 재실행** — §6.12 미끼는 셋째 자리만 이동(p 0.0054 → 0.0051 · r −0.617 → −0.619),
  순서대응은 20/27 → **25/33**(개선) · 듣기는 29/30 → **26/27**. **뒤집힌 결론 없음**
- 교란 검사가 발견을 죽인 게 아니라 **가짜 하나를 죽이고 진짜 하나를 키웠다**

명제 73 → **77** · 판정 98.7% · 전수 57.5% → **59.7%** ·
규칙 17.8% → **16.9%**(HARD 는 12 그대로, **분모가 늘어서** 내려간 값 — 세 축을 곱하지 않는 이유)
