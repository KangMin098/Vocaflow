...
**하네스 사용법 함정** — Git Bash(Windows)에서 라우트를 **하나만** 줄 때 MSYS 경로 변환이
`/library/textbooks/5` 를 `C:/Program Files/Git/library/...` 로 바꿔 아무 라우트와도 안 맞는다.
쉼표가 섞이면 변환이 꺼져서 여러 개를 줄 때는 멀쩡하다 — 그래서 더 헷갈린다.
`MSYS_NO_PATHCONV=1` 로 끄면 된다. **빈 ROUTES 가드가 이걸 잡아 줬다** — 없었으면
0장 찍고 성공으로 보였을 것이다(그 가드를 만든 이유가 정확히 이것이었다). 스펙 머리에 적어 뒀다.
