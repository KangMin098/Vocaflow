...
**금지색은 이름이 아니라 색상환으로 검사한다** (2026-09-20 · DD-62). 평균 신호 라쳇이
`packages/design-tokens/src` 까지 훑고(표면 `tokens`), 그 안의 **모든 색 리터럴을 HSL 로 바꿔**
보라·인디고 구간(색상 235°–309° · 채도 ≥30 · 명도 10–97)에 앉은 값을 찾는다. 주석 안의 색은 세지 않는다.
리터럴 목록(`#8B5CF6`…)으로는 한 글자만 달라도 새기 때문이다 — 실제로 `#6E3AF0` 은 목록을 그냥 통과했다.
남아 있는 자리는 **아케이드 `--combo`**(게임 색은 판면 규칙 밖 · 「변경 금지」) · **CEFR C1 척도색**
(등급 A1→C2 의 한 칸 = 의미) · **단어장 표지 duotone 「unique」**(`iosColors.purple`/`purpleTint` —
색을 바꾸면 이미 각인된 표지와 어긋나므로 재각인 여부를 먼저 정한다) 셋뿐이고, 각각 이유가 테스트에 적혀 있다.
쓰는 곳이 0 이던 `--ios-indigo`·`--ios-indigo-vivid`(+ `iosColors.indigo`)는 지웠다 — 정의만 남은 색은
화면에 못 나오면서 팔레트를 넓혀 보이게 한다.
