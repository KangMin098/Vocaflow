...
| **I8** | 증명은 **접힌 위(above the fold)** 에서 끝나야 한다 — 390×844 · 1280×900 둘 다 | 실측 하단 좌표. 현행 커버리지 숫자 모바일 787 · 데스크톱 736 |
...
| **D7** | 가입 후 **첫 학습 1회 완료까지 화면 전환 ≤ 3** | 경로 계수 |
...
**항상 금지**: 폭죽 · 트로피 · 콘페티 · 배지 팝업 · 루프 애니메이션 · 자동재생 캐러셀 ·
perpetual micro-motion(쉬지 않는 미세 모션).
