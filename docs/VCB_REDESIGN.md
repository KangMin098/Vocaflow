...
### Round 30~33 — 표제어를 눈으로 열어 보고 고친 것 (2026-08-15)

7지표는 **필드 충족도와 구조**를 잰다. "이게 맞는 표제어인가" 는 재지 않는다. 그래서 28/28
우위인 카탈로그에서 표제어 첫 12개를 뽑아 읽었더니 네 유형이 무너져 있었다.

| # | 무엇이 나왔나 | 원인 | 고친 것 |
|---|---|---|---|
| 30 | 혼동어 1군이 **18개** — `bearing·caring·cleaning·clearing·daring·darling·dating·engineering…` | 편집거리 1 판정은 맞는데 **union-find 전이성**이 그것을 사슬로 이어 접미사 계열 전체를 한 군으로 만들었다. 라임 병합(`-ther` 계열)이 겹쳐 더 커졌다 | 군 크기 상한 4 + 라임 병합 제거(그건 `rhyme-phonics` 의 일이다). 결과 `able·cable·stable·table` · `again·gain·grain·main` |
| 31 | 구동사 세트가 `a bone of contention · a buyer's market · a chink in…` 로 시작 | 구는 `frequency_rank` 가 **3,635건 전부 NULL** 이라 `order_within: 'frequency'` 가 사실상 알파벳순이었다. "빈출" 을 약속하고 근거가 없었다 | 없는 빈도를 쓰는 대신 `phrase_brevity`(예문 有 → 낱말 수 → 길이) 신설 + `, etc.` 열거 표제어 배제. 결과 `do up · go by · go on · act on · add up · get up` |
| 32 | 미수록 세트가 `further · wearing · worn · trading · voting · drinking` | "아무 단어장에도 없는 말" 이 아니라 **기본형이 이미 실려 굴절형만 남은 것**. `drop_pool_inflections` 는 같은 풀에 기본형이 있을 때만 도는데, 기본형은 이미 빠져나간 뒤였다 | `exclude_inflections` 신설 — 사전 컬럼(`base_word`, 커버리지 7%)과 어휘집 대조 판정을 함께 본다. 굴절 144건 제외 |
| 33 | 32 를 고쳤는데 `listing·shaped·dressed` 가 남았다 | 대조군을 **필터 걸린 풀**로 삼았다. `미수록` 은 `v_level_min: 3` 이라 `list·dress·shape`(V1~2)가 풀에 없어 "기본형 없음 = 굴절 아님" 이 됐다. 기본형은 늘 난이도가 낮은 쪽이라 이 실수는 항상 같은 방향으로 샌다 | 대조군을 **사전 전체**로. 그 과정에서 `fetchLexicon` 이 `.order()` 없이 페이징해 45,688 중 **33,412 개만 모으던 것**(27% 누락)도 함께 잡았다 |

Round 33 뒤 미수록 첫 항목: `stuff · earnings · leggings · unhappy · lamp · viewer · documentary · sofa`.

### Round 34 — 단어장 표지 (2026-08-15)

이모지 하나로는 카탈로그에서 29권이 구별되지 않는다. 표지는 "그 책이 뭐였지" 를 되살리는
손잡이인데 손잡이가 다 같이 생겼다. 결을 두 겹으로 만들었다 —
**소재를 퍼블릭도메인 도판으로 한정**(이 도서관 자체가 구텐베르크 고전이라 시대가 맞는다) +
**계열 5색 듀오톤**(출처가 달라도 한 색으로 누르면 한 시리즈로 읽힌다).

수집기(`scripts/vcb/fetch-covers.mts`)를 만들며 실측으로 걸러낸 것:

| 증상 | 원인 | 조치 |
|---|---|---|
| 첫 실행 1/29 | `category=illustration,digitized_artwork` 가 **전 검색어 0건** | 카테고리 대신 소장 기관으로 거른다 |
| 4단어 검색어 1~5건 | Openverse 관련성이 긴 질의에 약하다 | 2~3단어로 |
| NASA 위성사진·콘크리트 텍스처 | Flickr 는 Commons(PD)와 현대 사진이 같은 provider | Flickr 가중치 절반 + 현대어 제목 배제 |
| `tree roots` 에 새 도판, `tuning fork` 에 은하 | 점수에 **관련성이 없었다** | 검색 순위 + 제목 낱말 일치 |
| 4개 세트가 같은 성좌도 | 중복 검사 없음 | 실행 내 사용 이미지 제외 |
| `426013`·`page294` | 스캔 정리번호 제목 | 무엇이 그려졌는지 알 수 없는 제목 배제 |
| 자화상·지리 교과서가 붙음 | 점수기는 늘 "그중 최선" 을 고른다 | **하한 0.55** — 미달이면 붙이지 않고 그라디언트로 남긴다 |

결과 **25/29 · 미확보 4**(하한 미달 → 그라디언트 유지) · 중복 0 · PD/CC0 우선.
표지는 학습자 카탈로그·어드민 발행 컬렉션·hub 추천·도서 상세 네 곳에 모두 배선했다.

### Round 29 — 발음(IPA)·소리(TTS)를 단어장 범위에서 제외 (2026-08-15)

**제품 결정**: 단어장은 발음 표기도, 소리 재생도 다루지 않는다. Round 28 의 TTS 확정을 되돌린
것이고, 되돌리면 세 가지가 함께 움직인다 — 그중 둘은 **오히려 좋아졌다**.

| 무엇이 | 전 | 후 | 결과 |
|---|---|---|---|
| IPA 요구 | 모든 낱말 유형이 `require_fields: ipa` | **없음** | 아무도 안 쓰는 필드 때문에 후보를 9.5%(구는 91.7%) 버리던 것이 사라졌다. 구동사 세트가 그 자리에서 다시 살아난다 |
| Sound 면 요구 | `speakable`(TTS 로 읽히나) | `audio_url`(녹음) | 들려줄 것은 녹음뿐이고 그것이 0% 다. IPA 는 표기이지 소리가 아니라 대신 세지 않는다 |
| D26 오디오 유형 | `ready` | **`asset_gap`** | 재생 경로가 녹음 하나뿐인데 0% — 그림 단어장과 같은 처지. 0건을 내는 것이 정상이다 |
| 시중 `발음 표기` 요소 | 판정 대상 | `applicable: false` | 요소를 **목록에서 지우지는 않았다** — 지우면 "16요소 전부 우위" 가 슬그머니 15요소 이야기가 된다. 남겨 두면 리포트가 무엇을 겨루지 않기로 했는지 보여 준다 |
| U3 이름 | 6면 보장 (`여섯 면이 다 열리는`) | **5면 보장** | Sound 를 뺐으니 여섯이라고 부르면 카탈로그가 거짓말을 한다 |

`audio_playable` 규칙과 `speakable` 필드는 쓰는 곳이 없어져 삭제했다. **WordVault 흘려듣기는
그대로 둔다** — 그건 기존 학습 기능이고, 빠진 것은 "TTS 를 단어장 상품의 전달 수단으로 세는 것"
이다. Round 28 에서 고친 재생 버그 2건(음성 미선택 · 실패 시 큐 정지)도 그대로 남는다.

발행 카탈로그: **29세트 · 15,488단어 · 유형 26종** (오디오 내림 · 구동사/라임/5면 재발행).

### Round 28 — 오디오 전달 방식 확정: 브라우저 TTS (2026-08-15 · Round 29 에서 범위 제외로 정정)
...
컴포저가 발행한 세트 **29개 · 15,488단어 · 유형 26종**. 전부 `curation_query` 에 레시피·채점표·
...
| 전달 기반 2 | 30일 완성 600 · 연상 500 |
| 플랫폼 고유 7 | Fables 해금 300 · Pride 해금 200 · Pinocchio 재등장 80 · Pride 재등장 80 · 다섯 면 500 · 다섯 면 300 · 미수록 600 |
...
| 단어 발음 **녹음** | D26 오디오 단어장 · F3 Sound 면 | `audio_url` 0 / 45,688 — TTS 를 범위에서 제외했으므로(Round 29) 남은 경로는 녹음뿐이다 |

둘 다 설계로 못 메운다. 데이터가 없으면 0건을 내고, 그것이 정상 동작이다 —
빈 세트를 만들어 "있는 척" 하지 않는다.

---

## 지면 지수 — 세 번째 자 (2026-09-06 신설 · 실측)

### 왜 자를 하나 더 만들었나

단어장에는 이미 자가 둘 있었고 **둘 다 이기고 있었다**:

| 자 | 무엇을 재나 | 값 |
|---|---|--:|
| `scripts/vocab/market-benchmark.mjs` | 표제어 칸에 무엇이 들어 있나 (DB 값) | **1.635** |
| `scripts/vocab/choice-benchmark.mts` | 한 권을 **고를** 근거가 있나 (책 앞뒤 정보) | **1.288** |
...
상호참조 · 활용형 · 어근 헤더 · 회독 체크칸.

### 시장 기준선 — 능률VOCA 4권 실측

`scripts/textbook-corpus/vocab-market-spec.mjs` 의 `pageApparatus` 블록. 검출 가능한 장치
**17종**을 4권에서 재어 한 권당 **14.75개**. (미리보기본이라 **하한**이다.)

유의·반의 기호(⇔ =)는 지면에 있으나 **심볼 폰트 글리프가 임베드돼 있지 않아 추출로는
사라진다** — 없는 것으로 세면 시장 기준선이 내려가 우리 지수가 공짜로 오르므로
`undetectable` 로 빼고 분모에서도 뺐다.

### 우리 값 — 렌더된 DOM 실측

`scripts/vocab/design-benchmark.mts` (Playwright · 렌더된 것만 센다).

**한 시트당 1.5개 · 지면 지수 0.102.** 시중의 **1/10** 이다.

학습자가 `/library/vocab` 에서 한 세트를 열면 나오는 것은 **낱말과 뜻뿐**이다 —
품사 · 뜻 번호 · 파생어 · 예문 · 예문 번역 · 어법 · 테스트 · 누적 복습 · 학습 계획 ·
색인 · 상호참조 · 활용형이 **하나도 없다**. 재료는 DB 에 다 있다(내용 지수 1.635 가 그 증거다).
없는 것은 **지면**이다.

### 함께 드러난 것 — 판권면이 열리지 않는다

표본 전부가 `carousel-sheet`(`NetflixDetailSheet`)였다. 카테고리를 골라 격자를 띄운 뒤
카드를 눌러도 열리는 것은 같은 시트다 — **`VocabSetPreviewModal` 은 `/library/vocab` 에서
열리지 않는다.** `VocabSetGrid` 가 그것을 렌더하고 `VocabSetCard` 가 `onPreview` 를 부르는데도
그렇다(실측: `#vocab-preview-title` 0 · `[aria-label="판권면"]` 0).

그래서 **선택 지수 1.288 은 그만큼 부풀어 있다** — 그 지수는 열리지 않는 컴포넌트의
렌더 조건으로 우리 값을 세고 있다. 다음 사이클에서 같은 자로 다시 잰다.

### 이 자가 지키는 것

- 렌더된 DOM 에만 묻는다 — 코드에 있어도 그 라우트에서 열리지 않으면 0.
- 경로가 둘이면 **더 나은 쪽**만 센다(합집합이 아니다).
- 판정 규칙은 시장 쪽과 같은 모양(추출 글자 + 정규식). `runningHead` 한 축만 DOM 을 보는데,
  그 기준(`position: sticky|fixed`)은 글자 판정보다 **엄격**해서 우리 점수를 부풀리지 않는다.
- 시트의 껍데기(안내문 · 통계 칸 · CTA)는 판정 전에 지운다 — 첫 실행에서 안내문 한 줄이
  `exampleEn`·`exampleKo` 에 걸려 지수가 0.39 로 부풀었다. 지금 값 0.102 가 그것을 걷어낸 값이다.

---

## 지면 조판 — 파이프라인이 지면을 만든다 (2026-09-06)

### 무엇을 했나

지면 지수 0.102 의 원인은 **재료가 아니라 자리**였다. `shared_dictionary` 한 행이 뜻 갈래
(품사·예문·예문 한국어역까지) · 발음 · 유의/반의 · 연어 · 학습자 노트 · 굴절/파생형을 이미
들고 있었는데, 그것을 지면에 앉히는 규칙이 어디에도 없었다.

`packages/library-pipeline/src/vocab/typeset.ts` — 순수 함수 `typesetVocabSet()`.
`ComposedSet` 의 낱말을 받아 **PART → DAY → 표제어 칸**으로 앉히고 17종 장치를 채운다.

**화면에 두지 않은 이유 둘**

1. **파이프라인이 만들어야 한다** — 세트는 컴포저가 발행하고, 발행물이 곧 지면이어야
   "게시된 것을 새 파이프라인으로 다시 낸다" 가 성립한다. JSX 안에 있으면 재생성이 안 된다.
2. **브라우저 없이 잴 수 있어야 한다** — 지면 지수는 렌더된 DOM 에서 재지만, 조판이 옳은지는
   그 전에 단위 테스트로 잠근다(19종). 개발 서버가 흔들려도 이 층은 흔들리지 않는다.

### 규칙 (테스트가 잠근 것)

| 규칙 | 왜 |
|---|---|
| 없는 값은 **지어내지 않는다** | 빈 칸을 채우면 그 순간 시장 비교가 거짓이 된다 |
| 굴절형과 파생어를 **다른 칸**에 | 시중 지면은 활용형을 표제어 옆 괄호에, 파생어를 아래 줄에 싣는다. 한 칸에 몰면 둘 다 아닌 것이 된다 |
| 상호참조는 **이 권 안의 낱말만** | 밖을 가리키면 학습자가 따라갈 곳이 없다 (시중의 `(p.208)` 은 늘 그 책 안이다) |
| 뜻이 하나면 **번호를 붙이지 않는다** | 번호가 하나뿐인 목록은 번호가 아니다 |
| 묶음(PART)은 컴포저가 정한 것을 **그대로** 쓴다 | 조판기가 다시 나누면 "목차가 왜 이렇게 갈렸나" 의 답이 두 곳으로 갈린다 |
| `apparatus` 는 **채워진 것만** 센다 | 칸을 만든 것과 값이 들어간 것은 다르다 |

### 닿는 자리에 냈다

학습자가 `/library/vocab` 에서 여는 것은 `NetflixDetailSheet` 다(`VocabSetPreviewModal` 은
아직 열리지 않는다). 그래서 **열리지 않는 쪽을 고치는 대신 닿는 쪽에** 지면을 붙였다 —
`VocabSpreadSheet.tsx` + `GET /api/vocab/[setId]/spread`.

라우트가 필요한 이유는 권한이다: `shared_dictionary` 는 **anon 이 한 행도 못 읽는다**(RLS).
`/fit` 이 같은 벽에서 `textfit_resolve_levels_public` 을 만든 것과 같은 상황이고, 여기서는
서버가 읽어 **조판 결과만** 내려 준다.

### 실측

표본 7권 **전부 17/17**. 지면 지수 **1.153**.

### 이 축의 천장 — 120% 는 여기서 불가능하다

장치는 있거나 없거나이므로 우리가 17종을 전부 채워도 지수는 `17 ÷ 14.75 = 1.153` 을 넘지
못한다. **그 축에 1.20 을 요구하면 영원히 미달로 남는다** — 독해 쪽 `market-benchmark.mjs` 가
A1·A7 축에서 겪은 것과 같은 문제이고, 같은 답을 쓴다: **이 축의 목표는 천장이다.**

120% 판정은 세 자를 합친 **종합 우위지수**로 한다:
...
`scripts/vocab/overall-benchmark.mts`. 기하평균인 이유는 **한 축이 0 에 가까우면 종합도
끌어내려야** 맞기 때문이다 — 지면이 없는 단어장은 내용이 아무리 좋아도 단어장으로 못 쓴다.

### 아직 남은 것

- **선택 지수 재검증** — 근거인 `VocabSetPreviewModal` 이 열리지 않는다. 지면이 생기며
  학습계획·색인·복습은 닿게 됐지만 판권면·머리말·시리즈 안내는 아직 시트에 없다.
- **브랜드·시리즈를 Claude Design 으로** — 표지는 아직 Openverse 도판 + 듀오톤이다.
- **발행 55권 재생성** — 지면은 요청 시 조판되므로 이미 전권에 적용되지만, 조판 결과를
  발행물에 각인(`curation_query.typeset`)하는 일은 아직 하지 않았다.

---

## 브랜드 드레인 — Claude Design 이 파이프라인 단계다 (2026-09-06)

### 왜 코드로 짜 넣지 않았나

표지는 이미 자동이었다 — PD 도판을 Openverse 에서 찾아 계열 색으로 누른다(`covers/design.ts`).
그것만으로 서가가 한 시리즈로 읽히지만 **브랜드는 아니다**: 로고 lockup 도, 권 번호 표기도,
표지 격자도, 계열 부제도 정해진 적이 없었다. 그 결정은 **눈으로 보고 손으로 옮겨야 하는 종류**라
코드로 짜 넣을 수 없다.

그래서 Claude Design 캔버스를 **드레인의 2단계**로 둔다.

### 3단 구조

| 단계 | 하는 일 | 재실행 안전 |
|---|---|---|
| `brand-drain-export.mts` | 계열 5 청크 (`chunk-NN.json`) | ✅ 이미 전권 각인된 계열은 건너뛴다 |
| `brand-drain-artboards.mts` | 토큰에서 아트보드 7장 + `canvas.json` | ✅ 파일만 덮어쓴다 |
...
이 규칙이 있는 이유는 저장소가 같은 사고를 두 번 겪었기 때문이다 — 교재 조판기가 자기 팔레트를
따로 갖고 있어 다섯 항목이 전부 토큰과 어긋났고, 단어장 듀오톤 열 개 중 여덟이 어디서도 오지 않은
값이었다. **아트보드조차 색을 손으로 적지 않는다** — `brand-drain-artboards.mts` 가 토큰을 읽어
칠하므로, 토큰이 바뀌면 다시 돌리는 것으로 따라온다.

### 실측 (2026-09-06)

- 발행 **55권 전부 각인** — list 13 · structure 28 · corpus 2 · delivery 2 · unique 10
- 기존 `curation_query` 키 12종 **사라진 것 0 · 값이 바뀐 것 0**
- export 재실행 → 청크 0 · 건너뜀 5
- 캔버스: <https://claude.ai/code/artifact/f2cce3c7-7b5f-4803-82f0-c8489473b615>

### 아트보드 7장

`Main`(표지 lockup 해부) · `Grid`(격자와 활자) · 계열 5(3:4 표지).
도판은 **선화만** — 표지 프로그램이 PD 판화라 채운 도형이나 그라디언트를 쓰면 수집한 도판과
나란히 놓였을 때 한 시리즈로 안 읽힌다.

---

## 세 자를 같은 화면에서 재게 만들었다 (2026-09-06)

### 선택 지수가 자기 규칙을 어기고 있었다

`choice-benchmark.mts` 는 머리주석에 **"DB 에 값이 있다 가 아니라 학습자가 본다 가 기준"** 이라고
적어 두고, 열한 신호를 전부 `VocabSetPreviewModal` 의 렌더 조건으로 세고 있었다.
그 모달은 `/library/vocab` 에서 **열리지 않는다.**

지면을 열어 다시 재니 **0.94** 였다. 1.288 은 닿지 않는 화면의 값이었다.

| 신호 | 재고(DB) | 지면(렌더) 처음 | 채운 뒤 |
|---|--:|--:|--:|
| isbn · seriesGuide · targetGrade | 100% | **0%** | 100% |
| 나머지 8 | 96~100% | 100% | 100% |
| **지수** | 1.29 | **0.94** | **1.294** |
...
| 줄 | 왜 |
|---|---|
| 판권 번호 `VF-<slug>-v<n>` | 학습자가 이 **판**을 특정해 인용·문의할 수 있어야 한다. slug 가 없으면 만들지 않는다 — 지어낸 번호는 인용할 수 없다 |
| 사다리 띠 | 다음에 무엇을 볼지. **계단 밖이어도 그린다**(어느 칸도 안 세울 뿐) — 빼면 그 권만 시리즈에서 떨어져 나온 것처럼 보인다 |
| 대상 수준 | 계단이 있으면 계단, 없으면 각인된 V-Level 중앙값. 사다리 밖이라고 수준이 없는 것이 아니다 |

셋 다 **값은 이미 갖고 있었다.** 지면에 자리가 없었을 뿐이다 — 지면 지수 0.102 때와 같은 모양의 문제다.

### 표본을 한 곳에서 연다

지면 지수와 선택 지수는 **같은 화면**을 재야 한다. 여는 방법을 두 벌 두면 한쪽만 고쳐졌을 때
두 지수가 다른 화면을 재게 되고, 그 어긋남은 숫자만 봐서는 보이지 않는다.
그래서 `scripts/vocab/_sheet-sampler.mts` 하나가 두 벤치마크의 표본을 연다.

### 최종

| 자 | 값 | 천장 | 무엇을 재나 |
|---|--:|--:|---|
| 내용 지수 | 1.635 | — | 표제어 칸에 무엇이 들어 있나 |
| 선택 지수 | **1.294** | — | 고르기 전에 줄 근거가 있나 |
| 지면 지수 | 1.153 | 1.153 | 펼쳤을 때 매 쪽 장치가 있나 |
| **종합 (기하평균)** | **1.346** | — | — |

세 축 모두 **학습자가 실제로 여는 화면**에서 잰 값이다.
