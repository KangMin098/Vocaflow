...
| 무엇이 | 전 | 후 | 왜 |
|---|---|---|---|
| Sound 면 등급 | `fallback`(0.7 가중) | `full` | 확정된 전달 방식을 결핍으로 계속 세면 그 유형은 영원히 감점된 채 남는다. 녹음 부재는 `audio_playable` 규칙이 채점표에 "녹음 0 · TTS 합성 300" 으로 그대로 적는다 |
...
단어 발음 녹음은 **과제에서 내렸다** — 브라우저 TTS 가 전달 방식으로 확정됐다(Round 28).
녹음이 생기면 그건 결핍 해소가 아니라 가산이다: 오프라인 저장 · 원어민 억양.
