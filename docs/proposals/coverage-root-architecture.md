...
- 사용자 방향성 정합: **커버리지는 도서 잔여 수집이 아니라 (a)포괄 사전 베이스 (b)생성 해소 엔진 (c)폐쇄집합 완비**에서 나온다.
- 3부류 근본해법: **A=규칙+음성+완비맵 하이브리드 · B=재귀 형태소 엔진 · C=Webster 완전판+생성**.
- **단일 알고리즘 만능 없음**(음성 30% 실측) → 생성 엔진 + 포괄 베이스 + 폐쇄집합의 3원 결합이 근본.
- 다음: **Webster 완전 재적재(베이스)** 를 첫 근본 조치로 착수 권장.
