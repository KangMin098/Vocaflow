...
| `/reports` | `(main)/reports/page.tsx` + `components/reports/ReportsClient.tsx` | **P2** 주간 Report Card(daily_activity 집계 + 격려 코멘트) + "이번 주 겹 쌓기" · (2026-09-19) 주마다 한 겹(두께 = 복습 수) · 분 표기 없음 · 조회 실패는 빈 상태와 구분 |
| `/teacher` | `(main)/teacher/page.tsx` + `components/teacher/TeacherClient.tsx` | **P4.2 L3 B2B** 교사 허브(클래스 개설·초대코드·참여·멤버수, classes/class_members) · (2026-09-19 DD-33) 첫 시선 = 초대장 한 장(`InviteSheet` — 반 이름·코드·`/join` QR, 반 없으면 미리보기) |
| `/hub` | `(main)/hub/page.tsx` | **메타 "Today"(forward)** — **TodayStage**(2026-09-19 「들어 올리는 곡선」 — 좌: 7일 기억 곡선 + 「오늘 다시 볼 단어」 슬라이더 + 세션 큐 순서 낱말 24개(권점) · 우: 처방 5블록 흐름, 미진단이면 1차 행동 = `/diagnostic`) + TodayPlanCard(수동계획 날의 정본). 「이 N개부터」 → `/flashcard/play?limit=N&from=/hub`. TodayFocus·NextWordsStrip 은 허브에서 은퇴. **단일 정본 유지** — 계획이 있으면 처방 흐름을 렌더하지 않는다(v06.108 META). async |
...
| `/dashboard` | `(main)/dashboard/page.tsx` + `layout.tsx` | (2026-09-19) 히어로 = 기억의 지층(`DurabilityLadder` — 층 안에 낱말 · 이번 주 권점 · 층 펼치기) · 카드 대신 번호 괘선 구획 · **메타 "회고"(backward, L7 단독)** — known-word 성장 헤더 · MemoryStatus · WeeklyHeatmap · **학습 관리 3카드(ManageSection: 진단·계획·리포트)** · RecentActivity. /manage 흡수(v06.108) |
...
| `/library/textbooks` | `(main)/library/textbooks/page.tsx` | **독해 코너.** 교재 서가 — 7계단 진열 + 3축 필터(학령·수준·유형) + 담기 |
| `/library/textbooks/[series]` | `(main)/library/textbooks/[series]/page.tsx` | **시리즈 코너**(독해·어휘·구문). 화면은 `ShelfScreen` 하나를 공유한다 — 옛 주소는 색인 대상이라 리다이렉트하지 않았다 |
...
| **Practice — 연습 단일 진입면** (v06.201) | `(main)/practice` | — | — | 사이드바 PRACTICE 5형제(Flashcard·WordBlitz·PairFlip·SpellForge·Game Lab) → 2개로 통폐합. **면(facet)으로 고른다** — (2026-09-19 DD-32) 「오늘의 연습지」: 번호 붙은 괘선 문항 6줄 + 문항마다 그 면에서 아직 통과 못 한 내 낱말(`FacetSummary.pending`) · 1번 = 가장 무른 면 · 면 요약 서버 조회. 도구 = 모듈 4 + Game Lab 게임 17(`lib/learner/practice-map.ts` 가 `GAME_CATALOG.layer` 에서 파생) + 활성 시 `Syntax`. 게임 링크는 `from=/practice` 필수(없으면 종료가 `/arcade` 로 튕김) |
...
| **`/fit/s/[payload]`** | **공유받은 결과** — 결과가 URL 에 통째로 담긴다(서버 저장 0). 2026-09-19 `SharedFitView`(전용): 원문이 없으니 가장 어려운 낱말 16개를 `/fit` 과 같은 `PaintedPassage` 로 칠하고 1차 「내 지문으로 해 보기」 — 도구(`PublicFitClient`)로 열던 때는 빈 입력칸이 먼저였다. `opengraph-image.tsx` 가 학년별 곡선을 그린 미리보기 PNG 생성(edge 런타임). `noindex` + canonical→`/fit`. ⚠️ 쿼리(`?r=`)가 아니라 **경로 세그먼트**인 이유: `opengraph-image` 는 `searchParams` 를 못 받는다 |
...
| `/pricing` | 요금 |
| **`/video/[id]`** | **편별 페이지 62개** — **이게 PR 체계의 핵심이다.** 영상 안의 말은 픽셀이라 기계가 못 읽는다. 여기에 **자막 전문을 서버 렌더 HTML** 로 깔고(I6) `VideoObject` 구조화 데이터를 낸다 → 「빈칸 추론이 뭐죠」 같은 검색이 닿는다. 전사는 버킷의 .vtt 가 아니라 **manifest 에 실어 둔 것**을 쓴다(크롤러는 왕복을 안 기다린다). sitemap 실측 **영상 면 1 → 63** |
| **`/video`** | **영상 서가** — 구성요소마다 한 편(`packages/video-factory` 가 찍는다). 공개인 이유는 첫 독자가 학습자가 아니라 **교사**라서다(허용 CAC ₩400 · 교사→학급 경로). 발행 전에는 목록 대신 `/fit` 로 가는 **다음 한 걸음**을 보여 준다(D5) — 빈 화면을 두지 않는다. 계측 `video_started` · `video_completed` |
| `/privacy` | 개인정보 |
| `/terms` | 약관 |
