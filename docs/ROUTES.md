...
| `/hub` | `(main)/hub/page.tsx` | **플랫폼 메인**(2026-09-22 재설계) — 배너 캐러셀 `PortalHero`(서가·만화·수능·아케이드·교사, 미진단이면 진단이 첫 장) + 「나의 오늘」 패널(셸 나침반과 같은 `buildWayfinder` 모델 · `data-today-flow`) → 바로 가기 10 → 새로 들어온 고전 → 홍보 격자 → 읽을거리(새 글 5 + 주제 트랙 7) → 아케이드 띠 → 목적별 단어장 → 서가 규모. 수치·표지는 `lib/learner/hub-portal-query.ts` 가 발행 게이트로 센다. 비로그인은 `/login` 으로. async |
