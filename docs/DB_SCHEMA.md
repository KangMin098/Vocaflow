...
**v06.34 FK 부분 인덱스** (`20260908051907`): `idx_sw_source_queue_id` on `shared_words(source_queue_id) where source_queue_id is not null`. 전체 인덱스가 아닌 이유 — 681,023행 중 채워진 행 1,998(0.29%)이라 btree 가 NULL 679,025개를 담아 ~15MB 가 된다. 부분 인덱스 실측 **64 kB**. advisor 의 unindexed_foreign_keys 를 따를 때 **채움률을 먼저 볼 것.**
...
20260926120000  video_request_supersede_restore            ← 요청 applied_at + 옛 failed 재시도 차단 + purge 뒤 다시 찍기 되살리기(rerender_requested_at · video_retire_rerendered) — **미적용, 사용자 승인 필요**
...
### 영상 내리기 · 교체 — `video_retirements` · `video_requests.mode` (2026-09-24)

| | |
|---|---|
| 테이블 | `video_retirements` — `video_id` PK · `reason` NOT NULL · `retired_at` · `restored_at`(null = 지금 내림) · `purged_at`(파일 삭제) · admin 읽기 RLS |
| 열 | `video_requests.mode` new/replace — replace 면 `video_id` = 대상 id(요청 때 확정) |
| 인덱스 | `video_requests_video_id_live_uniq` — 부분 유일(approved·applying·applied). 옛 `video_requests_video_id_key` UNIQUE 는 제거(같은 자리를 여러 번 교체) |
| RPC (화면) | `video_retire(id, reason)` · `video_restore(id)` — purged 면 거절 · `video_request_create(…, p_mode)` |
| RPC (service_role) | `video_retire_mark_purged(id)` · `video_job_restart(id, kind)` — 교체 시작 때 큐 행을 queued 로 |
| 20260926120000 | `video_requests.applied_at`(applied 전이 순간의 `video_jobs.published_at` — 평가 창의 시작) · `video_request_advance` 가 같은 자리에 더 새 요청이 있는 `failed` 의 재시도를 거절 · `video_retirements.rerender_requested_at` · `video_restore` 는 purged 면 거절 대신 다시 찍기 요청 · `video_retire` 는 재내림 때 요청을 지움 · `video_retire_rerendered(id)`(service_role — 렌더 완료 뒤 purged_at 해제 + 되살림) |

### 영상 평가 결과 — `video_jobs` 의 평가 열 (2026-09-14)

큐는 **어디까지 왔나**(단계)는 답해도 **잘 찍었나**는 못 답했다. 그래서 결함은 스틸을 띄워
눈으로 볼 때만 드러났고, 219개 컴포지션에서 되풀이할 수 없는 방식이었다.

| | |
|---|---|
| 열 | `eval_at` · `eval_pass` · `eval_fail` · `eval_unknown` · `eval_axes jsonb` |
| 인덱스 | `idx_video_jobs_eval_fail` — **부분**(`eval_fail > 0`). 대부분 0 이라 전체 인덱스는 낭비다 |
| RPC | `video_job_evaluate(video_id, pass, fail, unknown, axes)` → service_role · `video_eval_overview()` → service_role·authenticated |

**왜 같은 행인가** — 관리자가 묻는 것은 「이 편이 어디까지 왔고 규격 안인가」 하나다.
두 표를 맞대야 답이 나오면 화면이 그 조인을 매번 해야 하고, 한쪽만 갱신되면 둘이 갈린다.

**왜 축을 jsonb 로 두나** — 축은 늘어난다(지금 7종 + 규격별 음량 3). 축마다 열을 만들면
축을 더할 때마다 마이그레이션이 필요하고, 그러면 **축을 안 더하게 된다.** 판정 규칙의 정본은
`packages/video-factory/src/spec/evaluate.ts` 이고 DB 는 결과를 담기만 한다.

⚠️ **합계 세 개를 따로 두는 이유**: jsonb 안을 세면 화면이 매번 배열을 훑어야 하고, 무엇보다
**못 잰 축(unknown)이 조용히 합격으로 접힌다.** 세 수를 분리해 두면 `pass + fail ≠ 전체` 가
눈에 보인다.

⚠️ **`stage` 를 건드리지 않는다.** 평가는 제작의 단계가 아니라 그 위의 판정이다 —
섞으면 「평가에서 떨어졌다」가 「아직 안 찍었다」로 읽힌다. 그래서 기록 RPC 는 **행을 만들지도
않는다**(없는 편의 평가를 받아 행을 만들면 큐에 유령이 생긴다).
...
| `reviewed_digest` | **그 판정이 읽은 문항의 판** — sha256(canonical JSON of {payload, answer_key}). CHECK 64자 hex. **NULL 은 「같다」가 아니라 「모른다」**(2026-09-14 이전 516행) |
| `unique (item_id, persona)` | **이 제약이 표의 존재 이유다** — 같은 눈이 세 번 봐도 3인이 안 된다 |

인덱스 3: `(item_id)` · `(verdict, item_id)`(조판기 질의 모양 그대로 — 실측 0.1ms) ·
`(item_id, reviewed_digest)`([20260914120000](../supabase/migrations/20260914120000_csat_item_reviews_digest.sql)).
RLS 켜고 **정책 없음** — 기출 검수표와 같다(서버만 읽고 쓴다).

### 왜 판(digest)을 적는가 — 고쳐도 안 풀리던 판정 (2026-09-14)

세 회차의 3인 검수 통과율이 5/60 · 0/55 · 4/57 로 제자리였다. 재고 품질 탓인 줄 알았는데
아니었다. 차단 사유 1위가 「해설」(17.0% · 117문항)이었고 그 해설은 같은 날 고쳐져
**38,522문항이 다시 쓰였다.** 그런데 그때 내려진 `fail` 은 그대로 남았고, 드레인은 그 판정으로
문항을 ②「고칠 몫」에 넣어 **다시 뽑지 않았다.**

실측 — 후보에서 빠진 163문항 중 **123(75%)이 판정 시점과 해설이 다르다**
(`scripts/textbook/review-staleness.mjs`). 즉 품질이 오르는 동안 후보 풀이 단조롭게 줄었고,
통과율이 **구조적으로 회복될 수 없었다.**

규칙은 하나다 — **지금 판과 같은 판정만 센다.** 다르거나 모르면 pass 도 fail 도 아니고 그 문항은
검수 큐로 돌아간다. 계산의 정본은 `packages/library-pipeline/src/textbook/review-digest.ts` 하나이고,
지금 상태는 `scripts/textbook/review-freshness.mjs` 가 잰다(읽기만 · 재실행 안전).

⚠️ **판정을 지우지 않는다.** 사람(페르소나)이 읽고 내린 기록이라, 무효가 되는 것이 아니라
**다른 판에 대한 기록**이 될 뿐이다. 지우면 「왜 그때 막혔나」를 다시 물을 수 없다.

⚠️ **해설이 판에 들어간다.** 검수자 셋 중 `tutor` 는 「해설이 왜 나머지가 아닌지를 짚나」를
판정 근거로 쓴다 — 해설이 바뀌면 그 사람이 본 것이 바뀐 것이다. 그래서 해설 재생성은 검수를
되돌린다. 비용이지만 **숨은 비용이 아니라 보이는 비용**이라 옳다.

⚠️ jsonb 는 키 순서를 바꿔 저장하므로 **키를 정렬해** 해시한다. 그냥 `JSON.stringify` 로 비교했다가
「내용이 같으면 안 쓴다」가 한 번도 안 지켜져 802행이 불어난 전례가 이 저장소에 있다.
