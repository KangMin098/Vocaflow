...
| `coverage_lexicon` | **409,001** | — | **독해 커버리지 참조 사전 (학습 core와 분리, v06.271)** — 도서·스크립트의 비학습 롱테일 단어 뜻 조회용. kaikki 단일어·content POS·form_of(굴절) 제외 벌크(굴절-of-코어 정리 후 408k) + 추출 누락 실단어 **903(`source='content_residual'`)**. `word`(PK)·`pos`·`gloss_en`(즉시 폴백)·`ipa`·`meaning_ko`(**빈도순 77,501 완주 + residual 903 = 78,404**·미랭크 327k 대기)·`frequency_rank`·`source`(kaikki/content_residual/skip)·`seen_count`. RLS 공개읽기. 마이그 `create_coverage_lexicon`. **학습 로직(단어장·i+1·추천)은 조회 금지** — 오직 reader 폴백(`lookup_word_meaning` tier 6·7 `coverage`/`coverage_en`, 마이그 `lookup_coverage_fallback`). |
