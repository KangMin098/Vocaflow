...
`오늘의 흐름 0/5` 의 5 중 하나(Syntax)는 완료를 **관측할 경로가 없었다** — 채점 RPC 가 쓰던
`csat_item_attempts` 가 삭제됐기 때문이다. 5/5 는 구조적으로 도달 불가였다.
→ 완료를 관측할 수 없는 항목에는 `observable: false` 를 달고 **분모에서 뺀다**.
매일 도달 못 할 목표를 보여주는 것은 진행 표시가 아니라 압박이다(철학 ③).
