...
> **커버리지 규격(한 줄 · 2026-09-20 DD-55)** — 관리자 **파이프라인 화면**은 「가장 앞선 막힌 단계」를 **첫 뷰포트에 한 줄**로 말한다. 색이 아니라 **문장 + 주묵 표식**으로 말하고, 막힌 것이 없으면 그 줄을 **비운다**(없는 것을 색으로 꾸미지 않는다). 액센트 C 「공정 띠」를 대신해 흡수한 규격이다.
...
> 내리기는 DB 기록이고, 화면 반영은 `retire:sync --commit` + manifest 커밋·배포 뒤. 파일 삭제는 터미널 `--purge` 로만(되돌릴 수 없음). 상세: VIDEO_FACTORY §2-4.
...
- 하위 화면 3 (레일 + 사이드바 하위 메뉴):
  - `/admin/csat/evidence` — ① 기출 원천 운영. **운영 현황 · 작업 큐 · 문항 탐색** 3개 뷰와
    URL로 복원되는 문항 검토 패널. 학습 준비는 실제 `/csat/dissect` 후보 판정을 사용하며
    원천 결함 6종과 별도로 보여 준다. 우선순위는 원문·채점 → 분석·연결 → 메타 → 보고서.
    영향 수는 중복될 수 있고 파이프라인 제외 수는 처음 막힌 단계만 센다.
    ...
    선택 대상 내보내기는 문항 ID·조치 안내·미리보기 명령의 JSON이며 재분석 실행이 아니다.
    지금 재검증은 관리자 GET API로 캐시 없이 다시 읽으며 실패하면 판정을 보류한다.
    재분석·대량 수정·발행을 자동 수행하지 않는다. 전체 MD/JSON·함정 계열·유형 보고서는
    기술 상세에 보존한다. 감사 및 설계 근거: `reports/csat-evidence-operations-20260918.md`.
...
- 화면도움말: `lib/admin/help/csat.ts` — `csat`(현황판) · `csat-evidence`(운영 현황·작업 큐·문항 탐색)
