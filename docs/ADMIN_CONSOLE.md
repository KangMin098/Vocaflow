...
## /admin/csat — CSAT (평가원 기출 분석)

관측 전용 화면. 분석·검수는 Claude Code 드레인이 터미널에서 돌리고, 이 화면은 **어디까지 됐고
다음에 무엇을 돌릴지**만 말한다.

**이 화면이 답하는 질문은 하나다 — "이 회차를 지금 풀면 99점이 나오나".**
총점 100 = 듣기 37 + 독해 63이고 배점 단위가 2·3점이므로 **99점 이상 = 독해 실점 0**.
그래서 진행률을 백분율로 그리지 않는다 — 96%는 99점이 아니다. 회차마다 **덮은 배점 /
사정권 배점**을 두 수로 적고, 같을 때만 「가능」이 켜진다.

| 탭 | 보는 것 |
|---|---|
| 회차 커버리지 | 회차별 사정권 문항·분석·검수 통과·덮은 배점·99점 판정. 수능/모의평가 필터 |
| 유형별 진행 | 유형별 문항·검수 통과·**남은 몫**(내림차순)·유형 리포트 유무 |

상단 지표 4: 99점 가능 회차 · 검수 통과 문항 · 검수 기록 · **정답 미상**(평가원 정답표 PDF 가
실제로는 듣기 대본인 회차 7개 = 196문항).

- 데이터: `lib/csat/client.ts` (`csat_coverage()` RPC + 유형 집계)
- 화면도움말: `lib/admin/help/csat.ts` — 드레인 4단계에 **재실행 안전 여부 명시**
- AdminSidebar 등재 (`사용자 & 콘텐츠` → `CSAT Pipeline`)
- 회귀: `app/admin/csat/__tests__/console-render.test.tsx` 7종 (61/63 → 미달 · 63/63 → 가능 경계 고정)
...
| 조판된 권 | `textbook_volume_renders` | 권별 단원·문항·자동 검수·해설 없음·**유형-학년 적합도**·겹치지 않는 권수·**규격 최신 여부**·마지막 조판 + 표 아래 **사다리 병목** 한 줄 |
...
- **"문항 없는 원글"** — 조판이 세는 재고는 원글 전체가 아니라 문항이 붙은 원글이다. 그 격차(실측 12,018편)를 요약 카드 · 표 열 · "먼저 할 일" 한 줄로 낸다. 못 잰 것(null)과 0 을 구별한다.
