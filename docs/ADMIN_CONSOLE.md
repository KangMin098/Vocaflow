...
운영 집계 후속 성능 보강: `csat_source_operations_summary` 한 번의 조회로 14개 큐를 계산한다. 관련 SQL `20260918232551_csat_source_operations_summary.sql`은 2026-09-19 사용자 승인 후 적용했다. 실제 API 3회 모두 200·14개 수치 정상이며 첫 조회 6,037ms, 이후 1,552/1,577ms를 확인했다.

### 원문 운영 정책 v3 (2026-09-18)

최종 판정은 `evaluateSource`. 조판에 항상 강제되며 `VOCAFLOW_SOURCE_STRICT=0`은 오류다.
아래 v2 및 2026-09-06 경고 모드 설명은 과거 기록이며 현재 허용 규칙이 아니다.

상단 수치에서 개별 원문 목록으로 이동한다. 내용·CEFR·품질·발췌·분석 큐는 겹칠 수 있다.
inspector는 본문, 사유, 발췌 후보, 문항, 교재 manifest, 시도 기록과 이력을 보여준다.
과거 manifest가 없으면 노출은 unknown이다. 원문 변경 후에는 재검증까지 사용을 차단한다.
재검증은 파생 캐시만 갱신하며 원문·정답·내용 verdict를 자동 변경하지 않는다.
...
후속 보강(2026-09-19): 오래되거나 없는 판정 캐시는 재검증 큐(`stale`)에서 확인하고, 적격 수치에서 제외한다.
캐시 적재는 원문의 현재 revision과 기존 캐시 측정 시각을 확인하며, 음수·100 초과 구문 점수는 분석 오류로 차단한다.
revision·캐시 측정 시각 검증을 DB에서도 강제하는 `20260918222517_csat_source_cache_freshness.sql`(트리거·운영 view)은 **2026-09-19 사용자 승인 후 적용**했다.
적용 전후 체크포인트를 비교했으며 원문·정답·풀이 기록은 변경하지 않았다.
...
## 원문 관리 작업 공간 (2026-09-18)

`/admin/csat/sources`는 **원천 관리 → 적격 판정 → 코퍼스 분포 → 처리 안내** 네 보기로 구성한다.
기본 보기는 원천 집계 목록이며 검색·문제 필터·정렬과 선택한 원천의 근거 패널을 제공한다.
검토 → 원문 검수하기는 ACP의 `stage=review&status=all&src=<원천>`으로 연결한다.
실패 건은 `status=failed` 링크로 좁힌다. 수정·보관·삭제·일괄 액션은 기존 ACP가 소유한다.

목록의 조치 순서는 실패·법적 제약·발췌 경로·학령 분석·내용 판정·상위 차단 기록이다.
각 집계는 겹칠 수 있고 적격 통과를 뜻하지 않는다. 원천별 적격 통과 수는 미측정으로 표시한다.
재고와 적격 판정의 측정 시각을 각각 표시하며, 등록 시각은 `created_at` 기준이다.
원천에 맞지 않는 배치 명령 자동 처방과 서로 다른 모집단을 빼서 미통과 수로 표시하던 계산은 제거했다.
보충 계획이 유형 재고보다 오래되면 재계산 안내를 표시한다.

화면 도움말 정본은 `lib/admin/help/textbook.ts`의 `csat-sources`이며 네 보기의 라벨과 `tabs` 키가 같다.

코퍼스 분포(2026-09-19)는 전수 재고와 usable 본문 재측정 스냅샷을 서버에서 축약한다. CEFR·주제·저장 형식·공급 의도 학령을 고르면 사용 가능·조건부 발췌를 분리하고 출처 집중도를 계산한다. 주제 필터에서 조건부의 교집합은 미측정이다. 저장 형식의 기본값 오염과 미검증 연령/문항 유형 적합성을 명시한다. 상단도 사용 가능 9,103과 조건부 4,296을 합산하지 않는다(해당 스캔 시점).

원천 선택 → 역할·권리·수집 건강 근거 → 기존 검수 화면으로 이동한다. `source-discovery-profiles.json`은 기존 21종과 외부 후보/기존 통합 개선 18건을 같은 15항목으로 기록한다. 미측정은 null/명시 상태이며 역할 추천이 active를 바꾸지 않는다. 후보 API/라이선스의 범위와 pilot의 미적재 상태도 구분한다. [실측·권리·파일럿 기록](./reports/csat-corpus-discovery-20260919.md).
조사·IA 비교·브라우저 검증: [재설계 기록](./reports/csat-sources-workspace-20260918.md).
...
[ 어휘 ]     (accent: #8B5CF6)
   ...
   = 4 항목
[ 운영 ]     (accent: var(--info))
   사용자 · 플랫폼 분석 · 영상 공장(VFP) · 신고/문의(실 데이터 뱃지) · 결제/구독
   = 5 항목
...
렌더는 헤드리스 크롬으로 프레임을 한 장씩 찍는다 — 186편 · 약 90분 · 177MB.
서버리스 런타임에서 돌릴 수 있는 일이 아니고, 돌릴 수 있게 만들어도 관리자가 기다릴 수 있는
시간이 아니다. 그래서 이 저장소의 다른 드레인과 같은 모양을 쓴다: **화면은 밀린 것을 보여 주고,
실행할 명령을 그대로 건넨다**(복사 버튼 포함).
...
|---|---|
| ① 구성요소 (커밋된 원천) | — |
| ② manifest (커밋됨) | ①에 있고 ②에 없다 → **안 만듦**. 플랫폼은 자랐는데 영상이 안 따라왔다 |
| ③ `storage.objects` (실제 파일) | ②에 있고 ③에 없다 → **유실**. 기록은 있는데 파일이 없다 = 화면에서 깨진다 |
| | ②에 있고 ①에 없다 → **고아**. 구성요소가 사라졌다 |

**③이 가장 중요하다.** ①②만 보면 "발행했다는 기록"만 보는 것이고, 그 기록은 파일이 지워져도
그대로 남는다.
...
| 탭 | 답하는 질문 |
|---|---|
| 현황 | 지금 볼 것이 있는가 (없으면 경보 칸을 펴지 않는다) |
| 구성요소 | 무엇이 밀렸고 **어떤 영상이 실제로 보이는가**(편별 재생 수) |
| 수치 낡음 | 영상에 박힌 수가 **지금과 얼마나 다른가** |
| 내보내기 | 무슨 명령을 돌리는가 · YouTube 업로드(사람이 해야 하는 단계 포함) |

### 수치 낡음 — 임계값을 두지 않는다

영상은 **찍은 날의 스냅샷**이다. DB 는 계속 자라므로 화면에 박힌 수는 반드시 묵는다.
"20% 넘으면 경고" 같은 수를 근거 없이 정하면 그건 판단이 아니라 짐작이라, 대신 **실제 차이를
크기순**으로 낸다 — 다시 찍을지는 사람이 정한다.

다시 셀 수 있는 것만 비교한다(사전 · 문항 · 도서 · 챕터 퀴즈 · 유형별 재고).
논문 인용이나 계산식은 건너뛴다 — 억지로 0 으로 만들지 않는다.
**실측 2026-09-13: 수치 근거 39건 중 27건이 대조된다.** 이 수가 0 이 되면 판정이 죽은 것이고,
회귀 `video-drift.integration.test.ts` 가 그것을 잡는다(조용한 0 을 막는다).

### 새 테이블이 없다

manifest(파일) · `storage.objects` · `funnel_events` 셋으로 전부 답한다.
구성요소 분모는 커밋된 원천(`lib/video/components.ts`)에서 직접 센다 — 공장의 번들은
커밋하지 않아 운영 환경에 없기 때문이다.
...
| 4 | 소재 | 지문 섭외·저작권 | **지문으로 채우는** 밴드에 지문이 있는가 | `csat_source_snapshots` (6h 크론 · `csat_source_rollup()`) |
...
- 하위 화면 3 (레일 + 사이드바 하위 메뉴):
  - `/admin/csat/evidence` — ① 기출 원천. **탭 0 · 축 8**(회차 · 유형 · 함정 · 절차 단계 ·
    어휘 수 · 난이도 사정권 · 연도 · 결함). 행 축 · 열 축을 고르면 같은 802문항이 그 교차로
    다시 접히고(8×8 = 64 조합), 피벗 가장자리의 **총합은 어느 조합에서도 802** 여야 한다 —
    회귀가 64 조합을 전부 돌려 확인한다. 맨 윗줄은 「내보내도 되나」에 답하는 한 줄
    (문항 × 필드 커버리지 매트릭스 요약)이고, 그 아래 결함 6종 칩마다 **막는 하류 공정**이
    붙는다. 문항 전문은 목록에 펼치지 않고 오른쪽 오버레이(drawer)에서만 연다.
    조건은 URL 에 실려 링크로 넘어간다(`?defect=body&type=R-BLANK&row=defect&col=type`).
    축이 고른 것에 따라 **패널 하나**가 붙는다(피벗이 구조상 못 말하는 것만) — 함정은 라벨
    518종을 접은 **계열 280개** 트리, 결함은 **고치는 순서**(막는 가장 이른 공정 + 다음 한 걸음),
    유형은 리포트의 계수 정합·배포 가능(문항 줄에 없는 값이라 집계로 안 나온다).
    ⚠️ 옛 4탭(회차 커버리지 · 유형별 진행 · 문항 분석 · 가이드 원천)은 **폐지**했다 —
    같은 문항 집합을 축 하나씩 잘라 둔 표라 두 축을 겹칠 자리가 구조적으로 없었다
  ...
  - `/admin/csat/sourcing` — ④ 소재. 단계 밴드별 **조판 풀** 재고 + 소스 타겟 + 등록부 대조.
    값은 6시간 크론이 떠 두는 스냅샷에서 읽고(`csat_source_rollup()` · 실측 1.4~2.6초),
    화면은 **언제 잰 값인지**와 직전 대비 증감을 함께 낸다. 조작 1개 — **「지금 다시 잰다」**
    (크론과 같은 RPC `csat_source_snapshot_take` · 재실행 안전 · 드레인 직후에 누른다)
    - ⚠️ **2026-09-15 에 분모를 고쳤다.** 여기 있던 「지문 재고 562편 · S5 0편」은 둘 다 틀렸다:
      `csat_stage_catalog` 는 뷰이고 양쪽 갈래가 `status='published'` 라 562는 **출고분**이었고
      (실측 조판 풀 87,556편 = 그 156배), S5 의 합격선은 `listening` 하나뿐이라 **지문으로는
      영영 안 차는 칸**을 「재료 없음」으로 세고 있었다. 지금은 밴드 판정을 게이트의 metric 에
      맡긴다(`source-rollup.ts` §6) — 목록이 아니라 규칙이라 게이트가 바뀌면 그날 같이 바뀐다
...
- 화면도움말: `lib/admin/help/csat.ts` — `csat`(현황판) · `csat-evidence`(탭 없음 — 화면 단일 항목)
...
### 계기 넷 — 「자격」 다음에 오는 질문들 (2026-09-08 ~ 09-13)

적격 판정은 **「이 원문을 써도 되는가」**까지만 답한다. 그 답을 다 받고도 교재가 안 나오는
동안, 같은 화면에 네 질문이 차례로 붙었다. 각각 **스캔 하나 · 스냅샷 하나 · 표 하나**다.

| 표 | 답하는 질문 | 계기 | 스냅샷 |
|---|---|---|---|
| 유형 재고 | 시중 구성 그대로 **몇 권**까지 가는가 | `type-inventory-scan.mjs` | `type-inventory-snapshot.json` |
| 채울 몫 | **그래서 무엇부터** 쓰는가 | `item-fill-plan.mjs` | `item-fill-plan.json` |
| 쓰고도 못 싣는 것 | 이미 쓴 것 중 **지금 게이트에 막힌** 것 | `item-drain-audit.mjs` | `item-drain-audit-snapshot.json` |
| 원천 생존율 | **어느 원천을 더 수확**할 것인가 | `source-yield-scan.mjs` | `source-yield-snapshot.json` |

네 계기 모두 **읽기만 한다 — 재실행 안전**이고, 규격을 새로 정하지 않는다
(`rungMix` · `itemWordSpec` · `isPrintablePassage` · `checkDrainItem` 한 벌을 그대로 부른다).
사본을 두면 표가 조판과 다른 말을 하는 날이 온다.

**이 넷이 각각 무엇을 잡아냈는가** — 표가 없었으면 안 보였을 것들이다:

- **권수는 가장 얇은 유형이 정한다.** 결정론 유형은 이미 수천 권 분량인데(`order` 6,466권 ·
  `grammar_fix` 4,852권) 생성형 유형이 밴드당 0~2권이라 **22만 개 쌓인 `blank_word` 가 권수를
  1도 못 올린다.** 「재고가 많다」와 「권이 나온다」는 다른 말이다.
- **「재고가 있다」로 세면 안 된다.** 유형당 26개도 「있다」가 되지만 한 권이 120문항이고
  목표 14% 면 권당 17개가 필요하다 — 한 권 쓰면 바닥이다.
- **계기 자신이 틀릴 수 있다.** 재고 스캔이 V1 을 「0권」이라 말한 적이 있다 — 초등 3종은
  저장되지 않고 조판 때 사전에서 즉석 생성되는데 저장된 문항만 세고 있었다(실제 11권).
  감사는 「해설만 고치면 되는 것」을 두 배 넘게 부풀려 말했다 — 게이트가 **첫 실패에서
  반환**하므로 「사유가 해설」이 「해설이 유일한 관문」은 아니었다(142 → 실제 17).
- **편수는 공급이 아니다.** 같은 밴드·같은 규격인데 **원천이 다르면 생존율이 스무 배** 갈린다 —
  PLOS/Europe PMC 초록 **5.6%** vs Gutenberg 고전 산문 **100%**. 사인은 논문 서식과 어수 상한
  초과 둘뿐이고 지문을 고쳐서는 못 푼다. 그래서 「9만 편 있다」가 상위 밴드 독해에는 참이 아니다.

⚠️ **안 잰 것을 0 으로 칠하지 않는다.** 네 표 모두 스냅샷이 없거나 비면 `null` 을 돌려
아예 안 보인다 — 0% 로 칠하면 화면이 「전부 못 쓴다」고 말하게 된다. 회귀가 그 성질을 잠근다.
