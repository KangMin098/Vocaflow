...
## 8. 건너뛴 스킬을 뒤늦게 돌렸다 — 위반 넷이 나왔다

요청에 「frontend-design / claude design 스킬 사용」이 있었고 CLAUDE.md 도 「UI 작업 전
`vocaflow-design` 을 먼저 읽는다」고 못 박는데 **안 읽고 구현했다.** 뒤늦게 돌리니
`vocaflow-design` §1 라우팅이 Admin 화면은 `minimalist-ui`, 차트·통계는 내장 `dataviz` 를
부르라고 명시하고 있었다. `dataviz` 를 적용해 나온 것:

| # | 위반 | 근거 | 조치 |
|---|---|---|---|
| 1 | **상태 색을 범주 색으로 썼다** | 검토 대기는 재고의 80%인 정상 상태인데 `warning-ink` 로 칠해 없는 경보를 만들었다 | 단일 색조 순차 램프(t2→t1→t4)로 교체 |
| 2 | **정상 시야에서 구분 불가** | 검증기 실측 `#1F6B49`↔`#7A5200` ΔE **12.7** (기준 15) — 하드 FAIL | 〃 (명도로 구분이 보장된다) |
| 3 | **보이지 않는 마크** | 발행 세그먼트가 21원천 중 **11곳에서 1px 미만**, 9곳은 0px | 0 이 아니면 최소 2px. 0 은 0 으로 둔다 |
| 4 | **범례 없음 · 세그먼트 틈 없음 · 상태색 단독** | 2계열 이상은 범례 필수 · 인접 칸 2px 틈 · 상태색은 아이콘+라벨과 함께 | 범례 한 줄 · `gap-[2px]` · 빨간 0 을 「⚠ 0 판정 없음」으로 |

⚠️ **범주 3색으로 고치지 않은 이유**: 검증을 통과하는 3색 조합이 이 시스템 토큰 밖에 있었다
(Reading Room 팔레트가 의도적으로 저채도다). 색을 새로 만드는 대신 **인코딩을 바꿨다** —
세 상태는 실은 파이프라인 순서(그 외 → 대기 → 발행)라 순차 램프가 애초에 맞는 형식이었다.

재검증: axe 위반 0 유지 · 접힌 위 310/412/827px 유지 · 정보 누락 0 유지.
