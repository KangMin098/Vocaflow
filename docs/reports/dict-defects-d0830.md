...
# T6-3 실행 기록 — 예문 교체 (2026-08-30)

파이프라인 `scripts/dict/w0830-exfix.mjs` · 청크 `scripts/dict/w0830-exfix/`
`example_en` 을 덮는 **유일한 배치**다.

## 사본 어긋남을 코드로 막았다

T5(exko)가 `example_en` 을 `meanings_ko[0].example` 로 복사하고 그 문장의 해석을 달아 놨다.
여기서 `example_en` 만 바꾸면 **뜻 단위 사본은 옛 문장을 그대로 들고, 그 해석은 이제 존재하지 않는
문장을 옮긴 것이 된다.** 화면에는 둘 다 나온다.

apply 는 **같은 문자열이 있는 모든 자리를 함께 바꾼다.** 한 자리라도 해석 없이 남으면
그 낱말을 통째로 거부한다(`ko_missing`) — 반쪽만 바꾸느니 안 바꾸는 게 낫다.
실측 `ko_missing` **0**.

## ⚠️ 게이트가 좋은 예문을 버리고 있었다 (내 결함)

`containsWord` 매처를 senseex 에서 베껴 올 때 **불규칙 굴절표를 안 가져왔다.**
그래서 `wring` 의 `wrung` · `befall` 의 `befell` · `rend` 의 `rent` 를 쓴 예문이
"표제어가 없다" 로 **조용히 버려졌다.**

서브에이전트 **셋이 각자 이 함정을 만나** 원형이 드러나는 문장으로 다시 썼다("파이프라인 차원의
확인이 필요하다" 고 보고까지 했다). 표를 `scripts/dict/_irregular.mjs` 로 빼서 공유하도록 고쳤고,
다시 돌리니 `no_headword` **5 → 0**, 30 낱말이 추가로 살아났다.

**교훈**: senseex 는 배치 도중 이 표를 **세 차례** 보강했다(`chose`·`strode`·`undertook`…).
그 학습이 표 안에만 남고 매처를 베껴 간 곳에는 안 따라왔다. **같은 표를 두 곳에 두면
한 곳만 고쳐지고 다른 곳은 계속 버린다.**
⚠️ `w0830-senseex.mjs` 는 아직 자기 사본을 쓴다 — 다음에 손댈 때 통합할 것.

## 실측

| | |
|---|--:|
| 대상 | 1,454 낱말 / 97 청크 (시험 밴드 704) |
| 적재한 낱말 | **1,409** |
| 예문 교체 | `example_en` **123** · 뜻 단위 **1,853** |
| 사본 동기화 | 5 |
| 게이트 거부 | `ko_missing` 0 · `non_ascii` 0 · `too_long` 0 · `no_headword` 0(수정 후) |

## 기계 신호가 못 잡은 것들 — 사람이 읽어야 나온 갈래

기계는 `끝맺음 부호 없음`·`소문자 시작`·`4낱말 미만`만 센다. 서브에이전트들이 읽고 잡은 것:

| 갈래 | 실례 |
|---|---|
| **뜻과 예문이 서로 뒤바뀜** | `sack` 뜻0(자루)에 '해고' 예문, 뜻3(해고)에 '약탈' 예문 · `canon` 뜻3 이 뜻4 의 예문 보유(뜻4는 빈칸) · `scrub`·`compact`·`spit`·`stag`·`purl` — **여섯 낱말 이상에서 같은 모양** |
| **사실이 틀림** | `distillation` — "증류에 여러 해가 걸린다"(오래 걸리는 건 숙성) · `slapdash` — 아르놀피니 초상 서명은 공들여 쓴 장식체 · `obverse` — 같은 조건을 정반대로 서술 |
| **비문** | `the interior of the house **are**` · `The police **is** going to rumble` · `Do you have **a gum**?` · `today evening` · `rended`(→rent) |
| **마침표가 있어 통과하던 조각** | `The Protestant work ethic.` · 주절 없는 관계절 인용 · 대시로 표제어를 푼 사전 항목 |
| **난이도 역전** | v2 `salt` 에 `seawater infrastructure` · v1 `season` 에 `invasive species reproduces` · `currency` 에 `Wampum/Amerindians` |
| **학습자 화면에 부적절** | 나치 수용소(`impersonal`) · 참수(`sever`·`lop`) · 마약(`hooked`·`overdose`) · 성적 내용(`solicit`·`erect`·`fetish`) — Calm UI 원칙에 따라 **뜻은 살리고 문맥만** 중립화 |

## 예문 교체로는 못 고치는 것 — 표제어 자체가 틀렸다

서브에이전트들이 **일부러 예문을 내지 않고** 넘긴 자리들이다. 옳은 판단이다 —
예문을 만들면 **틀린 표제어를 문장째 학습자에게 내보내게 된다.**

| 표제어 | 무엇이 문제인가 |
|---|---|
| `re` | 접두사 `re-` 를 부사 표제어로 올렸다. 예문 `Please re send the document…` 은 영어가 아니다 |
| `epicopal` | `episcopal` 오철자 |
| `clich` | `cliché` 오철자 |
| `ville` · `bona` · `por` | 독립 낱말이 아니다 (지명 접미사 · `bona fide` 조각 · 영어 아님) |
| `marched` · `finisher`(`finsher`) | 굴절형·오철자가 표제어로 올라 있다 |

**다음 배치(T7) 의 대상**은 이것이다 — 예문이 아니라 **표제어 정리**.

## 곁가지로 확인한 것

- 보이지 않는 유니코드(U+2060 등)를 **뜻 단위 예문까지** 전수 조사했다(앞서 `example_en` 만 봤다).
  사전 전체에서 **1자리**(`manure`)뿐이라 배치를 만들지 않고 그 자리에서 제거했다.
- 굽은 따옴표(U+2019·U+201C/D)는 `ASCII_OK` 가 허용하고 있어 기계가 못 잡는다.
  서브에이전트들이 발견한 자리만 곧게 폈다 — 전수 정리는 별도 건이다.

## T6-3 마무리 — "두라" 도 판정이다

apply 가 `nothing`(고칠 것 없음) 45건에 도장을 안 찍고 있었다. 서브에이전트가 읽고
**"멀쩡하니 두라"** 고 판정한 것인데, 도장이 없으면 다음 chunk 가 그 낱말을 영원히 다시 묻는다.
T6-2 에서 같은 구멍을 고쳤는데 여기 또 있었다.

다만 **제안을 냈는데 게이트가 전부 버린 경우**는 판정이 아니다 — 결함이 그대로 남은 것이므로
도장을 찍지 않고 다음 회차가 다시 묻게 둔다. 그래서 도장은 **제안이 아예 없을 때만** 찍는다.

실측: `replaced:d0830-t6` **1,409** + `kept:d0830-t6` **45** = 대상 **1,454 전량**.

---

# T6 전체 마무리 (2026-08-30)

| 배치 | 대상 | 적재 | 되돌리기 근거 |
|---|--:|--:|---|
| T6-1 동음이의어 오염 | 6,456 | 117 항목 제거 | `t6_removed` |
| T6-1b 대표 뜻 연어 | 93 | 91 낱말 / 269 연어 | `t6_repcolloc_added` |
| T6-2 뜻 갈래 정리 | 1,317 | 580 낱말 재구성 | `t6_senses_before` (원본 배열 전체) |
| T6-3 예문 교체 | 1,454 | 1,409 낱말 / 1,976 자리 | `t6_example_before` |

사전 최종: 낱말 48,969 · 뜻 72,969 · 뜻별 예문 **72,124** · 예문 해석 **72,129**

## 이 회차에서 배운 것 — 배치가 자기 결함을 네 번 잡았다

전부 **실측으로 확인하고 그 자리에서 고쳤다.** 넷 다 같은 뿌리다 —
**"안전하다고 믿은 규칙이 다른 방향으로 새고 있었다."**

| # | 무엇이 샜나 | 어떻게 드러났나 |
|---|---|---|
| 1 | 병합 시 합치는 쪽이 비면 예문이 그냥 사라짐 | 드라이런에서 예문 492 → 412. 재보니 순수 손실은 1건, 나머지는 정상 |
| 2 | 모집단 오탐 — 품사 구멍 667행 중 **533행(80%)** 이 표기 차이 | 서브에이전트 **세 팀이 각자** "고칠 게 없다" 고 보고 |
| 3 | 매처에 불규칙표가 빠져 `wrung`·`befell` 예문이 조용히 버려짐 | 서브에이전트 **셋이 각자** 함정을 만나 문장을 다시 씀 |
| 4 | 큐·도장이 재실행 시 빈 채로 덮이거나 안 찍힘 | FLAGGED 가 184 → 83 으로 덮이는 것을 목격 |

**공통 교훈**: 게이트는 "무엇을 막을지" 만큼 **"막은 것이 어디로 가는지"** 를 세야 한다.
`example_lost: 0` 은 맞았지만 예문은 병합 경로로 새고 있었고, `no_headword` 는 정직하게
세고 있었지만 **그 수치를 결함이 아니라 정상으로 읽고 있었다.**

## 남은 것 — T7 (표제어 정리)

예문·갈래·연어로는 못 고치는 자리다. 서브에이전트들이 **일부러 손대지 않고** 넘겼다.

| 갈래 | 실례 |
|---|---|
| 접두사·조각을 표제어로 올림 | `re` · `ville` · `bona` · `panky` · `jiggery` · `pocus` · `pourri` · `nilly` |
| 오철자 표제어 | `epicopal`(episcopal) · `clich`(cliché) · `exongenous` · `hypothesi` · `fillete` · `finsher` |
| 굴절형이 표제어 | `marched` · `slagged` · `cottoned` · `limed` · `beefed` |
| 영어가 아님 | `por` · `acte` · `mes` · `artis` · `accademia` |
| OCR 오독 | `eduldamer` · `peles` |
| **중립 예문이 존재할 수 없음** | `klux` — 낱말이 아니라 인종차별 단체명 조각 |
