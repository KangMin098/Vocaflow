...
| 결함 | 실측 | 결과 |
|---|---|---|
| **`lib-topic.mjs` 오분류** ✅**해소 2026-09-07** | 정찰 20편 → 손판독 **144편**(소스 15곳)으로 다시 쟀다: 오분류 **23.6%**. 원인은 정규식 오른쪽 경계 없음(`art`→article·artery) · 낱말 반복 무한 득점 · 동점이 조용히 첫 칸으로. 수정 후 **8.3%** | **빈 칸을 재는 자가 고장이었다.** 고친 뒤 「8칸 전부 부족 0」이 **부족 3,010편(병목 교육·언어)** 로 바뀌었다 — §8-1 참조 |
...
1. ~~**`lib-topic.mjs` 분류기 수정**~~ — ✅ **2026-09-07 완료.** 오분류 23.6% → **8.3%**
   (손판독 144편 · 회귀 48종 · 변이 6종 전부 잡힘). 그 결과 **우선순위가 실제로 바뀌었다**:
   기존 「8칸 전부 부족 0」 → 3단계(5만) 기준 **부족 3,010편**, 병목은 **교육·언어 1,464**.
   균형 사정권 50,001 → **37,234**(표본 5,000 · 95% 구간 33,069~41,399).
   ⚠️ 기존 라벨 6.9만 행은 **옛 자로 잰 것**이다(표본 6,000 중 37.8%가 바뀐다).
   전량 재분류는 별도 결정 — `node scripts/csat/backfill-topic.mjs --commit`.
   정확도는 이제 상설로 잰다: `node scripts/csat/topic-accuracy.mjs --score`
