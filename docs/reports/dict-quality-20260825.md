...
큐를 가득 채운 베트남어(cua · nhung · khong · duoc …)의 출처를 **문서 단위**로 좁히니
소스가 아니라 **1,935편 중 4행**이었다 — 자막이 통째로 비영어인 TED talk 3편(1편은 중복 적재):

| 문서 | unique | gap | gap율 |
|---|--:|--:|--:|
| Is Taste a Blessing Or a Curse (ESG) | 710 | 654 | **92.1%** |
| Una teoría para acabar con las burbujas… | 694 | 504 | **72.6%** |
| The island itself | TEDxHANU (×2) | 369 | 228 | **61.8%** |

(초판에 "카탈로그가 번역 제목·설명을 담기 때문"이라고 적었으나 **메타데이터가 아니라 자막 자체**였다.)
조치는 **적재 쪽 언어 게이트** — 2026-08-25 마이그레이션 `20260825114639` 로 넣었다.
...
#### 2단계 (큐 처분) — 규칙 기각을 하려다 멈춘 이유

계획은 "하이픈 3,088행 규칙 기각" 이었다. `triage.ts` 가 그 갈래를 **"부분이 이미 해석됨 —
등재 불필요"** 로 규정하고 3순위에 두고 있었기 때문이다. 실행 직전에 확인해 보니 그 전제가 틀렸다:

```
resolve_dict_headword('well-being')      → NULL
resolve_dict_headword('decision-making') → NULL
resolve_dict_headword('face-to-face')    → NULL
```

**해석기는 하이픈 토큰을 분해하지 않는다.** 부분이 해석되는 것과 전체형이 해석되는 것은 별개이고,
학습자가 보는 것은 전체형이다. 기각했다면 **실낱말 2,473개(8,124회)** 를 "무시해도 되는 것" 으로
확정할 뻔했다.

| 하이픈 3,238 | 수 | 처분 |
|---|--:|---|
| 접두사 파생 (non-·anti-·self-) | 615 | **보류** — 뜻이 뒤집혀 해석기로도 못 풀고 등재도 판단이 필요 |
| 진짜 합성어 | 2,473 | 상위 **53낱말 등재** (나머지는 드레인 대상) |
| 머리 낱말도 미해석 | 150 | 보류 — 고유명사·OCR 파편 혼재 |

| | |
|---|---|
| 등재 | 하이픈 합성어 53 (well-being · decision-making · face-to-face · meta-analysis · light-year …) |
| 고유명사 | 339행 → `proper_noun_forms` 등재 후 기각. 적재 함수가 이 표를 보므로 **다시 큐에 안 들어온다** |
| triage 정정 | 라벨 '하이픈 노이즈'→'하이픈 합성' · 우선순위 3→1 · 조치 문구 교체 (화면도움말 동반) |
