...
### 파일럿 실측 (2026-08-26 · 청크 3개 동시)

세 서브에이전트를 한 번에 띄워 180낱말을 판정했다. 각 에이전트가 **적재기의 검증 로직을
직접 읽어 스스로 대조**했고, 실제 import DRY-RUN 에서도 결과가 같았다:

```
files 4 · add 105 · defer 53 · proper_noun 27 · noise 55 · 검증 탈락 0
적재 완료 — 사전 +86 · proper_noun_forms +27 · noise_blacklist +55
큐 정리 — added 86 · rejected 52 · 보류(reviewing) 42
```

에이전트가 스스로 잡아낸 경계 사례가 이 작업의 값어치를 보여 준다:

- **굴절형은 `add` 가 아니다** — `headsets`·`thinkers`·`suppliers`. 기본형이 표제어이고
  굴절형을 넣으면 틀린 형태가 사전에 영구히 남는다. 한 청크 `defer` 16건의 최대 원인.
- **무한히 생산되는 패턴** — `six-year-old`·`third-generation`. 하나를 표제어로 삼으면
  그 계열 전체를 불러들인다.
- **레지스터가 없는 낱말** — `heck`. `standard` 를 붙이면 학습자 단어장 후보가 된다.

이 셋은 규칙으로 못 가르는 것들이고, **파일럿에서 배운 것을 에이전트 사양에 다시 적었다**
(legal `word_register` 8종 명시 · 굴절형 절.)
...
14,577건 소진 ≈ **5.1시간** · 에이전트 실행 **약 243회**.
⚠️ 실행당 서브에이전트 토큰이 7~8만이라 **전량 소진은 1,800만 토큰 규모**다.
한 번에 다 돌릴 일이 아니라 물결로 나눠 도는 것이 맞다(`--wave-size`).
