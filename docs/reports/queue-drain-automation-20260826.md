...
**자동화의 병목은 LLM 이 아니라 두 가지다** — ① `ANTHROPIC_API_KEY` 가 없어서 이미 만들어 둔
자동 경로(Batch API · `--sdk` 폐루프)가 못 돈다, ② **LLM 이 전혀 필요 없는 큐가 가장 크게
밀려 있는데**(85,179건) 그걸 도는 것이 브라우저 탭 하나다.

그리고 그 가장 큰 큐는 **자동화보다 먼저 걷어내야 한다** — 대기분의 **88.6% 가 수확 불가**다.
...
→ 이 다섯은 **키만 생기면 사람 없이 끝까지 돈다.** 전부 적재기에 값 검증이 박혀 있어
쓰레기가 DB 에 들어가지 않는다. 자동화 위험이 가장 낮은 묶음이다.
...
**즉 40시간을 돌려 88.6% 를 버린다.** 자동화가 이 낭비를 빠르게 만들 뿐이다.

**먼저 할 일**: 적재(enqueue) 시점에 자막 유무를 걸러라. 지금은 수확기가 페이지를 받아 본
뒤에야 안다. 그 판정을 앞당기면 큐가 **9,750건 수준**으로 줄고, 그때 자동화는 4~5시간짜리가 된다.
...
| 순위 | 무엇 | 근거 | 선행조건 |
|---|---|---|---|
| **P0** | `topic_corpus_queue` **적재 필터** (자막 없는 항목 제외) | 88.6% 를 버리는 큐를 자동화하면 낭비가 자동화된다 | 없음 |
| **P1** | 그 뒤 **헤드리스 수확 워커** | 순수 기계 · 재실행 안전 · 지금은 브라우저 탭이 필요 | P0 |
| **P2** | **교재 기계 단계 야간 잡** (health · series · verify · render) | 전부 읽기 전용 또는 멱등 | 없음 |
| **P3** | `ANTHROPIC_API_KEY` 확보 → **B군 5종 자동화** | 코드가 이미 있다. 적재기 검증도 이미 있다 | 키 + 예산 결정 |
| **P4** | `pending_words` 자동 판정 + `add` 표본 검수 | 8,962 대기 · 사전에 쓰므로 게이트 필요 | P3 |
