...
### ⚠️ 낭비의 진짜 원인은 드레인이 아니라 **적재 방식**이었다

무인 드레인을 몇 시간 돌린 뒤 **소스별로** 다시 쟀다:

| 소스 | 적재 방식 | done | skipped | 대기 | 수확률 |
|---|---|---:|---:|---:|---:|
| `ted:catalog` | **카탈로그 대량 투입** | 2,622 | 53,582 | 40,780 | **4.7%** |
| `ted:sleep` | 주제 페이지 발견(`discover`) | 15 | 0 | 1 | **100%** |

**큐가 나쁜 것이 아니라 카탈로그 대량 투입이 나빴다.** 주제 발견 경로는 TED 가
주제 페이지에 **내세우는**(= 대개 자막이 있는) 강연만 주는데, 카탈로그 덤프는
자막 없는 것까지 전부 집어넣는다.

⚠️ `ted:sleep` 은 **표본 15건**이라 100% 를 그대로 믿을 수는 없다. 다만 4.7% 와의
차이는 표본으로 설명되지 않고, 기전(주제 페이지 = 큐레이션된 목록)도 설명이 된다.
TEDx 여부는 원인이 아니었다 — TEDx 2.9% · 그 밖 5.0% 로 **둘 다 낮다**.

**그래서 남길 규칙**: 주제 코퍼스는 **`discover` 로 주제별로 적재**한다.
카탈로그 전량 투입은 40,780건을 넣고 그중 약 1,900편만 건지는 일이다
(≈22시간). 지금 큐는 이미 들어와 있으므로 **무인으로 끝까지 돌리되**,
**다시 그렇게 채우지는 않는다.**
...
### ⚠️ 팬아웃에 claim 이 없었다 — 3차 물결에서 드러났다

두 에이전트가 **"내 청크에 이미 다른 판정본이 있었다"** 고 보고했다. 워크스페이스를
여러 세션이 공유하는데, 청크 폴더에는 TCP 큐의 `FOR UPDATE SKIP LOCKED` 같은 장치가 없었다.

DB 는 안전했다 — 적재기가 이미 있는 표제어를 건너뛴다("이미 등재 296건 건너뜀").
**낭비된 것은 판정 노동이고, 위험한 것은 서로 다른 판정본이 덮어쓰는 경합이다.**

→ 오케스트레이터에 **claim 파일**을 넣었다. 스폰 전에 `chunk-NN.claim` 을 찍고,
   신선한 claim 이 있는 청크는 건너뛴다. 30분이 지난 claim 은 죽은 것으로 보고 가져간다
   (`compose` 큐가 쓰는 회수 시간과 같은 값). `.claim` 은 `.gitignore` 로 커밋에서 뺀다.

**교훈**: 이 저장소는 DB 큐에는 동시성 장치를 잘 걸어 뒀는데(`SKIP LOCKED` · 30분 회수),
**파일로 도는 드레인에는 그게 없다.** 청크 폴더를 쓰는 다른 드레인(교재 해설 · VCB)도
같은 구멍을 갖고 있다.

### 속도 — 유입은 **TCP 를 얼마나 세게 도느냐**에 달려 있다

처음엔 "팬아웃 51낱말/분 vs 유입 4낱말/분 → 순배출 47" 이라고 적었다. **그 4는 내 드레인
한 대 기준이었다.** 워크스페이스에 여러 세션이 함께 돌면서 TCP 처리 속도가 뛰자 유입도 같이 뛰었다:

| 물결 | `topic_corpus_queue` | `pending_words` | 우리가 판정 |
|---|---:|---:|---:|
| 1차(3청크) 전 | 85,179 | 8,962 | — |
| 2차(6청크) 후 | 40,411 | 14,332 | 600낱말 |
| 3차(8청크) 후 | **23,706** | **16,208** | 480낱말 |

3차 구간 실측: TCP **14,530편**을 처리하는 동안 `pending_words` 가 **2,236건 늘었다**
(우리가 360건을 빼낸 뒤에도). → **편당 0.154 낱말**.

**그래서 지금은 팬아웃이 유입을 못 이긴다.** 다만 이건 일시적이다 —
TCP 는 유한하고(잔여 23,706), 마르면 유입이 **0 이 된다**.

**투영**: TCP 가 다 마르면 `pending_words` 총량 ≈ **19,856건 (약 331청크)**.
그때부터 팬아웃만 돌리면 순배출 전량이 감소로 잡힌다.

⚠️ 실행당 서브에이전트 토큰이 5~8만이라 **331청크 전량은 2,000만 토큰 규모**다.
한 번에 다 돌릴 일이 아니라 물결로 나눠 돈다.

⚠️ **"재작업이 없다" 는 앞 문장은 과했다.** 판정된 낱말은 영구히 큐에서 빠지므로
실제 재작업은 없다 — 큐 숫자가 안 줄어 보일 뿐이다. TCP 를 먼저 마르게 하면
**보고가 읽기 쉬워지는** 것이지 일이 줄지는 않는다.

### ⚠️ 큐가 나에게 **판단이 필요 없는 일**을 주고 있는가 (6물결 2,520낱말 실측)

가설: 굴절형·하이픈 조어가 `defer` 의 대부분이니, 규칙으로 걸러 LLM 을 아끼자.
**반은 틀렸다.** 낱말 모양별 `add` 율을 재 보면:
...
하이픈은 통념과 달리 **일반어와 같은 수율**이었다(드레인 스크립트 머리말도 같은 말을 한다 —
"하이픈은 등재 불필요 → 틀렸다"). 규칙으로 버렸으면 `anti-slavery`·`cost-of-living`·
`family-friendly` 같은 표제어 247개를 잃었을 것이다.

남는 후보는 복수형 하나다. 계측기(`scripts/dict/plural-lane-eval.mjs`)로
**규칙 판정과 이미 내려진 사람 판정을 대조**했다 — 어간이 이미 표제어면 굴절형으로 본다:

```
rule:굴절형  총 29 · add 0 (0.0%)  → 선필터를 켜도 표제어 손실 0
rule:모름    총 220 · add 23 (10.5%)
```

**손실 0 이지만 이득도 작다** — 큐 17,688건 중 이 규칙에 걸리는 것은 299건(1.7%, 약 5청크).
쓸 만하되 "큰 절감" 으로 말하면 거짓이다.

### 더 큰 것은 상류에 있다 — **판정의 18%가 애초에 영어가 아니었다**

`noise` 348건의 내역(2,520낱말 판정 기준):

| 범주 | 건수 | 무엇인가 |
|---|---:|---|
| `foreign_word` | 254 | 베트남어·터키어 음절, 학명 종소명 |
| `corrupt_token` | 116 | LaTeX(`leq`·`mathrm`) · HTML 엔티티(`hellip`) · URL·태그 조각(`nasahubble`·`alexanderpalace`) · 웹 UI 문구 접합(`downloadshare`·`articlereuse`) · 낱말 접합(`photonslight`·`takenmay`) |
| `interjection_noise` | 13 | |
| `proper_noun_marker` | 5 | |

한 에이전트가 `homeanimal`·`homechild`·`homefood`·`homehunger`·`homemigration` 7종을
**같은 패턴**으로 짚었다 — 메뉴어 "Home" 이 다음 낱말에 붙은 것이다. 즉 개별 낱말의
판단 문제가 아니라 **토크나이저·본문 추출 결함**이다.

큐 잔량에 투영하면 **약 3,200낱말**이 "영어가 아니라서 버릴 것" 이고, 지금은 그것을
낱말 하나하나 LLM 이 읽어서 버리고 있다. 복수형 선필터(299건)의 **10배**다.

**그러나 여기서 멈춘다** — 원인이 수집기 어느 지점인지(자막 언어 필터 부재인지, 본문
추출의 chrome 제거 실패인지) 확인하지 않았다. 확인 없이 필터를 넣으면 실단어를 버린다.
다음 작업의 첫 단계는 `ted-transcript.ts`·`harvest.ts` 의 토큰화 경로를 읽고,
접합 토큰이 **어느 단계에서 생기는지**를 재현하는 것이다.
