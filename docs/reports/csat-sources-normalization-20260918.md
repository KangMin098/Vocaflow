...
**배포 경계:** SQL 4개·최종 코드·DB·브라우저 검증 완료. 별도 기능 브랜치로 커밋/PR을 준비하며 main 병합·서비스 배포·원격 일정 감사 활성화는 별도다. 원래 localhost:3000은 다른 세션의 워크트리를 사용하므로 이번 분리 브랜치 화면을 자동 반영하지 않는다.
