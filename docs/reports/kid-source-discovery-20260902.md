...
## 13. 적용 후 실측 — B3 FAIL 이 사라졌다

마이그레이션 승인·적용(`20260902160000_storyweaver_source.sql`, `convalidated: true`) 후
StoryWeaver Level 1 을 실제로 적재해 축을 다시 쟀다.

| 축 | 배선 전 | 배선 후 |
|---|---|---|
| B1 지문 출처 명시율 | 73.7% | **77.8%** |
| B2 재배포 가능 라이선스 | 83.3% | **85.9%** |
| B4 진본 원문 비율 | 56.1% | **63.0%** |
| B3 register 다양성 | 3종 (**narrative 0** · FAIL) | **4종 · FAIL 해소** |
| B5 발행일 명시율(사실문) | 99.2% | 99.2% |
| 초·중 창 지문 | 228편 | **270편** |

적재 **59편 · 실패 0 · 라이선스 미확인 건너뜀 0**(전량 CC BY 4.0 이 책 안에서 확인됨) ·
처리 **59편 · 실패 0**(A1 24 · A2 35).

### 넣고 나서 세 가지가 더 나왔다

**① `queued` 로 두면 아무 데도 안 보인다.** 재고 질의가 `status in ('ready','published')` 로
세기 때문에, 넣기만 하고 끝냈으면 **"넣었는데 0"** 이 됐다. 그래서 적재 스크립트에
`--process` 를 붙여 화면과 **같은 라우트**(`/api/acp/dev-process`)를 부르게 했다 —
처리 로직을 베끼면 화면에서 본 것과 스크립트가 만든 것이 갈린다.

**② B5 가 99.2% → 74.7% 로 떨어졌다.** 미상 43편 중 **42편이 그림책**이고, 그림책에는
발행일이 없다(어댑터가 일부러 `null` 을 넣는다 — 없는 날짜를 지어내지 않는다).
여기서 갈림길이 둘이었다:

- 그대로 두면 74.7% 를 보고 "발행일 관리가 나빠졌다" 고 읽는다 — **틀린 읽기**다.
- 분모에서 빼면 99.2% 가 되지만 **지표를 좋아 보이게 고친 것과 구별되지 않는다.**

둘 다 안 하고 **둘 다 낸다.** 축 이름을 `발행일 명시율(사실문)` 로 바꾸고
진본 전체 기준값(`oursAllAuthentic`)을 JSON 에 함께 남겼다. 이 축의 존재 이유가
"시의성을 판단하고 낡은 사실을 거르는 것" 이므로 그 잣대가 닿는 것은 사실문이다.

**③ StoryWeaver 는 이미 배선돼 있었다 — 다른 표에.** `library_books` 에 **20권**이
발행돼 있다(2026-06 LCP 경로, `20260614190000_lcp_storyweaver_source.sql`).
교재 지문 재고는 `library_articles` 만 세므로 글로도 넣어야 하는 것이 맞지만,
**내가 넣은 59편 중 7편이 그 20권과 같은 이야기**다. 학습자가 같은 이야기를 서가와
지문 양쪽에서 만난다.

지우는 것은 데이터 결정이라 하지 않았다. 대신 적재 스크립트가 `library_books` 를 조회해
겹친 수를 **세어서 말하도록** 했다 — 모르고 지나가는 것과 알고 두는 것은 다르다.
⚠️ **판단이 필요한 지점**: 7편을 남길지 지울지는 사람이 정할 일이다.

### 남은 결함 셋 (측정값)

- 우리가 쓴 글 **100편 (37%)** — 자작 62 · 재저작 38
- 사실문인데 발행일 미상 **1/128편**
- 재배포 불가 라이선스 **38편**

셋 다 소스를 더 배선하면 분모가 커지며 옅어진다. Simple Wikipedia 도입부(중창 후보
142,379편)와 Frontiers for Young Minds(중창 적중 100%)가 다음 차례다.
