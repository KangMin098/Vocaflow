...
## 10. ⚠️ Node 런타임이 DB 에 못 붙는다 (미해결)

적재 뒤 처리를 돌리니 **"처리 대상 0건"** 이 나왔다. DB 에는 152편이 `queued` 였다.

원인은 내 코드였다 — `const { data: queued } = await db…` 로 **`error` 를 버리고 있었다.**
`error` 를 던지게 고치자 진짜 이유가 나왔다:

```
Error: 대기 목록 조회 실패: TypeError: fetch failed
```

**0건은 "없다" 일 수도 "못 물어봤다" 일 수도 있고, 둘은 완전히 다른 상황이다.**

그리고 그 `fetch failed` 는 내 스크립트만의 일이 아니었다:

| 경로 | 결과 |
|---|---|
| Node Supabase 클라이언트 (스크립트) | `fetch failed` — 모든 질의 |
| **dev 서버** `/api/acp/dev-process` | `Article not found … (TypeError: fetch failed)` |
| **MCP `execute_sql`** | **정상** (152편 확인) |

즉 **이 머신의 Node 런타임이 PostgREST 엔드포인트에 못 붙는다.** 이 세션에서 네 번째
"소스가 죽은 게 아니라 클라이언트가 못 붙는" 사례다(africanstorybook TLS 악수 ·
undici connectTimeout · gutenberg.org TLS 거부 · 이번).

⚠️ dev 라우트도 같은 혼동을 한다 — `fetch failed` 를 **"Article not found"** 로 보고한다.
괄호 안에 원인을 함께 적어 두어 진단은 되지만, 첫 줄만 읽으면 "그 글이 없다" 로 읽힌다.

**할 일**: 연결이 돌아오면 `frym-ingest.mjs --process` 를 다시 돌린다(재실행 안전).
