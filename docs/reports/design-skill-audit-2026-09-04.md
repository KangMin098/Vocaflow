...
| 조작 가능성 | 없음 | **레벨 슬라이더 8칸** | 네트워크 0(사전 계산), 반응 즉시 |
| 랜딩 내부 이탈 관측 이벤트 | 0개 | **2개** | `landing_demo_moved` · `landing_section_reached` |
| 지침의 첫인상·이탈 규범 | 0조 | **21조** | CLAUDE.md · DESIGN_SYSTEM.md |
| 히어로 데모 계산 시간 | — | **3.7초** (전체 맵 경로였다면 88초) | `loadLevelsFor()` 표적 조회 |

실측 곡선(2026-09-04, 실 사전): 초등 65% · 중1–2 68% · 중3 78% · **고1 89%** · 고2 93% ·
수능심화 94% · 토익 96% · 학술 96%. 같은 글의 숫자가 **31%p** 움직인다 — 이것이 증명이다.

## 4. 미해결 — 승인이 필요한 것

**`/fit` 본경로의 레벨 맵이 상시 잘려 있다.** 조사 중 실측으로 드러났다(§CHANGELOG 2026-09-04).
anon 가시 `shared_words` 656,257행을 PostgREST 가 1,000행씩 돌려주는데 로더 상한은 200,000행이라,
**콜드 88초를 쓰고도 30%만 읽고 멈춘다.** 빠진 낱말은 '미지어' 로 세어져 커버리지가 낮게 나온다.

이번에는 (a) 잘림을 `console.error` 로 드러내고 (b) 히어로만 표적 조회로 우회했다.
근본 해결은 **마이그레이션 1줄** — 이미 있는 `textfit_resolve_levels`(2026-08-26 적용,
`shared_dictionary` 48,969행을 `resolve_dict_headword` 로 해석, 읽기 전용 STABLE)에 anon 실행 부여:

```sql
GRANT EXECUTE ON FUNCTION public.textfit_resolve_levels(text[]) TO anon;
```

그러면 전량 적재를 통째로 버릴 수 있다. **마이그레이션 자동 적용 금지 규약에 따라 승인 대기.**
