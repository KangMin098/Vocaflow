# DESIGN_SYSTEM — 이력 보관본 (2026-09-18 이동)

> **이 파일은 정본이 아니다.** 2026-09-18 디자인 거버넌스 개정에서 [docs/DESIGN_SYSTEM.md](../../DESIGN_SYSTEM.md) 를
> 현행 SSoT 만 남기고 줄이면서, 그 직전 원문 1,509줄을 **한 글자도 고치지 않고** 여기로 옮겼다(삭제 0 · 이동만).
> 값을 복사하지 않는다 — 현재 값은 `packages/design-tokens/src/tokens.css` · `apps/web/src/app/globals.css`.
>
> 절 분류(무엇이 본문에 남고 무엇이 여기에만 있는가): [../DECISIONS.md](../DECISIONS.md) §DS-분류.
> 이 원문 안의 상대 링크(`./X.md`)는 `docs/` 기준으로 쓰였다 — 여기서 열면 한 단계 어긋난다.
...
`learning-home.module.css`는 기출 두 소재 → 하나의 출제 공식 관계를 CSS Grid와 괘선으로 표현한다. 홈 wrapper만 최대 68rem, 세션·공식은 기존 42rem 읽기 폭이다. 패턴 교체는 native button의 aria-pressed, 심화 지도는 details/summary, 필터는 label/select를 사용한다. 새 토큰·글꼴·애니메이션 패키지는 없다.


### CSAT 관계 표식 (2026-09-18)

오답 앵커의 origin이 `tempt`이면 배제 근거가 아니라 유인 표현이다. 이때 점선 **화살표**와 「오답 유인」 라벨을 쓰고, `reject`의 점선 **막대 끝**과 구별한다. 원문 주석도 같은 origin을 따른다.

이 도메인의 시각 문법은 기존 주묵 토큰을 사용한다. `--ju` 실선+화살표+정답 라벨은 근거의 지지 관계, `--t2` 점선+막대 끝+오답 배제 라벨은 배제 관계다. 선택된 정답은 `--ju-light` 면과 실선 테두리, 선택된 배제는 `--bg3` 면과 점선 테두리로 구분한다. 음성 focus에는 별도 「현재 설명」 라벨을 함께 표시한다. 같은 공식은 정렬된 두 문항에서 공통 공식으로 합류하는 연결선과 = 기호로 표현한다. 기록 지도는 탐색 전 ○/점선, 살펴봄 •/실선, 공식 보관 ✓/이중선, 재확인 ↻/라벨을 사용한다. 색만으로 의미를 구분하지 않는다. 막대 길이는 문장 길이이며 중요도/숙련도 수치가 아니다. `visual-analysis.module.css`와 재사용 컴포넌트가 소유하고 공용 토큰을 신설하지 않는다.
