...
| 의뢰서의 진단 | 실측 | 판정 |
|---|---|---|
| 연보라(violet/purple) 포인트 | 학습자 표면에서 `purple-*` 6회(3파일) · `violet-*` **0회** · `indigo-*` **0회**. 실행 중 화면의 액센트는 deep ink `#0F2540` 과 muted gold `#B0843A` | ❌ **틀렸다.** 보라는 `/admin` 전용 액센트(`--admin: #8B5CF6`)다. 단 `/wordvault/browse` 에 떠돌이 `rgb(79,70,229)`(indigo-600) 1건이 실측됨 |
| 흰 카드 | 카드 면은 흰색이 아니라 크림 `#FBFAF6` / `#F4F0E9`. **하지만 형태는 정확히 그 지적대로다** — 아래 0-3 | ⚠️ **색은 아니고 형태는 맞다** |
| 시스템 폰트 | **정확하다. 그리고 생각보다 심각하다** — 아래 0-2 | ✅ 맞다 |

> 요약: 팔레트는 이미 "Reading Room"(종이·잉크·금)이라는 방향을 갖고 있다. 문제는 **팔레트가 아니라
> ① 한글에 글꼴이 없다는 것 ② 형태 언어가 한 종류뿐이라는 것 ③ 브랜드 자산이 0개라는 것** 이다.
