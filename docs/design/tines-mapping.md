...
| `FinalCta` 흩어진 물건 띠 | 정지 | `view()` 시차 `±0.75rem` — 액자를 두르고 그림을 위아래 `3rem` 크게 잡는다 | ③ | 0 |
...
| `FinalCta` 띠 | `scroll(root)` `6rem` | `view()` `±0.75rem` + **액자** | 딱 맞는 배경 그림을 옮기면 띠 가장자리에 **빈 줄**이 생긴다. `overflow-hidden` 액자를 두르고 그림을 위아래로 `3rem` 크게 잡아 넘치는 부분을 자른다 |
