...
공개 화면은 §19 에서 이미 참조 막대 + 메가메뉴 셋을 쓰고 있었는데, **로그인 뒤의 셸만 왼쪽 240px 레일**이었다(`components/layout/Sidebar.tsx`). 같은 제품 안에서 내비 문법이 둘이었고, 레일은 1440 에서 가로 17%를 상시 점유하면서 열세 개 주소를 어느 화면에서나 펴 놓고 있었다. 사용자 지시(2026-09-23 「좌측 메뉴를 tines 스타일 상단 메뉴 체계로」)로 셸을 하나로 맞췄다.

**IA 는 그대로다.** 항목·주소·순서·`owns` 의 정본은 여전히 `components/layout/sidebar-config.ts` 이고, 새 파일 둘은 그것을 **배치**만 한다.

| 새 파일 | 하는 일 |
|---|---|
| `layout/top-nav-data.ts` | 설정 → 막대 칸 · 패널(틀 · 원색 카드 · 목록 행). 카드 면 색과 타일은 `lib/design/route-art` 의 범주 색을 그대로 읽는다 — 메뉴가 색을 새로 정하지 않는다 |
...
**막대(60px)** — 로고 · 메타 둘(Today · Growth) · 세로 구분선 · 흐름 레일 다섯 · (오른쪽 끝) 더 보기. 사이드바의 세로 순서를 그대로 눕힌 것이다.

- 레일은 번호 배지 다섯이 짧은 실선 조각으로 이어진다(세로 레일 선의 가로판). 번호는 **늘 그 단계의 색**이다 — 글자만 남기면 막대가 보라 한 색이 되어 §12 진단으로 되돌아간다.
- 항목이 하나인 단계(② Words · ④ Conquer · ⑤ Complete)는 패널을 열지 않고 **모듈 이름**을 그대로 판다(WordVault · ScriptQuiz · Dictation). 카드 한 장짜리 패널은 클릭만 한 번 늘린다.
- 지금 있는 구역은 면(색 12%) + 밑줄 + 굵기 + sr-only 「(현재 구역)」 — 색만으로 알리지 않는다.

**패널 셋**

| 메뉴 | 골격 | 내용 |
|---|---|---|
| ① Read | 라벤더 틀(눈썹 「자료별 — 공용 서가」, 눈썹이 `/library` 링크) + 원색 카드 4 · 떨어진 살구 목록 「내 라이브러리」(눈썹이 `/text`) | 서가 네 면은 `lib/library/tabs.ts` 에서, 색은 `MATERIAL_TONE` 에서 — 도서 초록 · 기사 주황 · 단어장 자홍 · 교재 보라 |
| ③ Practice | 라벤더 틀(「연습 방식」) + 큰 카드(Practice, 소품 1점) + 카드(Game Lab) | 접힌 도구 넷(Flashcard·WordBlitz·PairFlip·SpellForge)은 **다시 펴지 않는다**(v06.202 결정) — `/practice` 가 `owns` 로 대신 불을 켠다 |
| 더 보기 | 노랑 틀(「Comics — 같은 이야기를 그림으로」) + 원색 카드 2 · 떨어진 카드 2(CSAT 자홍 → · Class 초록 ↗) · 청록 목록 「도구」(Settings · Sitemap) | 레일 밖 = 단계가 아닌 것. DOM 순서로도 레일 뒤라 「여섯 번째 단계」로 읽히지 않는다 |

**지킨 것 · 바뀐 것**

- 사이드바 시절 **셰브런 한 번**을 눌러야 나오던 서가 네 면이, 이제 메뉴를 여는 동작 하나로 **부모와 함께** 나온다(패널은 언제나 하나만 열린다 — Books 가 두 벌 서던 2026-08-16 문제는 한 패널 안 눈썹 둘로 갈라 해결).
- 닫힌 패널도 DOM 에 남긴다(`hidden`) — 링크가 사라지면 스크린리더가 목록을 잃고 회귀 테스트도 주소를 셀 수 없다.
- **모바일은 그대로다.** 레일도 `hidden md:flex` 였고, 폰 셸은 이미 세 줄(유틸리티 바 · 나침반 띠 · 하단 탭)이다 — 메가메뉴 서랍을 더하면 같은 주소가 두 벌이 된다.
- `--sidebar-w` 는 0 으로 알린다(하단 player 가 `md:left-[var(--sidebar-w,240px)]` 로 읽는다 — 안 넣으면 왼쪽이 240px 비어 보인다).
- 회귀: `components/layout/__tests__/top-nav.test.tsx`(정본의 모든 주소가 그려지는가 · 번호 = 순서 · 만화는 레일 뒤 · 현재 위치 표식 정확히 하나) · e2e `12-navigation`(전 항목 이동 · 패널 직행 · 한 번에 하나) · `09-arcade-access` · `11-comic-discovery`.
