...
### 29-12. D 단계 — 공개 화면으로 넓히기 (2026-09-23 · 브랜치 `feat/motion-d`)

**새 CSS 0 · 새 JS 0.** A 가 세운 다섯 클래스만 쓴다. 넓히는 방법을 **화면이 아니라 부품**으로 잡았다 —
`/`·`/about`·`/library/books` 에 따로 붙이면 다음 화면이 또 자기 것을 붙이게 된다.

**공유 부품 넷** (한 번 고치면 여러 화면이 함께 산다)

| 부품 | 넣은 것 | 함께 사는 화면 |
|---|---|---|
| `layout/AreaHero` | 구역 타일 뜨기(`5s` · `5%`). **판은 가만히 둔다** — 제목·수치를 읽는 면이다 | 도서 · 글 · 만화 · 단어장 · 교재 · 교사 |
| `marketing/sections` `Frame` | 액자 진입(`16px ↑` · `.6s`) — 참조 `threeBVisualAppear` 자리 | `/about` 영상 액자 · 다른 점 3열 |
| `marketing/sections` `Bento` | 칸 그림 뜨기 + `0.35s` 계단 — **참조 `tileFloat` 의 원자리** | `/about` 학습 과학 7 |
| `marketing/sections` `ToneCards` | 구석 소품 뜨기(`5s` · `4%`) + `0.3s` 계단 | `/about` 모듈 · 그 밖의 색면 카드 줄 |
| `site/ScatterCta` | 끝 띠 시차 — `/hub` 에서 검증한 **6% 균일 확대** 그대로(§29-11) | 공개 화면 **전부**의 끝 |

**화면 둘**

| 화면 | 넣은 것 |
|---|---|
| `/`(랜딩) | 히어로 계단 4(알약 · 제목 · 부제 · CTA) · 선언문 소품 뜨기 · **보라 통판 꽃밭 시차** · 제품 액자 진입 · 문 카드 타일 둘 계단 뜨기 |
| `/about` | 히어로 계단 4 · 근거 카드 진입(`200ms`) · 카드 아래 꽃밭 시차 · 영상 없을 때의 대체 타일 뜨기 |

**꽃밭 시차에서 방향이 갈렸다** — 같은 그림인데 두 화면의 처리가 다르다:

- 랜딩 보라 통판: 꽃밭이 `bottom-0` 에 붙어 있고 판이 `overflow-hidden` 이다. **아래로만** 민다
  (`0 → 1.5rem`) — 위로 올리면 판 밑변에 빈 줄이 생기고, 아래로 민 만큼은 판이 자른다.
- `/about` 근거 카드 아래: 자르는 액자가 없지만 **잘릴 것도 없다**(떠 있는 그림 한 장). 양방향(`±1rem`).

**하지 않은 것**: 랜딩 히어로의 리본 셋(`Ribbon`)은 `-rotate-[5deg]` 로 **자기 transform 을 쓴다** —
§29-11 의 규칙대로 겹쳐 걸지 않았다(겹치면 합쳐지지 않고 덮는다). 리본을 움직이려면 한 층 더 싸야 하는데,
장식 하나 때문에 히어로의 DOM 을 깊게 만들 이유가 없다.

**확인**: tsc · eslint clean · 관련 스위트 **162건 통과**. 격리 워크트리의 dev 서버(포트 3100)로
`/` · `/about` · `/library/books` 를 모션 걸린 채(`data-motion-freeze`) 1440 슬라이스로 찍어 봤다 —
넘침 0 · 판 밖으로 나간 그림 0 · 끝 띠 가장자리 빈 줄 0.

⚠️ `show-screen.mjs` 로는 이 확인이 **안 된다**: 그 하네스는 컨텍스트에 `reducedMotion: 'reduce'` 를
강제해서 §4.5 규칙이 아예 존재하지 않는 프레임만 나온다(= 「변화 없음」만 증명한다). 모션이 걸린
프레임을 보려면 reduce 를 켜지 않고 `installFreezeMotion()` 으로 세워야 한다.

**왜 별도 워크트리인가**: 작업 시작 시점에 `design/replica-first` 의 잠금을 **다른 Claude 세션**
(pid 17540)이 쥐고 있었다. AGENTS.md 「공유 워크스페이스」대로 그 워크트리에는 쓰지 않고
`pnpm wt new motion-d design/replica-first` 로 갈라 나왔다. 덤으로 `.next` 와 dev 서버가 분리돼
**포트 3100 에서 마음대로 띄우고 찍을 수 있었다** — 지난 단계에서 캡처를 막던 것이 바로 그 공유였다.
