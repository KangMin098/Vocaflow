...
| 10 | `/text/new` | **완료** 2026-09-19 | A 「붙여 넣으면 칠해지는 입력칸」(채색 지문 · `/fit` 부품) | 21→**4** · 390 그림자 3→**0** | [compare/text-new.md](../compare/text-new.md) · [golden/text-new.md](../golden/text-new.md) · DD-30 |


**B1 다음 — 여정 ①② 의 남은 화면**

| 순서 | 화면 | 상태 | 선택안 | 평균 신호 | 기록 |
|--:|---|---|---|---|---|
| 11 | 인증 3화면 `/login` · `/reset-password` · `/verify-email` | **완료** 2026-09-19 | A 「칠해진 지문 옆의 폼」(`/signup` 틀) | 6·11·6 → **0·0·0** | [compare/auth.md](../compare/auth.md) · [golden/auth.md](../golden/auth.md) · DD-31 |

**B1 다음 — 학습자 화면**(점수 순, 같은 입구는 한 단위)

| 순서 | 화면 | 상태 | 선택안 | 평균 신호 | 기록 |
|--:|---|---|---|---|---|
| 12 | `/practice` + `/practice/dcp` | **완료** 2026-09-19 | A 「오늘의 연습지」(시험지 사물) | 렌더 1280 큰 모서리 10→**0** · 카드형 5→**0** | [compare/practice.md](../compare/practice.md) · [golden/practice.md](../golden/practice.md) · DD-32 |
| 13 | `/teacher` | **완료** 2026-09-19 | A 「교실에 붙일 초대장」(시험지 사물) | 렌더 카드형 2→**1**(초대장 자체) | [compare/teacher.md](../compare/teacher.md) · [golden/teacher.md](../golden/teacher.md) · DD-33 |
| 14 | 모듈 허브 `/flashcard` · `/spellforge` | **완료** 2026-09-19 | A 「오늘 담길 낱말」(망각 · `/hub` 권점 몸짓) | 렌더 카드형 8→**0** · 9→**1** | [compare/module-hubs.md](../compare/module-hubs.md) · [golden/module-hubs.md](../golden/module-hubs.md) · DD-34 |
| 15 | 게임 허브 `/wordblitz` · `/pairflip` | **완료** 2026-09-19 | A 「이번 판 낱말」(시험지 사물 · 권점) | 정적 4·11 → **0·1** | [compare/game-hubs.md](../compare/game-hubs.md) · [golden/game-hubs.md](../golden/game-hubs.md) · DD-35 |
| 16 | `/dictate` | **완료** 2026-09-19 | A 「오늘의 받아쓰기 문제지」(시험지 사물) | 정적 6→**1** | [compare/dictate.md](../compare/dictate.md) · [golden/dictate.md](../golden/dictate.md) · DD-36 |
| 17 | `/scriptquiz` | **완료** 2026-09-19 | A 「읽은 챕터 장부」(망각 · 권점) | 렌더 카드형 2→**1** | [compare/scriptquiz.md](../compare/scriptquiz.md) · [golden/scriptquiz.md](../golden/scriptquiz.md) · DD-37 |

- **보류**: 없음.
- **라쳇**: average-signal learner `float-hover` 66→**65**(`/hub` — `TodayPlanCard` 칩) · `gradient` 171→**169** · `glass` 52→**50**(`/diagnostic`) · `grid-3eq` 60→**59** · `gradient` 169→**165** · `float-hover` 65→**64** · `infinite-anim` 12→**10**(`/flashcard/play`) · `rounded-big` 41→**39**(`/signup`) · `ai-purple` 38→**37** · `glass` 50→**49**(`/text/[id]`) · `shadow-heavy` 29→**25** · `rounded-big` 39→**34** · `gradient` 165→**164** · `float-hover` 64→**61** · `infinite-anim` 10→**9**(`/wordvault`) · `grid-3eq` 59→**56** · `ai-purple` 37→**36** · `float-hover` 61→**60**(회고) · `grid-3eq` 56→**54** · `shadow-heavy` 25→**22** · `rounded-big` 34→**25** · `gradient` 164→**162** · `float-hover` 60→**59**(`/text/new`) · `shadow-heavy` 22→**18** · `rounded-big` 25→**15**(인증 3화면) · `gradient` 162→**160**(모듈 허브) · `grid-3eq` 54→**49** · `gradient` 160→**158** · `float-hover` 59→**58**(게임 허브) · `shadow-heavy` 18→**17** · `gradient` 158→**153** · `float-hover` 58→**55**(`/dictate`). form-declaration 무선언 목록 −1(`/diagnostic`) · −1(`/reports`) · −1(`/text/new`) · −3(인증 3화면) · −2(`/practice`) · −1(`/teacher`) · −2(모듈 허브) · −2(게임 허브) · −1(`/dictate`) · −1(`/scriptquiz`). `/hub` 선언은 렌더와 일치하게 정정.
- **승인 대기 마이그레이션**: `supabase/migrations/_pending_funnel_allow_hub_curve.sql`(관측 `hub_curve_interacted` · `retrospect_layer_opened` · `text_created` — 한 파일).
