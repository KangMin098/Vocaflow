...
2. **방향**: 필요하면 코드 전에 vocaflow-design §G2 세 줄(골격 · 서명 · N4)을 적어 둔다(선택).
   **큰 재설계(새 화면·전면 재설계)는 발산 4안**을 만들 수 있다 — 서명 요소가 서로 다른 사물(§G1 축이 서로 다른 넷)이면 비교가 쉽고,
   최고 성능 모델·고노력으로 만든다(싼 모델은 이름만 넷인 한 안을 돌려준다). 산출물 `compare.md`(4안 표).
   고르는 것은 사람이다. 선택 이유와 버릴 요소를 기록하고, 고른 안은 다듬어 [골든](golden/README.md)으로 남긴다.
3. **구현**: 기존 토큰·컴포넌트·기능을 활용한다. 외부 라이브러리 추가는 기존 도구로 해결되지 않는 요구가 있을 때 검토한다.
4. **검증**: 실제 브라우저에서 캡처하고 핵심 동작을 실행한다. 아래 명령은 자동 검사의 출발점이다.
5. **비평·수정**: 이미지를 직접 연다. 아래를 차례로 본다.
   ((a) 익명성 · (b) 평균 회귀 통과 판정은 DD-66 으로 삭제.)
   ...
   (e) 규칙(AGENTS.md)과 접근성.
   출력: 눈에 띄는 차이 · 가장 나쁜 것 1개. 가장 나쁜 것 하나만 고치고 재검증. 루프는 2회까지, 그 뒤는 사람 판단.
   개수를 채우기 위한 결함을 지어내지 않는다 — 없으면 없는 만큼만 적는다.
6. **기록**: 전후 증거 경로, 라우트·상태·테마·뷰포트·실행 버전, (c)–(e) 관찰, 수정 이유, 미검증 범위를 남긴다.
   캡처 산출물에는 골든 대조 쌍(같은 뷰포트의 골든 경로 + 이번 캡처 경로)을 함께 적는다.

Codex 진입 스킬은 `.agents/skills/vocaflow-design-loop/SKILL.md`다(비평 스킬 `vocaflow-visual-critic` 은
DD-65 로 삭제 — 비평 순서는 위 5단계가 정본). 기존 `.claude/skills/vocaflow-design/SKILL.md`는
발명 절차 참고서다(선택). 스킬 위치는 [공식 Codex 문서](https://learn.chatgpt.com/docs/build-skills)의
...
(평균 금지 유닛 라쳇 — `@form` 골격 선언 · 평균 신호 출현 수 — 은 DD-66 으로 삭제.)
