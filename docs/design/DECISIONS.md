...
| **DD-35** | 게임 허브 `/wordblitz` · `/pairflip` = **A 「이번 판 낱말」**(시험지 사물 — 게임이 실제로 쓸 낱말이 괘선 두 단, PairFlip 난이도의 쌍 수만큼 권점 — 모듈 허브와 같은 몸짓). 시작은 하나(주묵). `HubStartCard` 에 `cta.onStart`(sessionStorage 로 설정을 넘기는 PairFlip). 준비 중 매칭 모드 선택지 제거 · 죽은 `PairFlipLevelSelector`·`ModeSelector`·`StartScreen` 삭제. 최고 기록 줄은 기록이 없어도 선다 — e2e 18 의 낡은 실패도 함께 풀림. 탈락: B 「게임 판 미리보기」(모션 예산 밖) · C 「기록판」 · D 「게임몰 톤다운」(N4 불통과) | B4 (2) A 만 모듈 허브 골든과 같은 부품·몸짓. 수정 2회 — PairFlip 390 시작 버튼이 여전히 3px 걸침(학습자 화면이라 I8 대상 아님, 남은 것에 기록). 정적 4·11 → 0·1. C6 미실행. 골든 [golden/game-hubs.md](golden/game-hubs.md) |
| **DD-36** | `/dictate` = **A 「오늘의 받아쓰기 문제지」**(시험지 사물 — 번호 다섯 줄, 낱말 수만큼 빈칸 · 폭 = 낱말 길이 · 내 낱말 자리만 주묵, **글자는 보이지 않는다** — `DailySheet`). 1차 행동 하나(주묵 「받아쓰기 시작하기」 — e2e 17 의 `/시작하기/` 계약 유지). 자료·약점·최근 세션은 괘선 구획. 탈락: B 「귀로 읽는 지문」(받아쓰기 전에 답을 보여 준다 — 과제 파괴) · C 「약점 먼저」(조회 1) · D 「카드 정리」(N4 불통과) | B4 (2) A·B 가 가까웠으나 B 는 (3) 자기검토 탈락. 정적 6→1 · 렌더 그라디언트 9→6(셸 몫). 수정 1회. e2e 23 G·H 실패는 DB 조회 -1(서비스 키 없는 실행 환경). C6 미실행. 골든 [golden/dictate.md](golden/dictate.md) |
| **DD-37** | `/scriptquiz` = **A 「읽은 챕터 장부」**(망각 — 읽고 오래된 챕터부터 괘선 장부, 확인 안 한 챕터 번호에 권점). 빈 상태는 페이지가 이미 읽던 카탈로그에서 **퀴즈가 준비된 책 5권**을 실제 목록으로. 1차 행동 주묵 하나. 탈락: B 「곡선 위의 챕터」(챕터엔 S 가 없어 곡선을 지어낸다) · C 「문제지 한 장」(문항 미리보기 = 확인 무력화) · D 「카드 정리」 | B4 (2) A 만 권점 몸짓. 두 검증 계정 모두 읽은 챕터 0 — 장부 상태는 **캡처 없이** 렌더 계약 테스트로 고정. `@form` 은 단일 축만 받는다(「망각 × 시험지 사물」 거부 → 「망각」). 수정 1회. C6 미실행. 골든 [golden/scriptquiz.md](golden/scriptquiz.md) |
