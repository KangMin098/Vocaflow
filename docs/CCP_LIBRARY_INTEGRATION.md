...
| **D4** | **도서 단위 만화 라우트 `/comics/book/[bookId]`** 신설 — 미등록·미로그인도 프리뷰 N컷 열람, "이어서 보기"는 자동 enroll 후 `/text/[id]/comic` | G3. 유입 자산은 유입 전에 보여야 함 | 현행(등록 후에만 열람) |
...
├── Comics    /comics    도서의 만화 표현형 (CCP)                     ← D2 (NEW · 최상위)
│             └── /comics/book/[bookId]  만화 상세(프리뷰+포맷 선택)
├── My Scripts /text
└── Restored  /restored  퍼블릭도메인 복원 만화 (PDCP — 별도 기능)
```
...
| 지점 | 화면 | 동작 |
|---|---|---|
| ① 만화 메뉴 | `/comics` (사이드바 최상위) | 카탈로그 그리드 + 이어서 보기 레인 + 레벨 필터 |
| ② 도서 탐색 중 | `BookGridCard` / rails / 상세 시트 | `만화` 배지 → 탭하면 형식 선택 |
| ③ 만화 상세 | `/comics/book/[bookId]` | 프리뷰 3컷(무료·미등록 허용) + 형식 선택 3종 |
