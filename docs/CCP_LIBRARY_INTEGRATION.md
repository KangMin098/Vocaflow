...
| 그럼 왜 탭을 따로 두나? | 학습자의 멘탈 모델(=무엇을 할지 고르는 입구)과 데이터 축(=무엇에 속하는지)은 다른 축이기 때문. 탭은 **입구**, 데이터는 **포맷 facet**. |
...
| 산출 | 4탭 IA + 포맷 facet + 도서 단위 만화 라우트 + 통합 카탈로그 뷰 + 단어장 정합 게이트. P0~P3 로드맵(§13). |
...
| **D2** | 발견 UI 는 **4번째 탭 `/library/comics`** 로 독립 코너화 | 사용자 요구("또 다른 장르로 제시") + G1/G4. 탭=입구 축, facet=데이터 축으로 분리하면 둘 다 만족 | 히어로 확장만(재방문 경로 없음) / GenreBucket 추가(축 오염, D1 위배) |
| **D3** | 동시에 도서 축에 **포맷 facet(`만화`·`오디오`·`퀴즈`)** 추가 — 교차 필터 가능 | G2·G6. "SF이면서 만화" 질의 성립 | 만화 탭만 두고 도서 탐색은 방치(같은 책이 두 세계로 갈라짐) |
| **D4** | **도서 단위 만화 라우트 `/library/comics/[bookId]`** 신설 — 미등록·미로그인도 프리뷰 N컷 열람, "이어서 보기"는 자동 enroll 후 `/text/[id]/comic` | G3. 유입 자산은 유입 전에 보여야 함 | 현행(등록 후에만 열람) |
...
```
/library
├── /books    도서      장편 원서 (Work 원본)        ← 기존
├── /comics   만화      도서의 만화 표현형 (NEW)      ← D2
├── /scripts  스크립트  ACP 짧은 글                   ← 기존
└── /vocab    공용 단어장  어휘 세트                   ← 기존
```

탭 라벨 밑 한 줄 설명으로 **중복이 아님**을 명시: `만화 — 같은 책, 그림으로 먼저 만나는 입구`.
아이콘 `BookImage`(리더/히어로와 동일), 액센트는 만화 계열의 gold `var(--active)` 유지(도서=오렌지, 스크립트=보라, 단어장=퍼플과 구분).

### 4.2 축 정의 (혼동 차단)

| 축 | 값 | 저장 | 용도 |
|---|---|---|---|
| **종류(kind)** | book · article · word_set | 테이블 자체 | 탭(도서/스크립트/단어장) |
...
`만화` 탭은 **kind=book ∧ format∋comic** 의 저장된 뷰(saved view)로 정의된다. 즉 탭은 UI 편의이고, 진실은 facet 하나다 — 이것이 D1·D2 를 동시에 만족시키는 장치다.
...
| 지점 | 화면 | 동작 |
|---|---|---|
| ① 만화 탭 | `/library/comics` | 카탈로그 그리드 + 레인 3종(이어서 보기 · 지금 딱 맞아요 · 새로 나온 만화) |
| ② 도서 탐색 중 | `BookGridCard` / rails / 상세 시트 | `만화` 배지 → 탭하면 형식 선택 |
| ③ 만화 상세 | `/library/comics/[bookId]` | 프리뷰 3컷(무료·미등록 허용) + 형식 선택 3종 |
...
- [ ] `vocab_orphans` 검증 + Admin 리뷰 표시 + 만화 상세 "단어장에서 보기"
- [ ] 도서 카드 진행률에 "만화 n/N 미리 봄" 분리 표기(R2)

### P3 — 통합 홈 · 적응
- [ ] `/library` 루트를 통합 홈으로 승격(현재 `/library/books` 리다이렉트)
