...
for await (const rows of walk('csat_source_eligibility', 'article_id,source_updated_at,policy_version,grade:result->>grade', {}, 'article_id')) {
...
for await (const rows of walk('library_articles', select)) {
  for (const row of rows) {
    ...
    const input = {
      title: row.title, status: row.status, articleVLevel: row.article_v_level,
      wordCount: row.word_count, register: row.register, cefrLevel: row.cefr_level,
      syntaxScore: row.syntax_score?.score == null ? null : Number(row.syntax_score.score),
      displayOnly: row.display_only, licenseClass: row.license_class,
      copyrightSafeInKr: row.copyright_safe_in_kr,
      gatePublishable: gate.publishable ?? null, gateBlockedBy: gate.blockedBy ?? null,
      gateVerdict: gate.verdict ?? null, gatePurpose: gate.purpose ?? null,
      gateGenre: gate.genre ?? null,
      excerptWindows: Array.isArray(row.windows) ? row.windows.length : null,
      hasItems: refs.has(row.id), outsidePct: null,
    }
    const result = judgeSource(input)
    const cached = policyCache.get(row.id)
    if (!cached) flag('cache_missing', row, null)
    else {
      if (cached.source_updated_at !== row.updated_at || cached.policy_version !== ELIGIBILITY_SPEC_VERSION) flag('cache_stale', row, { cached: cached.source_updated_at, current: row.updated_at })
      if (cached.grade !== result.grade) flag('cache_grade_drift', row, { cached: cached.grade, current: result.grade })
    }
...
console.log(JSON.stringify({ ...report, duplicateHashes: report.duplicateHashes.length, findings: Object.fromEntries(Object.entries(findings).map(([code,f]) => [code,{count:f.count,candidates:f.candidates,bySource:f.bySource}])) }, null, 2))
