...
export function extractExplanationSpec(db, pub = null) {
  ...
  //
  // ⚠️ **"글자가 깨진 것뿐이니 OCR 하면 된다" 도 기각됐다** (실측 2026-09-01).
  //    쎄듀 첫단추 본책은 `ko_chars = 0` — 226쪽에서 한국어가 **한 글자도** 안 나온다(영어는
  //    213,902자 정상). 임베드 폰트 CMap 문제라 OCR 이면 되살아난다고 볼 만했고, 실제로
  //    한국어 OCR 품질은 좋았다. 그래서 183쪽을 뽑아 같은 정규식으로 세어 봤다:
  //
  //      해설 블록 **20개** — 표본은 "본 자료는 수업용으로…"(저작권 문구)와
  //      "모의고사는 실제 수능처럼…"(학습 요령 Q&A). **문항 해설이 아니다.**
  //      그리고 본문에 `[별책] 정답 및 해설` · `정답 및 해설 p.2` 가 있다.
  //
  //    쎄듀의 해설은 **별책**이고 이 코퍼스에 없다. 본책은 그 별책을 가리킬 뿐이다.
  //    즉 `insufficient: true` 는 추출의 한계가 아니라 **자료의 부재**다 — 코드로 풀 수 없다.
  //    (덧붙여, 통째로 OCR 로 덮으면 지금 멀쩡한 영어가 `only→이11y` 로 열화된다.
  //     그러면 상대의 인용률이 우리 때문에 낮아진다 — 이기는 게 아니라 자를 망치는 것이다.)
  if (blocks.length === 0) {
    return {
      blocksMeasured: 0,
      insufficient: true,
...
/**
 * ⛔ **여기에 "지문 출처 표기율" 축을 만들지 말 것 — 2026-09-01 에 만들었다가 되돌렸다.**
 *
 * 동기는 옳았다. 구속 출판사(EBS)는 정답해설이 코퍼스에 없어 잴 수 있는 축이 A6·A7
 * 둘뿐이고, 그래서 천장이 **1.199 < 1.200** 으로 눌려 있다. 답지 없이 **본책만으로**
 * 잴 수 있는 축을 더하면 그 천장이 풀린다 — 출처 표기가 그 후보였다.
 *
 * 그런데 **텍스트 추출로는 이 축을 신뢰성 있게 잴 수 없다.** 두 방향 다 실패한다:
 *
 *  · 느슨한 패턴(`/출처|Source:|ⓒ/`) → 지문 **한국어 해석 안의 보통명사**가 걸린다.
 *      "신뢰할 수 있는 출처를 선택하는 것이…"  ← 출처 표기가 아니라 지문 내용이다
 *      이 오검출로 NE능률 12.0% · EBS 4.6% 가 나왔다. 둘 다 실제 값이 아니다.
 *
 *  · 엄격한 패턴(줄 모양·URL·연도) → **출판사 홍보 URL과 지문 속 실용문**이 걸린다.
 *      "무료 부가 자료 | www.nebooks.co.kr"      ← 판권 안내
 *      "- Computer Lab, Oakwood University"      ← 안내문 **지문 본문**
 *
 * 세 출판사 모두에서 남는 진짜 출처줄은 사실상 0 이다. 즉 **시중 교재는 지문 아래에
 * 출처를 인쇄하지 않는다** — 이 결론 자체는 견고하다. 다만 그래서 **비율 축이 못 된다**:
 * 분모가 0 이면 지수가 발산해(쎄듀 0/423 → 천장 약 111) 기하평균을 통째로 삼킨다.
 * 우리가 100% 라는 이유로 종합이 1.12 → 2.6 으로 뛰는데, 그건 우위가 아니라 **자의 고장**이다.
 *
 * 그래서 출처 표기는 **차별 요소(beyond-market)** 로 다룬다 — `vocab-choice-benchmark` 의
 * `beyondMarket`(C1~C3)과 같은 취급이다. 시장이 아예 안 하는 것에 "120% 낫다" 는 말은
 * 성립하지 않는다. 그리고 우리 우위의 일부는 실력이 아니라 **구조**다(우리 지문은 PD·CC 라
 * 표기 비용이 0 이고, 상업 출판사는 라이선스 계약상 표기하지 않을 수 있다).
 *
 * 같은 이유로 어휘주석·난이도표시·구문해설도 축이 못 된다 — 그것들은 **다른 쪽**
 * (어휘 정리면·해설지)에 실려서, 지문 쪽만 보는 검출기에는 0~12% 로 보인다.
 * 낮은 값이 "안 한다" 가 아니라 "여기서는 안 보인다" 인 것이다.
 *
 * 결론: **EBS 천장 1.199 는 본책 축을 늘려서 풀 수 없다.** 그 출판사의 정답해설을
 * 코퍼스에 넣어 A1~A4 를 여는 길뿐이다. 상세: docs/reports/textbook-body-axis-rejected.md
 */
