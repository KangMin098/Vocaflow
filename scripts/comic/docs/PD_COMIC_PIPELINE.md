...
### 남은 선택지 — 말풍선 텍스트를 어떻게 얻을 것인가

hOCR 재활용으로는 여기까지다(캡션 포함 24%). 손레터링 말풍선은 다음 중 하나가 필요하다:

| 안 | 내용 | 비용 |
|---|---|---|
| A | 손글씨 특화 OCR 을 컷 단위로 재실행 (영어 고정 → 키릴 오인 제거) | 엔진 도입 |
| B | 멀티모달 모델에 컷 이미지를 주고 대사 추출 | API 비용 · 정확도 검증 필요 |
| C | 캡션만 자동, 말풍선은 사람 입력 | 인건비 · 품질 최상 |
| D | 텍스트 없이 그림만 게시(vocab 칩·reveal 포기) | 학습 가치 급감 |

**언어 고정만으로도 개선 여지가 크다** — 검수 필요 1위 사유가 비라틴 문자 12건이고,
이는 Tesseract 가 단어별로 스크립트를 자동 판별해서 생긴 문제다. A 안의 우선 검토 근거.
