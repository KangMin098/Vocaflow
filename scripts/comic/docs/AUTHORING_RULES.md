...
- **R30 FLUX.2 ReferenceLatent 다중참조는 다인 일관 만화에 부적합**(전권 실증 2026-08-07, 음성 결과): 캐릭터 시트를 `ReferenceLatent`로 조건화하면 — (a) **2차 캐릭터가 주인공 복제로 붕괴**(Bob·Tim → Scrooge, 인물 중복), (b) **faceless 팬텀이 얼굴/눈/인간복제**로 무너짐(S4 6/8 위반), (c) **텍스트 baked**(말풍선·간판·묘비, FLUX.2 텍스트 강점 역효과). 전권 게이트 **56/90 vs GPT 90/90**. ⇒ **다인·일관 만화엔 GPT Image 2(16장 images.edit)가 우월**. FLUX.2는 **단일 인물·단일 참조엔 강함**(반투명 최고, 화풍 통일)이나 다중참조 캐스트락은 다른 기법(IP-Adapter·낮은 ref 강도·단일 지배참조) 필요. **교훈: "무료·통일화풍"이 "다인 일관성·통제"를 못 이긴다** — 백엔드 선택은 과제(캐릭터 수·텍스트 정책)에 종속.
- **R29 앵커는 얼굴·소품뿐 아니라 핵심 의상도 고정**(홀리스틱 평가서 도출): 참조 이미지는 **얼굴/정체성만 락**하고 의상은 scene·생성에 맡겨져 **패널·스테이브 간 색·형태가 드리프트**한다(실측: Fred 코트가 S1 흰색 ↔ S3/S5 검은 연미복 — 앵커에 "holly sprig on his coat"만 있고 색 미지정이 원인). ⇒ 재등장 캐릭터 앵커에 **의상 색·종류를 명시**("a holly sprig on his **dark tailcoat**"). 이래야 "한 작가가 그린 한 권" 환영이 유지된다. (propLines가 앵커를 매 패널 주입하므로 앵커 고정이 곧 전권 고정.)
- **R28 verbatim 태그는 정본 대조 필수**(홀리스틱 평가서 도출 2026-08-07): `verbatim:true`는 **완전 word-for-word Dickens**만. 축약·패러프레이즈를 1차 인용으로 오표기 금지(영어학습 제품 정확성). 구조 가드 `verbatim-audit.mjs`가 정본 참조와 대조해 미스매치를 플래그(릴리스 전 0 필수). 실측: 24건 중 9건 오표기 → 완전원문 복원(6) + 축약은 `adapted_from`으로 강등(3) → 미스매치 0. ⚠주의: **같은 화자 버블이 한 패널에 복수**일 수 있으니 텍스트 패치는 verbatim 플래그로 정확히 타겟팅(전체덮어쓰기 금지).
- **R27 전수 게이트가 드러낸 3대 신규 패턴**(2026-08-07 GONICK 전권 폐루프서 도출·수렴):
  - **구원 패널의 유령 환각**: Stave 5(성탄절 아침·낮·구원) 패널에 모델이 **사슬 유령을 무단 삽입**(S5 P4). "A Christmas Carol"→유령 연상 편향. ⇒ **구원/낮 패널엔 scene-aware NEG("no ghost/spirit/chains")** 를 스테이브 조건부로. 전역 NEG 금지(1~4권은 유령 필수).
  - **참조 없는 재등장 엔티티의 렌더 드리프트**: faceless 팬텀(noref)이 패널마다 **검은 후드↔흰 베일로 흔들림**(S4 P17). 캐릭터 시트가 없는 엔티티는 **고정 묘사 앵커**("tall solid BLACK hooded shroud, pure black void")를 매 패널 강제.
  - **아동 크기 드리프트**: Tim이 **성인으로** 렌더(S3 P11). t2i/생성 편향. ⇒ 앵커에 **"very small young CHILD, not an adult"** 명시. (교정힌트로 5/5 수렴 확인.)
- **R26 다중참조는 "격리된 캐릭터 시트"로만**(백엔드 무관 · 2026-08-07 FLUX.2 dev 실증): 참조 이미지로 **전체 장면**을 주면 얼굴뿐 아니라 **배경·구도까지 배어난다**(FLUX.2 ReferenceLatent 실측: 묘지 scene을 ref로 쓰자 침실 장면에 묘지가 배어남). ⇒ 참조는 반드시 **흰 배경 단일 인물 캐릭터 시트**여야 정체성만 전이. *적용→검증 완결*: 깨끗한 시트로 교체하니 배어남 사라지고 동일 인물 유지. FLUX.2(ReferenceLatent)·GPT(다중참조)·Qwen-Edit(참조편집) 모두 동일 원칙. (백엔드별 워크플로: `wf/flux2-ref.api.json`, `flux2-ref-test.mjs`.)
