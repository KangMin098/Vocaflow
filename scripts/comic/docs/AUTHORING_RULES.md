...
- **R22 힌트-저항 의도는 구조로 푼다**(프롬프트 천장 인정): realistic 레지스터에서
  - **묘비/간판 텍스트** → "blank" 힌트 성공률 낮음. **철자 정확한 고유명사는 diegetic 수용**하거나, **텍스트 영역 crop/inpaint 로 비우기**, 또는 **각도·거리로 판독불가**하게.
  - **얼굴없는 유령** → 얼굴 지우기 힌트보다 **후드 안을 순흑(백라이트/깊은 그림자)로 조명 연출**하거나 **후드 영역 후처리 어둡게**.
  - **반투명(Marley·과거 그림자)** → realistic은 불투명하게 그림. **이중노출/글로우 컷 명시**하거나 **합성(opacity)** 으로.
  - **캐릭터 노화(현재 유령)** → 젊은 ref 조건화가 저항. **"aged" 상태의 별도 ref** 를 만들거나 그 패널만 **noref t2i**.
  - **구현(`post-fix.mjs`)**: 좌표 기반 결정적 후처리 3연산 — `void`(후드/그림자 순흑) · `blank`(묘비/간판 baked 글자 → 주변 톤 평탄화, HTML 레터링이 대신 얹힘) · `translucent`(유령 몸통 종이톤 블렌딩). 좌표는 Claude vision 로케이트(`--sdk`, pod 불필요) 또는 에이전트 스펙(`--spec`). 원본은 `qc/prefix-backup/` 백업(가역). `gen-verified --post-fix` 로 R21 미해결(flagged) 패널의 마지막 폴백으로 배선. 재롤이 아니라 확정 편집이므로 수렴 보장.
