...
function parseArgs(argv) {
  // psm 11 = sparse text — 레이아웃 가정 없이 흩어진 글자를 훑는다.
  //
  // ⚠️ 직전 결정(psm 4)을 뒤집은 것이다. 그때 채점식이 **정밀도만** 쟀기 때문이다 —
  //    "찾은 대사 중 몇 개가 깨끗한가"만 보고 **놓친 대사는 세지 않았다.**
  //    정답(사람이 센 말풍선 수)을 분모로 재현율을 넣자 순위가 통째로 뒤집혔다.
  //
  // 자기발전 스윕(tune.mjs, 12조합 × 표본 2종 · 정답 기반 재현율) 실측 2026-08-11:
  //           All Top(점수/재현율)   Classics Illustrated(점수/재현율)
  //   psm4/55   14.93 / 0.455          3.97 / 0.50     ← 직전 기본. 대사의 절반을 놓쳤다.
  //   psm6/55   24.85 / 0.818          5.33 / 1.00
  //   psm11/55  27.18 / 0.955          7.47 / 1.00     ← 채택(두 표본 모두 1위)
  //
  // 대가는 있다: 대사 조각이 81→139개로 늘고 검수 필요 비율이 0.58→0.76 이 된다.
  // 그래도 이쪽이 낫다 — 그대로 쓸 수 있는 개수는 34→33 으로 사실상 같고,
  // **조용히 사라지던 대사의 절반이 화면에 올라온다.** 검수자가 지우는 쓰레기는 보이지만,
  // 안 찾아진 대사는 아무에게도 안 보인다. 현대화 지우기도 이 재현율에 묶여 있다.
  const a = { psm: 11, minConf: 55 }
