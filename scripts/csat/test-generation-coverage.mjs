...
const DECISIONS = [
  { id: 'D1', name: '지문 선정', grade: 'SOFT',
    why: '**2026-08-26 에 NONE 에서 올라왔다(§10.33).** 이제 셋이 있다: '
      + '① 세 관문을 통과한 규칙 **D1a·D1b**(`verify-d1-rules.mjs`) — 지문은 내용어를 되풀이한다'
      + '(최빈 ≥2회 · 반복어가 어휘의 3% 이상). 전 회차 235편 위반 0 · 돌연변이 2/2 · 모평 홀드아웃 0. '
      + '② **두괄식 경향** — 최빈 내용어가 첫 문장에 있는 비율 66.5%, 기저(임의 두 문장, 순열 20,000회) 대비 **p<0.0001**. '
      + '끝 문장은 48.3%로 기저와 구분 안 됨(p=0.986) — **주제문은 앞에 온다, 뒤가 아니다.** '
      + '③ 유형별 계측 대역(`check-passage-band.mjs`). '
      + '**그러나 HARD 가 아니다** — D1a·D1b 는 "글이 아닌 것"(나열·목록)을 걸러낼 뿐 **어떤 글을 고를지 정하지 못하고**, '
      + '66.5% 는 규칙이 아니라 경향이다. 초안 §1 의 **사슬 밀도 가설은 기각됐다**(사슬이 안 끊기는 지문 27/191 = 14%, 끊긴 비율 중앙 43%). '
      + '**V1(기출 지문 vs 동일 출처 인접 단락 대조군)은 여전히 미실행이다.**' },
