  ...
  for (const a of j.analyses ?? []) {
    ...
    if (a.answer_unknown) {
      ...
      for (const c of correct) {
        ...
        // **이관된 서술이 실은 「왜 지우게 되나」일 수 있다.**
        // 정답 선지의 `why_tempting` 에 적혀 있던 것을 기계가 `why_correct` 로 옮겼는데,
        // 그중 일부는 「이 정답이 왜 배제되기 쉬운가」(오답 심리 서술)였다 — 뜻이 정반대다.
        // 실측 2026-09-03: 이관 398건 중 배치 검수가 4건, 어휘 검사가 4건을 더 찾았다.
        // 사람이 다시 읽어야 하는 자리이므로 **막지는 않고 표시한다**(내용이 틀린 게 아니라 자리가 틀렸다).
        // 좁게 잡으면 안 걸린다 — 배제 어휘만 보면 4건, **어미까지 넓히면 52건**이다(실측).
        // 불일치 찾기 유형(도표·내용일치·무관한 문장·어법·지칭)에서 특히 잦다: 정답이 「틀린 것」이라
        // 분석자가 「왜 이 틀린 것을 놓치기 쉬운가」를 쓰고 만다. 값진 관찰이지만 **정답 근거는 아니다** —
