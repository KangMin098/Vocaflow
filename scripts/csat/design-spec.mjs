...
// **번호 → 유형** (2019 개편 이후). E8 은 능력군까지, 이것은 유형까지 정한다 — E11 이 읽는다.
// 유형은 classify-types.mjs 가 지시문 패턴으로 붙인 것이고 번호를 안 쓴다(순환 아님).
export const TYPE_BY_NO = {
  18: 'R-PURPOSE', 19: 'R-MOOD', 20: 'R-CLAIM', 21: 'R-IMPLY', 22: 'R-GIST', 23: 'R-TOPIC', 24: 'R-TITLE',
  25: 'R-CHART', 26: 'R-FACT', 27: 'R-NOTICE', 28: 'R-NOTICE', 29: 'R-GRAMMAR', 30: 'R-VOCAB',
  31: 'R-BLANK', 32: 'R-BLANK', 33: 'R-BLANK', 34: 'R-BLANK', 35: 'R-IRRELEVANT',
  36: 'R-ORDER', 37: 'R-ORDER', 38: 'R-INSERT', 39: 'R-INSERT', 40: 'R-SUMMARY',
  41: 'X-TITLE', 42: 'X-VOCAB', 43: 'X-ORDER', 44: 'X-REFER', 45: 'X-FACT',
}
...
export const SPEC = {
  exam: [
    ...
    { id: 'E11', grade: 'HARD', name: '2019학년도부터 번호마다 **유형**이 고정 (18~45, 28자리)',
      check: (ex) => {
        if (Number(ex.exam.slice(0, 4)) < 2019) return true
        return ex.items.filter((i) => i.no >= 18).every((i) => !TYPE_BY_NO[i.no] || i.type === TYPE_BY_NO[i.no])
      },
      why: '**E8 은 "능력군" 까지만 말한다.** 이것은 그보다 강하다 — 번호가 **유형**까지 정한다. '
        + '2019~2026 수능 8 + 모평 3 = 11회차 · 28자리 × 11 = **308문항 예외 0**. '
        + '기저: E8 이 능력군을 고정하므로 남은 자유는 **군 안의 유형**이다. '
        + '군이 1종인 자리(21·31~34)는 자유가 없어 E8 의 재진술이고, **실질 자리는 23개**다. '
        + '한 회차가 우연히 이 배열일 확률 exp(−32.9) = **5.2e-15**, 11회차가 다 같을 확률은 사실상 0. '
        + '⚠️ **순환이 아니다** — 유형은 classify-types.mjs 가 **지시문 패턴**으로만 붙이고 '
        + '번호를 판정에 쓰지 않는다(그 파일 주석이 명시). '
        + '⚠️ E6(장문 41·43·44·45)와 겹치는 자리가 있다 — E11 은 그것을 포함하고 18~40 까지 넓힌 것이다.' },
  ],
  item: [
