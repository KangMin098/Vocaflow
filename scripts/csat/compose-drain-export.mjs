...
const SUBJECTS = {
  ...
  '역사·인류': [
    '문자 없는 사회의 기억 전달', '농경의 시작은 진보였는가',
    '교역망이 만든 도시', '기념물과 권력의 가시화',
    '친족 용어가 드러내는 사회 구조', '전염병이 바꾼 노동과 토지',
    '연대기와 사료 비판', '이주와 정체성의 재구성',
    '의례의 반복이 만드는 공동체', '식민 기록의 침묵을 읽는 법',
  ],
  '철학·윤리': [
...
for (let c = 0; c < CHUNKS; c++) {
  ...
  for (let i = 0; i < PER; i++) {
    ...
    items.push({
      topic: slot,
      subject: pool[(next * PER + i) % pool.length],
      title: '',
      content: '',
    })
  }
