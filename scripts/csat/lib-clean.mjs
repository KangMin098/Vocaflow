...
export function looksLikeBookMatter(text) {
  ...
  // ⑤ **편집자 주석·서지 블록은 여기서 못 잡는다** — 네 번 시도했고 네 번 다 실패했다.
  //
  //   첫 수확에서 《마르코 폴로 여행기》 학술판의 주석 문단이 지문으로 적재됐다:
  //     "…lete downe drawe." (Weber, II. 228.) So Correa calls a golden idol…
  //   ①~④ 는 이런 것을 전혀 못 잡는다. 문장이고, 대문자도 적고, 마침표도 있다.
  //   그래서 네 가지 신호를 차례로 재 봤고, 전부 **기존 재고를 대량 오탐하거나
  //   판별력이 아예 없었다**(실측 2026-09-04, PLOS 적합 원문 800편 대조):
  //
  //     · 출처 표기 `(이름, 권/쪽)`      → 오탐 **24.4%**. `(Qiagen, CA)` · `(Sigma, MO)`
  //                                       같은 제조사 표기가 그대로 걸린다.
  //     · 쪽수 `p. 228` · 로마숫자 `II. 228` → 오탐 4.3% · 2.8%. 학술 글에도 있다.
  //     · 고유명사 밀도                    → **판별력 없음.** 중앙값이 주석본 15.4%,
  //                                       본문 좋은 예(타키투스) 9.5%, PLOS 9.2% 로
  //                                       분포가 겹친다. 문턱을 어디 두어도 한쪽이 무너진다.
  //     · 책 단위 각주 밀도                 → **방향이 반대.** 타키투스(좋은 예) 17.8/천어,
  //                                       마르코 폴로(주석본) 1.6/천어. 좋은 판본일수록
  //                                       각주 *표시*가 많고 주석 본문은 뒤에 몰려 있다.
  //
  //   그러므로 이 한계는 남는다: **수확분의 일부는 본문이 아니라 주석이다.**
  //   대역에는 맞고 산문이지만 맥락 없이 시작한다. 자동으로 가를 방법을 아직 모르므로
  //   여기서 잡는 척하지 않는다 — 오탐 13% 짜리 규칙을 넣는 것보다 한계를 적어 두는
  //   편이 낫다. 가르려면 책을 고르는 단계에서 사람이 판단해야 한다.
