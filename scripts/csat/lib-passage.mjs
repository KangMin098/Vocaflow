...
export function itemBlocks(exam, no) {
  ...
  // ⚠️ 대체 진입점을 **줄머리 후보가 없을 때만** 쓰면 안 된다. 줄머리 후보가 있어도 그 블록이
  //    빈손인 경우가 있다(문항 번호가 옆 단 꼬리에 붙어 있고, 진짜 본문은 다른 자리에 있다).
  //    그래서 뒤에 **덧붙이고**, 고르는 일은 호출자가 한다(build-corpus 가 가장 실한 블록을 쓴다).
  //    줄머리 후보를 앞에 두므로 `blocks[0]` 을 쓰는 기존 호출자의 동작은 바뀌지 않는다.
  //
  ...
  //
  // ⚠️ **아무 세트 머리글이나 쓰면 안 된다.** `[31~34]` · `[36~37]` 은 **발문을 묶은 것**이지
  //    지문을 공유하는 것이 아니다(지문을 공유하는 세트는 장문 41~45 뿐이다 — `setBlockFor` 참조).
  //    이 구분을 놓치면 31번 지문이 32·33·34번에 통째로 복사된다. 2026-09-02 에 실제로 그랬고,
  //    게이트의 인용 대조 23건이 한꺼번에 어긋나서야 드러났다.
  //    그래서 ① 다른 후보가 하나도 없을 때만, ② 지문을 실제로 공유하는 장문 세트에서만 쓴다.
  if (!starts.length) {
    ls.forEach((l, i) => {
      const m = l.match(/\[\s*(\d{1,2})\s*[~～∼〜–—-]\s*(\d{1,2})\s*\]/)
      if (m && +m[1] >= 41 && no >= +m[1] && no <= +m[2]) starts.push(i)
    })
  }
...
/**
 * **①~⑤ 를 손가락으로 가리켜 답하는 유형 — 별도 선지 블록이 없다.**
 *
 * `build-corpus.mjs` 의 `SYMBOL_CHOICE_TYPES`(지문에 기호가 있어도 정상인 유형)와 **다르다**:
 * 저쪽은 순서(R-ORDER·X-ORDER)를 포함하는데, 순서 문항의 ①~⑤ 는 진짜 선지 블록이라
 * 절단을 꺼서는 안 된다. 두 목록이 하는 일이 달라서 하나로 합치지 않는다.
 */
export const INLINE_SYMBOL_TYPES = new Set([
  'R-REFER',
  'R-VOCAB',
  'R-GRAMMAR',
  'R-IRRELEVANT',
...
// ⚠️ **장문(X-*)은 하나도 넣지 않는다 — 표시 기호가 다르다.**
//    장문 세트의 어휘(42)·지칭(44)은 본문을 `(a)`~`(e)` 로 표시하고 ①~⑤ 는 **진짜 선지 블록**
//    (`① (a) ② (b) …`)이다. 실측 2026-09-13: 49문항 전부 지문에 ①~⑤ 가 0개이고 `(a)`~`(e)` 는
//    5개 다 있었다. 여기에 X-VOCAB·X-REFER 를 넣었더니 2024#44 지문 끝에 43번 순서 선지
//    `① (B)－(D)－(C)…` 와 44번 선지와 쪽번호 `88` 이 함께 빨려 들어왔다(1,993 → 2,137자).
//    순서(X-ORDER·R-ORDER)를 빼는 이유도 같다 — 본문에 박히는 것은 (A)(B)(C) 다.

/**
 * `opts.symbolsInline` — **①~⑤ 가 지문 안에 박히는 유형**(지칭·어휘·어법·무관·삽입·도표)에서
 * 켠다. 이 유형들은 밑줄·기호를 손가락으로 가리켜 답하므로 **별도 선지 블록이 아예 없다.**
 * 그런데 `choiceStart` 는 유형을 모르고 「줄머리 ① + 이후 ②③④⑤ 순서대로」만 보므로,
 * 줄바꿈이 마침 ① 앞에서 일어나면 그 자리를 선지 블록 머리로 오인해 **지문을 거기서 끊는다.**
 *
 * 실측 2026-09-13(이 옵션을 넣은 이유): M1809#30(지칭)은 지문이 1,000자인데 DB 에 **106자**만
 * 있었다 — 추출본 545~561행에 본문이 온전히 있는데도 547행이 `① her mother dealt with…` 로
 * 시작해 그 앞에서 잘린 것이다. 잘린 조각은 영어이고 한글도 없고 기능어로 끝나지도 않아
 * `suspectBody` 9종 중 **어느 것도 걸지 못했고**(`body_ok = true`), 그 상태로 분석이 쓰였다.
 * 인용 대조 게이트도 잘린 지문을 대조하므로 함께 통과했다 — 조용히 틀리는 경로였다.
 *
 * ⚠️ **R-ORDER·X-ORDER 는 켜지 않는다.** 순서 문항에서 본문에 박히는 것은 (A)(B)(C) 이고
 *    ①~⑤ 는 진짜 선지 블록(`① (A)-(C)-(B)`)이다. 여기서 끄면 선지가 지문에 섞인다.
 */
export function passageOf(block, opts = {}) {
  const cut = opts.symbolsInline ? -1 : choiceStart(block)
...
function trimChoice(s) {
  ...
  // 옆 단에서 넘어온 한글 토막 — **영어 선지에만** 적용한다.
  // ⚠️ 한국어 선지(요지·목적·주장·심경 · 2014A 회차)에 그대로 걸면 **끝 명사를 잘라 먹는다**:
  //    `시간적 압박하에서의 휴식의 중요성` → `시간적 압박하에서의 휴식의`.
  //    실측 2026-09-02: 사정권 14문항이 조사로 끝나 있었고 `body_ok` 는 true 였다
  //    (서브에이전트가 오답 분석이 성립하지 않는 것을 보고 잡았다).
  if (!/[가-힣]/.test(t.replace(/[가-힣]{1,3}\s*$/, ''))) t = t.replace(/\s+[가-힣]{1,3}\s*$/, '')
