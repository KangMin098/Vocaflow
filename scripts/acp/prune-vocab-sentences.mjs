...
// 부풀어 디스크가 위험하다(DB 가 이미 7.6 GB). 그래서 배치로 끊는다.
//
// ⚠️ **이 스크립트는 `VACUUM` 을 돌리지 않는다 — 돌릴 수단이 없다.** supabase-js 는 RPC 만
//   부를 수 있는데 `VACUUM` 은 트랜잭션 안에서 못 돌아 RPC 로 감쌀 수 없고, 이 DB 에 범용
//   SQL 실행 RPC 도 없다(2026-09-14 확인: exec_sql·run_sql·vacuum 류 함수 0개).
//   그러므로 죽은 튜플 회수는 **autovacuum**, 아니면 **사람이 따로 도는 것**에 달렸다.
//   (2026-09-14 실측 — 이 자리에 "몇 배치마다 일반 VACUUM 으로 공간을 재사용시킨다" 고
//    적혀 있었으나 코드에는 그런 호출이 없었다. 하필 이 주석이 근거로 든 것이 디스크
//    안전이라, 믿고 20M 행을 한 번에 돌리면 그 근거가 통째로 빈 말이 된다.)
//
// 디스크가 빠듯하면 `--max` 로 끊어 돌리고 **청크 사이에 사람이** 다음을 돌린다:
//     VACUUM public.library_article_vocabularies;
// 마지막 `VACUUM FULL` 은 **사람이 판단해서** 따로 돌린다
