...
//
//   node --env-file=apps/web/.env.local scripts/design/shot-authed.mjs /library/books /hub --slices 2
//
...
const routes = argv.filter((a, i) => a.startsWith('/') && argv[i - 1] !== '--base')
...
for (const r of routes) {
  ...
  for (let i = 0; i < Math.min(SLICES, Math.ceil(H / 900)); i++) {
    await page.evaluate((y) => window.scrollTo(0, y), i * 900); await page.waitForTimeout(250)
    const f = path.join(OUT, `authed-${name}-1440-${String(i + 1).padStart(2, '0')}.png`)
