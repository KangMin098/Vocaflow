...
3. **20~160자.**
4. **아포스트로피 금지 — 단, 표제어 자체에 아포스트로피가 있으면 예외.**
   기본은 금지입니다(TTS·따옴표 처리). `don't` → `do not`, 소유격은 우회.
   **그러나 표제어가 `hobson's choice`·`devil's advocate`·`adam's apple` 처럼 아포스트로피를 품고 있으면
   예문에도 그대로 쓰세요.** 빼면 `Hobsons choice` 가 되어 **틀린 철자를 가르치게 됩니다.**
   곡선 `’` 대신 ASCII `'` 를 쓰세요(apply 가 정규화하지만 처음부터 ASCII 로 쓰는 편이 낫습니다).
