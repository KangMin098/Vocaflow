...
// 입력: scratchpad-foreign/r8/residual.jsonl → 출력: scratchpad-foreign/r8/classified.jsonl {w,freq,books,sent,cls,lang}
...
const rows = []
for (const l of fs.readFileSync('scratchpad-foreign/r8/residual.jsonl', 'utf8').split('\n')) { if (!l) continue; try { rows.push(JSON.parse(l)) } catch {} }
console.error('잔여:', rows.length)
const byKey = new Map(rows.map(r => [r.w.toLowerCase(), r]))
const outPath = 'scratchpad-foreign/r8/classified.jsonl'
const done = new Set()
