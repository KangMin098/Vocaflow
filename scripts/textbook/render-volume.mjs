...
// ── 문서 조립 ─────────────────────────────────────────────────────────
// 여기 있던 170줄짜리 템플릿은 `textbook/volume-document.ts` 로 옮겼다(2026-09-06).
// 스크립트 최상위 스코프에 있으면 **임포트가 안 돼 회귀를 못 붙인다** — 판형·인쇄 규칙·
// 활자 스케일·판권면이 맞는지 확인하려면 사람이 이걸 돌리는 수밖에 없었다.
// 이제 스크립트는 DB 에서 모아 온 값을 넘기기만 하고, 문서의 모양은 저쪽이 책임진다.
const html = renderVolumeDocument({
  ...
  autoPassed: passed,
  autoTotal: card.auto.length,
  passageChip: PASSAGE_CHIP,
  answerBias: bias,
  proof,
  unitsHtml: unitHtml.join(''),
  answers: answerRows,
})
...
// ⚠️ **0단원이면 기록하지 않는다.** 남길 권이 없고, 표의 `units_check`(> 0)가 거부해
//    「조판 기록 실패」로 찍힌다 — 결함처럼 보이지만 실제로는 재료가 모자라 책이 안 만들어진
