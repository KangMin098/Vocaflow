...
const tasks = per.map(({ s, lemmas }) => {
  ...
  return {
    ...
    rule:
      `**밖% 는 종수가 아니라 토큰 비율**이다 — 소재 낱말을 되풀이하면 오르고, ` +
      `되풀이는 서로 다른 낱말을 늘리지 않으므로 V-Level p75 를 밀어 올리지 않는다. ` +
      `⚠️ "어려운 낱말을 더 넣어라" 로 고치면 밴드가 위로 튄다(실측: 한 편이 V3→V5). ` +
      `문장을 짧게 자르는 것도 답이 아니다 — FK 는 내려가지만 밖 낱말 밀도가 함께 떨어진다.`,
