 ...
 *   한 번도 검증하지 않은 값이었다.
 *
 * ⚠️ **낱말 정의는 `countPassageWords` 한 벌이다** — 여기서 다시 세지 말 것.
 *   풀과 벤치마크가 같은 문항에서 다른 수를 냈던 적이 있다(`purpose` 188 vs 194 ·
 *   `title` 167 vs 156, 실측 V6 전수). 원인은 읽는 글이 아니라 **세는 정의**였다:
 *   조합기는 공백 토큰, 코퍼스·벤치마크는 알파벳 토큰. `U.S. Supreme` 은 공백 2 · 낱말 3,
 *   `125 tons` 는 공백 2 · 낱말 1 이라 양방향으로 어긋난다. 창을 그은 자로 통일하자
 *   A6 미달 6건이 **0** 이 됐다(2026-09-01). 회귀 `word-count-unity.test.ts`.
 */
