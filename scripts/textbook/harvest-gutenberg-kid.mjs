 ...
 *
 * ── 4차 (2026-09-06) — 뺐다가 **재서 되돌렸다** ───────────────────────
 * `children's literature` 를 넣은 뒤 최신순으로 12편을 판정하니 3/12 라 그 서가를 뺐다.
 * **그 판단이 틀렸다.** 최신순 표본은 서가가 섞여 있어 무엇을 재는지 모르는 표본이었고,
 * 그때 그 서가에서 온 것은 **27편뿐**이었다(대부분 `book series` 에서 왔다).
 *
 * `source_id` 의 책 번호로 서가에 귀인해 다시 뽑으니 셋이 같은 자리였다:
 *
 *   children's instructional   **4/12**
 *   children's literature      **4/12**
 *
 * 그래서 되돌린다. 남는 교훈은 서가가 아니라 **표본 뽑는 법**이다 —
 * ⚠️ **최신순 표본으로 서가를 비교하지 말 것.** 무엇이 섞였는지 모르는 표본은 숫자를
 *   내지만 답을 내지 못한다. 서가를 견줄 때는 `source_id` 로 귀인해서 뽑는다.
 * ⚠️ 그리고 **수율만으로 고르지도 말 것** — 둘을 함께 재야 한다.
 *
 * ── 시도했고 물러섰다 — 운문 판별 (2026-09-06) ──────────────────────
 * `literature` 서가 실패 12편 중 **셋이 운문**이었다(생일시 · 자장가 · 정글북 챈트).
 * 줄머리마다 대문자를 쓰니 '문장당 대문자 수' 로 잡힐 줄 알았는데 재 보니 겹친다:
 *
 *   운문 0.5 ~ 2.0   ·   산문 0.33 ~ 0.75(고유명사가 많으면 0.75)
 *
 * 정글북 챈트(0.5)가 `Mr. Harrison … Ginger … Anne` 같은 평범한 산문(0.75)보다 **낮다.**
 * 조각을 만들 때 줄바꿈이 사라지므로 운문의 진짜 표지가 남지 않는다. 넣지 않는다.
 */
const TOPICS = (
  arg(
    'topics',
