 ...
 * 한 권(20단원)에 필요한 원글 수.
 *
 * 단원 하나는 서로 다른 원글 4편을 쓴다. 한 원글이 여러 단원에 나눠 쓰일 수 있으므로
 * 80편이 다 필요하진 않지만, **원글이 부족하면 단원 수가 원글 수에 묶인다.**
 * 실측 기준선: V5 는 원글 75편으로 20단원, V6 은 58편으로 20단원.
 * 그래서 60편을 한 권의 실무 하한으로 둔다.
 */
const VOLUME_ARTICLES = 60
  ...
  JSON.stringify(
    {
      ...
      counts: {
        ...
        [`V${BAND}`]: '12~14개',
        [`V${BAND + 1}~V${BAND + 2}`]: '7~9개  ← **이 꼬리가 없으면 한 계단 아래로 떨어진다**',
        [`V${BAND - 1} 이하`]: '나머지 전부 (절반 이상)',
      },
      rule:
        `서로 다른 낱말 기준으로 V${BAND} 을 12~14개, V${BAND + 1}~V${BAND + 2} 를 7~9개 쓰고 ` +
        `나머지는 쉬운 층으로 채운다. **쉽게만 쓰면 목표보다 아래로 떨어진다.**`,
      at_band: lexicon.at,
