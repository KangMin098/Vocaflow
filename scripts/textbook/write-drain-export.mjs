...
/**
 * 설명문 소재 축. **축 × 하위 주제가 곧 시리즈가 쓸 수 있는 글의 수다.**
 *
 * ⚠️ 2026-08-30 에 축마다 여덟 개씩 더해 8×8=64 → **8×16=128** 로 늘렸다.
 *   V3 몫 40 편을 뽑으려는데 64 조합 중 **이미 쓴 것이 그만큼**이라, 열일곱 슬롯이
 *   V2 에서 막 쓴 소재와 똑같이 나왔다(「옛 지도의 오류」·「줄 서기와 차례」…).
 *   그대로 쓰면 학습자가 2단에서 읽은 글을 3단에서 다시 읽는다 — 사다리로 파는
 *   시리즈에서 이건 브랜드 결함이다.
 *
 *   **오프셋을 더하는 것으로는 안 된다** — 창이 밀릴 뿐 겹침의 총량은 그대로다.
 *   조합을 늘리는 것만이 답이다. 다음에 또 바닥나면 여기에 더한다(계산: 밴드 7 ×
 *   한 권 40 편 = 280 이므로 사다리 전체를 한 번씩 채우려면 결국 280 조합이 필요하다).
 */
const TOPIC_AXES = [
  {
    key: 'life_science',
    ...
    subs: ['먹이를 찾는 방식', '추위·더위를 견디는 몸', '무리 지어 사는 규칙', '씨앗과 번식', '위장과 신호', '기생과 공생', '밤에 활동하는 동물', '되살아나는 숲', '잠들지 않는 식물', '알을 지키는 방식', '섬에서만 사는 종', '냄새로 하는 대화', '뿌리와 흙 속의 이웃', '떼지어 옮겨 가는 길', '몸집과 수명', '버려지는 부위의 쓸모'],
  },
  {
    key: 'earth_space',
    label: '지구·우주',
    hint: '날씨·지형·바다·행성, 관측으로 알아낸 것',
    subs: ['해류와 기온', '흙과 침식', '지하수와 샘', '조수와 달', '얼음과 빙하', '화산과 온천', '대기의 층', '먼 곳을 재는 법', '모래가 만들어지는 자리', '강이 굽는 이유', '별빛이 오는 시간', '바람이 부는 까닭', '동굴이 자라는 속도', '땅이 흔들리는 방식', '밤하늘이 어두운 이유', '비가 소금을 씻어 내는 길'],
  },
  {
    key: 'technology',
    label: '기술·공학',
    hint: '도구와 재료가 문제를 푸는 방식, 설계의 절충',
    subs: ['접합과 이음', '열을 다루는 구조', '무게를 견디는 형태', '물을 막고 흘리는 법', '소리를 다루는 설계', '일부러 약하게 만든 부품', '규격이 생긴 이유', '오래 쓰기 위한 정비', '마찰을 줄이는 방법', '빛을 휘게 하는 것', '멈추게 만드는 장치', '한 번 쓰고 버리는 설계', '실을 꼬아 얻는 힘', '공기를 가두는 재료', '눈금을 읽기 쉽게 만드는 법', '고장을 미리 알리는 신호'],
  },
  {
    key: 'society',
    label: '사회·경제',
    hint: '사람들이 모여 정하는 규칙, 자원의 배분, 도시',
    subs: ['줄 서기와 차례', '공유지의 관리', '가격이 정해지는 자리', '길과 통행', '쓰레기와 재사용', '이웃 간의 소음', '공공장소의 규칙', '일과 휴식의 배분', '빌리고 돌려주는 약속', '이름을 붙여 파는 일', '한 줄로 세운 시간표', '누가 먼저 고르는가', '싼 것이 비싸지는 순간', '함께 쓰는 물의 몫', '보이지 않는 일의 값', '문턱을 낮추는 설계'],
  },
  {
    key: 'history',
    label: '역사·문화',
    hint: '옛 사람들의 생활과 그것이 남긴 흔적, 관습의 유래',
    subs: ['이름과 호칭의 유래', '옛 저장 기술', '길과 표지의 역사', '측정 단위의 통일', '기록을 남기는 방법', '축제와 계절', '옷과 신분', '옛 지도의 오류', '불을 옮기는 방법', '숫자를 세던 도구', '길 위의 시장', '남의 말을 옮겨 적기', '집의 방향', '아이들의 놀이', '병을 막으려던 규칙', '버려진 도시가 남긴 것'],
  },
  {
    key: 'arts',
    label: '예술·매체',
    hint: '음악·미술·건축·이야기의 형식과 그 형식이 하는 일',
    subs: ['반복이 하는 일', '여백과 침묵', '색을 섞는 규칙', '시점과 거리', '무대와 관객의 자리', '제목이 하는 일', '모작과 변주', '재료가 정하는 형식', '소리가 비는 자리', '틀을 벗어난 그림', '이야기의 첫 문장', '같은 노래 다른 속도', '작은 것을 크게 그리기', '보이지 않는 주인공', '광고가 빌려 쓰는 형식', '오래된 형식이 살아남는 법'],
  },
  {
    key: 'mind',
    label: '심리·학습',
    hint: '주의·기억·습관·판단이 작동하는 방식',
    subs: ['익숙함과 앎의 차이', '방해를 받은 뒤 돌아오기', '순서가 기억에 남기는 것', '한꺼번에 담을 수 있는 양', '미루는 이유', '남의 판단을 따라가기', '틀린 기억이 만들어지는 법', '쉬는 동안 일어나는 정리', '이름을 잊는 순간', '여럿이 있을 때의 판단', '기다림이 길게 느껴질 때', '손으로 쓰면 남는 것', '어려운 것이 더 기억되는 이유', '처음 본 것에 끌리는 마음', '규칙을 몸이 먼저 아는 일', '실수를 되짚는 법'],
  },
  {
    key: 'health_sport',
    label: '건강·운동',
    hint: '몸을 쓰는 일과 회복, 음식과 수면',
    subs: ['준비 운동이 하는 일', '땀과 체온', '자세와 부담', '회복에 걸리는 시간', '물과 갈증', '잠의 단계', '나이와 유연성', '통증이 알리는 것', '숨을 고르는 법', '근육이 기억하는 것', '아침과 저녁의 몸', '허기와 식사의 간격', '무게를 옮기는 걸음', '소리가 몸에 주는 부담', '손의 힘과 도구', '쉬는 날의 몸무게'],
  },
]
...
const LONG_NARRATIVE_SHAPES = [
  {
    ...
    hint:
      ...
      '**이름 있는 인물 둘이 같은 성별**이어야 한다(44번 지칭이 그 둘을 헷갈리게 묻는데, ' +
      '남녀 한 쌍이면 he/she 로 저절로 갈려 물을 것이 없다). ' +
      '대명사를 문단마다 여러 번 쓰되, 누구를 가리키는지 문맥으로 분명해야 한다.',
  },
  {
    key: 'two_people',
    label: '두 사람 — 한 사건을 사이에 두고',
    hint:
      '이름 있는 두 인물이 번갈아 초점이 된다 — **둘은 같은 성별이어야 한다**(지칭 문항의 전제). ' +
      '(A) 에서 둘을 세우고, (B)(C)(D) 에서 한 사람의 행동이 다른 사람에게 닿는 순서로 잇는다. ' +
      '각 문단에 상대를 가리키는 대명사가 최소 두 번 나와야 한다. ' +
...
/**
 * 문장 수 하한 — **어수만큼이나 딱딱한 규격인데 지금까지 슬롯에 안 적혀 있었다.**
 *
 * `write-drain-import.mjs` 의 `MIN_SENTENCES` 와 같은 값이고 근거도 같다:
 * 순서 문항은 문단이 4~6문장이어야 나오고, 교재용 삽입은 문단이 6~10문장이어야 나온다.
 * 겹치는 값이 6뿐이라 한 편에 **6+6 = 열두 문장**이 필요하다.
 *
 * ⚠️ 이 값을 슬롯에 안 실어 보내면 배치는 그것을 알 방법이 없다. 실측 2026-08-30:
 *   어수·어휘 규격을 다 지킨 V2 40편 중 **29편이 9~11문장이라 적재에서 통째로 걸렸다.**
 *   글은 멀쩡했고 밴드 적중은 40/40 이었는데, 아무도 말해 주지 않은 조건 하나 때문에
 *   다시 써야 했다. 규격은 지침에 있어야 하고, 검사에만 있으면 안 된다.
 */
...
for (let i = 0; i < need; i++) {
  ...
  tasks.push({
    ...
    words_max: WORDS_MAX,
    /**
     * **시중 지문과 같은 결로 쓰기 위한 목표.** V-Level 만으로는 여기가 안 맞는다.
     *
     * ── 왜 생겼나 (실측 2026-09-05) ────────────────────────────────
     * 이 드레인이 쓴 368편의 시중 자리 중앙이 **14.7** 이었다(0=가장 쉬움 · 50=시중 중앙).
     * 외부 소스는 40~79 다. 밴드를 통제해도 같아서 — 초6~중1 칸이 우리 글을 빼면
     * **16.5 → 72.6** 으로 뛴다 — **소스가 모자란 게 아니라 우리가 쉽게 쓰고 있었다.**
     *
     * 원인은 자가 없어서가 아니라 **다른 자를 댔기 때문**이다. V-Level 꼬리 수는 우리
     * 사다리에서의 자리를 정하지 **시중 지문과 비교되는 축이 아니다.** V 목표를 정확히
     * 맞히고도 시중 15번째에 떨어질 수 있고, 실제로 그랬다.
     *
     * 그래서 축을 하나 더 준다: **내용어 중 2022 교육과정 기본어휘 3,000 밖의 비율.**
     * 아래는 시중 지문 207쪽 실측 분포다. 적재기가 이 축의 하한으로 거른다.
     */
    outside_curriculum_pct: {
      note:
        '내용어(기능어 제외) 중 교육과정 기본어휘 3,000 **밖**인 것의 비율. ' +
        '낮을수록 쉽다. 이 값이 곧 시중 분포에서의 자리다.',
      elementary: { p25: 20, p50: 27, p90: 38.6 },
      middle: { p25: 23.5, p50: 28.6, p90: 41.6 },
      rule:
        '초등 밴드로 떨어지면 **20~38.6%**, 중등이면 **23.5~41.6%** 안에 든다. ' +
        '어느 쪽이 될지 쓰기 전엔 모르니 **중앙(27 · 28.6)을 겨냥하면 양쪽 다 안전하다.** ' +
        '⚠️ 그 아래는 적재기가 거른다 — 시중 지문보다 쉬우면 지문 구실을 못 한다.',
    },
    // ⚠️ 적재기가 이 아래를 통째로 거른다 — 어수를 맞춰도 문장이 모자라면 못 넣는다.
...
console.log(
  `  문장 규격 **최소 ${SENTENCES_MIN}문장** — 어수를 맞춰도 이 아래는 적재기가 통째로 거른다` +
    ' (순서는 문단 4~6문장 · 삽입은 문단 6~10문장 → 6+6). 한 문장 평균 15~16어가 된다.',
)
