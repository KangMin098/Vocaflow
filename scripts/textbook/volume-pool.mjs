...
export async function loadVolume(
  ...
  //
  // ⚠️ **기본은 경고이지 차단이 아니다.** — 그런데 **그 이유가 낡았다.**
  //
  //   처음 적을 때의 근거는 「강제하면 V2 는 3편 · V3 는 1편만 남아 그 학년 권이 아예 안
  //   나온다」였다(2026-09-06 오전 실측). 재고를 못 만들면서 규칙만 켜는 것은 규칙을 지키는
  //   것이 아니라 파이프라인을 세우는 것이므로 옳은 판단이었다.
  //
  //   같은 날 저녁 기사 5,245편을 전부 판정한 뒤 다시 재니 **V2 230편 · V3 220편 · V4 704 ·
  //   V5 3,383 · V6 1,912 · V7 408** 이다. 조판 가능이 2,219 → 6,900(10.2% → 32.5%).
  //   V1 은 5편뿐이지만 **그 학년은 지문을 안 쓴다**(유형 셋이 전부 no-passage) — 그러므로
  //   지문 재고가 강제를 막는 학년은 지금 **하나도 없다.**
  //
  //   그래도 기본을 아직 안 바꿨다. 강제는 조판 산출물을 바꾸는 변경이라 **권을 실제로 뽑아
  //   비교한 뒤** 정할 일이고, 그 비교를 아직 안 했다. 근거가 바뀌었다는 사실만 여기 남긴다 —
  //   낡은 근거를 그대로 두면 다음 사람이 「재고가 없다」고 믿고 영영 안 켠다.
