...
/**
 * ⚠️ **내보낸다** — 재고 스캔이 같은 표를 봐야 한다(실측 2026-09-08). 초등 세 유형은
 *   `csat_dcp_items` 에 저장되지 않고 여기서 **즉석 생성**되는데, 재고 스캔이 저장된
 *   문항만 세어 V1 을 **0권**이라 말하고 있었다. 실제로는 운율 470 · 뜻 805 · 철자 528
 *   (권당 40) 로 **11권**이다. 사본을 두면 그 어긋남이 다시 생긴다.
 */
export const ELEMENTARY_TAG = { 1: 'kcurr2022_1', 2: 'kcurr2022_1', 3: 'kcurr2022_2' }
...
export async function loadVolume(
  ...
  //
  // ── 기본이 「차단」이다 (2026-09-06 저녁 전환) ────────────────────────
  //
  // 오래 기본이 「경고」였고 그 근거는 **재고**였다: 「강제하면 V2 는 3편 · V3 는 1편만 남아
  // 그 학년 권이 아예 안 나온다」(같은 날 오전 실측). 재고를 못 만들면서 규칙만 켜는 것은
  // 규칙을 지키는 것이 아니라 파이프라인을 세우는 것이므로 그때는 옳았다.
  //
  // 기사 5,245편을 전부 판정한 뒤 **권을 실제로 뽑아 강제 전후를 비교했다**:
  //
  //   밴드   비강제 문항풀 → 강제 문항풀      비강제 채점 → 강제 채점
  //   V2         2,148 →  2,133                9/10 → **10/10**  (강제가 더 낫다)
  //   V3             — →  2,456               10/10 → 10/10
  //   V4         9,635 →  9,598               10/10 → 10/10
  //   V5        14,856 →  7,172               10/10 → 10/10
  //   V6       157,622 →  3,024               10/10 → 10/10
  //   V7        66,385 →    127               10/10 → **8/10**
  //
  // 여섯 학년은 **같은 권이 나온다.** 문항 풀이 98% 잘려도(V6) 뽑는 것은 120문항이라
  // 결과가 안 바뀐다. V2 는 오히려 좋아졌다.
  //
  // V7 만 다르다 — 판정된 원문이 59편뿐이라(2,468편이 미판정 plos raw) 6단원짜리가 되고
  // 같은 글을 되풀이한다. **그런데 그것이 사실이다.** 지금의 20단원 V7 권은 미판정 원문
  // 위에 세워져 있고, 그건 「단 하나의 원문도 사용이 어려운 원문이면 안 된다」는 이 저장소의
  ...
  return {
    ...
    //   조판은 V7 에서 「문항 없는 원글 2,985편 — store-new-types 를 돌려야 쓰인다」고
    //   찍었는데, 그중 **지금 채울 수 있는 것은 506편**뿐이었다. 나머지 2,479편은
    //   내용 판정을 아직 못 받아 게이트가 막는다 — 문항을 만들어도 안 실린다.
