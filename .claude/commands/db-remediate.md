...
**MCP 로 무엇이 되고 무엇이 안 되는가** (2026-09-16 실측 — 같은 것을 다시 알아내지 말 것):

| | |
|---|---|
| `VACUUM` / `VACUUM FULL` **한 문장만** | **된다.** 단일 문장은 트랜잭션에 안 감싸인다 |
| `set statement_timeout = '30min'; <문장>` | **안 된다.** 문장이 둘이면 통째로 감싸여 `VACUUM`·`CONCURRENTLY` 가 `25001` |
| 시간 제한 | `postgres` 역할엔 **없다**(`pg_roles.rolconfig` 확인). 2분은 이 경로가 세션에 거는 값이라 `ALTER ROLE` 은 필요 없다 — 다만 위 이유로 한 호출 안에서 올려 쓸 수는 없다 |
| `REINDEX ... CONCURRENTLY` | **안 된다** → `_pending_*.sql` + 사람이 psql (위 §스키마 조치) |

**회수량은 재고 정한다** — `pgstattuple_approx(<표>)` 로 힙 빈 공간, `pgstatindex(<색인>)` 의
`avg_leaf_density`(새 btree 는 ~90%)로 색인 몫. 둘 다 2분 안에 끝난다. 이걸 안 재면
「크니까 줄겠지」로 락을 잡게 된다 — 2026-09-14 기록이 실제로 그랬고, 실측 회수는
기대치의 **절반 이하**였다(`CHANGELOG.md` §2026-09-14 정정 블록).
