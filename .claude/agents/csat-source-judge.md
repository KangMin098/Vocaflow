...
| `content` | **본문 전문.** 발췌가 아니다 (보관 판정 청크에는 없고 대신 `sections` · `sections_found` · `words` · `has_items`) |
...
`$OUT_PATH` 에 JSON 배열. **입력과 같은 개수·같은 순서**로, 항목마다 정확히 일곱 키(보관 판정이면 `basis` 까지 여덟 키):
...
**판정 전에 `docs/SOURCE_JUDGMENT_CRITERIA.md` 를 처음부터 끝까지 읽고 그대로 따른다.** verdict · genre ·
...
| 입력에 있는 키 | 판정 | 출력 |
|---|---|---|
| `content` | **내용 판정** — 본문 전문을 읽는다 | 일곱 키 |
| `sections` · `basis:"sections"` | **보관 판정** — 서론·고찰 절을 전부 읽는다 | 일곱 키 + `"basis": "sections"` |
