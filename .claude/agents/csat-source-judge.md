...
`$OUT_PATH` 에 JSON 배열. **입력과 같은 개수·같은 순서**로, 항목마다 정확히 일곱 키(보관 판정이면 `kind`·`basis` 까지 아홉 키):
...
| `content` + `kind:"retain"` | **보관 판정** — 본문 전문을 읽는다 | 일곱 키 + `"kind": "retain"` · `"basis": "full"` |
