...
`$OUT_PATH` 에 JSON 배열. **입력과 같은 개수·같은 순서**로, 항목마다 정확히 일곱 키:
...
## uses — 이 원문으로 어떤 교재를 만들 수 있는가

**`use`·`narrative` 면 하나 이상. `reject` 면 빈 배열.** 정본은 `gate-rules.SOURCE_USES`.

⚠️ **이것은 수능 목록이 아니다.** 아래 대응은 `textbook/market-spec.json` — 시중 영어 교재 **79종**
(94문서 · 5,229쪽 · 초등 6 · 중등 10 · 고등 30권)에서 실측한 표준 발문 41종의 매핑이다.
...
⚠️ **길이로 고르지 않는다.** 「짧아서 장문은 안 되겠다」 같은 판단을 하지 말라 —
어수창과 밴드는 기계가 이미 잰다(`compose-unit.itemWordSpec`). 당신이 답할 것은
**그 재료가 이 글에 있는가**뿐이다. 1,000어짜리에도 `mood` 가 있을 수 있고
100어짜리에도 `argument` 가 있을 수 있다.

넉넉히 달지 말고 **실제로 그 재료가 있는 것만** 단다. 전부 여덟 개를 다는 글은 거의 없다.
