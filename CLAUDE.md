...
| 축 | 컬럼 | 출처 | 의미 |
|---|---|---|---|
| **V-Level Centroid** | `library_books.book_v_level` (smallint) + `v_level_centroid_precise` (numeric) + `vrl_components` (jsonb) | Vocaflow 자체 (`compute_book_vrl`) — `library_book_vocabularies.lemma → shared_dictionary.v_level` **type-based** p75 centroid (V11 excluded · DISTINCT lemma · ★v06.34 token→type 교체) | 한국 학습자 어휘 부담 |
...
산정 위치 (단일 출처):
- `compute_book_vrl(book_id)` — V-Level Centroid + p50/p75/p90 · **type-based** (★v06.34 migration `compute_book_vrl_type_based_p75`)
...
#### V-Level 측정 방식 — token vs type (★v06.34 정정)

**v06.34 이전 (token-weighted p75, 폐기)** — `generate_series(1, freq_in_book)` 으로 빈도 가중 확장 → 'the' (V1) 가 freq=2000 → 2000 row 복제 → PERCENTILE_DISC 계산.

문제: Zipf 분포 영향으로 영어 텍스트 토큰의 **50~65% 가 V1-V2 function word** (the/a/of/and 등 콘텐츠 무관 고정). p75 가 도서 본질 어휘 권역이 아닌 Zipf 보편 영역을 가리킴 → 실측 결과 Christmas Carol/Treasure Island/Connecticut Yankee/Study in Scarlet 4권 모두 V5 측정 (실 어휘 부담 V7-V8).

**v06.34 신 (type-based p75)** — `SELECT DISTINCT lbv.lemma, sd.v_level` 후 PERCENTILE_DISC. 각 distinct lemma 1회만 카운트 → Zipf 편향 배제 → 도서 본질 어휘 부담 반영.

학술 정합: Lexile · ATOS · CEFR-J Text Profile 모두 type-frequency 기반. Krashen i+1 정합 — 학습자 challenge zone 측정엔 function word 토큰 빈도 무의미.

실측 효과 (12 도서 재측정):
- Christmas Carol: V5 → **V7** (A2.2 → B1.2)
- Treasure Island: V5 → **V8** (A2.2 → B2.1)
- Connecticut Yankee: V5 → **V8** (A2.2 → B2.1)
- Study in Scarlet: V5 → **V8** (A2.2 → B2.1)
- Twenty Years After: V5 → **V9** (A2.2 → B2.2) — Dumas advanced literature 정합
- Romeo and Juliet: (null) → **V8** (B2.1)
- The Four Feathers: (null) → **V8** (B2.1)
- Alice / Frankenstein / Pride & Prejudice / Sherlock / Dorian: 변화 없음 (이미 V6 / V8 정확)

**method tag** — `vrl_components.method='p75_type_v11_excluded_l1_l2_inflections'` 으로 변경 시점 표기.

**다운스트림 효과** —
- `extract_book_vocabulary_admin` v_threshold = book_v_level 자동 갱신 (단어장 추천 quality 상향)
- `publish_book_word_sets` 의 `v_level >= book_v_level` strict 필터 → Twenty Years After 챕터 단어장 4,804→1,576 단어 (avg 17.5/ch, min 1, <5단어 챕터 7개 = 8% — CLAUDE.md v06.30 "8개 <10단어 그대로 보존" 결정 정합)
- LibraryCard `cefr_band` 표시 자동 갱신 (B1 → B2 다수)
- `recommend_word_sets_for_user` i+1 zone 계산 정밀도 향상
...
### 변경 이력 (append-only) — 파생어 사전 seed (2026-05-31)

**파생어 사전 seed (freq≥50)** — `shared_dictionary` 에 파생어 독립 row 2,315건 추가 (`source='derivational-seed'` · `classified_by='claude_code_derivational'` · 누적 2,824). 대상: `inflections.forms` 에 있으나 독립 row 없는 파생어 중 freq≥50. POS 접사별 결정(noun/adjective/adverb) · verb 접사(-ate/-ize/-ise)·-ry 제외(POS 오탐+철자변이 차단). `meaning_ko` 규칙 합성 352건(나머지 NULL=2차 LLM) · 3-pass 품질 fix(구/이중파생-ly/verb-base-ly/부정접두사 polarity 오역 NULL). v_level=LEAST(base_v+1,10) · verified=false · 멱등 INSERT. 검증: 굴절오염0·자기참조0·dangling0. 회수: "Twenty years after" 553건 신규 L1 바인딩. 인프라: `scripts/derivational-seed.mjs`. 상세: `CHANGELOG.md` [Unreleased]. 정합: ADR 0001 D1-D3 (파생=독립 row, 굴절=inflections 회수). 동반: `find_unbound_book_lemmas.cluster_base` 컬럼 + `classify_archaic_candidates()` 분류기(재출현 게이트).

### 변경 이력 (append-only) — V11 word_register 3종 분류 (2026-05-31)

**V11 word_register 분류** — `shared_dictionary.word_register` (TEXT DEFAULT 'standard' + CHECK 5값) 신설, v_level=11 의 17,452건 전량 태그(standard 0). 분포: modern_advanced 12,414 · phrase_unit 4,319 · archaic_literary 435 · period_cultural 284. 도서 단어장 register 배지(📜 고어 / 🏛 시대어)·학습 차등(archaic→SpellForge 제외 등) 표시용. **분류 방식**: 규칙(archaic=meaning_ko 고어 마커+known list · phrase_unit=frequency_band phrase/compound · period_cultural=meaning_ko 시대 키워드) + 나머지 modern_advanced 기본값. **spec 의 LLM batch(12,600단어·126 req) 불요로 판정** — 잔여 빈도 신호(frequency_rank 0/list_tags 9) + 표본 검증 결과 잔여가 현대 고급/기술/과학 어휘 압도적(keratin·biogeochemistry·yakuza…)이라 기본값으로 ~95% 정확(spec 허용 <10%). spec 의 어미 규칙(est/eth)은 전부 구·현대어 오탐("be no contest"·palimpsest·shibboleth)으로 폐기. 기존 의미/레벨 무수정. Migration `add_word_register`. 배지 UI 연동은 별도(범위 밖).
