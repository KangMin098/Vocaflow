...
| **기출을 학습자에게 내놓는 방식 (화면·단계)** | **[docs/csat-learner-brief.md](./docs/csat-learner-brief.md)** · [docs/csat-learner/DECISIONS.md](./docs/csat-learner/DECISIONS.md) · [docs/CSAT_LEARNER_DELIVERY.md](./docs/CSAT_LEARNER_DELIVERY.md) · [docs/MODULES.md](./docs/MODULES.md) |
...
**항상 금지**: 폭죽 · 콘페티 · 배지 팝업 · 자동재생 캐러셀 · **장식적 상시 모션**(perpetual micro-motion).

⚠️ **로더·스켈레톤은 허용된다** — 판정 기준은 "반복하는가" 가 아니라 **"끝나는 상태가 있는가"** 다.
(2026-09-06 실측: "루프 애니메이션 금지" 로 적었더니 정당한 `Loader2` 로더 20여 곳이 걸렸다 —
규칙이 틀렸지 코드가 틀린 게 아니었다.) 아케이드(`components/game/`)는 이 절의 대상이 아니다.
회귀 `components/__tests__/learning-tone.test.ts`.
...
> 마지막 생성 **2026-09-17**. 낡았는지 확인만 하려면 `--check` (파일을 안 고치고 exit 1).

**수요 측** — 이 줄이 이 문서에서 가장 중요하다. 공급이 아무리 늘어도 여기가 안 늘면 진단은 `risk` 다.

- 가입자 **3** (프로필 3) · 학습기록 **665** · 읽기 세션 287 · 일별 활동 60 · 점수 78
- 교사 채널: 학급 **1** · 학급 구성원 0 · 학급 과제 **0** · 퍼널 이벤트 7,890
...
- `shared_word_sets` 11,308 (published 11,099) · `library_chapter_quiz` 2,453
