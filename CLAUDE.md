...
- ⚠️ **승인 대기 마이그레이션 1건** — `/fit` 레벨 맵이 상시 잘려 있다(anon 가시 `shared_words` **656,257행** · 콜드 **88초** · `MAX_ROWS` 200,000 에서 조용히 중단 → 커버리지 과소). 잘림 로그 + 히어로 표적 조회 우회(88초 → 3.7초)까지 했고, 근본 해결은 `GRANT EXECUTE ON FUNCTION public.textfit_resolve_levels(text[]) TO anon;` 한 줄. 절차·근거는 [CONVENTIONS.md §전량 적재 캐시](./docs/CONVENTIONS.md)
