...
## 📚 어휘 관리 원칙 (always-on · 사용자 명시 지침 2026-07-19)

**1. 사전 DB 보완의 목적 = 학습 대상 누락 방지.** 파생형·굴절형(annoyingly·steepness·went·children 등)이 **학습 대상에서 누락되지 않도록** 사전을 지속 보완한다. base 가 사전에 있는 파생/굴절 표면형은 학습 대상으로 등재·연결(→ [docs/adr/0001](./docs/adr/0001-dictionary-derivational-enrichment.md) Phase 6).

**2. 추출되는 실단어는 전부 사전 DB에 존재해야 한다 — "실단어 미등재"는 줄여야 할 상태.** 독서 중 추출되는 단어는 학습 대상이 아니어도 **사전 DB에 등재(뜻 포함)** 하여 추출·조회되게 한다(조용히 버리지 않음). 어휘를 **이원 관리**:
- **학습 대상** = 일반 학습 어휘(파생·굴절 포함) → SRS 학습.
- **비학습 대상 = 독서 지원용** = 도서 특유 희귀어(항해 전문어·방언 철자·희귀 복합 등) → **사전 DB에 등재하되 학습 대상에서 제외**(SRS 미포함). 단어장엔 "독서 지원용" 비학습으로 함께 보존 — 뜻 조회·문맥 참조는 가능.

→ 전문어·방언도 사전에 넣어 **추출되게** 하고, 학습/비학습은 **별도 플래그**로 구분한다(사전 등재 자체는 전부 대상).
